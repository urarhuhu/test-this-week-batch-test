...
設為 true 可將漣漪效果添加到視圖的前景而非背景。這在子視圖已有自身背景，或您正在顯示圖片等情況下非常有用，可避免漣漪效果被遮蓋。

請先檢查 TouchableNativeFeedback.canUseNativeForeground()，因為此功能僅在 Android 6.0 及以上版本可用。若在較舊版本上使用，將收到警告並回退至背景模式。
