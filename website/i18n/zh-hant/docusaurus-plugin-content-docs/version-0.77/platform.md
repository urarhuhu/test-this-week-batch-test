...
`select` 方法會回傳最適合當前執行平台的值。也就是說，若您在手機上執行，會優先使用 `android` 和 `ios` 鍵。若未指定這些鍵，則會使用 `native` 鍵，接著是 `default` 鍵。
...
由於對應平台鍵的值可以是 `any` 類型，[`select`](platform.md#select) 方法也可用來回傳平台特定的元件，如下所示：
