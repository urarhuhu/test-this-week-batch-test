...
建議在輕度使用之外的情境中，應基於 `AsyncStorage` 再封裝抽象層，而非直接操作全域性的 `AsyncStorage`。

在 iOS 上，`AsyncStorage` 底層由原生程式碼實現，小數值會序列化儲存在字典中，較大數值則存於獨立檔案。Android 平台則會根據可用性選擇使用 [RocksDB](https://rocksdb.org/) 或 SQLite。

`AsyncStorage` 的 JavaScript 層是提供清晰 API 的門面模式，包含真實的 `Error` 物件與非批次操作方法，每個 API 方法皆回傳 `Promise` 物件。
...
根據 `key` 取得對應項目並於完成時觸發回調。回傳 `Promise` 物件。
...
為指定 `key` 設定數值並於完成時觸發回調。回傳 `Promise` 物件。
...
合併現有 `key` 值與輸入值（假設兩者皆為 JSON 字串）。回傳 `Promise` 物件。
...
清除 _所有_ `AsyncStorage` 資料（含其他客戶端、函式庫的資料）。通常不應直接呼叫此方法，請改用 `removeItem` 或 `multiRemove` 僅清除應用程式自身的鍵值。回傳 `Promise` 物件。
...
取得應用程式 _所有_ 已知的鍵值（含其他呼叫者、函式庫的鍵值）。回傳 `Promise` 物件。
...
此方法允許您批次獲取給定`key`陣列對應的項目。回調函數將被調用並傳入一組找到的鍵值對陣列：
...
使用此方法批次儲存多組鍵值對。操作完成時將透過單一回調函數返回任何錯誤：
