...
現在是時候撰寫一些 Android 平台程式碼來實現網頁視圖的渲染。你需要遵循的步驟如下：
...
`ReactWebView` 是封裝 Android 原生視圖的元件，當使用我們的自訂元件時，React Native 會渲染此視圖。
...
該類別定義了三個 Android 建構函式，但將其實際實現委託給私有的 `configureComponent` 函式。此函式負責初始化所有元件特定的屬性：在此範例中，你設定了 `WebView` 的佈局，並定義了用於自訂 `WebView` 行為的 `WebClient`。在這段程式碼中，`ReactWebView` 在頁面載入完成時透過實作 `WebClient` 的 `onPageFinished` 方法來發送事件。

接著程式碼定義了一個輔助函式來實際發送事件。要發送事件，你需要：

- 取得 `ReactContext` 的參考；
- 取得當前視圖的 `surfaceId`；
- 取得與視圖關聯的 `eventDispatcher` 參考；
- 使用 `WritableMap` 物件建立事件的負載；
...
- 實際的 `OnScriptLoadedEvent`，需要繼承 React Native 的 `Event` 類別。
...
`ReactWebViewManager` 繼承了 React 的 `SimpleViewManager` 類別，並實作了由 Codegen 生成的 `CustomWebViewManagerInterface`。
...
然後，視圖管理器需要定義所有 React 元件屬性如何更新原生視圖。在此範例中，你需要決定如何處理 React 將在 `WebView` 上設定的 `sourceURL` 屬性。
