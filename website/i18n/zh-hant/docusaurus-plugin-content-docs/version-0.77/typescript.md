...
[TypeScript][ts] 是一種透過添加類型定義來擴展 JavaScript 的語言。新的 React Native 專案預設採用 TypeScript，但也支援 JavaScript 和 Flow。

## 開始使用 TypeScript

由 [React Native CLI](getting-started-without-a-framework#step-1-creating-a-new-application) 或熱門模板（如 [Ignite][ignite]）建立的新專案將預設使用 TypeScript。
...
> 您應保留 `./index.js` 入口檔案不變，否則在打包生產版本時可能會遇到問題。
...
React Native 預設新專案使用 TypeScript，但仍可使用 JavaScript。副檔名為 `.jsx` 的檔案將被視為 JavaScript 而非 TypeScript，且不會進行類型檢查。JavaScript 模組仍可被 TypeScript 模組導入，反之亦然。

## TypeScript 與 React Native 的運作方式

開箱即用，TypeScript 原始碼在打包時會由 [Babel][babel] 轉換。我們建議僅使用 TypeScript 編譯器進行類型檢查。這是新創建專案中 `tsc` 的預設行為。如果您有現有的 TypeScript 代碼移植到 React Native，使用 Babel 而非 TypeScript 時有[一兩個注意事項][babel-7-caveats]。
