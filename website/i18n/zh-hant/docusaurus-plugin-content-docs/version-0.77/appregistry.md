...
`AppRegistry` 是所有 React Native 應用程式的 JavaScript 進入點。應用程式的根元件應透過 `AppRegistry.registerComponent` 註冊自身，接著原生系統便能載入應用程式的套件包，並在準備就緒時透過呼叫 `AppRegistry.runApplication` 實際運行應用程式。
...
當視圖需要銷毀時，若要「停止」應用程式，請使用傳入 `runApplication` 的標籤呼叫 `AppRegistry.unmountApplicationComponentAtRootTag`。這些方法應始終配對使用。

`AppRegistry` 應在 `require` 序列的早期階段引入，以確保在其他模組被引入前，JavaScript 執行環境已正確設置。
