...
- 在多數整合開發環境中，使用 `StyleSheet.create()` 會提供靜態類型檢查和建議，幫助你撰寫有效的樣式。
...
將兩種樣式組合，使 `style2` 覆蓋 `style1` 中的任何樣式。如果任一樣式為假值，則直接返回另一個樣式而不分配陣列，節省記憶體分配並維持 PureComponent 檢查的引用相等性。
...
設置一個函數用於預處理樣式屬性值。此功能內部用於處理顏色和變換值。除非你確切知道自己在做什麼且已窮盡其他選項，否則不應使用此功能。
...
一個非常常見的模式是創建具有絕對定位和零邊距的覆蓋層（`position: 'absolute', left: 0, right: 0, top: 0, bottom: 0`），因此 `absoluteFill` 可用於方便且減少這些重複樣式的冗餘。如果需要，absoluteFill 可用於在 StyleSheet 中創建自定義條目，例如：
...
有時你可能需要 `absoluteFill` 但稍作調整 - `absoluteFillObject` 可用於在 `StyleSheet` 中創建自定義條目，例如：
...
此常數始終為像素的整數（因此由其定義的線條看起來會很清晰），並會嘗試匹配底層平台上細線的標準寬度。然而，你不應依賴其為固定大小，因為在不同平台和螢幕密度下，其值可能以不同方式計算。

如果你的模擬器被縮小，具有髮絲寬度的線條可能不可見。