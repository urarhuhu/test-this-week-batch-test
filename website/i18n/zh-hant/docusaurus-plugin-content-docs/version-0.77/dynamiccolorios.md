...
`DynamicColorIOS` 接收一個物件作為參數，該物件必須包含 `dark` 和 `light` 兩個必要鍵，以及可選的 `highContrastLight` 和 `highContrastDark` 鍵。這些鍵值對應於您希望在 iOS 的「淺色模式」和「深色模式」下使用的顏色，以及當高對比度無障礙模式啟用時的高對比版本。

在運行時，系統會根據當前的系統外觀和無障礙設定自動選擇要顯示的顏色。動態色彩適用於品牌色彩或其他需要自動響應系統設定變更的應用程式特定色彩。
