...
React Native 提供了一系列用於展示數據列表的組件。通常情況下，您會想要使用 [FlatList](flatlist.md) 或 [SectionList](sectionlist.md)。
...
`FlatList` 組件需要兩個屬性：`data` 和 `renderItem`。`data` 是列表的數據來源，而 `renderItem` 則從數據源中提取一個項目並返回一個格式化後的組件進行渲染。
...
如果您想要渲染一組按邏輯分段的數據，可能需要加上分段標題（類似 iOS 的 `UITableView`），那麼 [SectionList](sectionlist.md) 就是您需要的解決方案。
