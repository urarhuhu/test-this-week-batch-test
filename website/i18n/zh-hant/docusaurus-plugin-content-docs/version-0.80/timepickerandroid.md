...
回傳一個 Promise，當使用者選擇時間時，將以包含 `action`、`hour` (0-23)、`minute` (0-59) 的物件呼叫。若使用者關閉對話框，Promise 仍會以 `action` 為 `TimePickerAndroid.dismissedAction` 且其他鍵值為未定義的方式解析。**務必**在讀取值之前檢查 `action`。
