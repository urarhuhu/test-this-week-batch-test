...
用於使視圖正確響應觸控操作的包裝元件。按下時，被包裝視圖的不透明度會降低，使底色顯現，從而讓視圖變暗或著色。
...
決定觸控激活時被包裹視圖的不透明度值。該值應介於 0 和 1 之間，預設為 0.85。需設定 `underlayColor` 方可生效。
