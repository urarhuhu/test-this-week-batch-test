...
`DynamicColorIOS` 接收一個物件作為參數，該物件必須包含兩個必要鍵值：`dark` 和 `light`，以及兩個可選鍵值 `highContrastLight` 和 `highContrastDark`。這些鍵值對應於 iOS 上「淺色模式」和「深色模式」要使用的顏色，以及當高對比度無障礙模式啟用時的高對比版本顏色。
