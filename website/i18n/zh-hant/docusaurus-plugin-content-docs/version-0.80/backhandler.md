...
> **模態視窗使用者請注意**：若您的應用程式顯示開啟中的 `Modal`，`BackHandler` 將不會發布任何事件（參見 [`Modal` 文件](modal#onrequestclose)）。
...
若您使用 React Navigation 進行畫面導航，可參考其[自訂 Android 返回按鈕行為](https://reactnavigation.org/docs/custom-android-back-button-handling/)指南。
