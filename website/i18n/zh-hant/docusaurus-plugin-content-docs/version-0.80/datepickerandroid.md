...
回傳一個 Promise，當使用者選擇日期時，將以包含 `action`、`year`、`month`（0-11）、`day` 的物件呼叫。若使用者關閉對話框，Promise 仍會以 `action` 為 `DatePickerAndroid.dismissedAction` 且其他鍵值為 undefined 的物件解析。**務必**在讀取值之前檢查 `action` 是否等於 `DatePickerAndroid.dateSetAction`。
