...
[`flex`](layout-props#flex) 屬性決定了子元素如何沿主軸**「填滿」**可用空間。空間將根據每個元素的 flex 屬性進行分配。
...
佈局 [`direction`](layout-props#direction) 指定了子元素和文字在層級結構中的排列方向，同時影響 `start` 和 `end` 所指的邊緣。預設情況下，React Native 採用 LTR（左至右）佈局方向，此時 `start` 指左側，`end` 指右側。
...
[`justifyContent`](layout-props#justifycontent) 描述如何沿容器的主軸對齊子元素。例如，你可以使用此屬性在 `flexDirection` 設為 `row` 的容器中水平置中子元素，或在 `flexDirection` 設為 `column` 的容器中垂直置中子元素。
...
[`alignItems`](layout-props#alignitems) 描述如何沿容器的交叉軸對齊子元素。它與 `justifyContent` 非常相似，但作用於交叉軸而非主軸。
...
:::info
要使 `stretch` 生效，子元素在次要軸上不能有固定尺寸。在以下範例中，除非移除子元素的 `width: 50`，否則設置 `alignItems: stretch` 不會產生效果。
:::
...
[`alignSelf`](layout-props#alignself) 的選項和效果與 `alignItems` 相同，但它是針對單個子元素設置，以改變其在父容器內的對齊方式。`alignSelf` 會覆蓋父容器通過 `alignItems` 設置的任何選項。
...
[alignContent](layout-props#aligncontent) 定義沿交叉軸的多行分布方式。僅在使用 `flexWrap` 換行時有效。
...
- `space-between` 沿交叉軸均勻分布換行後的內容，剩餘空間分配在行與行之間。

- `space-around` 沿交叉軸均勻分布換行後的內容，剩餘空間環繞各行分配。容器兩端的空間大小為行間空間的一半。

- `space-evenly` 沿交叉軸完全均勻分布換行後的內容，所有間距（包括容器兩端）大小相同。
...
[`flexWrap`](layout-props#flexwrap) 屬性設置在容器上，用於控制當子元素在主軸方向上超出容器尺寸時的處理方式。預設情況下，子元素會被強制排列在單一行內（這可能會壓縮元素）。如果允許換行，則會根據需要將項目沿主軸方向換行排列成多行。
...
- [`flexBasis`](layout-props#flexbasis) 是一種與軸向無關的方式，用於設定項目在主軸上的預設尺寸。設定子元素的 `flexBasis` 類似於在父容器為 `flexDirection: row` 時設定該子元素的 `width`，或在父容器為 `flexDirection: column` 時設定其 `height`。項目的 `flexBasis` 是該項目的預設尺寸，即在進行任何 `flexGrow` 和 `flexShrink` 計算之前的尺寸。

- [`flexGrow`](layout-props#flexgrow) 描述容器內的主軸空間應如何在其子元素之間分配。在佈局其子元素後，容器會根據子元素指定的 flex grow 值分配剩餘空間。

  `flexGrow` 接受任何大於或等於 0 的浮點數值，預設值為 0。容器會根據子元素的 `flexGrow` 值加權分配剩餘空間。

- [`flexShrink`](layout-props#flexshrink) 描述當子元素的總尺寸在主軸上超出容器尺寸時，如何沿主軸縮小子元素。`flexShrink` 與 `flexGrow` 非常相似，可以將任何溢出的尺寸視為負的剩餘空間來理解。這兩個屬性也能很好地協同工作，允許子元素根據需要進行擴展和收縮。

  `flexShrink` 接受任何大於或等於 0 的浮點數值，預設值為 0（在網頁上，預設值為 1）。容器會根據子元素的 `flexShrink` 值加權縮小子元素。
...
- `static` 當元素採用靜態定位時，它會依照正常的佈局流排列，並忽略 `top`、`right`、`bottom` 和 `left` 的值。此 `position` 設定也會導致元素不會為其絕對定位的子元素形成包含區塊，除非有其他優先的樣式屬性存在（例如 `transform`）。這允許絕對定位元素可以相對於非父元素的物件定位。注意：**`static` 僅在新架構（New Architecture）中可用**。
...
React Native 中「包含區塊」的運作方式與 [網頁上的行為](https://developer.mozilla.org/en-US/docs/Web/CSS/Containing_block) 非常相似，但由於缺少某些網頁特性而有所簡化。
...
- 若該元素的 `position` 類型為 `absolute`，則其包含區塊為滿足以下任一條件的最接近祖先元素：
  - 其 `position` 類型不為 `static`
  - 其設有 `transform` 屬性

## 深入探討

你可以透過互動式的 [yoga playground](https://www.yogalayout.dev/playground) 來更深入理解 flexbox 的運作方式。

我們已涵蓋基礎知識，但佈局可能還需要許多其他樣式。完整控制佈局的屬性列表可參考 [此文件](./layout-props.md)。

此外，你也可以查看 [Wix 工程師](https://medium.com/wix-engineering/the-full-react-native-layout-cheat-sheet-a4147802405c) 提供的一些範例。