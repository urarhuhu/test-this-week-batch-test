...
裝置名稱對應 Xcode 中可用的裝置列表。可透過終端機執行 `xcrun simctl list devices` 來查看可用裝置。
...
您可指定從 `xcrun simctl list devices` 指令返回的裝置 UDID。例如：要使用 UDID `AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA` 運行應用程式，請執行以下指令：
