...
`<Text>` 元素在佈局上是獨特的：其內部不再使用 Flexbox 佈局，而是使用文字佈局。這意味著 `<Text>` 內部的元素不再是矩形，而是在看到行尾時會自動換行。
...
您也失去了為整個子樹設置預設字體的能力。同時，`fontFamily` 僅接受單一字體名稱，這與 CSS 中的 `font-family` 不同。建議在應用程式中使用一致的字體和大小的方式是創建一個包含這些樣式的元件 `MyAppText`，並在整個應用程式中使用此元件。您還可以使用此元件來創建更具體的元件，如 `MyAppHeaderText` 用於其他類型的文字。
...
以這種方式組合 `MyAppText` 確保我們從頂層元件獲取樣式，同時保留了在特定使用情況下添加/覆蓋它們的能力。
...
在 iOS 上，這些角色映射到對應的輔助功能特徵。圖片按鈕具有與同時設置為「image」和「button」特徵相同的功能。詳見 [輔助功能指南](accessibility.md#accessibilitytraits-ios) 以獲取更多資訊。

在 Android 上，這些角色在 TalkBack 上的功能類似於 iOS 中 Voiceover 添加輔助功能特徵的功能。
...
輔助功能操作允許輔助技術以程式方式呼叫元件的操作。`accessibilityActions` 屬性應包含一個操作物件列表。每個操作物件應包含欄位名稱和標籤。
...
當使用者執行輔助功能操作時呼叫。該函數的唯一參數是一個包含要執行操作名稱的事件。
...
設置在 Android API 等級 23+ 上確定斷字時使用的自動斷字頻率。
...
此視圖是否想要「聲明」觸控響應？當視圖不是響應者時，每次觸控移動都會調用此函數。
...
視圖現在正在響應觸控事件。此時應高亮顯示並向用戶展示正在發生的操作。

在 Android 上，從此回調返回 true 可阻止任何其他原生組件成為響應者，直到此響應者終止。
...
響應者已從視圖中移除。可能是在調用 `onResponderTerminationRequest` 後被其他視圖接管，也可能是在未經詢問的情況下被操作系統接管（例如 iOS 上的控制中心/通知中心）。
...
當其他 `View` 試圖成為觸控回應者並要求此 `View` 釋放回應權時觸發。返回 `true` 表示允許釋放。
...
若父層 `View` 需阻止子層 `View` 在觸碰開始時成為回應者，應設置此處理函數並返回 `true`。
...
當滾動視圖禁用時，此屬性定義觸碰點可偏離按鈕多遠才會停用按鈕。停用後若移回範圍內會重新啟用。建議傳入常數以減少記憶體分配。
...
設為 `true` 時，文字被按下不會有視覺變化。預設情況下按下會顯示灰色橢圓高亮。
