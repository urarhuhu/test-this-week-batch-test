...
現在是時候編寫一些 iOS 平台代碼，以確保 `localStorage` 在應用程式關閉後仍然保留。

## 準備你的 Xcode 專案

我們需要使用 Xcode 準備你的 iOS 專案。完成以下 **6 個步驟** 後，你將擁有實現生成的 `NativeLocalStorageSpec` 接口的 `RCTNativeLocalStorage`。
...
最後一步是更新 `package.json`，告訴 React Native 關於原生模組的 JS 規範與原生代碼中這些規範的具體實現之間的連結。
...
此時，你需要重新安裝 pods，以確保 codegen 再次運行並生成新文件：
