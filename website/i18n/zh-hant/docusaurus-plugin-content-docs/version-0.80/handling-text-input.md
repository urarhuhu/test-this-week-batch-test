...
在此範例中，我們將 `text` 儲存於 state 中，因為它會隨時間變化。

文字輸入還有許多其他應用方式。例如，您可以在使用者輸入時驗證文字內容。更詳細的範例請參閱 [React 受控元件文件](https://react.dev/reference/react-dom/components/input#controlling-an-input-with-a-state-variable) 或 [TextInput 參考文檔](textinput.md)。
