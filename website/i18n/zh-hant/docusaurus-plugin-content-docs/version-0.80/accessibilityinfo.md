...
有時了解裝置當前是否有啟用螢幕閱讀器非常實用。`AccessibilityInfo` API 正是為此目的設計。您可以用它查詢螢幕閱讀器的當前狀態，並註冊在狀態變更時接收通知。
...
發送字串供螢幕閱讀器朗讀，並可設定選項。預設會中斷當前語音，但在 iOS 可透過選項物件設定 `queue` 為 `true` 來排隊等候當前語音結束。
...
檢查是否啟用任何無障礙服務。包含 TalkBack 及可能安裝的第三方無障礙應用程式。若僅需檢查 TalkBack 狀態，請使用 [isScreenReaderEnabled](#isscreenreaderenabled)。回傳一個解析為布林值的 Promise，當有無障礙服務啟用時結果為 `true`，否則為 `false`。
...
查詢粗體文字功能是否啟用。回傳一個解析為布林值的 Promise，當粗體文字啟用時結果為 `true`，否則為 `false`。
...
查詢灰階模式是否啟用。回傳一個解析為布林值的 Promise，當灰階模式啟用時結果為 `true`，否則為 `false`。
...
查詢反轉顏色功能是否啟用。回傳一個解析為布林值的 Promise，當反轉顏色啟用時結果為 `true`，否則為 `false`。
...
查詢減少動畫效果是否啟用。回傳一個解析為布林值的 Promise，當減少動畫效果啟用時結果為 `true`，否則為 `false`。
...
查詢當前是否啟用了減少動畫效果並偏好使用淡入淡出過場設定。回傳一個解析為布林值的 Promise，當偏好淡入淡出過場設定啟用時結果為 `true`，否則為 `false`。
