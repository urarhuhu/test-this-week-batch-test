...
當圖片具有圓角時，指定 overlayColor 會使角落剩餘空間填充純色。這在以下 Android 圓角實作不支援的情況特別有用：

- 特定縮放模式（如 `'contain'`）
- 動態 GIF

典型用法是將圖片顯示在純色背景上，並將 `overlayColor` 設為與背景相同的顏色。

關於底層運作原理，請參閱 [Fresco 文件](https://frescolib.org/docs/rounded-corners-and-circles.html)。
...
- `cover`: 均勻縮放圖片（保持圖片的寬高比），使得：
  ...
  - 圖片的寬度和高度都會等於或大於視圖的對應尺寸（減去內邊距）
  - 縮放後的圖片至少有一個維度會等於視圖的對應尺寸（減去內邊距）

- `contain`: 均勻縮放圖片（保持圖片的寬高比），使得圖片的寬度和高度都會等於或小於視圖的對應尺寸（減去內邊距）。
