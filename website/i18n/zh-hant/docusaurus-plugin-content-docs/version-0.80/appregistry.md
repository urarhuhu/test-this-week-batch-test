...
若要「停止」應用程式（當視圖需要銷毀時），請調用 `AppRegistry.unmountApplicationComponentAtRootTag` 並傳入當初傳給 `runApplication` 的標籤參數。這兩者應始終配對使用。

`AppRegistry` 應在 `require` 序列的早期階段引入，以確保 JavaScript 執行環境在其他模組載入前已完成設定。
...
註冊可取消的無介面任務。無介面任務是指不需使用者介面即可執行的程式片段。
...
註冊無介面任務。無介面任務是指不需使用者介面即可執行的程式片段。

此機制允許應用程式在背景狀態下執行 JavaScript 任務，例如同步最新資料、處理推播通知或播放音樂。
