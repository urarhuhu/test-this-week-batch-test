...
使用 React Native 時，您的 JavaScript 程式碼可能會在三種環境中運行：

- 多數情況下，React Native 會使用 [Hermes](hermes)，這是一個專為 React Native 優化的開源 JavaScript 引擎。
- 如果停用 Hermes，React Native 會使用 [JavaScriptCore](https://trac.webkit.org/wiki/JavaScriptCore)，這是 Safari 瀏覽器採用的 JavaScript 引擎。請注意，在 iOS 上由於應用程式缺乏可寫入的執行記憶體，JavaScriptCore 不會使用 JIT 編譯。
- 當啟用 Chrome 調試功能時，所有 JavaScript 程式碼會在 Chrome 瀏覽器內部執行，並透過 WebSockets 與原生程式碼通訊。Chrome 使用 [V8](https://v8.dev/) 作為其 JavaScript 引擎。

雖然這些環境非常相似，但您可能會遇到一些不一致的情況。建議避免依賴任何特定運行環境的特性。

## JavaScript 語法轉換器

語法轉換器讓編寫程式碼更加愉悅，允許您使用新的 JavaScript 語法，而無需等待所有直譯器的支援。

React Native 內建了 [Babel JavaScript 編譯器](https://babeljs.io)。詳情請查閱 [Babel 文件](https://babeljs.io/docs/plugins/#transform-plugins) 中關於其支援的轉換功能。

React Native 啟用的完整轉換清單可在 [@react-native/babel-preset](https://github.com/facebook/react-native/tree/main/packages/react-native-babel-preset) 中找到。
...
許多標準函式在所有支援的 JavaScript 運行環境中都可使用。
