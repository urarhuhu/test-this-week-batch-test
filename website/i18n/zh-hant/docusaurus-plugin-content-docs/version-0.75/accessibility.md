...
當設為 `true` 時，表示該視圖是一個無障礙元素。當視圖是無障礙元素時，它會將其子元素群組為一個可選取的元件。預設情況下，所有可觸碰元素都是無障礙的。
...
當視圖被標記為無障礙時，最好在視圖上設置一個 `accessibilityLabel`，這樣使用 VoiceOver 或 TalkBack 的使用者就能知道他們選擇了什麼元素。螢幕閱讀器會在選取相關元素時朗讀此字串。
...
在上面的例子中，TouchableOpacity 元素的 `accessibilityLabel` 預設為「Press me!」。此標籤是透過串接所有 Text 節點子元素並以空格分隔來構建的。
...
當僅靠無障礙標籤無法清楚表達動作的結果時，可以使用無障礙提示來為使用者提供額外的上下文。
...
在上面的例子中，如果使用者在裝置的 VoiceOver 設定中啟用了提示，VoiceOver 會在標籤之後朗讀提示。有關 `accessibilityHint` 的指南，請參閱 [iOS 開發者文件](https://developer.apple.com/documentation/objectivec/nsobject/1615093-accessibilityhint)。
...
反轉螢幕顏色是 iOS 和 iPadOS 中為色盲、低視力或視力障礙使用者提供的無障礙功能。如果有您不想在此設定開啟時反轉的視圖（例如照片），請將此屬性設為 `true`。
...
上述範例中，`addOne` 方法會改變狀態變數 `count`。當 TouchableWithoutFeedback 被觸發時，由於 Text 視圖設有 `accessibilityLiveRegion="polite"` 屬性，TalkBack 會朗讀其中的文字。
...
- **timer** 用於表示計時器。
- **togglebutton** 用於表示切換按鈕，需搭配 accessibilityState 的 checked 屬性指示切換狀態。
- **toolbar** 用於表示工具列（動作按鈕或元件的容器）。
- **grid** 與 ScrollView、VirtualizedList、FlatList 或 SectionList 搭配使用表示網格，為 Android 的 GridView 添加網格內/外公告。
...
表示元素將被更新，並描述使用者代理、輔助技術和使用者可以從動態區域中預期的更新類型。
...
在上述範例中，`yellow` 佈局及其子元素對 TalkBack 和其他無障礙服務完全不可見。因此，我們可以使用同父節點的重疊視圖而不會混淆 TalkBack。

### `onAccessibilityEscape` <div class="label ios">iOS</div>

將此屬性賦予一個自定義函數，當用戶執行「返回」手勢（雙指 Z 形手勢）時會調用該函數。返回函數應在用戶界面中按層級向上移動，例如在導航層級中返回或關閉模態界面。如果選定元素沒有 `onAccessibilityEscape` 函數，系統會嘗試向上遍歷視圖層級，直到找到具有該函數的視圖，或發出提示音表示未找到。
...
將此屬性賦予一個自定義函數，當用戶執行「魔法點擊」手勢（雙指雙擊）時會調用該函數。魔法點擊函數應執行用戶在組件上最相關的操作。例如 iPhone 上的電話應用中，魔法點擊可接聽或結束通話。如果選定元素沒有 `onMagicTap` 函數，系統會向上遍歷視圖層級直到找到具有該函數的視圖。
...
- **scrollbar** 用於表示捲軸。
- **searchbox** 當文字輸入框同時應被視為搜尋欄位時使用。
- **slider** 當元素可被「調整」時使用（例如滑桿）。
- **spinbutton** 用於表示可打開選項清單的按鈕。
...
`accessibilityActions` 屬性應包含一個操作物件的清單。每個操作物件應包含以下欄位：
...
- `'longpress'` - 僅限 Android - 當使用者將輔助功能焦點置於元件上，然後雙擊並在螢幕上按住一根手指時，會生成此動作。無論是否使用輔助技術，此動作應執行相同的操作。
...
`AccessibilityInfo` API 允許您確定螢幕閱讀器當前是否處於活動狀態。詳情請參閱 [AccessibilityInfo 文檔](accessibilityinfo)。
...
要在您的 iOS 或 iPadOS 裝置上啟用 VoiceOver，請前往「設定」應用，點擊「一般」，然後點擊「輔助功能」。在那裡，您會找到許多工具，供人們啟用以使其裝置更易於使用，包括 VoiceOver。要啟用 VoiceOver，請點擊「視覺」下的「VoiceOver」，然後切換頂部出現的開關。
