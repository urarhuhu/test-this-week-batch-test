...
如果使用者先前關閉了某個權限提示，系統會建議您的應用程式說明需要該權限的理由。可選的 `rationale` 參數僅在必要時顯示對話框提示——否則將顯示正常的權限提示。
...
向用戶請求啟用權限，並回傳一個解析為字串值的 Promise（參見上方的結果字串），表示用戶是允許、拒絕該請求，或選擇不再詢問。

若提供 `rationale`，此函式會向作業系統確認是否需要顯示對話框說明為何需要該權限（參見 https://developer.android.com/training/permissions/requesting.html#explain），然後顯示系統權限對話框。
...
在同一個對話框中向用戶請求啟用多個權限，並回傳一個物件，其鍵為權限名稱，值為字串（參見上方的結果字串），表示用戶是允許、拒絕該請求，或選擇不再詢問。
