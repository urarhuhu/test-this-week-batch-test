...
> 對於 React 元件來說，[`useWindowDimensions`](usewindowdimensions) 是更推薦的 API。與 `Dimensions` 不同，它會隨著視窗尺寸的變化而自動更新，這與 React 的設計模式更加契合。
...
> 雖然尺寸資訊可以立即取得，但它們可能會發生變化（例如由於設備旋轉、折疊式設備等），因此任何依賴這些常數的渲染邏輯或樣式都應該在每次渲染時調用此函數，而不是快取值（例如使用內聯樣式而非在 `StyleSheet` 中設定值）。

如果您針對的是折疊式設備或可以改變螢幕尺寸或應用視窗大小的設備，可以使用 Dimensions 模組中的事件監聽器，如下例所示。
