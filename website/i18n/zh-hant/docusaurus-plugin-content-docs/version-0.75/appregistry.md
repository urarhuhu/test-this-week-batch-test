...
當需要「停止」應用程式以銷毀視圖時，請呼叫 `AppRegistry.unmountApplicationComponentAtRootTag` 並傳入當初傳給 `runApplication` 的標籤。這兩者應始終配對使用。
...
`Task` 是一個接受任意數據作為參數並返回解析為 `undefined` 的 Promise 的函式。
...
`TaskCanceller` 是一個不接受參數且無返回值的函式。
...
有效的 `TaskCancelProvider` 是一個返回 [`TaskCanceller`](appregistry#taskcanceller) 的函式。
...
有效的 `TaskProvider` 是一個返回 [`Task`](appregistry#task) 的函式。
