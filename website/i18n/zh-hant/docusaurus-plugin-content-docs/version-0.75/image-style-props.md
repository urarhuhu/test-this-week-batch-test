...
當圖片具有圓角時，指定 `overlayColor` 會使角落的剩餘空間填充為純色。這在以下 Android 圓角實現不支援的情況下特別有用：
...
- `cover`: 均勻縮放圖片（保持圖片的寬高比），使得：
  ...
  - 圖片的寬度和高度都會大於或等於視圖的對應尺寸（減去內邊距）
  - 縮放後的圖片至少有一個維度會等於視圖的對應尺寸（減去內邊距）

- `contain`: 均勻縮放圖片（保持圖片的寬高比），使得圖片的寬度和高度都會小於或等於視圖的對應尺寸（減去內邊距）。

- `stretch`: 獨立縮放寬度和高度，這可能會改變原始圖片的寬高比。

- `repeat`: 重複圖片以填滿視圖的框架。圖片將保持其大小和寬高比，除非它大於視圖，在這種情況下它將被均勻縮小以使其包含在視圖中。
