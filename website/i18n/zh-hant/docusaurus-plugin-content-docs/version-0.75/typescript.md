...
透過 [React Native CLI](getting-started-without-a-framework#step-1-creating-a-new-application) 或熱門模板（如 [Ignite][ignite]）建立的專案將預設使用 TypeScript。

TypeScript 也可與 [Expo][expo] 搭配使用，後者維護了 TypeScript 模板，或在專案中加入 `.ts` 或 `.tsx` 檔案時會自動提示安裝並配置 TypeScript。
...
1. 將 TypeScript、型別定義及 ESLint 插件加入您的專案。
...
:::note
此命令會添加所有依賴項的最新版本。可能需要調整版本以匹配專案現有套件。您可使用 [React Native Upgrade Helper](https://react-native-community.github.io/upgrade-helper/) 等工具查看 React Native 內建的版本。
:::

2. 添加 TypeScript 設定檔。在專案根目錄建立 `tsconfig.json`：
...
> 應保留 `./index.js` 入口檔案的原命名，否則可能在打包生產版本時遇到問題。

4. 執行 `tsc` 進行 TypeScript 檔案的型別檢查。
...
預設情況下，TypeScript 原始碼會在打包時由 [Babel][babel] 轉換。建議僅使用 TypeScript 編譯器進行型別檢查，這是新建專案中 `tsc` 的預設行為。若需將現有 TypeScript 程式碼移植到 React Native，使用 Babel 替代 TypeScript 時需注意 [一兩個注意事項][babel-7-caveats]。

## React Native + TypeScript 實例展示

您可以透過 `React.Component<Props, State>` 為 React 元件的 [Props](props) 和 [State](state) 提供介面，這將在 JSX 中使用該元件時啟用型別檢查與編輯器自動完成功能。
...
您可在 [TypeScript 遊樂場][tsplay] 進一步探索語法。

## 實用資源推薦

- [TypeScript 手冊][ts-handbook]
- [React 官方 TypeScript 文檔][react-ts]
- [React + TypeScript 速查表][cheat] 提供 React 與 TypeScript 搭配使用的完整概述

## 在 TypeScript 中使用自訂路徑別名

要讓自訂路徑別名同時在 Babel 和 TypeScript 中生效，需進行以下設定：

1. 編輯 `tsconfig.json` 設定 [自訂路徑映射][path-map]。將 `src` 根目錄下的內容設為無前綴路徑即可訪問，並允許透過 `tests/File.tsx` 引用測試檔案：
