...
若 React Native 應用程式在正式版建置中拋出未捕獲的例外，輸出內容可能經過混淆而難以閱讀。
...
您可透過將堆疊追蹤與產生的原始碼對應表傳遞給 [`metro-symbolicate`](http://npmjs.com/package/metro-symbolicate)，來符號化上述的最小化函式名稱與位元組碼。

### 啟用原始碼對應表

符號化堆疊追蹤需使用原始碼對應表。請確認目標平台的建置配置中已啟用原始碼對應表生成功能。
...
- 建置過程可能生成多份原始碼對應表，請確保使用範例中指定路徑的版本。
- 使用的原始碼對應表必須與當機應用程式的提交版本完全對應，原始碼的微小變更可能導致偏移量大幅差異。
- 若 `metro-symbolicate` 立即成功退出，請確認輸入來源為管道或重定向而非終端機直接輸入。