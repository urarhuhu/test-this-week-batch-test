...
輔助函數，創建一個包含`create`、`update`和`delete`字段的對象，以傳入[`configureNext`](layoutanimation.md#configurenext)。`type`參數是[動畫類型](layoutanimation.md#types)，`creationProp`參數是[佈局屬性](layoutanimation.md#properties)。
