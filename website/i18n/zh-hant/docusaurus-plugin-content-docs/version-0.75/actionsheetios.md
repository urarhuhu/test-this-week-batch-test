...
關閉最上層的 iOS 動作選單，若無動作選單顯示則會發出警告。
...
- `message` (字串) - 要分享的訊息
- `subject` (字串) - 訊息的標題
- `excludedActivityTypes` (陣列) - 要從動作選單排除的活動類型
...
'failureCallback' 函式接收一個參數，即錯誤物件。此物件唯一定義的屬性是選用的 `stack` 屬性，類型為 `字串`。
