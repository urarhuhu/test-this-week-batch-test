...
回傳一個布林值，定義裝置是否為 Apple Vision。_若您使用 [Apple Vision Pro (專為 iPad 設計)](https://developer.apple.com/documentation/visionos/checking-whether-your-app-is-compatible-with-visionos)，`isVision` 會是 `false` 但 `isPad` 會是 `true`_
...
回傳一個布林值，定義應用程式是否在開發者模式下執行且設定了測試標記。
...
select 方法會回傳最適合當前執行平台的數值。也就是說，若您在手機上執行，會優先採用 `android` 和 `ios` 鍵值。若未指定這些鍵值，則會使用 `native` 鍵值，接著是 `default` 鍵值。
...
這將導致容器在所有平台上具有 `flex: 1`，在 Android 上具有綠色背景色，在 iOS 上具有紅色背景色，在其他平台上則具有藍色背景色。

由於對應平台鍵值的類型可以是 `any`，[`select`](platform.md#select) 方法也可用於回傳平台特定的元件，如下所示：
