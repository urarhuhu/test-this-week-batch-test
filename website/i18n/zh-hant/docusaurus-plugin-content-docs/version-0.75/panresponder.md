...
`PanResponder` 將多個觸控點協調為單一手勢。它使單點觸控手勢能抵禦額外觸點的干擾，並可用於識別基礎的多點觸控手勢。
...
`PanResponder` 與 `Animated` API 協同工作，幫助構建 UI 中的複雜手勢。以下範例包含一個可動畫化的 `View` 組件，可在螢幕上自由拖曳：
...
`config` 物件提供了所有響應回調的增強版本，不僅包含 [`PressEvent`](pressevent)，還包含 `PanResponder` 手勢狀態。具體方式是將典型 `onResponder*` 回調中的 `Responder` 替換為 `PanResponder`。例如，`config` 物件可能如下所示：
