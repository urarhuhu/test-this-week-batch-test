...
在 iOS 平台上，可自訂背景顏色。當開關值為 `false` 或開關被禁用時（且開關呈半透明狀態），此背景顏色會顯現。
...
_iOS_：當開關值為 `false` 時，軌道會向邊界收縮。若要調整收縮軌道露出的背景顏色，請使用 [`ios_backgroundColor`](switch.md#ios_backgroundColor)。
...
開關的數值。若為 true 則開關會呈開啟狀態。預設值為 false。
