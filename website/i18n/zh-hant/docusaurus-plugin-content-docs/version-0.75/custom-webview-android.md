...
雖然內建的 WebView 具備許多功能，但 React Native 無法涵蓋所有使用情境。不過，您無需複製現有 WebView 程式碼或分叉 React Native，即可透過原生程式碼擴展 WebView 功能。
...
進行此操作前，您應熟悉 [原生 UI 元件](native-components-android) 的概念。同時建議預先了解 [WebView 的原生程式碼](https://github.com/react-native-webview/react-native-webview/blob/master/android/src/main/java/com/reactnativecommunity/webview/RNCWebViewManagerImpl.kt)，雖然不需要深入理解，但在實作新功能時需將其作為參考。
...
您可在 WebView 客戶端觸發事件。若事件基於現有處理程序，可掛接這些處理程序。
...
最後需透過 `getExportedCustomDirectEventTypeConstants` 在 `CustomWebViewManager` 中公開事件。請注意，目前預設實作會返回 `null`，但未來可能變更。
...
取得原生元件時，需與常規自訂元件相同使用 `requireNativeComponent`，但需傳入額外的第三參數 `WebView.extraNativeComponentConfig`。此參數包含僅原生程式碼需要的屬性類型。
...
對於事件處理，事件處理器必須始終設定為一個函式。這意味著直接從 `this.props` 使用事件處理器並不安全，因為使用者可能沒有提供。標準做法是在你的類別中建立一個事件處理器，然後在 `this.props` 中提供的事件處理器存在時調用它。
...
與常規的原生元件類似，你必須在元件中提供所有的屬性類型，以便將它們轉發到原生元件。然而，如果有些屬性類型僅在元件內部使用，你可以將它們添加到前面提到的第三個參數的 `nativeOnly` 屬性中。對於事件處理器，你必須使用值 `true` 而不是常規的屬性類型。

例如，如果你想添加一個名為 `onScrollToBottom` 的內部事件處理器，你會這樣使用：
