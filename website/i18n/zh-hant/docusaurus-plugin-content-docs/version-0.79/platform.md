...
`select` 方法會回傳最適合當前執行平台的值。也就是說，如果您在手機上執行，會優先使用 `android` 和 `ios` 鍵值。若未指定這些鍵值，則會使用 `native` 鍵值，接著是 `default` 鍵值。
...
這會讓容器在所有平台上都有 `flex: 1`，在 Android 上背景為綠色，在 iOS 上背景為紅色，其他平台則為藍色背景。
