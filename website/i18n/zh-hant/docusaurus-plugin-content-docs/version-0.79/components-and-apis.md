...
React Native 提供了許多內建的[核心組件](intro-react-native-components)，可直接用於您的應用程式。您可以在左側邊欄（或窄螢幕上方的選單）中找到所有組件。如果不確定從何開始，請參考以下分類：
...
您不僅限於使用 React Native 內建的組件和 API。React Native 擁有數千名開發者組成的社群。如需尋找特定功能的函式庫，請參閱[尋找函式庫指南](libraries#finding-libraries)。

## 基礎組件

大多數應用程式最終會使用一個或多個這些基礎組件。
...
與更通用的 [`ScrollView`](./scrollview) 不同，以下列表視圖組件僅渲染當前顯示在螢幕上的元素。這使它們成為顯示長數據列表的高效能選擇。
...
這些組件可能對特定應用程式有用。如需完整的組件和 API 列表，請查看左側邊欄（或窄螢幕上方的選單）。
