...
在向用戶發布應用程式之前，實際裝置測試始終是個好主意。本文件將引導您完成必要的步驟，以便在裝置上運行您的 React Native 應用程式並為生產環境做好準備。

:::info
若您使用 `create-expo-app` 建立專案，可透過掃描執行 `npm start` 時顯示的 QR Code，在 Expo Go 中於裝置上運行應用程式。詳情請參閱 Expo 的[在裝置上運行專案](https://docs.expo.dev/get-started/expo-go/)指南。
:::
