...
手指並非最精確的工具，使用者常會意外啟動錯誤元素或錯過啟動區域。為此，`Pressable` 提供可選的 `HitRect`，可用來定義觸碰在包裝元素多遠距離外仍能註冊。按壓可在 `HitRect` 範圍內的任何位置開始。

`PressRect` 允許按壓在維持啟動狀態的同時，移動超出元素及其 `HitRect` 範圍，並仍有資格成為「按壓」——想像您按住按鈕時慢慢滑開手指的情況。

> 觸碰區域絕不會超出父視圖邊界，且若觸碰同時命中兩個重疊視圖，兄弟視圖的 Z-index 永遠優先。
...
在此元件外部的額外距離範圍內，觸碰仍會被視為按壓狀態，直到觸發 `onPressOut`。
