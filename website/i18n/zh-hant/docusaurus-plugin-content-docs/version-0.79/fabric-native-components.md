...
您的規格必須使用 [TypeScript](https://www.typescriptlang.org/) 或 [Flow](https://flow.org/) 定義（詳見 [Codegen](the-new-architecture/what-is-codegen) 文件）。Codegen 會根據此規格生成 C++、Objective-C++ 和 Java 程式碼，將您的平台程式碼與 React 運行的 JavaScript 運行時連接。

規格文件必須命名為 `<模組名稱>NativeComponent.{ts|js}` 才能與 Codegen 配合使用。後綴 `NativeComponent` 不僅是慣例，更是 Codegen 識別規格文件的關鍵字。
...
請注意，在 iOS 端我們需明確將規格導出的 JS 元件名稱（`CustomWebView`）與實作該元件的原生 iOS 類別進行映射。

## 2. 建置原生程式碼

現在是時候撰寫原生平台程式碼，以便當 React 需要渲染視圖時，平台能夠建立正確的原生視圖並將其渲染到畫面上。
