...
- `duration` 以毫秒為單位
- `create`，用於動畫新增視圖的配置（可選）
- `update`，用於動畫更新視圖的配置（可選）
- `delete`，用於動畫移除視圖的配置（可選）
...
輔助函數，創建一個包含`create`、`update`和`delete`字段的對象，供傳入[`configureNext`](layoutanimation.md#configurenext)使用。`type`參數是[動畫類型](layoutanimation.md#types)，`creationProp`參數是[佈局屬性](layoutanimation.md#properties)。
...
動畫類型的枚舉，用於[`create`](layoutanimation.md#create)方法，或[`configureNext`](layoutanimation.md#configurenext)的`create`/`update`/`delete`配置中。（用法示例：`LayoutAnimation.Types.easeIn`）
