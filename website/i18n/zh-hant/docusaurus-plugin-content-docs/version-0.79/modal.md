...
模態框的背景顏色（或模態框容器的背景色）。若未提供且 `transparent` 為 `false`，則預設為 `white`。若 `transparent` 為 `true` 則此屬性無效。
...
`navigationBarTranslucent` 屬性決定模態框是否應延伸至系統導航列下方。但需同時將 `statusBarTranslucent` 設為 `true` 才能使導航列透明。
...
`onOrientationChange` 回調函式會在模態框顯示期間設備方向改變時被呼叫。提供的方向僅為 'portrait'（豎屏）或 'landscape'（橫屏）。此回調也會在初始渲染時呼叫，無論當前方向為何。
...
當用戶在 Android 上點擊硬體返回按鈕或在 Apple TV 上點擊選單按鈕時，會呼叫 `onRequestClose` 回調函式。由於此屬性是必需的，請注意只要模態框處於開啟狀態，`BackHandler` 事件將不會被觸發。
...
`presentationStyle` 屬性控制模態框的顯示方式（通常在較大設備如 iPad 或加大版 iPhone 上）。詳見 https://developer.apple.com/reference/uikit/uimodalpresentationstyle。
