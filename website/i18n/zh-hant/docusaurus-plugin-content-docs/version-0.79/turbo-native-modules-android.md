...
現在需要編寫一些 Android 平台代碼，以確保應用程式關閉後 `localStorage` 仍能保留。
...
接著我們需要建立 `NativeLocalStoragePackage`。它通過將模組封裝為基礎原生套件，提供在 React Native 運行時註冊模組的物件：
...
在此情況下，您需要將其添加到 [getPackages](https://github.com/facebook/react-native/blob/8d8b8c343e62115a5509e1aed62047053c2f6e39/packages/react-native/ReactAndroid/src/main/java/com/facebook/react/ReactNativeHost.java#L233) 方法的返回值中。

:::info
後續您將學習如何將原生模組作為 [npm 套件](the-new-architecture/create-module-library.md#publish-the-library-on-npm) 發佈，我們的建置工具會自動為您進行連結。
:::
