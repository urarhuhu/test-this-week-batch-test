...
純 C++ Turbo 原生模組屬於 Turbo 原生模組。它們需要一個規格檔案（也稱為 spec 檔案），以便 Codegen 能為我們生成框架程式碼。此規格檔案也是我們在 JS 中存取 Turbo 原生模組的途徑。

規格檔案需以類型化 JS 方言編寫。React Native 目前支援 Flow 或 TypeScript。
...
編寫 C++ Turbo 原生模組可讓您在 Android 和 iOS 之間共享程式碼。因此，我們只需編寫一次程式碼，然後探討需要對平台進行哪些變更，以便 C++ 程式碼能被正確引用。
...
- `NativeSampleModule` 類別是實際的 Turbo Native Module 類別，它繼承自 `NativeSampleModuleCxxSpec` 類別，該類別包含了一些粘合程式碼和樣板程式碼，使這個類別能夠作為 Turbo Native Module 運作。
- 最後，我們有構造函數，它接受一個指向 `CallInvoker` 的指針，以便在需要時與 JS 進行通信，以及我們需要實作的函數原型。
...
- 使用 `target_sources` 指令添加我們需要構建的模組 C++ 原始碼。預設情況下，React Native 已經會將預設原始碼填充到 `appmodules` 庫中，這裡我們包含了我們自定義的部分。你可以看到我們需要從 `jni` 資料夾回溯到我們的 C++ Turbo Module 所在的 `shared` 資料夾。
...
這個區塊告訴 Gradle 檔案在哪裡尋找 CMake 檔案。路徑是相對於 `build.gradle` 檔案所在的資料夾，因此我們需要添加指向 `jni` 資料夾中的 `CMakeLists.txt` 檔案的路徑。
...
ModuleProvider 是一個 Objective-C++ 檔案，負責將純 C++ 模組與 iOS 應用程式的其他部分連接起來。
...
4. 點擊 Next 產生檔案。
5. 將 `SampleNativeModuleProvider.m` 重新命名為 `SampleNativeModuleProvider.mm`。`mm` 副檔名表示 Objective-C++ 檔案。
...
最後一步是更新 `package.json`，以告知 React Native 關於原生模組的 JS 規格與原生程式碼中具體實現之間的連結。
...
如果您現在從 Xcode 建置應用程式，應該能夠成功建置。

## 5. 測試您的程式碼

現在是時候從 JS 存取我們的 C++ Turbo 原生模組了。為此，我們需要修改 `App.tsx` 檔案，以導入 Turbo 原生模組並在程式碼中呼叫它。
...
- `import SampleTurboModule from './specs/NativeSampleModule';`：這一行將 Turbo 原生模組導入應用程式，
- `const revString = SampleTurboModule.reverseString(value);` 在 `onPress` 回呼中：這是您在應用程式中使用 Turbo 原生模組的方式。

:::warning
為了這個範例的簡潔性，我們直接將規格檔案導入應用程式中。
最佳實踐是建立一個單獨的檔案來封裝規格，並在應用程式中使用該檔案。
這讓您可以在 JS 中準備規格的輸入並對其有更多控制權。
:::
