...
`runAfterInteractions` 接受一個普通回調函數，或一個帶有返回 `Promise` 的 `gen` 方法的 `PromiseTask` 對象。如果提供 `PromiseTask`，則會完全解析（包括通過 `runAfterInteractions` 安排更多任務的異步依賴項），然後才開始處理可能之前同步排隊的下一個任務。

默認情況下，排隊的任務會在一個 `setImmediate` 批次中一起執行。如果使用正數調用 `setDeadline`，則任務只會在截止時間（以 js 事件循環運行時間為準）接近時執行，此時執行會通過 setTimeout 讓步，允許觸控等事件開始互動並阻止排隊任務執行，使應用程式更具響應性。
