...
建議在輕量使用情境外，應基於 `AsyncStorage` 封裝抽象層，而非直接操作全域實例。

在 iOS 上，`AsyncStorage` 底層透過原生程式碼實現，小量資料會序列化儲存於字典，較大資料則存為獨立檔案。Android 平台會根據環境自動選用 [RocksDB](https://rocksdb.org/) 或 SQLite 作為儲存引擎。
...
透過 `key` 取得對應項目並於完成時觸發回調。返回 `Promise` 物件。
...
為指定 `key` 設置數值並於完成時觸發回調。返回 `Promise` 物件。
...
清除 _所有_ 客戶端、函式庫等的 `AsyncStorage` 資料。通常不建議直接呼叫此方法，應改用 `removeItem` 或 `multiRemove` 僅清除應用相關鍵值。返回 `Promise` 物件。
...
取得應用中 _所有_ 呼叫方、函式庫等已知的鍵值列表。返回 `Promise` 物件。
...
此方法允許您批次獲取給定鍵名陣列對應的項目。回調函數將被調用並傳入一組找到的鍵值對：
...
此方法用於批次儲存多組鍵值對。操作完成時將透過單一回調返回任何錯誤：
