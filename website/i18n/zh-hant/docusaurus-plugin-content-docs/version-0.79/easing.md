...
您可以在 https://easings.net/ 找到一些常見緩動函數的可視化範例。

### 預定義動畫

`Easing` 模組透過以下方法提供了幾種預定義動畫：

- [`back`](easing.md#back) 提供基礎動畫效果，物件會先略微後退再向前移動
...
預設彈性值為1，會稍微過衝一次。彈性值為0則完全不過衝，而彈性值N > 1時會過衝約N次。
...
與`Animated.parallel()`搭配使用，可創造物件在動畫開始時略微回縮的基礎效果。
...
使任何緩動函數對稱執行。前一半時間正向執行，後一半時間反向執行。