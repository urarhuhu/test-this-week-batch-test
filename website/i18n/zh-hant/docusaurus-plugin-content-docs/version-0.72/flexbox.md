...
通常您會結合使用 `flexDirection`、`alignItems` 和 `justifyContent` 來達成正確的佈局。
...
[`flex`](layout-props#flex) 屬性決定了子元素如何沿主軸「填滿」可用空間。空間將根據每個元素的 flex 屬性比例分配。
...
佈局[`direction`](layout-props#direction) 指定了子元素和文字在層級結構中的排列方向，同時影響 `start` 和 `end` 所對應的邊緣。React Native 預設採用 LTR（由左至右）佈局方向，此時 `start` 表示左側，`end` 表示右側。

- `LTR`（**預設值**）文字和子元素由左至右排列。應用於元素起始處的外邊距和內邊距會作用於左側。
- `RTL` 文字和子元素由右至左排列。應用於元素起始處的外邊距和內邊距會作用於右側。
...
[`justifyContent`](layout-props#justifycontent) 描述如何沿容器主軸對齊子元素。例如，您可透過此屬性在 `flexDirection` 設為 `row` 的容器中水平置中子元素，或在 `flexDirection` 設為 `column` 的容器中垂直置中。
