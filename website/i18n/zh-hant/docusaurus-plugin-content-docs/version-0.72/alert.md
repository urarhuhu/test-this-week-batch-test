...
這是一個在Android和iOS上均可運作的API，能夠顯示靜態警示。僅在iOS上提供讓使用者輸入資訊的提示型警示。
...
在iOS上，您可以指定任意數量的按鈕。每個按鈕可選擇性地指定樣式或設為強調，可用選項由[AlertButtonStyle](#alertbuttonstyle-ios)枚舉和[AlertButton](alert#alertbutton)上的`isPreferred`欄位表示。

## Android

在Android上最多可指定三個按鈕。Android有中性、負面和正面按鈕的概念：

- 若指定一個按鈕，它將是「正面」按鈕（如「確定」）
- 兩個按鈕表示「負面」、「正面」（如「取消」、「確定」）
- 三個按鈕表示「中性」、「負面」、「正面」（如「稍後」、「取消」、「確定」）

Android上的警示可透過點擊警示框外部關閉。預設為禁用，可透過提供可選的[AlertOptions](alert#alertoptions)參數並將cancelable屬性設為`true`（即`{cancelable: true}`）來啟用。

取消事件可透過在`options`參數中提供`onDismiss`回調屬性來處理。
