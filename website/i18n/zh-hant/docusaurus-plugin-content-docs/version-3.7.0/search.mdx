...
該服務對任何開發者文件或技術部落格**完全免費**：請務必閱讀[申請條件](https://docsearch.algolia.com/docs/who-can-apply/)並[提交 DocSearch 計畫申請](https://docsearch.algolia.com/apply)。
...
當您的申請通過並部署後，您將收到一封包含所有詳細資訊的電子郵件，以便將 DocSearch 添加到您的專案中。您可以透過[網頁介面](https://crawler.algolia.com/)編輯和管理爬取設定。索引在部署後即可立即使用，因此通常不需要手動配置。

:::danger[使用推薦的爬蟲配置]

強烈建議使用我們官方的 [**Docusaurus v3 爬蟲配置範本**](https://docsearch.algolia.com/docs/templates/#docusaurus-v3-template)。若您選擇不同的爬蟲配置，我們將無法提供支援。
...
若您更新了 `initialIndexSettings` 爬蟲設定，雖然可以透過介面手動更新索引，但 [Algolia 團隊建議刪除現有索引後重新執行爬取](https://github.com/facebook/docusaurus/issues/9200#issuecomment-1667338492)，以確保索引能完整重新初始化為新設定。
...
請參閱其 [官方 DocSearch 文件](https://docsearch.algolia.com/docs/api#searchparameters) 以了解可能的參數值。
...
若在重大變更後搜尋仍無法運作，請使用 Algolia 儀表板 **手動觸發新的爬取**。
...
假設您有 2 個文件版本 (**v1** 和 **v2**) 及 2 種語言 (`en` 和 `fr`)。

當瀏覽 v2 文件時，若出現 v1 文件的搜尋結果會顯得突兀。有時 v1 與 v2 文件內容高度相似，可能導致相同查詢出現重複結果（每個版本各出現一次）。

同理，瀏覽法語網站時，若出現英語文件的搜尋結果也不合理。

為解決此問題，上下文搜尋功能會自動識別您當前瀏覽的文件版本與語言，並動態建立對應的搜尋過濾條件。
...
當啟用 `contextualSearch: true`（預設值）時，上下文篩選條件會與 `algolia.searchParameters.facetFilters` 提供的篩選條件合併。
...
若僅在停用上下文搜尋時才能獲得結果，極可能是由於 [索引設定問題](#algolia-no-search-results) 所致。

:::

### 自訂 Algolia 搜尋樣式 {#styling-your-algolia-search}

DocSearch 預設配備了經過微調的主題，專為無障礙設計，確保色彩與對比度符合標準。
...
Algolia DocSearch 支援 [一系列選項](https://docsearch.algolia.com/docs/api/)，您可將這些參數傳遞至 `docusaurus.config.js` 檔案中的 `algolia` 欄位。
...
若需直接編輯 Algolia 搜尋的 React 元件，請對 `@docusaurus/theme-search-algolia` 中的 `SearchBar` 元件進行 [swizzle](swizzling.mdx) 操作：
...
請參閱逐步教學：[如何執行 typesense-docsearch-scraper](https://typesense.org/docs/guide/docsearch.html#step-1-set-up-docsearch-scraper) 以及[如何在 Docusaurus 網站安裝搜尋列](https://typesense.org/docs/guide/docsearch.html#option-a-docusaurus-powered-sites)。

## 👥 使用本地搜尋 {#using-local-search}

當搜尋索引規模較小且可於使用者瀏覽網站時下載至其瀏覽器時，您可使用本地搜尋外掛。

您可在[社群資源清單](https://docusaurus.io/community/resources#search)找到社群支援的本地搜尋外掛。
...
這將在專案目錄中建立 `src/theme/SearchBar` 檔案。重新啟動開發伺服器並編輯元件後，您會發現 Docusaurus 現在使用您的自訂 `SearchBar` 元件。
