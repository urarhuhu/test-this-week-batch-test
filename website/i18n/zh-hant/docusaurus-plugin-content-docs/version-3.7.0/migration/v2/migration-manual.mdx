...
此手動遷移流程應在[自動遷移流程](./migration-automated.mdx)之後執行，以補齊缺失部分或除錯遷移 CLI 輸出的問題。
...
此變更明確區分了官方套件與社群維護套件。換言之，所有官方套件均歸屬於 `@docusaurus/` 命名空間下。
...
Docusaurus 1 的建置產出物位於 `website/build/<PROJECT_NAME>`。

Docusaurus 2 則簡化為 `website/build`，請確認部署配置已更新至正確讀取新目錄。
...
既有 D1 專案的 README 檔案可修改為反映 D2 變更，或直接套用預設的 [Docusaurus v2 README](https://github.com/facebook/docusaurus/blob/main/packages/create-docusaurus/templates/shared/README.md)。
...
Docusaurus 2 將各功能（部落格、文件、頁面）拆分為模組化插件。預設集為插件組合包，為保持相容性，我們提供 `@docusaurus/preset-classic` 預設集，內含 Docusaurus 1 的核心插件。
...
如果您正在遷移 Docusaurus v1 網站，且有待處理的文檔拉取請求，可以暫時將 `/docs` 資料夾保留在原處，以避免產生衝突。
...
建議使用 [ColorBox](https://www.colorbox.io/) 來為您選擇的主色尋找不同的色調。

或者，使用以下工具生成網站的不同色調，並將變數複製到 `src/css/custom.css` 中。
...
網站元信息（如資源、SEO、版權信息）現在由主題處理。要自訂這些內容，請在 `docusaurus.config.js` 中使用 `themeConfig` 欄位：
...
在 Docusaurus 1 中，標題圖標和標題連結是 `siteConfig` 的根欄位：
...
您可以聯繫 DocSearch 團隊（@shortcuts, @s-pace）尋求支援。他們可以為您更新配置並觸發重新爬取網站以恢復搜索功能（否則您可能需要等待最多 24 小時以進行下一次計劃爬取）。
...
**重大變更**：`editUrl` 應指向 (website) Docusaurus 專案，而非 `docs` 目錄。
...
在 v1 中，所有頁面均可使用或不使用 `.html` 副檔名訪問。
...
舊版本不允許嵌套側邊欄分類，且分類僅能包含文件 ID。而 v2 版本支援無限嵌套側邊欄，並提供多種[側邊欄項目類型](../../guides/docs/sidebar/items.mdx)（不限於文件）。
...
請勿僅為在左側添加標誌而替換頁尾元件。v2 版本已刻意移除左側標誌並移至底部，請直接在 `docusaurus.config.js` 中透過 `themeConfig.footer` 設定頁尾：
...
請參閱[建立頁面](guides/creating-pages.mdx)了解 Docusaurus 2 的頁面運作方式。閱讀後請注意，需將 v1 的 `pages/en` 檔案移至 `src/pages`。

Docusaurus v1 中，頁面會接收 `siteConfig` 物件作為 props。

Docusaurus v2 需改從 `useDocusaurusContext` 獲取 `siteConfig` 物件。

v2 版本中，需在每個頁面套用主題佈局元件。Layout 元件接受元資料 props。
...
此功能已由[內嵌目錄](../../guides/markdown-features/markdown-features-toc.mdx#inline-table-of-contents)取代

### 將 Markdown 語法更新為 MDX 相容格式 {#update-markdown-syntax-to-be-mdx-compatible}

Docusaurus 2 已將 Markdown 語法改為 [MDX](https://mdxjs.com/)，因此現有文件可能需修正不兼容語法。常見範例如 HTML 中有效的自閉合標籤 `<img>` 和 `<br>`，現需明確閉合為 `<img/>` 和 `<br/>`。MDX 文件中的所有標籤都必須符合 JSX 規範。
...
欄位 `authorFBID` 與 `authorTwitter` 已被棄用。這些欄位僅用於產生作者頭像，現可透過 `authors` 欄位實現相同功能。
