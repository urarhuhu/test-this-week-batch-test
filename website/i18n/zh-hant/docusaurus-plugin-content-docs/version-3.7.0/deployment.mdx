...
將潛在敏感信息放在環境中是常見的做法。然而，在典型的 Docusaurus 網站中，`docusaurus.config.js` 檔案是與 Node.js 環境的唯一接口（參見[我們的架構概述](advanced/architecture.mdx)），而其他所有內容（MDX 頁面、React 組件等）都是客戶端，無法直接訪問 `process` 全域變數。在這種情況下，您可以考慮使用 [`customFields`](api/docusaurus.config.js.mdx#customFields) 將環境變數傳遞到客戶端。
...
Docusaurus 可使用 [`docusaurus serve`](cli.mdx#docusaurus-serve-sitedir) 指令進行自架托管。透過 `--port` 變更連接埠，`--host` 變更主機位址。
...
後續章節將介紹常見託管服務商及其最佳化部署配置。Docusaurus 與這些服務無商業合作，資訊僅供參考。部分內容由第三方提供，API變更可能未即時同步。若發現過時內容，歡迎提交PR修正。

由於我們僅能提供有限支援，現已停止收錄新增託管方案的PR。您可將部署指南發表於外部平台（如個人部落格或服務商官網），並申請將連結收錄至此處。

:::

## 部署至 Netlify {#deploying-to-netlify}

部署至 [Netlify](https://www.netlify.com/) 前，請確認以下配置正確：
...
若未預先配置，仍可於站點建立後至「Site settings」→「Build & deploy」補設定。

完成配置後，站點將自動部署並於合併至預設部署分支（通常為 `main`）時觸發重新部署。

:::warning

部分 Docusaurus 專案將 `docs` 目錄置於 `website` 外（多見於 Docusaurus v1 舊專案）：
...
若選擇以 `website` 目錄作為 Netlify 基礎目錄，需配置 [自訂忽略指令](https://docs.netlify.com/configure-builds/common-configurations/ignore-builds/) 以避免 `docs` 更新時未觸發建置：
...
建議關閉 Netlify 設定項 `Post Processing > Asset Optimization > Pretty Urls` 以避免強制小寫URL、非必要重導與404錯誤。

**請注意**：全域選項 `Disable asset optimization` 存在缺陷，無法實際關閉 `Pretty URLs` 設定，請務必手動獨立取消勾選。
...
將專案部署至 [Vercel](https://vercel.com/) 可獲得[多重優勢](https://vercel.com/)，包含效能提升與操作簡化。
...
專案導入後，所有後續推送至分支的動作都會產生 [預覽部署](https://vercel.com/docs/platform/deployments#preview)，而對 [生產分支](https://vercel.com/docs/git-integrations#production-branch)（通常為 "main" 或 "master"）所做的更動則會觸發 [生產環境部署](https://vercel.com/docs/platform/deployments#production)。
...
通常發布流程會涉及兩個儲存庫（或至少兩個分支）：包含原始檔的分支，以及包含要透過 GitHub Pages 服務之建置輸出的分支。在以下教學中，將分別稱它們為 **「來源」** 與 **「部署」**。
...
GitHub Pages 會從預設分支（通常為 `master` / `main`）或 `gh-pages` 分支，以及根目錄或 `/docs` 資料夾中擷取可部署檔案（即 `docusaurus build` 的輸出）。您可透過儲存庫中的 `設定 > Pages` 來配置此選項。此分支將稱為「部署分支」。
...
這些欄位也有對應的環境變數版本，且優先級更高：`ORGANIZATION_NAME`、`PROJECT_NAME` 與 `DEPLOYMENT_BRANCH`。

:::warning

GitHub Pages 預設會為 Docusaurus 網址加上尾部斜線。建議設定明確的 `trailingSlash` 配置（`true` 或 `false`，而非 `undefined`）。
...
預設情況下，GitHub Pages 會透過 [Jekyll](https://jekyllrb.com/) 處理發布的檔案。由於 Jekyll 會捨棄所有以 `_` 開頭的檔案，建議您在 `static` 目錄中加入名為 `.nojekyll` 的空檔案來停用此功能。
...
以下工作流程範例假設您的網站原始碼位於儲存庫的 `main` 分支（_原始碼分支_為 `main`），且您的[發佈來源](https://help.github.com/en/github/working-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site)已設定為[使用自訂 GitHub Actions 工作流程發佈](https://docs.github.com/en/pages/getting-started-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site#publishing-with-a-custom-github-actions-workflow)。

我們的目標是：

1. 當有新的拉取請求提交到 `main` 時，執行一個確保網站成功建置的動作，但不會實際部署。此工作將稱為 `test-deploy`。
2. 當拉取請求合併到 `main` 分支或有人直接推送到 `main` 分支時，將建置並部署到 GitHub Pages。此工作將稱為 `deploy`。

以下是使用 GitHub Actions 部署文件的兩種方法。根據您的部署儲存庫位置，選擇下方相關的標籤：

- 原始碼儲存庫和部署儲存庫為**同一個**儲存庫。
- 部署儲存庫是一個**遠端**儲存庫，與原始碼不同。此情境的說明假設[發佈來源](https://help.github.com/en/github/working-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site)為 `gh-pages` 分支。
...
雖然您可以將兩個工作定義在同一個工作流程檔案中，但原始的 `deploy` 工作流程在 PR 檢查套件狀態中總是會顯示為已跳過，這無法反映實際狀態且對審查過程沒有價值。因此，我們建議將它們作為獨立的工作流程來管理。
...
跨儲存庫發佈的設定較為困難，因為您需要有權限推送到另一個儲存庫。我們將使用 SSH 進行身份驗證。
...
2. 預設情況下，您的公鑰應建立於 `~/.ssh/id_rsa.pub`；若您在前一步驟使用了自訂名稱，請使用該名稱將金鑰新增至 [GitHub 部署金鑰](https://developer.github.com/v3/guides/managing-deploy-keys/)。
3. 使用 `pbcopy < ~/.ssh/id_rsa.pub` 將金鑰複製到剪貼簿，並將其貼上作為部署儲存庫的 [部署金鑰](https://developer.github.com/v3/guides/managing-deploy-keys/#deploy-keys)。若命令列無法使用，請直接複製檔案內容。在儲存部署金鑰前，請勾選 `允許寫入存取` 選項。
...
5. 使用 `pbcopy < ~/.ssh/id_rsa` 複製您的私鑰，並在原始碼儲存庫中建立名為 `GH_PAGES_DEPLOY` 的 GitHub 秘密。若命令列無法使用，請直接複製檔案內容。儲存您的秘密。
...
- 原始碼儲存庫：已設定 GitHub 工作流程，並將 SSH 私鑰設為 GitHub 秘密
- 部署儲存庫：已在 GitHub 部署金鑰中設定 SSH 公鑰
...
持續整合 (CI) 服務通常用於在原始碼控制提交新變更時執行例行任務。這些任務可能包括執行單元測試與整合測試、自動化建置、將套件發佈至 npm，以及將變更部署至您的網站。要自動化網站部署，您只需在網站更新時呼叫 `yarn deploy` 腳本即可。以下章節將說明如何使用熱門的持續整合服務供應商 [Travis CI](https://travis-ci.com/) 來達成此目的。

1. 前往 https://github.com/settings/tokens 並生成一組新的 [個人存取權杖](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/)。建立權杖時，請授予其 `repo` 權限範圍以取得必要權限。
2. 使用您的 GitHub 帳戶，[新增 Travis CI 應用程式](https://github.com/marketplace/travis-ci) 至您要啟用的儲存庫。
3. 開啟您的 Travis CI 儀表板 (網址格式為 `https://travis-ci.com/USERNAME/REPO`)，並導覽至儲存庫的 `更多選項 > 設定 > 環境變數` 區段。
4. 建立一個名為 `GH_TOKEN` 的新環境變數，其值為您剛生成的權杖，接著建立 `GH_EMAIL` (您的電子郵件地址) 和 `GH_NAME` (您的 GitHub 使用者名稱)。
...
現在，每當有新的提交進入 `main` 分支時，Travis CI 將會執行您的測試套件，若所有測試通過，系統將透過 `yarn deploy` 腳本部署您的網站。

### 使用 Buddy 觸發部署 {#triggering-deployment-with-buddy}

[Buddy](https://buddy.works/) 是一款易用的 CI/CD 工具，可讓您將入口網站自動部署至不同環境，包括 GitHub Pages。

請按照以下步驟建立管道，以便在您推送變更至專案的選定分支時，自動部署網站的新版本：
...
3. 選擇 GitHub 作為 git 託管服務提供商，並選取包含網站程式碼的儲存庫。
...
2. 為您的私鑰和公鑰命名以確保其特定性，避免覆蓋其他[SSH 金鑰](https://docs.github.com/en/free-pro-team@latest/github/authenticating-to-github/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent)。
...
2. 創建一個專案。
   - 點擊 **Create project** 並為您的專案命名。
   - 點擊 **Next**。
3. 創建一個新環境。
   - 點擊 **Create environment** 並為其命名（例如 staging、production）。
4. 添加一個應用程式。
   - 點擊 **Create an application**，為其命名並選擇您的 GitHub 或 GitLab 倉庫，其中包含您的 Docusaurus 應用程式。
   - 定義主分支名稱和應用程式的根路徑。
   - 點擊 **Create**。應用程式創建後：
   - 導航至您的應用程式 **Settings**
   - 選擇 **Port**
...
5. 部署
   - 現在您只需導航至您的應用程式並點擊 **Deploy**。

![部署應用程式](https://hub.qovery.com/img/heroku/heroku-1.png)

就是這樣。查看狀態並等待應用程式部署完成。要在瀏覽器中打開應用程式，請在應用程式概覽中點擊 **Action** 和 **Open**。
...
3. 配置建置設定。
   ...
   - 接下來，將出現**網站自訂**視窗。從框架列表中選擇**靜態網站**選項。
...
Surge 是一個[靜態網頁託管平台](https://surge.sh/help/getting-started-with-surge)，您可以用來在幾秒內從命令列部署您的 Docusaurus 專案。將您的專案部署到 Surge 既簡單又免費（包括自訂網域和 SSL 憑證）。

使用 Surge 只需以下步驟即可在幾秒內部署您的應用：

1. 首先，使用 npm 安裝 Surge，執行以下指令：
   ```bash npm2yarn
   npm install -g surge
   ```
2. 為了在專案的根目錄中為生產環境建置靜態檔案，執行：
   ```bash npm2yarn
   npm run build
   ```
3. 然後，在專案的根目錄中執行以下指令：
...
使用以下指令將您的網域儲存在 CNAME 檔案中以供未來部署：
...
詳見[文件](https://docs.quantcdn.io/docs/cli/continuous-integration)與[部落格](https://www.quantcdn.io/blog)，了解更多部署至 QuantCDN 的範例與使用情境。

## 部署至 Layer0 {#deploying-to-layer0}

[Layer0](https://www.layer0.co) 是一站式平台，用於開發、部署、預覽、實驗、監控與運行無頭前端。專注於大型動態網站，透過 EdgeJS（基於 JavaScript 的內容傳遞網路）、預測性預取與效能監控實現頂尖效能。Layer0 提供免費方案。只需幾分鐘即可開始，請遵循 [Layer0 的 Docusaurus 部署指南](https://docs.layer0.co/guides/docusaurus)。
...
[Azure Static Web Apps](https://docs.microsoft.com/en-us/azure/static-web-apps/overview) 是一項服務，可直接從程式碼儲存庫自動建置並部署全端網頁應用至 Azure，簡化 CI/CD 的開發者體驗。此服務將網頁應用的靜態資源與動態（API）端點分離，靜態資源透過全球分佈的內容伺服器提供，使客戶端能從鄰近伺服器更快取得檔案；動態 API 則採用無伺服器架構，以事件驅動的函式為基礎進行擴展，更具成本效益且能按需擴容。按照[此逐步指南](https://dev.to/azure/11-share-content-with-docusaurus-azure-static-web-apps-30hc)，幾分鐘內即可開始使用。

## 部署至 Kinsta {#deploying-to-kinsta}

[Kinsta 靜態網站託管服務](https://kinsta.com/static-site-hosting) 可免費部署最多 100 個靜態網站，支援自訂網域與 SSL、每月 100 GB 流量，並透過 260+ 個 Cloudflare CDN 節點加速。

按照我們的 [Kinsta 上的 Docusaurus](https://kinsta.com/docs/docusaurus-example/) 文章指引，僅需點擊幾下即可開始使用。