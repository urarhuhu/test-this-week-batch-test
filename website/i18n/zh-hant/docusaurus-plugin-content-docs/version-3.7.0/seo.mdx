...
與[全域元數據](#global-metadata)類似，Docusaurus 也允許為單個頁面添加元信息。請按照[本指南](./guides/markdown-features/markdown-features-head-metadata.mdx)配置 `<head>` 標籤。簡而言之：
...
Docusaurus 部落格預設支援[豐富搜尋結果](https://search.google.com/test/rich-results)，以獲得最佳搜尋引擎體驗。這些資訊會根據您在部落格/全域設定中的元資訊自動生成。要獲得豐富搜尋資訊的優勢，請填寫文章發布日期、作者和圖片等相關資訊。更多關於元資訊的說明請參閱[此文件](./blog.mdx)。

## Robots 檔案 {#robots-file}

`robots.txt` 檔案用於規範搜尋引擎的行為，決定哪些內容應該被顯示、哪些不應該。您可以將其作為[靜態資源](./static-assets.mdx)提供。以下設定允許所有請求存取所有子頁面：
...
Docusaurus 提供預設內建於 `preset-classic` 的 [`@docusaurus/plugin-sitemap`](./api/plugins/plugin-sitemap.mdx) 外掛。它會自動生成 `sitemap.xml` 檔案，在正式環境建置後可透過 `https://example.com/[baseUrl]/sitemap.xml` 存取。此網站地圖元資料能幫助搜尋引擎爬蟲更精確地抓取您的網站。
...
Docusaurus 使用檔案名稱作為連結，但您隨時可以透過 slug 功能修改，詳見此[教學文件](./guides/docs/docs-create-doc.mdx#document-id)。
