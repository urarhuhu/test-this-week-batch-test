...
詳盡選項清單請參閱 [文件插件 API 參考文件](./../../api/plugins/plugin-content-docs.mdx)。

:::

您網站的文件系統分為四個層級，由低至高分別為：
...
所有文件預設會透過子路徑 `docs/` 提供服務。但若**您的站點僅包含文件**，或希望將文件置於根目錄以提升優先級該怎麼辦？
...
請注意，您**不一定需要放棄使用部落格**或其他插件功能；`routeBasePath: '/'` 的作用僅是將文件服務路徑從 `https://example.com/docs/some-doc` 變更為站點根目錄 `https://example.com/some-doc`。若啟用部落格功能，仍可透過 `blog/` 子路徑存取。
...
若您已透過 `slug: /` 將某個文件設為首頁，應刪除現有的 `./src/pages/index.js` 首頁檔案，否則會出現兩個檔案映射至相同路由的情況！
...
對於僅需使用 Docusaurus 部落格功能的用戶，也存在「純部落格模式」。您可採用上述相同方法，具體設定步驟請參閱[純部落格模式](../../blog.mdx#blog-only-mode)。
