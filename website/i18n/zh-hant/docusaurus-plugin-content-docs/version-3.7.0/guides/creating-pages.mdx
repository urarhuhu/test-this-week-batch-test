...
儲存檔案後，開發伺服器將自動重新載入變更。現在打開 [`http://localhost:3000/helloReact`](http://localhost:3000/helloReact)，您將看到剛剛建立的新頁面。

每個頁面預設沒有樣式。如果您希望顯示導覽列和/或頁尾，需要從 `@theme/Layout` 導入 `Layout` 元件並將內容包裹在該元件中。
...
您可能會意外建立多個應透過相同路由存取的頁面。發生此情況時，Docusaurus 會在執行 `yarn start` 或 `yarn build` 時警告您有關重複路由的問題（可透過 [`onDuplicateRoutes`](../api/docusaurus.config.js.mdx#onDuplicateRoutes) 配置調整此行為），但網站仍會成功建置。最後建立的頁面將可存取，但會覆蓋其他衝突頁面。若要解決此問題，您應修改或移除任何衝突的路由。