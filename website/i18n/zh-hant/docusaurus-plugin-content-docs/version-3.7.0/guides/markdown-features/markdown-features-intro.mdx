...
Docusaurus 使用現代化工具協助您建立**互動式文件**。

**[MDX](https://mdxjs.com/)** 編譯器會將 **Markdown 檔案轉換為 React 元件**，並允許您在 Markdown 內容中使用 JSX。這讓您能輕鬆地在內容中嵌入 React 元件，創造愉悅的學習體驗。
...
Docusaurus 使用 MDX 編譯器將 `.md` 和 `.mdx` 檔案編譯為 React 元件，但**語法解讀方式可能因設定而異**。
...
您可以透過 [`siteConfig.markdown.format`](../../api/docusaurus.config.js.mdx#markdown) 設定或 `mdx.format: md` 前置資料**選擇改用 CommonMark**。
...
前置資料用於為 Markdown 檔案添加元數據。所有內容插件都有各自的前置資料結構描述，並利用前置資料來豐富從內容或其他配置推斷出的預設元數據。

前置資料位於檔案最頂端，以三個破折號 `---` 包圍。內容以 [YAML](https://yaml.org/spec/1.2.2/) 格式解析。
...
你可以重複使用預設解析器，並用自有的專屬邏輯進行包裝。這讓你能實作便利的 front matter 轉換、快捷方式，或是透過 front matter 整合外部系統（即使 Docusaurus 外掛不支援這些功能）。
...
Markdown 可以嵌入 HTML 元素，而 [`details`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/details) HTML 元素也經過美觀的樣式設計：
...
建議將 `<summary>` 保持在同一行。請注意 [MDX 會為換行符號建立額外的 HTML `<p>` 段落](https://mdxjs.com/migrating/v2/#jsx)。若有疑問，可使用 [MDX 遊樂場](https://mdxjs.com/playground/) 來排查 `<details>` 的渲染問題。
