...
您可以透過三種方式顯示圖片：Markdown 語法、CommonJS require 或 ES 模組導入語法。
...
同樣地，您可以透過 `require` 引用現有資源，並將返回的 URL 用於 `video`、`a` 錨點連結等元素。
...
若使用 Markdown 圖片或連結語法，Docusaurus 會將所有資源路徑解析為檔案路徑並自動轉換為 `require()` 調用。除非使用 JSX 語法（此時需自行處理），否則無需在 Markdown 中手動使用 `require()`。
...
當您需要透過 CSS 修改 SVG 圖像的局部樣式時，這項功能非常實用。例如，可根據當前主題變更 SVG 的某部分顏色。
...
GitHub 採用其獨特的[圖片主題化方案](https://github.blog/changelog/2021-11-24-specify-theme-context-for-images-in-markdown/)，透過路徑片段實現，您可輕鬆自行實作。

若要使用路徑片段（GitHub 採用 `#gh-dark-mode-only` 和 `#gh-light-mode-only`）切換圖片可見性，請在自訂 CSS 中加入以下代碼（若不想與 GitHub 耦合，也可使用自訂後綴）：
...
若 Markdown 連結或圖片使用絕對路徑，該路徑將被視為檔案路徑，並從靜態資源目錄解析。例如，若您設定[靜態資源目錄](../../static-assets.mdx)為 `['public', 'static']`，則對於以下圖片：
...
1. 您無需處理基礎 URL 問題，Docusaurus 會在提供資源時自動處理；
