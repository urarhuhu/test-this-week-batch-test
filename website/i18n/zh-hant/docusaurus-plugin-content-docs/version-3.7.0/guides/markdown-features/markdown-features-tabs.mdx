...
亦可透過 `values` 和 `defaultValue` 屬性配置 `<Tabs>`：
...
預設情況下，所有標籤頁都會在建置階段預先渲染，搜尋引擎可索引隱藏的標籤頁。
...
預設顯示第一個標籤頁，若要覆寫此行為，可在任一標籤項目添加 `default` 屬性。亦可透過 `<Tabs>` 元件的 `defaultValue` 屬性指定標籤值。例如上方案例中，無論是為 `value="apple"` 標籤設置 `default` 屬性，或是為 `<Tabs>` 設置 `defaultValue="apple"`，都會強制預設開啟「Apple」標籤頁。
...
您可能希望同類型的標籤選擇能相互同步。例如，針對 Windows 與 macOS 用戶提供不同操作指南時，希望單擊即可切換所有作業系統專用的指南標籤頁。為實現此功能，可為所有關聯標籤頁設置相同的 `groupId` 屬性。需注意，此操作會將選擇狀態持久化至 `localStorage`，且所有具有相同 `groupId` 的 `<Tab>` 實例會在任一標籤值變更時自動更新。請注意群組 ID 為全域命名空間。

若同群組 ID 的標籤群組可能值不需完全相同。當某標籤群組選擇的值在另一同群組 ID 的標籤群組中不存在時，缺少該值的標籤群組將保持不變。參見以下案例：選擇 Linux 時，上方標籤群組不會變動。
...
您可能需自訂特定標籤頁組的外觀。可透過 `className` 屬性傳入字串，指定的 CSS 類別將被添加至 `<Tabs>` 元件：
...
您亦可透過 `attributes` 字段獨立自訂每個標籤標題。額外屬性可透過 `<Tabs>` 的 `values` 屬性傳遞，或如同宣告 `label` 的方式在各 `<TabItem>` 的屬性中設置。
...
可以將選取的標籤頁狀態持久化儲存至網址的搜尋參數中。這讓您能分享已預選特定標籤頁的頁面連結——例如從您的 Android 應用程式連結至已預選 Android 標籤頁的說明文件。此功能不提供錨點連結，瀏覽器不會自動滾動至該標籤頁。
...
當點擊標籤頁時，網址末端會加入搜尋參數：例如 `?current-os=android` 或 `?current-os=ios`。
...
當頁面載入時，會優先還原網址查詢字串中的標籤頁選擇（而非使用 `localStorage` 儲存的 `groupId` 選擇）。
