...
每個 Markdown 標題都會自動成為目錄條目。

### 標題 ID {#heading-ids}

每個標題都有一個可自動生成或手動指定的 ID。標題 ID 允許您透過 Markdown 或 JSX 連結到特定文件段落：
...
- 生成的 ID 可能不夠美觀
- 當您需要**修改或翻譯**標題文字時，可能希望保留原有 ID

透過特殊 Markdown 語法可設定**明確的標題 ID**：
...
使用 **[`write-heading-ids`](../../cli.mdx#docusaurus-write-heading-ids-sitedir)** CLI 命令可為所有 Markdown 文件添加明確 ID。

:::

:::warning[避免 ID 衝突]

自動生成的標題 ID 在單一頁面中保證唯一，但若使用自訂 ID，請確保每個 ID 在頁面中只出現一次，否則會導致多個 DOM 元素共用相同 ID，這違反 HTML 語義規範，並會使其中一個標題無法被連結。
...
`themeConfig` 選項會影響網站上所有目錄（包含[行內目錄](#inline-table-of-contents)），但 front matter 參數僅影響右上角的目錄。您需要使用 `minHeadingLevel` 和 `maxHeadingLevel` 屬性來客製化每個 `<TOCInline />` 元件。

:::

## 行內目錄 {#inline-table-of-contents}

透過 MDX 功能，您還可以在 Markdown 文件內直接嵌入行內目錄。

MDX 文件中可透過 `toc` 變數取得所有標題，預設僅顯示 `h2` 和 `h3` 標題。您可透過為個別 `TOCInline` 元件設定 `minHeadingLevel` 或 `maxHeadingLevel` 來調整顯示的標題層級範圍。
...
請注意 `toc` 全局變數是扁平化陣列，因此可輕鬆過濾不需要的節點或插入額外節點，重新建立目錄樹狀結構。
...
目錄是透過 [Remark 插件](./markdown-features-plugins.mdx) 解析 Markdown 原始碼所生成。已知在某些邊緣情況下會產生誤判（假陽性與假陰性）。
...
非 Markdown 標題不會出現在目錄中。此特性可用來解決前述問題。
