...
若您操作過 [MDX 遊樂場](https://mdxjs.com/playground/)，會注意到 MDX 轉譯有兩個中間步驟：Markdown 抽象語法樹（MDAST）和超文本抽象語法樹（HAST），最終才輸出 JSX。MDX 插件也分兩種形式：
...
使用插件為專案中最常用的 JSX 元素建立簡潔語法。我們提供的[警示框語法](./markdown-features-admonitions.mdx)也是透過 Remark 插件生成，您可依此模式實現自訂需求。
...
許多官方 Remark/Rehype 插件僅支援 [**ES Modules**](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)（JavaScript 模組系統），Docusaurus 已支援此規範。建議使用 [**ES Modules**](https://flaviocopes.com/es-modules/) 格式的設定檔，以便導入這類套件。
...
以下說明**並非**建立外掛程式的完整指南，僅展示如何使其在 Docusaurus 中運作。如需更深入的工作原理說明，請參閱 [Remark](https://github.com/remarkjs/remark/blob/main/doc/plugins.md#create-plugins) 或 [Rehype](https://github.com/rehypejs/rehype/blob/main/doc/plugins.md#create-plugins) 的官方文件。

:::

舉例來說，我們建立一個外掛程式，為每個 `h2` 標題加上 `第 X 節. ` 前綴。首先，在任何位置建立你的外掛程式原始檔——你甚至可以將其發佈為獨立的 npm 套件並如前述方式安裝。我們會將檔案放在 `src/remark/section-prefix.js`。remark/rehype 外掛程式本質上是一個接收 `options` 並返回操作 AST 的 `transformer` 的函式。
...
`transformer` 的第二個參數 [`vfile`](https://github.com/vfile/vfile) 可用於存取當前 Markdown 檔案的路徑，相當實用。
...
Docusaurus 的預設外掛程式會先於自訂 remark 外掛程式執行，這意味著圖片或連結已被轉換為帶有 `require()` 呼叫的 JSX。例如在上例中，即使所有 `h2` 標題都加上了 `第 X 節. ` 前綴，生成的目錄仍保持不變，因為目錄生成外掛程式在我們的自訂外掛程式之前執行。若你需要在預設外掛程式處理前操作 MDAST，請使用 `beforeDefaultRemarkPlugins` 和 `beforeDefaultRehypePlugins`。
