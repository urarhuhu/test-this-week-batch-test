...
如果你想引用 **由同一個插件處理** 的其他 Markdown 文件，可以使用目標文件的相對路徑。Docusaurus 的 Markdown 加載器會將檔案路徑轉換為目標文件的 URL 路徑（因此會移除 `.md` 副檔名）。
...
相對檔案路徑是根據當前文件所在目錄來解析。而絕對檔案路徑則是相對於 **內容根目錄** 來解析，通常是 `docs/`、`blog/` 或本地化目錄如 `i18n/zh-Hans/plugin-content-docs/current`。

絕對檔案路徑也可以相對於網站根目錄。但請注意，以 `/docs/` 或 `/blog/` 開頭的連結 **不可移植**，因為當你創建新版本或本地化時需要手動更新這些連結。
...
使用帶有 `.md` 副檔名的相對 _檔案_ 路徑而非相對 _URL_ 連結有以下優點：

- 連結在 GitHub 介面和許多 Markdown 編輯器中仍能正常運作
- 你可以自訂文件的 slug 而無需更新所有連結
- 移動檔案時，你的編輯器可能會追蹤這些變更，部分編輯器甚至會自動更新檔案連結
- [版本化文件](../docs/versioning.mdx) 會連結到相同版本的另一個文件
- 相對 URL 連結在更新 [`trailingSlash` 配置](../../api/docusaurus.config.js.mdx#trailingSlash) 時很可能會失效

:::warning

Markdown 檔案引用僅在來源和目標檔案由同一個插件實例處理時才能運作。這是我們 Markdown 處理架構的技術限制，未來將會修復。如果你需要在插件之間連結檔案（例如從部落格文章連結到文件頁面），則必須使用 URL 連結。
