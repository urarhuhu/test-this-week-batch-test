...
- **模組化**：允許插件作者提供國際化支援
- **低運行負擔**：文件主要是靜態內容，不需要繁重的 JavaScript 函式庫或填充程式
...
- **自動語言檢測**：此功能較為主觀，最好由[伺服器（您的託管服務提供商）](../deployment.mdx)處理
...
1. **配置**：在 [`docusaurus.config.js`](../api/docusaurus.config.js.mdx#i18n) 中宣告預設語言和替代語言
2. **翻譯**：將翻譯檔案放在正確的檔案系統位置
3. **部署**：使用單一或多重網域策略建置和部署您的網站
...
部分插件會讀取需整體本地化的外部資料檔案。例如部落格插件使用的[`authors.yml`](../blog.mdx#global-authors)檔案，可透過在`i18n/[locale]/docusaurus-plugin-content-blog/authors.yml`路徑下建立副本來實現翻譯。

### 翻譯檔案存放位置 {#translation-files-location}

翻譯檔案需建立在正確的檔案系統路徑下。

每個語言版本與插件都有專屬的`i18n`子目錄：
...
為簡易法語版Docusaurus網站進行翻譯後，目錄結構如下：
