...
Docusaurus 的路由系統遵循單頁應用程式慣例：一個路由對應一個元件。本節將先介紹三種內容外掛（文檔、部落格和頁面）內部的路由機制，接著深入探討底層路由系統的運作原理。

## 內容外掛中的路由機制 {#routing-in-content-plugins}

每個內容外掛都提供 `routeBasePath` 選項，用於定義外掛路由的基礎路徑。預設情況下，文檔外掛的路由前綴為 `/docs`；部落格外掛為 `/blog`；頁面外掛則為 `/`。您可以將路由結構視為以下形式：
...
系統會將所有路由請求與此嵌套路由配置進行匹配，直到找到相符的路由。例如當請求 `/docs/configuration` 路徑時，Docusaurus 會先進入 `/docs` 分支，然後在文檔外掛創建的子路由中進行搜索。

修改 `routeBasePath` 可有效改變網站的路由結構。例如在[純文檔模式](../guides/docs/docs-introduction.mdx#docs-only-mode)中，將文檔外掛設為 `routeBasePath: '/'` 意味著文檔路由將不再帶有 `/docs` 前綴，但這並不影響其他外掛（如部落格外掛）創建 `/blog` 等子路由。

接下來，我們將分別說明這三種外掛如何組織各自的「子路由區塊」。

### 頁面路由 {#pages-routing}

頁面路由的映射關係最為直接：檔案路徑會直接轉換為 URL 路徑，且無法透過其他方式自訂。詳見[頁面文件](../guides/creating-pages.mdx#routing)。
...
- **標籤文章頁**：`/tags/adoption`, `/tags/beta`...
  ...
  - 路由基礎路徑由 `tagsBasePath` 定義，子路徑可透過標籤的 `permalink` 欄位自訂
...
文檔外掛是唯一會創建**嵌套路由**的外掛。頂層會註冊[版本路徑](../guides/docs/versioning.mdx)：`/`, `/next`, `/2.0.0-beta.13`... 這些路徑提供版本上下文（包括佈局和側邊欄）。這確保切換文檔時能保持側邊欄狀態，並可透過導航欄下拉選單切換版本而不離開當前文檔。使用的元件是 `@theme/DocPage`。
...
個別文檔會渲染在導航欄、頁尾、側邊欄等由 `DocPage` 元件提供的框架之後的剩餘空間。例如本頁面 <URLPath /> 是由 <FilePath /> 檔案生成。使用的元件是 `@theme/DocItem`。

文件的 `slug` 前置元數據可自訂路由的最後部分，但基礎路由始終由外掛的 `routeBasePath` 和版本的 `path` 定義。

### 檔案路徑與 URL 路徑 {#file-paths-and-url-paths}

在整個文件中，我們始終力求明確區分檔案路徑和 URL 路徑。內容外掛通常會將檔案路徑直接映射到 URL 路徑，例如 `./docs/advanced/routing.md` 會變成 `/docs/advanced/routing`。然而，透過 `slug`，您可以讓 URL 完全與檔案結構脫鉤。

在 Markdown 中撰寫連結時，您可能指的是 _檔案路徑_ 或 _URL 路徑_，Docusaurus 會使用多種啟發式方法來判斷。

- 如果路徑有 `@site` 前綴，則始終是資源檔案路徑。
- 如果路徑有 `http(s)://` 前綴，則始終是 URL 路徑。
- 如果路徑沒有副檔名，則是 URL 路徑。例如，在 URL 為 `/docs/advanced/routing` 的頁面上，連結 `[page](../plugins)` 會連結到 `/docs/plugins`。Docusaurus 僅在建置網站時（當它知道完整路由結構時）檢測斷開的連結，但不會對檔案的存在做出任何假設。這完全等同於在 JSX 檔案中撰寫 `<a href="../plugins">page</a>`。
...
關於內容外掛的部分就這麼多。讓我們退一步，談談路由在 Docusaurus 應用中的一般工作原理。

## 路由成為 HTML 檔案 {#routes-become-html-files}
...
路由 `/docs/advanced/routing` 可以對應於 `/docs/advanced/routing/index.html` 或 `/docs/advanced/routing.html`。某些託管提供商會根據尾部斜線的存在來區分它們，並且可能容忍或不容忍另一種形式。詳情請參閱 [尾部斜線指南](https://github.com/slorber/trailing-slash-guide)。
...
所有 HTML 檔案都會使用絕對 URL 引用其 JS 資源，因此為了正確定位資源，您必須配置 `baseUrl` 欄位。請注意，`baseUrl` 不會影響生成的打包檔案的檔案結構：基礎 URL 位於 Docusaurus 路由系統之上的一層。您可以將 `url` 和 `baseUrl` 的組合視為 Docusaurus 網站的實際位置。
...
在客戶端，我們提供 `@docusaurus/router` 來存取頁面路由。該套件重新導出 [`react-router-dom`](https://www.npmjs.com/package/react-router-dom/v/5.3.0) 的功能，例如可用 `useLocation` 取得當前頁面的[位置物件](https://developer.mozilla.org/en-US/docs/Web/API/Location)，或透過 `useHistory` 存取[歷史物件](https://developer.mozilla.org/en-US/docs/Web/API/History)（功能類似瀏覽器 API，但具體用法請參考 React Router 文檔）。
...
若將 HTML 檔案置於 `static` 資料夾，它們會被複製到建置輸出中而成為網站可存取內容，但不會納入 Docusaurus 路由系統。我們提供 `pathname://` 協議，可像外部連結般以非 SPA 方式重新導向網域內其他路徑。
...
`pathname://` 協議特別適合參照 static 資料夾內的內容。例如 Docusaurus 會[將所有 Markdown 靜態資源轉為 require() 呼叫](../guides/markdown-features/markdown-features-assets.mdx#static-assets)，使用此協議可維持普通連結形式，避免被 Webpack 雜湊處理。
