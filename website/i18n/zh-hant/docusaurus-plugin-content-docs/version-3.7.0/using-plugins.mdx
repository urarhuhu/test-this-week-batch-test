...
您無需逐一手動安裝常見插件：它們可以透過[預設集](#using-presets)打包分發。對多數使用者而言，插件是透過預設集配置來設定的。
...
但插件可以透過在配置中將名稱和選項物件包裝成二元組來指定選項。此風格通常稱為「Babel 風格」。
...
主題的載入方式與插件完全相同。從使用者角度來看，在安裝和配置插件時，`themes` 和 `plugins` 條目可以互換使用。唯一細微差別在於主題是在插件之後載入，且[主題可以覆寫插件的預設主題組件](./advanced/client.mdx#theme-aliases)。

:::tip

`themes` 和 `plugins` 選項會導致不同的[模組簡寫解析方式](#module-shorthands)，因此若想利用簡寫功能，請務必使用正確的條目！
...
如果名稱是有作用域的（以 `@` 開頭），則會先以第一個斜線將名稱分割為作用域與套件名稱：
...
以下是在 `plugins` 欄位中註冊插件的一些範例。請注意，與 [ESLint](https://eslint.org/docs/user-guide/configuring/plugins#configuring-plugins) 或 [Babel](https://babeljs.io/docs/en/options#name-normalization) 強制要求插件命名遵循一致慣例不同，Docusaurus 允許更大的命名自由度，因此解析結果並非絕對，而是遵循上述優先順序。
