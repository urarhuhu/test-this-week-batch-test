...
要在部落格發布文章，請在部落格目錄中建立一個 Markdown 檔案。
...
在部落格文章中使用 `<!--truncate-->` 標記來表示在查看所有已發布部落格文章時將顯示為摘要的內容。`<!--truncate-->` 上方的任何內容都將成為摘要的一部分。請注意，截斷標記上方的部分必須是可獨立渲染的 Markdown。例如：
...
您還可以使用 `blogSidebarTitle` 選項來更改側邊欄的標題文字。例如，如果您設定了 `blogSidebarCount: 'ALL'`，與其使用預設的「最近文章」，您可能更希望顯示「所有文章」：
...
此命名慣例為可選項，您也可以透過 front matter 提供日期。由於 front matter 遵循支援日期時間標記法的 YAML 語法，若需更精確的發佈時間，可使用 front matter。例如，若有多篇文章於同一天發佈，可依照當天時間排序：
...
使用 `authors` front matter 欄位來宣告部落格文章作者。作者至少需具備 `name` 或 `image_url`。Docusaurus 會使用 `url`、`email` 和 `title` 等資訊，但其他任何資訊皆可保留。
...
對於固定部落格文章作者而言，在每篇文章中維護行內作者資訊可能相當繁瑣。
...
無論透過 front matter 或作者對映宣告的作者，都需具備名稱或頭像（或兩者皆有）。若某篇文章的所有作者皆未提供名稱，Docusaurus 會以緊湊方式顯示其頭像。效果請參見[此測試文章](/tests/blog/2022/01/20/image-only-authors)。

:::warning[訂閱源生成]

[RSS 訂閱源](#feed)需設定作者電子郵件，該作者才會出現在訂閱源中。
...
您可以讓 Docusaurus 網站不顯示專用首頁，而直接將部落格文章列表頁設為首頁。將 `routeBasePath` 設為 `'/'` 即可透過根路徑 `example.com/`（而非子路徑 `example.com/blog/`）提供部落格內容。
...
請務必刪除現有的首頁檔案 `./src/pages/index.js`，否則會出現兩個檔案映射到相同路徑的情況！

:::

:::warning

若停用文檔插件，請記得同時刪除設定檔中其他對文檔插件的參照。特別注意需移除導覽列中與文檔相關的項目。

:::

:::tip

針對僅需使用文檔功能的用戶，另有「純文檔模式」可供選擇。詳見[純文檔模式](./guides/docs/docs-introduction.mdx)獲取詳細說明或關於 `routeBasePath` 的更深入解釋。

:::

### 多重部落格 {#multiple-blogs}

預設情況下，經典主題假設每個網站僅有一個部落格，因此只包含一個部落格插件實例。若您需要在單一網站上運行多個部落格，這同樣可行！您可以在 `docusaurus.config.js` 的 `plugins` 選項中指定另一個部落格插件來新增部落格。

將 `routeBasePath` 設定為您希望第二個部落格被存取的 URL 路由。請注意，此處的 `routeBasePath` 必須與第一個部落格不同，否則可能發生路徑衝突！同時，將 `path` 設定為包含第二個部落格條目的目錄路徑。
