...
若使用 `@docusaurus/preset-classic` 預設套件，無需額外安裝此外掛作為依賴項。
...
- 設為 `string` 時，需提供相對於部落格內容目錄的自定義 `.xsl` 文件路徑。依慣例，需同時提供同名 `.css` 文件。
...
Markdown 文件可使用以下 [前置元數據](../../guides/markdown-features/markdown-features-intro.mdx#front-matter) 欄位，需以 `---` 分隔線包圍。
...
此檔案可包含預定義的 [全域部落格作者](../../blog.mdx#global-authors) 清單。可透過 Markdown 文件中的 [`authors` 前置元數據](#markdown-front-matter) 以作者鍵值引用這些作者。
