...
此插件會將額外的 HTML 頁面寫入您的靜態網站，透過 JavaScript 將用戶重定向至現有的 Docusaurus 頁面。
...
此插件還會讀取 [`siteConfig.onDuplicateRoutes`](../docusaurus.config.js.mdx#onDuplicateRoutes) 配置，以調整當多個文件將被發送到相同位置時的日誌級別。
...
這就是為什麼同一個「目標」可以有多個「來源」：我們會創建多個 HTML 文件，它們都重定向至同一個目的地。另一方面，一個「來源」絕不能有多個「目標」：寫入的 HTML 文件需要有一個確定的目的地。
