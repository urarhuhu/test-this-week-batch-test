...
此插件在開發模式下始終處於非活動狀態，且**僅在生產環境生效**，因為它作用於構建輸出結果。
...
- [`noIndex`](../docusaurus.config.js.mdx#noIndex)：將導致不生成站點地圖
- [`trailingSlash`](../docusaurus.config.js.mdx#trailingSlash)：決定站點地圖中的URL是否包含尾部斜杠
