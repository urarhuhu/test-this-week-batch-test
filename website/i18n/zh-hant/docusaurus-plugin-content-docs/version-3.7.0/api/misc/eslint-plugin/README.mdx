...
[ESLint](https://eslint.org/) 是一個靜態分析代碼的工具，能透過編輯器提示和命令行報告問題或建議最佳實踐。Docusaurus 提供了一個 ESLint 插件來強制執行 Docusaurus 的最佳實踐。
...
將 `plugin:@docusaurus/recommended` 加入你的 `.eslintrc` 配置文件的 `extends` 區塊：
