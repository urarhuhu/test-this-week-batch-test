...
Docusaurus 提供一組指令碼協助您生成、運行和部署網站。

當您的網站初始化完成後，網站原始碼將包含可透過套件管理器調用的 Docusaurus 指令碼：
...
使用 [Webpack 開發伺服器](https://webpack.js.org/configuration/dev-server) 本地建構並預覽您的網站。
...
有多種方式可取得憑證，此處以 [mkcert](https://github.com/FiloSottile/mkcert) 為例說明。
...
針對 CSS 套件的進階壓縮，我們採用 [cssnano 進階預設配置](https://github.com/cssnano/cssnano/tree/master/packages/cssnano-preset-advanced)（搭配多個 PostCSS 插件）與 [clean-css 第 2 級優化](https://github.com/jakubpawlowicz/clean-css#level-2-optimizations)。若此進階 CSS 壓縮導致樣式異常，可透過環境變數 `USE_SIMPLE_CSS_MINIFIER=true` 改用 [cssnano 預設配置](https://github.com/cssnano/cssnano/tree/master/packages/cssnano-preset-default) 進行壓縮。**若遭遇 CSS 壓縮錯誤，請[提交問題報告](https://github.com/facebook/docusaurus/issues/new?labels=bug%2C+needs+triage&template=bug.md)。**

透過環境變數 `SKIP_HTML_MINIFICATION=true` 可跳過 HTML 壓縮步驟。
...
清除 Docusaurus 網站生成的資源、快取及建置產物。

建議在提交錯誤報告、升級版本後，或任何遇到 Docusaurus 網站問題時執行此命令。
