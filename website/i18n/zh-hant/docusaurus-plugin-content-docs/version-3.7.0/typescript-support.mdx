...
Docusaurus 支援編寫和使用 TypeScript 主題元件。若初始化模板提供 TypeScript 變體，您可直接透過 `--typescript` 旗標[初始化網站](./installation.mdx#scaffold-project-website)，獲得完整的 TypeScript 支援。
...
對於支援 TypeScript 主題元件的主題，您可在 `swizzle` 指令結尾加上 `--typescript` 旗標以取得 TypeScript 原始碼。例如，以下指令將在 `src/theme/Footer` 中生成 `index.tsx` 和 `styles.module.css`：
