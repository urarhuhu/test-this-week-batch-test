...
指定字體粗細。多數字體支援 `'normal'` 和 `'bold'` 值。並非所有字體都具備每個數值對應的變體，此時會選擇最接近的數值。
...
設為 `false` 可移除為特定升部/降部預留的額外字體內邊距。某些字體在垂直置中時，此內邊距可能導致文字略微錯位。為達最佳效果，建議同時將 `textAlignVertical` 設為 `center`。
...
指定文字對齊方式。在 Android 上，'justify' 值僅支援 Oreo (8.0) 或更高版本 (API level >= 26)。較低版本 Android 會自動回退為 `left`。
