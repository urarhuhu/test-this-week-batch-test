...
`alignContent` 控制行在交叉方向上的對齊方式，覆蓋父元素的 `alignContent`。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/align-content。
...
`alignItems` 在交叉方向上對齊子元素。例如，若子元素垂直排列，`alignItems` 控制其水平對齊方式。功能類似 CSS 的 `align-items`（預設值：stretch）。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/align-items。
...
`alignSelf` 控制單一子元素在交叉方向上的對齊方式，覆蓋父元素的 `alignItems`。功能類似 CSS 的 `align-self`（預設值：auto）。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/align-self。
...
- 在設定寬度/高度的節點上，長寬比控制未設定維度的尺寸
- 在設定 flex basis 的節點上，長寬比控制未設定時交叉軸的節點尺寸
- 在具有測量函式的節點上，長寬比作用如同測量函式測量 flex basis
- 在具有 flex grow/shrink 的節點上，長寬比控制未設定時交叉軸的節點尺寸
- 長寬比會考慮最小/最大尺寸限制
...
`direction` 指定使用者界面的文字流向。預設值為 `inherit`，根節點會根據當前語言環境設定值。詳見 https://www.yogalayout.dev/docs/styling/layout-direction。
...
當 `flex` 為正數時，元件會具有彈性，其尺寸將按比例分配。例如 `flex` 設為 2 的元件所佔空間是 `flex` 設為 1 的兩倍。`flex: <正數>` 等同於 `flexGrow: <正數>, flexShrink: 1, flexBasis: 0`。

當 `flex` 為 0 時，元件尺寸由 `width` 和 `height` 決定，且不具彈性。

當 `flex` 為 -1 時，元件通常由 `width` 和 `height` 決定尺寸。但若空間不足，元件會縮小至 `minWidth` 和 `minHeight`。
...
`flexBasis` 是一種與軸向無關的方式，用於設定項目在主軸上的預設大小。設定子項目的 `flexBasis` 類似於在父容器為 `flexDirection: row` 時設定該子項目的 `width`，或在父容器為 `flexDirection: column` 時設定其 `height`。`flexBasis` 是項目在 `flexGrow` 和 `flexShrink` 計算之前的預設大小。
...
[`flexShrink`](layout-props#flexshrink) 描述了當子項目在主軸上的總大小超過容器大小時，應如何縮小子項目。`flexShrink` 與 `flexGrow` 非常相似，可以將溢出的空間視為負的剩餘空間。這兩個屬性可以很好地配合使用，讓子項目根據需要進行擴展或縮小。
...
`flexWrap` 控制子項目在到達彈性容器末端後是否可以換行。其功能類似於 CSS 中的 `flex-wrap`（預設值為 `nowrap`）。更多詳情請參閱 https://developer.mozilla.org/en-US/docs/Web/CSS/flex-wrap。注意，它不再與 `alignItems: stretch`（預設值）一起使用，因此您可能需要改用 `alignItems: flex-start`（重大變更詳情：https://github.com/facebook/react-native/releases/tag/v0.28.0）。
...
其功能類似於 CSS 中的 `height`，但在 React Native 中必須使用點數或百分比單位，不支援 em 等其他單位。更多詳情請參閱 https://developer.mozilla.org/en-US/docs/Web/CSS/height。
...
`justifyContent` 用於在主軸方向上對齊子元素。例如，若子元素是垂直排列，`justifyContent` 會控制它們在垂直方向上的對齊方式。其功能類似 CSS 中的 `justify-content`（預設值為 flex-start）。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/justify-content。
...
`overflow` 控制子元件的測量與顯示方式。`overflow: hidden` 會裁剪視圖，而 `overflow: scroll` 會讓視圖獨立於父元件的主軸進行測量。其運作方式類似 CSS 中的 `overflow`（預設值為 visible）。詳見 https://developer.mozilla.org/en/docs/Web/CSS/overflow 以獲取更多細節。
...
設定 `padding` 的效果等同於分別設定 `paddingTop`、`paddingBottom`、`paddingLeft` 和 `paddingRight`。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/padding 以獲取更多細節。
...
如果想使用特定的邏輯像素數值相對於父元素定位子元素，請將子元素的 `position` 設為 `absolute`。

如果想相對於非父元素的對象定位子元素，請不要使用樣式來實現。應使用組件樹結構。

有關 React Native 與 CSS 中 `position` 差異的更多詳情，請參閱 https://github.com/facebook/yoga。
...
`right` 表示此組件右邊緣偏移的邏輯像素數。

其作用類似於 CSS 中的 `right`，但在 React Native 中必須使用點數或百分比。不支援 em 等其他單位。

有關 `right` 如何影響佈局的更多詳情，請參閱 https://developer.mozilla.org/en-US/docs/Web/CSS/right。
...
`top` 表示此組件頂部邊緣偏移的邏輯像素數。

其作用類似於 CSS 中的 `top`，但在 React Native 中必須使用點數或百分比。不支援 em 等其他單位。

有關 `top` 如何影響佈局的更多詳情，請參閱 https://developer.mozilla.org/en-US/docs/Web/CSS/top。
...
其作用類似於 CSS 中的 `width`，但在 React Native 中必須使用點數或百分比。不支援 em 等其他單位。詳情請參閱 https://developer.mozilla.org/en-US/docs/Web/CSS/width。
...
`zIndex` 控制哪些組件顯示在其他組件上方。通常情況下不需要使用 `zIndex`。組件會按照其在文檔樹中的順序渲染，因此後面的組件會繪製在前面的組件之上。如果遇到動畫或自定義模態界面等不希望遵循此行為的情況，`zIndex` 可能會很有用。

其運作方式類似於 CSS 的 `z-index` 屬性——`zIndex` 值較大的元件會渲染在上層。可將 z 軸方向想像為從手機指向您的眼球。更多細節請參閱 https://developer.mozilla.org/en-US/docs/Web/CSS/z-index。
