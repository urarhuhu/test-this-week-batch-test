...
- 如果停用 Hermes，React Native 會使用 [JavaScriptCore](http://trac.webkit.org/wiki/JavaScriptCore)，這是 Safari 瀏覽器使用的 JavaScript 引擎。請注意，在 iOS 上由於應用程式缺乏可寫入的執行記憶體，JavaScriptCore 不會使用 JIT 編譯。
- 使用 Chrome 調試時，所有 JavaScript 程式碼會在 Chrome 內部執行，並透過 WebSockets 與原生程式碼通訊。Chrome 使用 [V8](https://v8.dev/) 作為其 JavaScript 引擎。

雖然這些環境非常相似，但您可能會遇到一些不一致的情況。最好避免依賴任何運行環境的特定行為。

## JavaScript 語法轉換器

語法轉換器讓編寫程式碼更加愉快，允許您使用新的 JavaScript 語法，而無需等待所有解譯器的支援。
...
React Native 啟用的完整轉換列表可在 [@react-native/babel-preset](https://github.com/facebook/react-native/tree/main/packages/react-native-babel-preset) 中找到。
...
許多標準函式在所有支援的 JavaScript 運行環境中也可用。
