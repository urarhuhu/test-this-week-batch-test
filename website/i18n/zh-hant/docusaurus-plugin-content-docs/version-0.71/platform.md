...
`select` 方法會回傳最適合當前執行平台的數值。也就是說，如果你在手機上執行，會優先使用 `android` 和 `ios` 鍵的值。如果這些鍵未指定，則會使用 `native` 鍵的值，最後才是 `default` 鍵的值。
...
由於對應平台鍵的值可以是任意類型，[`select`](platform.md#select) 方法也可以用於回傳平台特定的元件，如下所示：
