...
Docusaurus 支援編寫和使用 TypeScript 主題元件。若初始化模板提供 TypeScript 變體，您可直接透過 `--typescript` 標誌初始化具備完整 TypeScript 支援的網站。
...
Docusaurus 不會使用此 `tsconfig.json` 編譯專案。其僅用於提升編輯器體驗，但您仍可選擇執行 `tsc` 自行或在 CI 環境中進行型別檢查。

現在您可以開始編寫 TypeScript 主題元件。

## 設定檔案型別標註 {#typing-config}

Docusaurus 中可使用 TypeScript 設定檔案。
...
對於支援 TypeScript 主題元件的主題，您可在 `swizzle` 指令結尾加入 `--typescript` 標誌以取得 TypeScript 原始碼。例如以下指令將在 `src/theme/Footer` 中生成 `index.tsx` 和 `styles.module.css`。
