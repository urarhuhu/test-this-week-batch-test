...
**Docusaurus 核心本身不提供任何功能。** 所有功能都委託給各個插件：由[文檔插件](./api/plugins/plugin-content-docs.mdx)提供的[文檔功能](./guides/docs/docs-introduction.mdx)；由[博客插件](./api/plugins/plugin-content-blog.mdx)提供的[博客功能](./blog.mdx)；或由[頁面插件](./api/plugins/plugin-content-pages.mdx)提供的個別[頁面](./guides/creating-pages.mdx)。如果沒有安裝任何插件，網站將不包含任何路由。

不過，您可能不需要逐一手動安裝常見插件：它們可以以[預設套件](#using-presets)的形式分發。對大多數用戶來說，插件是通過預設配置來設定的。
...
然而，插件可以通過在配置中將名稱和選項對象包裝在一個雙成員元組中來指定選項。這種風格通常稱為「Babel 風格」。
...
所有 Docusaurus 內容插件都支持多個插件實例。例如，可能需要[多個文檔插件實例](./guides/docs/docs-multi-instance.mdx)或[多個博客](./blog.mdx#multiple-blogs)。必須為每個插件實例分配一個唯一的 ID，默認情況下，插件 ID 為 `default`。
...
最多只能有一個插件實例是「默認插件實例」，方法是省略 `id` 屬性，或使用 `id: 'default'`。

:::

## 使用主題 {#using-themes}

主題的加載方式與插件完全相同。從用戶的角度來看，在安裝和配置插件時，`themes` 和 `plugins` 條目是可以互換的。唯一的細微差別是主題在插件之後加載，並且[主題可以覆蓋插件的默認主題組件](./advanced/client.mdx#theme-aliases)。

:::tip

`themes` 和 `plugins` 選項會導致不同的[簡寫解析](#module-shorthands)，因此如果您想利用簡寫，請確保使用正確的條目！
...
## 使用預設套件 {#using-presets}

預設集（presets）是插件與主題的組合套件。舉例來說，與其讓您在設定檔中逐一註冊並配置 `@docusaurus/plugin-content-docs`、`@docusaurus/plugin-content-blog` 等插件，我們提供了 `@docusaurus/preset-classic` 預設集，讓您能在單一集中位置配置這些插件。
...
預設集是一個與[插件建構函式](./api/plugin-methods/README.mdx#plugin-constructor)具有相同結構的函式。它應該回傳一個 `{ plugins: PluginConfig[], themes: PluginConfig[] }` 物件，與網站設定中接受的形式相同。例如，您可以指定一個包含以下主題與插件的預設集：
...
當某些插件與主題需要一起使用時，這特別有用。您甚至可以將它們的選項連結在一起，例如將一個選項傳遞給多個插件。

## 模組簡寫 {#module-shorthands}

Docusaurus 支援插件、主題與預設集的簡寫形式。當它看到一個插件/主題/預設集名稱時，會嘗試依序載入以下模組：
...
如果名稱是有作用域的（以 `@` 開頭），名稱會先以第一個斜線分隔為作用域與套件名稱：
...
若名稱不存在（如 `@jquery`），則會載入 `[scope]/docusaurus-[moduleType]`（例如 `@jquery/docusaurus-plugin`）。否則，系統將嘗試以下路徑：

- `[scope]/[name]`（例如 `@jquery/content-docs`）
- `[scope]/docusaurus-[moduleType]-[name]`（例如 `@jquery/docusaurus-plugin-content-docs`）

以下為註冊於 `plugins` 欄位的插件範例。請注意，與 [ESLint](https://eslint.org/docs/user-guide/configuring/plugins#configuring-plugins) 或 [Babel](https://babeljs.io/docs/en/options#name-normalization) 強制要求插件命名規範不同，Docusaurus 允許更大的命名自由度，因此解析結果並非絕對，而是遵循上述優先級規則。
