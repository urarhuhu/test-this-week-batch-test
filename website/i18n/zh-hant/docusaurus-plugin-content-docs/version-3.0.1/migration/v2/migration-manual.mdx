...
此手動遷移流程應在[自動遷移流程](./migration-automated.mdx)之後執行，以補齊缺失部分或調試遷移 CLI 輸出的問題。

## 專案設定 {#project-setup}

### `package.json` {#packagejson}

#### 作用域套件名稱 {#scoped-package-names}

在 Docusaurus 2 中，我們採用作用域套件名稱：

- `docusaurus` → `@docusaurus/core`

這能明確區分 Docusaurus 官方套件與社群維護套件。換言之，所有 Docusaurus 官方套件皆命名於 `@docusaurus/` 之下。
...
請使用最新的 Docusaurus 2 版本，您可於[此處](https://www.npmjs.com/package/@docusaurus/core)查閱（使用 `latest` 標籤）。

:::

#### CLI 指令 {#cli-commands}

同時，CLI 指令更名為 `docusaurus <command>`（原為 `docusaurus-command`）。
...
Docusaurus 2 將各功能（部落格、文件、頁面）拆分為插件以實現模組化。預設集為插件組合包，為保持向後兼容性，我們建立了 `@docusaurus/preset-classic` 預設集，其中包含 Docusaurus 1 的大部分核心插件。

請將以下預設集配置加入您的 `docusaurus.config.js`。
...
我們建議將 `docs` 資料夾移至 `website` 資料夾內，這也是 v2 版本的預設目錄結構。[Vercel](https://vercel.com) 支援[開箱即用的 Docusaurus 專案部署](https://github.com/vercel/vercel/tree/main/examples/docusaurus)，前提是 `docs` 目錄必須位於 `website` 內。將文檔與網站程式碼集中存放在同一個 `website` 目錄下，通常也是較好的做法。

如果您正在遷移 Docusaurus v1 網站，且有待處理的文檔拉取請求(pull requests)，可以暫時保留 `/docs` 資料夾在原位置，以避免產生衝突。
...
已棄用。我們為 Docusaurus 2 開發了名為 [Infima](https://infima.dev/) 的自訂 CSS 框架，使用 CSS 變數進行主題設定。相關文檔尚未完全準備就緒，我們將在完成後更新此處說明。若要覆寫 Infima 的 CSS 變數，請建立自己的 CSS 檔案（例如 `./src/css/custom.css`），並通過將其作為選項傳遞給 `@docusaurus/preset-classic` 來全局導入：
...
建議使用 [ColorBox](https://www.colorbox.io/) 為您選擇的主色尋找不同色調。
...
您的 Algolia DocSearch v1 配置（位於[此處](https://github.com/algolia/docsearch-configs/blob/master/configs)）需要針對 Docusaurus v2 進行更新（[範例](https://github.com/algolia/docsearch-configs/tree/master/configs/docusaurus-2.json)）。

您可以聯繫 DocSearch 團隊 (@shortcuts, @s-pace) 獲取支援。他們可以為您更新配置並觸發站點重新爬取以恢復搜索功能（否則您可能需要等待最多 24 小時直到下一次計劃爬取）
...
已棄用。請改為在 `static` 資料夾中創建一個 `CNAME` 檔案，並填入您的自訂網域。執行建置命令時，`static` 資料夾中的檔案將被複製到 `build` 資料夾的根目錄。
...
- `defaultVersionShown` - 版本功能尚未移植。如果您使用版本功能，將無法遷移至 Docusaurus 2。請持續關注。
- `disableHeaderTitle`
- `disableTitleTagline`
...
為了 SEO 並避免連結失效，您應在託管服務商處設定伺服器端重新導向規則。

作為應急方案，您可以使用 [@docusaurus/plugin-client-redirects](../../api/plugins/plugin-client-redirects.mdx) 創建客戶端重新導向，將 `/installation.html` 導向 `/installation`。
...
舊版本中不允許嵌套側邊欄分類，且分類僅能包含文件 ID。然而 v2 版本允許無限嵌套側邊欄，並提供多種[側邊欄項目類型](../../guides/docs/sidebar/items.mdx)而不限於文件。

若您的側邊欄包含分類類型需進行遷移：將 `subcategory` 更名為 `category`，`ids` 更名為 `items`。
...
請參閱[建立頁面](guides/creating-pages.mdx)了解 Docusaurus 2 的頁面運作機制。閱讀後請注意需將 v1 的 `pages/en` 檔案移至 `src/pages`。

在 Docusaurus v1 中，頁面會接收 `siteConfig` 物件作為 props。

在 Docusaurus v2 中，請改用 `useDocusaurusContext` 獲取 `siteConfig` 物件。

v2 版本中需在每個頁面外層套用主題佈局元件。Layout 元件接收元資料 props。
...
Docusaurus 2 已將 Markdown 語法改為 [MDX](https://mdxjs.com/)，因此現有文件可能需修正不兼容語法。常見範例如 HTML 中有效的自閉合標籤 `<img>` 和 `<br>`，現在需明確閉合為 `<img/>` 和 `<br/>`。MDX 文件中的所有標籤都必須是有效的 JSX。

Front matter 由 [gray-matter](https://github.com/jonschlinkert/gray-matter) 解析。若您的 front matter 包含特殊字元（如 `:`），現在需加上引號：`title: Part 1: my part1 title` → `title: "Part 1: my part1 title"`。
...
部落格的 Docusaurus front matter 欄位已從駝峰式命名改為蛇底式命名，以與文件規範保持一致。

`authorFBID` 與 `authorTwitter` 欄位已棄用。這些欄位僅用於產生作者頭像，現可透過 `authors` 欄位實現相同功能。

## 部署 {#deployment}

GitHub Pages 使用的 `CNAME` 檔案不再自動生成，若使用自訂網域，請確認已在 `/static/CNAME` 建立該檔案。

部落格 RSS 訂閱網址變更為 `/blog/rss.xml`（原為 `/blog/feed.xml`）。建議設定伺服器端重新導向，以確保使用者訂閱持續有效。
