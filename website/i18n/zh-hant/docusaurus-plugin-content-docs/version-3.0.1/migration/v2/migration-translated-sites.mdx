...
Docusaurus v2 的國際化概念與 Docusaurus v1 相當類似，但有一些差異。
...
程式碼翻譯現在使用 Chrome i18n JSON 格式添加到 `i18n/[locale]/code.json`。
...
MDX 將 Markdown 檔案編譯為 React 組件，比 Docusaurus v1 的解析器更嚴格，並且會在出現錯誤時使您的建置失敗，而不是渲染一些不良內容。

此外，HTML 元素必須替換為 JSX 元素。

這對於國際化尤其重要，因為如果您的 Crowdin 翻譯不佳且使用了無效的標記，您的 v2 翻譯網站可能會建置失敗：您可能需要進行一些翻譯清理來修復錯誤。

## 遷移策略 {#migration-strategies}

本節將幫助您了解如何在遷移到 v2 後**保留現有的 v1 翻譯**。

有多種可能的策略可以遷移使用 Crowdin 的 Docusaurus v1 網站，每種策略都有不同的權衡。

:::warning

本文件是盡力幫助您進行遷移，如果您找到更好的方法，請幫助我們改進！
...
- 使用一個新的未翻譯的 Crowdin 專案和 [Crowdin 教學](../../i18n/i18n-crowdin.mdx) 讓 Crowdin 適用於您的 v2 網站

:::danger

不要在不了解 Crowdin 和 Docusaurus v2 國際化的情況下嘗試遷移。
...
- 根據i18n文件重新配置Docusaurus v2的`crowdin.yml`
...
- 先嘗試「100%匹配」（會比「Perfect」選項翻譯更多內容），並預翻譯您的原始檔
- 下載Crowdin翻譯至本地端
- 嘗試執行/建置您的網站，查看是否有錯誤

首次嘗試時很可能會遇到錯誤：預翻譯可能會嘗試翻譯不應翻譯的內容（front matter、admonition、程式碼區塊等），且翻譯後的MD檔案可能對MDX解析器無效。

您需要修正所有錯誤，直到網站能成功建置。可以透過修改本地端的翻譯MD檔案，並使用`docusaurus build --locale fr`一次針對一個語系修正網站。
...
您可能需要重複此預翻譯流程，最終嘗試「Perfect」選項並限制僅預翻譯部分語言/檔案。
...
您可能會注意到舊專案中已翻譯的內容，在新專案中卻未翻譯。

Crowdin的Markdown解析器會隨時間演進，且每個Crowdin專案的解析器版本可能不同，這可能導致預翻譯無法預翻譯所有字串。
...
Crowdin有「上傳翻譯」功能，但根據我們的經驗，對於Markdown的效果並不理想

:::

### 使用現有的Crowdin專案 {#use-the-existing-crowdin-project}

如果您不介意修改現有的Crowdin專案並承擔搞砸的風險，或許可以使用Crowdin的分支系統。
...
這樣您就不需要建立新的Crowdin專案、轉移翻譯記憶檔、套用預翻譯，以及嘗試修正預翻譯錯誤。

您可以為Docusaurus v2建立一個Crowdin分支，上傳v2原始檔，並在準備好後將Crowdin分支合併至主分支。
