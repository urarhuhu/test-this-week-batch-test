...
- `/blog` 目錄
- `/static` 目錄
- 若網站使用版本控制功能，則包含 `versioned_sidebar.json` 和 `/versioned_docs` 目錄

使用遷移 CLI 的步驟如下：

1. 執行遷移 CLI 前，請確認 `/docs`、`/blog`、`/static`、`sidebars.json`、`siteConfig.js`、`package.json` 符合預期結構。

2. 執行遷移 CLI 時需指定正確的檔案系統路徑：
...
3. 要預覽遷移後的網站，請進入 v2 網站目錄並啟動開發伺服器。
...
遷移 CLI 會直接修改現有檔案。請務必先提交這些檔案的當前版本！

:::

#### 選項 {#options}

您可透過選項標記讓遷移 CLI 自動轉換 Markdown 內容與頁面至 v2 格式。通常仍需手動調整才能達到理想效果。
...
建議先不帶選項執行遷移，提交變更後，再使用 `--page` 和 `--mdx` 選項重新執行遷移。
