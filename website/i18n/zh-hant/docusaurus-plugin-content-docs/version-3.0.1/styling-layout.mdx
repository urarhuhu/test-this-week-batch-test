...
Docusaurus 使用 `996px` 作為行動裝置螢幕寬度與桌機的分界點。若您希望在行動視圖中使用不同佈局，可使用媒體查詢。
...
若要使用 [CSS 模組](https://github.com/css-modules/css-modules) 為元件添加樣式，請將樣式表檔案命名為 `.module.css` 後綴（例如 `welcome.module.css`）。Webpack 會將此類 CSS 檔案視為 CSS 模組載入，您需透過導入的 CSS 模組屬性來引用類別名稱（而非使用純字串）。此慣例與 [Create React App](https://facebook.github.io/create-react-app/docs/adding-a-css-modules-stylesheet) 所用方式相似。
