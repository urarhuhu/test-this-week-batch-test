...
Docusaurus 對配置有獨特的見解。我們鼓勵您將網站相關信息集中到一個地方。我們會保護此文件的字段，並促進讓這個數據對象在整個網站中可訪問。

維護良好的 `docusaurus.config.js` 可以幫助您、您的協作者和開源貢獻者專注於文檔，同時仍然能夠自定義網站。
...
- **必需：** 使用 `export default /* 你的配置 */`（或 `module.exports`）導出 Docusaurus 配置
...
即使您在開發網站時，也不應該從頭開始編寫 `docusaurus.config.js`。所有模板都帶有一個包含常見選項默認值的 `docusaurus.config.js`。

然而，如果您對配置的設計和實現有高層次的理解，這會有所幫助。
