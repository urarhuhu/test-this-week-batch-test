...
要在部落格發佈內容，請在部落格目錄內建立 Markdown 檔案。
...
在部落格文章中使用 `<!--truncate-->` 標記來表示在瀏覽所有已發佈文章時顯示的摘要內容。該標記上方的所有內容都會成為摘要的一部分。請注意，截斷標記上方的部分必須是獨立可渲染的 Markdown。例如：
...
預設情況下，每個部落格列表頁面會顯示 10 篇文章，但您可以透過插件配置中的 `postsPerPage` 選項控制分頁。若設定 `postsPerPage: 'ALL'`，則會停用分頁功能，所有文章都會顯示在第一頁。您也可以為部落格列表頁面添加元描述以提升 SEO：
...
部落格側邊欄會顯示近期文章。預設顯示項目數為 5，但您可透過插件配置中的 `blogSidebarCount` 選項自訂。設定 `blogSidebarCount: 0` 會完全停用側邊欄，並移除容器，這將增加主容器的寬度。特別的是，若設定 `blogSidebarCount: 'ALL'`，則會顯示 _所有_ 文章。

您也可以透過 `blogSidebarTitle` 選項修改側邊欄標題文字。例如，若您設定 `blogSidebarCount: 'ALL'`，與其使用預設的「近期文章」，您可能更希望顯示「所有文章」：
...
使用資料夾可方便地將文章圖片與 Markdown 檔案放在同一位置。

:::

此命名規則為可選項，您亦可透過 front matter 提供日期。由於 front matter 遵循支援日期時間標記法的 YAML 語法，若需更精確的發布時間，可採用 front matter。例如當同一天發布多篇文章時，可依當日時間排序：
...
對於固定供稿的部落格作者而言，在每篇文章中維護行內作者資訊可能相當繁瑣。
...
無論透過 front matter 或作者對映宣告的作者，皆需具備名稱或頭像（或兩者兼具）。若某篇文章的所有作者均未提供名稱，Docusaurus 會將其頭像以緊湊方式顯示。效果請參見[此測試文章](/tests/blog/2022/01/20/image-only-authors)。

:::warning[Feed generation]

[RSS 訂閱](#feed)需要設定作者電子郵件，作者才會出現在訂閱源中。
...
`readingTime` 回調函數接收三個參數：部落格內容文字（字串形式）、front matter（字串鍵及其值的記錄）以及預設的閱讀時間函數。它返回一個數字（閱讀時間分鐘數）或 `undefined`（停用該頁面的閱讀時間）。

預設的閱讀時間函數可以接受額外選項：`wordsPerMinute`（數字，預設值：300）和 `wordBound`（從字串到布林值的函數）。如果傳遞給 `wordBound` 的字串應為單詞邊界（預設為空格、製表符和換行符），則該函數應返回 `true`。
...
您可以通過傳遞 `feedOptions` 來生成 RSS / Atom / JSON 訂閱源。預設情況下會生成 RSS 和 Atom 訂閱源。若要停用訂閱源生成，請將 `feedOptions.type` 設為 `null`。
...
您可以運行不帶專用首頁的 Docusaurus 站點，而是將部落格的文章列表頁面作為索引頁面。將 `routeBasePath` 設為 `'/'`，即可通過根路由 `example.com/` 而非子路由 `example.com/blog/` 來提供部落格服務。
...
對於僅需使用文檔功能的用戶，亦有「純文檔模式」可供選擇。詳見[純文檔模式](./guides/docs/docs-introduction.mdx)獲取詳細操作指南或關於`routeBasePath`的更深入說明。

:::

### 多部落格功能 {#multiple-blogs}

經典主題預設單一網站僅支援單一部落格，因此僅包含一個部落格插件實例。若需在單一網站架設多個部落格，您可透過在`docusaurus.config.js`的`plugins`選項中配置額外部落格插件實現此需求。

將`routeBasePath`設定為第二個部落格的目標URL路徑。請注意此處的`routeBasePath`必須與第一個部落格不同，否則可能發生路徑衝突！同時需將`path`指向存放第二個部落格條目的目錄路徑。

如[多實例插件](./using-plugins.mdx#multi-instance-plugins-and-plugin-ids)文件所述，您需要為各插件分配唯一ID。
...
實際範例可參考我們託管的第二個部落格[此處](/tests/blog)。