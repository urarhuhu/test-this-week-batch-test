...
更多背景資訊，可閱讀最初的 [RFC](https://github.com/facebook/docusaurus/issues/3317) 和 [PR](https://github.com/facebook/docusaurus/pull/3325)。
...
- **模組化**：允許插件作者提供國際化支援
- **低開銷運行時**：文件主要是靜態的，不需要繁重的 JS 函式庫或填充程式
...
- **RTL 支援**：支援從右至左閱讀的語言（阿拉伯語、希伯來語等）且易於實作
- **預設翻譯**：[多種語言](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-theme-translations/locales) 的經典主題標籤已為您翻譯

### 非國際化目標 {#i18n-non-goals}

我們不提供以下支援：

- **自動語言檢測**：此功能具有主觀性，最好由 [伺服器（您的託管服務商）](../deployment.mdx) 處理
- **翻譯 SaaS 軟體**：您需自行了解所選的外部工具
- **網址段翻譯**：技術複雜且 SEO 價值低

## 翻譯流程 {#translation-workflow}

### 概述 {#overview}

建立翻譯版 Docusaurus 網站的流程概述：
...
Markdown 和 MDX 文件會整體翻譯，以完整保留翻譯上下文，而非將每個句子拆分為單獨的字串。
...
翻譯一個簡易法文版Docusaurus網站會產生以下目錄結構：
...
JSON檔案可透過[`docusaurus write-translations`](../cli.mdx#docusaurus-write-translations-sitedir) CLI指令初始化。各插件在對應目錄下提供自有翻譯內容，而`code.json`檔案則定義React程式碼中使用的文字標籤。

每個內容插件或主題的**翻譯檔案存放位置定義各不相同**：
