...
- [typesense-docsearch-scraper](https://github.com/typesense/typesense-docsearch-scraper) - 用於爬取網站內容並將資料索引至 Typesense 叢集。
...
若搜尋索引規模較小，可下載至使用者瀏覽器時，可採用本地搜尋外掛程式。

社群支援的[本地搜尋外掛程式列表請見此](https://docusaurus.io/community/resources#search)。
...
此操作會在專案目錄中建立 `src/themes/SearchBar` 檔案。重新啟動開發伺服器並編輯元件後，Docusaurus 將改用您的自訂 `SearchBar` 元件。

**注意**：您也可以[從 Algolia SearchBar 替換元件](#editing-the-algolia-search-component)，並以此為基礎建立自訂搜尋元件。