...
絕對檔案路徑也可以相對於網站目錄。但要注意，以 `/docs/` 或 `/blog/` 開頭的連結 **不可移植**，因為當你建立新的文件版本或本地化時，需要手動更新這些連結。
...
Markdown 檔案引用僅在來源檔案和目標檔案由同一個插件實例處理時才有效。這是我們 Markdown 處理架構的技術限制，未來將會修復。如果你需要在插件之間連結檔案（例如從部落格文章連結到文件頁面），則必須使用 URL 連結。
