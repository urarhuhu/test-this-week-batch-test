...
- `fallback`: 可選的渲染回調函數，返回一個 JSX 元素。它將接收一個包含兩個屬性的對象：`error`，捕獲到的錯誤，以及 `tryAgain`，一個用於重置組件錯誤並嘗試重新渲染的函數回調（`() => void`）。如果未提供，則將渲染 `@theme/Error` 作為替代。`@theme/Error` 用於包裹站點的錯誤邊界，位於佈局之上。
...
此組件支持鏈接到內部頁面，以及一個稱為預加載的強大性能功能。預加載用於預取資源，以便用戶通過此組件導航時資源已經被獲取。我們使用 `IntersectionObserver` 在 `<Link>` 進入視口時發起低優先級請求，然後使用 `onMouseOver` 事件在用戶可能導航到請求的資源時觸發高優先級請求。

該組件是 react-router 的 `<Link>` 組件的封裝，添加了特定於 Docusaurus 的有用增強功能。所有屬性都會傳遞給 react-router 的 `<Link>` 組件。

外部鏈接也可以使用，並自動具有這些屬性：`target="_blank" rel="noopener noreferrer"`。
...
優先使用此組件而不是原生的 `<a>` 標籤，因為如果您使用 `<Link>`，Docusaurus 會進行許多優化（例如檢測斷開的路徑、預取、應用基礎 URL 等）。
...
當程式碼需要存取 `window` 或 `document` 物件而無法在 Node.js 環境執行時，可使用此元件進行整合。
...
翻譯字串將透過 [`docusaurus write-translations`](./cli.mdx#docusaurus-write-translations-sitedir) CLI 靜態提取，並在 `website/i18n/[locale]` 目錄下生成 `code.json` 翻譯檔案。
...
用於存取 Docusaurus Context 的 React Hook。此 context 包含來自 [docusaurus.config.js](api/docusaurus.config.js.mdx) 的 `siteConfig` 物件和一些額外的網站元資料。
...
`siteConfig` 物件僅包含**可序列化的值**（即經過 `JSON.stringify()` 後仍保留的值）。函式、正則表達式等內容會在客戶端遺失。

:::

### `useIsBrowser` {#useIsBrowser}

當 React 應用在瀏覽器中完成水合（hydration）後會回傳 `true`。

:::warning

在 React 渲染邏輯中，請使用此鉤子而非 `typeof windows !== 'undefined'`。

客戶端首次渲染輸出（瀏覽器中）**必須與**伺服器端渲染輸出（Node.js）**完全一致**。違反此規則可能導致非預期的水合行為，如 [The Perils of Rehydration](https://www.joshwcomeau.com/react/the-perils-of-rehydration/) 所述。
...
有時 `useBaseUrl` 功能不足。此鉤子會回傳與站點基礎 URL 相關的額外工具函式。

- `withBaseUrl`：若需同時為多個 URL 加上基礎 URL 時非常實用。
...
全域資料會依插件名稱與插件 ID 進行命名空間劃分。

:::info

插件 ID 僅在同個插件在站點中被多次使用時才有意義。每個插件實例都能建立自己的全域資料。
...
您可以在 `./docusaurus/globalData.json` 檢視站點的全域資料。
...
存取特定插件建立的全域資料。給定插件名稱後，會依插件 ID 回傳該名稱下所有插件實例的資料。
