...
有時需要在網站發布公告。為此可添加公告欄——這是位於導航欄上方、非固定且可選擇關閉的面板。所有配置皆位於`announcementBar`物件中。
...
標誌可置於[靜態資料夾](static-assets.mdx)。標誌URL預設設為網站根目錄。雖然可自訂標誌URL，但若為外部連結將在新分頁開啟。此外可覆寫標誌連結的target屬性值，當文檔網站託管於主站子目錄時特別有用——此情況下可能不需讓標誌連結至主站時開啟新分頁。
...
可透過`themeConfig.navbar.items`添加項目至導航欄。
...
項目行為可依據`type`欄位而異。以下章節將介紹所有可用的導航欄項目類型。

#### 導航欄連結 {#navbar-link}

預設情況下，導航欄項目是常規連結（內部或外部）。

React Router 應會自動為連結套用活動連結樣式，但在特殊情況下可使用 `activeBasePath`。若某連結需在多個路徑下保持活動狀態（例如同一側邊欄下有多個文件夾），可使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更靈活的替代方案且優先級更高——Docusaurus 會將其解析為正則表達式並與當前 URL 進行匹配。

外站連結會自動添加 `target="_blank" rel="noopener noreferrer"` 屬性。
...
若需連結至特定文件，此特殊導航欄項目類型會渲染指向指定 `docId` 文件的連結。只要瀏覽同一側邊欄的文件，該連結就會獲得 `navbar__link--active` 類別。
...
若使用版本化文件，此特殊導航欄項目類型會渲染包含所有可用版本的下拉選單。

使用者可在同一文件（只要文件 ID 在不同版本中保持一致）間切換版本。
...
若您使用版本化文件，此特殊導覽列項目類型會連結至當前瀏覽的文件版本（依據目前網址決定），並在無匹配版本時自動回退至最新版本。
...
若啟用[國際化功能](../../i18n/i18n-introduction.mdx)，此特殊導覽列項目類型會顯示包含所有可用語系的下拉選單。

使用者可切換不同語系，同時保持停留在當前頁面。
...
可啟用此精緻的UI功能：當使用者向下滾動頁面時自動隱藏導覽列，並在向上滾動時重新顯示。
...
目前提供兩種樣式選項：`dark`（深色）與`primary`（基於`--ifm-color-primary`色彩）。樣式預覽請參閱[Infima文件](https://infima.dev/docs/components/navbar/)。
...
Docusaurus採用[Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer)實現程式碼高亮，所有設定皆位於`prism`物件中。
...
預設使用[Palenight](https://github.com/FormidableLabs/prism-react-renderer/blob/master/packages/prism-react-renderer/src/themes/palenight.ts)作為語法高亮主題。您可從[可用主題清單](https://github.com/FormidableLabs/prism-react-renderer/tree/master/packages/prism-react-renderer/src/themes)選擇自訂主題，亦可為深色模式指定不同的高亮主題。
...
若您使用行標記的 Markdown 語法，可能需要為深色模式的語法高亮主題指定不同的標記背景色。請參閱[行標記說明文件](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)取得指引。

:::

### 預設語言 {#default-language}

您可為程式碼區塊設定預設語言，當開頭三個反引號（即```）後未指定語言時使用。注意必須傳入有效的[語言名稱](https://prismjs.com/#supported-languages)。
...
可透過`themeConfig.footer`新增標誌與版權聲明。標誌可置於[靜態資源目錄](static-assets.mdx)，其URL設定方式與導覽列標誌相同。
...
可透過`themeConfig.footer.links`新增頁尾連結。頁尾配置分為兩種型態：**多欄式頁尾**與**簡易式頁尾**。
...
簡易式頁尾僅需設定一列橫向排列的`FooterItem`清單。
...
可透過`themeConfig.tableOfContents`調整預設目錄樣式。
...
用於存取色彩模式的React鉤子。此上下文包含設定亮/暗模式的函式，並暴露當前使用模式的布林值變數。
