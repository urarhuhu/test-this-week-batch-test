...
此插件在開發環境始終處於非活躍狀態，且**僅在生產環境生效**，因其運作機制依賴於構建輸出。
...
本插件會同時讀取 [`siteConfig.onDuplicateRoutes`](../docusaurus.config.js.mdx#onDuplicateRoutes) 配置，以在檢測到多個文件將被生成至相同路徑時調整其日誌級別。
...
"來源"(from)與"目標"(to)是本插件的核心概念。"來源"表示您希望創建的路徑，即一個將被寫入的額外HTML文件；"目標"表示您希望重定向至的路徑，通常是Docusaurus已識別的路由。

因此同一個"目標"可對應多個"來源"：我們會創建多個HTML文件來重定向至同一目的地。反之，一個"來源"不能對應多個"目標"：生成的HTML文件必須有確定的重定向終點。
