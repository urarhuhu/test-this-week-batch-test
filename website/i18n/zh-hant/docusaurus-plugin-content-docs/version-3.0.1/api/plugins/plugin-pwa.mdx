...
僅安裝 service worker 並不足以讓您的應用成為 PWA。您至少需要包含 [Web 應用程式清單](https://developer.mozilla.org/en-US/docs/Web/Manifest) 並在 `<head>` 中加入正確標籤（參見 [選項 > pwaHead](#pwahead)）。
...
![應用程式安裝過程畫面錄製。瀏覽器網址列出現安裝按鈕，點擊後顯示「安裝此應用程式？」對話框。點擊「安裝」按鈕後，作業系統會開啟新應用程式並顯示 Docusaurus 首頁。](/img/pwa_install.gif)

:::note

應用程式安裝需使用 HTTPS 協定與有效清單文件。
...
> Service worker 的關鍵功能之一是在安裝時將一組文件保存到快取中，此過程通常稱為「預先快取」(precaching)，因為您是在 service worker 被使用前就快取內容。
>
> 這樣做的主要原因是讓開發者能控制快取，意味著他們可以決定文件何時被快取、快取多久，以及不透過網路直接提供給瀏覽器，從而建立可離線運作的網路應用程式。
>
> Workbox 透過簡化 API 並確保資源高效下載，大幅降低了預先快取的實作難度。

預設情況下，當網站被安裝為應用程式時會啟用離線模式。詳見 `offlineModeActivationStrategies` 選項說明。

當網站完成預先快取後，service worker 會為後續訪問提供快取回應。當新版本部署並伴隨新 service worker 時，新版本會開始安裝並最終進入等待狀態。在此等待狀態期間，會顯示重新載入彈窗，要求使用者刷新頁面以取得新內容。在使用者清除應用程式快取或點擊彈窗中的 `reload` 按鈕前，service worker 將持續提供舊內容。

:::warning

離線模式/預先快取需提前下載網站所有靜態資源，可能消耗不必要的頻寬。並非所有類型網站都適合啟用此功能。
...
適用於額外的 Workbox 規則。您可在此執行 service worker 能做的任何操作，並充分利用 workbox 函式庫。程式碼會經過轉譯，因此可使用現代 ES6+ 語法。
