...
**Docusaurus 核心本身不提供任何功能。** 所有功能都委派給個別插件實現：[文檔功能](./guides/docs/docs-introduction.mdx)由[文檔插件](./api/plugins/plugin-content-docs.mdx)提供；[部落格功能](./blog.mdx)由[部落格插件](./api/plugins/plugin-content-blog.mdx)提供；而獨立[頁面](./guides/creating-pages.mdx)則由[頁面插件](./api/plugins/plugin-content-pages.mdx)提供。若未安裝任何插件，網站將不會包含任何路由。

您無需逐一手動安裝常見插件：它們可以透過[預設集](#using-presets)以套件形式分發。對多數使用者而言，插件是透過預設集配置來設定的。

我們維護了一份[官方插件清單](./api/plugins/overview.mdx)，但社群也開發了部分[非官方插件](/community/resources#community-plugins)。若您想新增任何功能：自動生成文檔頁面、執行自訂腳本、整合其他服務...請務必查閱該清單：可能已有人為您實現了該功能！

若您精力充沛，也可以閱讀[插件指南](./advanced/plugins.mdx)或[插件方法參考文件](./api/plugin-methods/README.mdx)來了解如何自行開發插件。
...
然而，插件可以透過在配置中將名稱和選項物件包裝成二元組來指定選項。此風格通常稱為「Babel 風格」。
...
主題的載入方式與插件完全相同。從使用者角度來看，在安裝和配置插件時，`themes` 和 `plugins` 條目是可互換的。唯一的細微差別在於主題是在插件之後載入的，且[主題可以覆蓋插件的預設主題組件](./advanced/client.mdx#theme-aliases)。

:::tip

`themes` 和 `plugins` 選項會導致不同的[簡寫解析方式](#module-shorthands)，因此若您想利用簡寫功能，請確保使用正確的條目！
...
預設集是插件與主題的組合套件。舉例來說，與其讓您在設定檔中逐一註冊並配置`@docusaurus/plugin-content-docs`、`@docusaurus/plugin-content-blog`等插件，我們提供了`@docusaurus/preset-classic`預設集，讓您能在單一集中位置配置這些插件。

### `@docusaurus/preset-classic` {#docusauruspreset-classic}

經典預設集預設會隨[`create-docusaurus`](./installation.mdx#scaffold-project-website)建立的新Docusaurus網站一同安裝。它包含以下主題與插件：
...
預設集通常是npm套件，因此您可以使用npm像安裝其他npm套件一樣安裝它們。
...
預設集是一個與[插件建構函式](./api/plugin-methods/README.mdx#plugin-constructor)具有相同結構的函式。它應該返回一個`{ plugins: PluginConfig[], themes: PluginConfig[] }`物件，與網站設定中接受的形式相同。例如，您可以指定一個包含以下主題與插件的預設集：
...
Docusaurus支援插件、主題與預設集的簡寫形式。當它看到一個插件/主題/預設集名稱時，會嘗試按以下順序載入：
...
如果名稱是有作用域的（以`@`開頭），名稱會先按第一個斜線分割為作用域與套件名稱：
...
若名稱不存在（例如 `@jquery`），則會載入 `[scope]/docusaurus-[moduleType]`（即 `@jquery/docusaurus-plugin`）。否則，系統會嘗試以下路徑：

- `[scope]/[name]`（例如 `@jquery/content-docs`）
- `[scope]/docusaurus-[moduleType]-[name]`（例如 `@jquery/docusaurus-plugin-content-docs`）

以下是以註冊在 `plugins` 欄位中的插件為例的說明。請注意，與 [ESLint](https://eslint.org/docs/user-guide/configuring/plugins#configuring-plugins) 或 [Babel](https://babeljs.io/docs/en/options#name-normalization) 強制要求插件命名遵循統一規範不同，Docusaurus 允許更大的命名自由度，因此解析結果並非絕對，而是依照上述優先級進行。
