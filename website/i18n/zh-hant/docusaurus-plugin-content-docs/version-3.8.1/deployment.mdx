...
預設情況下，這將在 [`http://localhost:3000/`](http://localhost:3000/) 加載您的網站。

## 尾部斜線配置 {#trailing-slashes}

Docusaurus 有一個 [`trailingSlash` 配置](./api/docusaurus.config.js.mdx#trailingSlash)，允許自定義 URL/鏈接和生成的文件名模式。

預設值通常運作良好。不幸的是，每個靜態託管提供商都有**不同的行為**，將相同的網站部署到不同的主機可能會導致不同的結果。根據您的主機，更改此配置可能很有用。

:::tip

使用 [slorber/trailing-slash-guide](https://github.com/slorber/trailing-slash-guide) 更好地了解您主機的行為並適當配置 `trailingSlash`。
...
## 選擇託管提供商 {#choosing-a-hosting-provider}

有幾種常見的託管選項：

- 使用 Apache2 或 Nginx 等 HTTP 伺服器進行[自託管](#self-hosting)。
- Jamstack 提供商（例如 [Netlify](#deploying-to-netlify) 和 [Vercel](#deploying-to-vercel)）。我們將以它們為參考，但相同的推理可以應用於其他提供商。
...
沒有一種萬能的解決方案。您需要權衡您的需求和資源後做出選擇。

## 自行託管 {#self-hosting}

Docusaurus 可透過 [`docusaurus serve`](cli.mdx#docusaurus-serve-sitedir) 指令自行託管。使用 `--port` 參數變更連接埠，`--host` 參數變更主機位址。
...
相較於靜態託管服務商/CDN，此非最佳解決方案。

:::

:::warning

後續章節將介紹常見託管服務商及其最佳化部署 Docusaurus 網站的設定方式。Docusaurus 與這些服務無商業合作，提供資訊僅供參考。部分說明由第三方提供，最新 API 變動可能未即時更新。若發現內容過時，歡迎提交 PR 修正。

由於我們僅能基於盡力而為原則維護此內容，已停止接受新增託管選項的 PR。您可將教學發布於外部網站（如個人部落格或服務商官網），並提請我們加入相關連結。

:::

## 部署至 Netlify {#deploying-to-netlify}

部署 Docusaurus 網站至 [Netlify](https://www.netlify.com/) 前，請確認以下設定已正確配置：
...
若未預先設定建置選項，仍可於站點建立後，至「Site settings」→「Build & deploy」補設定。

完成上述設定後，網站將自動部署，並在程式碼合併至部署分支（預設為 `main`）時觸發重新部署。

:::warning

部分 Docusaurus 網站將 `docs` 目錄置於 `website` 外（多見於 Docusaurus v1 舊版結構）：
...
若選擇將 `website` 目錄設為 Netlify 的基礎目錄，當 `docs` 目錄更新時 Netlify 不會觸發建置，此時需設定 [自訂忽略指令](https://docs.netlify.com/configure-builds/common-configurations/ignore-builds/)：
...
建議關閉 Netlify 的「Post Processing > Asset Optimization > Pretty Urls」設定，以避免網址強制小寫、非必要重新導向及 404 錯誤。

**請特別注意**：全局選項「Disable asset optimization」實際無法正確關閉「Pretty URLs」設定，務必獨立取消勾選該項目。
...
透過 [Vercel 的 Git 整合功能](https://vercel.com/docs/concepts/git)部署前，請確認專案已推送至 Git 儲存庫。

透過 [匯入流程](https://vercel.com/import/git) 將專案匯入至 Vercel。在匯入過程中，您會發現所有相關選項都已預先設定完成；不過，您仍可選擇變更其中任何 [選項](https://vercel.com/docs/build-step#build-&-development-settings)。

專案匯入後，所有後續推送至分支的動作都會產生 [預覽部署](https://vercel.com/docs/platform/deployments#preview)，而對 [生產分支](https://vercel.com/docs/git-integrations#production-branch)（通常是 "main" 或 "master"）所做的任何變更，則會觸發 [生產環境部署](https://vercel.com/docs/platform/deployments#production)。
...
通常發布流程會涉及兩個儲存庫（或至少兩個分支）：包含原始檔的分支，以及包含要透過 GitHub Pages 服務之建置輸出的分支。在以下教學中，它們將分別稱為 **「來源」** 和 **「部署」**。
...
若您想為 GitHub Pages 使用自訂網域，請在 `static` 目錄中建立一個 `CNAME` 檔案。`static` 目錄內的任何內容都會被複製到 `build` 目錄的根目錄以供部署。使用自訂網域時，您應能將 `baseUrl: '/projectName/'` 改回 `baseUrl: '/'`，並將 `url` 設為您的自訂網域。

更多細節可參考 GitHub Pages 的文件 [使用者、組織與專案頁面](https://help.github.com/en/articles/user-organization-and-project-pages)。

:::

GitHub Pages 會從預設分支（通常為 `master` 或 `main`）或 `gh-pages` 分支，以及根目錄或 `/docs` 資料夾中擷取已準備好部署的檔案（即 `docusaurus build` 的輸出）。您可透過儲存庫中的 `設定 > Pages` 來配置此選項。此分支將稱為「部署分支」。
...
這些欄位也有對應的環境變數，其優先順序更高：`ORGANIZATION_NAME`、`PROJECT_NAME` 和 `DEPLOYMENT_BRANCH`。
...
預設情況下，GitHub Pages 會透過 [Jekyll](https://jekyllrb.com/) 處理發布的檔案。由於 Jekyll 會捨棄任何以 `_` 開頭的檔案，建議您在 `static` 目錄中新增一個名為 `.nojekyll` 的空檔案來停用 Jekyll。
...
以下工作流程範例假設您的網站原始碼位於儲存庫的 `main` 分支（_來源分支_為 `main`），且您的[發佈來源](https://help.github.com/en/github/working-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site)已設定為[使用自訂 GitHub Actions 工作流程發佈](https://docs.github.com/en/pages/getting-started-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site#publishing-with-a-custom-github-actions-workflow)。

我們的目標是：

1. 當有新的拉取請求提交到 `main` 分支時，有一個動作會確保網站能成功建置，但不會實際部署。此工作將稱為 `test-deploy`。
...
- 部署儲存庫是一個**遠端**儲存庫，與來源不同。此情境的說明假設[發佈來源](https://help.github.com/en/github/working-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site)為 `gh-pages` 分支。
...
雖然您可以將兩個工作定義在同一個工作流程檔案中，但原始的 `deploy` 工作流程在 PR 檢查套件狀態中會始終顯示為已跳過，這無法反映實際狀態且對審查過程沒有幫助。因此，我們建議將它們作為獨立的工作流程來管理。
...
跨儲存庫發佈的設定較為複雜，因為您需要推送到另一個具有權限檢查的儲存庫。我們將使用 SSH 進行身份驗證。
...
2. 預設情況下，您的公開金鑰應已建立於 `~/.ssh/id_rsa.pub`；否則，請使用上一步中提供的名稱將金鑰添加到 [GitHub 部署金鑰](https://developer.github.com/v3/guides/managing-deploy-keys/)。
3. 使用 `pbcopy < ~/.ssh/id_rsa.pub` 將金鑰複製到剪貼簿，並將其作為 [部署金鑰](https://developer.github.com/v3/guides/managing-deploy-keys/#deploy-keys) 貼到部署儲存庫中。如果命令列無法使用，請直接複製檔案內容。在儲存部署金鑰前，勾選 `允許寫入存取` 選項。
4. 您需要將私密金鑰設為 [GitHub 秘密](https://help.github.com/en/actions/configuring-and-managing-workflows/creating-and-storing-encrypted-secrets)，以允許 Docusaurus 為您執行部署。
5. 使用 `pbcopy < ~/.ssh/id_rsa` 複製您的私密金鑰，並在原始碼儲存庫中建立一個名為 `GH_PAGES_DEPLOY` 的 GitHub 秘密。如果命令列無法使用，請直接複製檔案內容。儲存您的秘密。
...
持續整合 (CI) 服務通常用於在原始碼控制中提交新提交時執行例行任務。這些任務可以是執行單元測試和整合測試、自動化建置、將套件發佈到 npm 以及將變更部署到您的網站的任意組合。要自動化網站的部署，您只需在網站更新時呼叫 `yarn deploy` 腳本。以下部分介紹如何使用 [Travis CI](https://travis-ci.com/)（一個流行的持續整合服務提供商）來實現這一點。

1. 前往 https://github.com/settings/tokens 並生成一個新的 [個人存取權杖](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/)。建立權杖時，請授予其 `repo` 範圍，以便其擁有所需的權限。
2. 使用您的 GitHub 帳戶，[將 Travis CI 應用程式](https://github.com/marketplace/travis-ci) 添加到您要啟用的儲存庫。
3. 開啟您的 Travis CI 儀表板。網址類似於 `https://travis-ci.com/USERNAME/REPO`，並導航至儲存庫的 `更多選項 > 設定 > 環境變數` 部分。
...
現在，每當有新提交進入 `main` 分支時，Travis CI 將執行您的測試套件，如果一切通過，您的網站將透過 `yarn deploy` 腳本部署。
...
按照以下步驟建立一個管道，該管道會在您將變更推送到所選專案分支時自動部署新版本的網站：
...
3. 前往 https://github.com/settings/tokens 生成新的[個人存取權杖](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/)，並授予 `repo` 範圍。
...
[Flightcontrol](https://www.flightcontrol.dev/?ref=docusaurus) 是一項服務，可自動從您的 Git 儲存庫直接建置並部署網頁應用至 AWS Fargate。它讓您能夠完整檢視並進行基礎架構變更，不受傳統 PaaS 的限制。
...
[Koyeb](https://www.koyeb.com) 是一個對開發者友善的無伺服器平台，可全球部署應用程式。該平台讓您能無縫運行 Docker 容器、網頁應用程式和 API，具備基於 Git 的部署、原生自動擴展、全球邊緣網路，以及內建的服務網格和服務發現功能。查看 [Koyeb 的 Docusaurus 部署指南](https://www.koyeb.com/tutorials/deploy-docusaurus-on-koyeb) 以開始使用。
...
4. 部署。
   ...
   - 開始後，您將進入部署日誌。如果程式碼有任何問題，您將在日誌中收到警告或錯誤訊息，指出問題原因。通常，日誌會包含您所需的所有除錯資料。
...
1. 首先，透過 npm 安裝 Surge，執行以下指令：
   ```bash npm2yarn
   npm install -g surge
   ```
2. 在專案的根目錄中，執行以下指令以建置網站的靜態檔案供生產環境使用：
   ```bash npm2yarn
   npm run build
   ```
3. 然後，在專案的根目錄中執行以下指令：
...
[Cloudflare Pages](https://pages.cloudflare.com/) 是專為前端開發者設計的 Jamstack 協作與部署平台。請依照[此頁面](https://developers.cloudflare.com/pages/framework-guides/deploy-a-docusaurus-site/)指引，幾分鐘內即可開始使用。

## 部署至 Azure Static Web Apps {#deploying-to-azure-static-web-apps}

[Azure Static Web Apps](https://docs.microsoft.com/en-us/azure/static-web-apps/overview) 是一項能自動從程式碼儲存庫建置並部署全端網頁應用至 Azure 的服務，簡化了 CI/CD 的開發者體驗。此服務將靜態資源與動態 API 端點分離，靜態檔案透過全球分發的內容伺服器提供，使客戶端能就近取得檔案以加速載入；動態 API 則採用無伺服器架構，基於事件驅動的函式進行擴展，更具成本效益且能隨需求彈性擴容。請參閱[這篇逐步指南](https://dev.to/azure/11-share-content-with-docusaurus-azure-static-web-apps-30hc)，幾分鐘內即可開始部署。

## 部署至 Kinsta {#deploying-to-kinsta}

[Kinsta 靜態網站託管服務](https://kinsta.com/static-site-hosting)免費提供最多 100 個靜態站點部署額度，支援自訂網域與 SSL 憑證、每月 100 GB 流量，並透過 260+ 個 Cloudflare CDN 節點加速。

請依照我們的 [Kinsta 上的 Docusaurus 部署教學](https://kinsta.com/docs/docusaurus-example/)，只需點擊幾下即可快速開始。