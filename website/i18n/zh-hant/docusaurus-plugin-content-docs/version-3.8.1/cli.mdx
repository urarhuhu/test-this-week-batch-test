...
Docusaurus 提供一系列腳本協助您生成、運行及部署網站。

當網站初始化完成後，專案原始碼將包含可透過套件管理器執行的 Docusaurus 腳本：
