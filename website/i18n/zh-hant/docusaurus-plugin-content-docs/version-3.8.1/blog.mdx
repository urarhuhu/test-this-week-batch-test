...
在部落格文章中使用 `<!--truncate-->` 標記來表示在查看所有已發布部落格文章時將顯示為摘要的部分。`<!--truncate-->` 上方的任何內容都將成為摘要的一部分。請注意，截斷標記上方的部分必須是獨立可渲染的 Markdown。例如：
...
部落格側邊欄會顯示最近的部落格文章。預設顯示的項目數量為 5，但您可以透過插件配置中的 `blogSidebarCount` 選項進行自訂。設定 `blogSidebarCount: 0` 將完全停用側邊欄，並移除容器，這將增加主容器的寬度。特別地，如果設定 `blogSidebarCount: 'ALL'`，則會顯示 _所有_ 文章。
...
此命名慣例為可選項，您也可以透過 front matter 提供日期。由於 front matter 遵循支援日期時間標記法的 YAML 語法，若您需要更精確的發布時間，可使用 front matter 來指定。例如，若您有多篇文章在同一天發布，可依照當天時間排序：
...
使用 `authors` front matter 欄位來宣告部落格文章作者。作者至少需具備 `name` 或 `image_url` 屬性。Docusaurus 會運用 `url`、`email` 和 `title` 等資訊，但其他任何資訊皆可自由添加。
...
無論透過 front matter 或作者對照表宣告的作者，都必須具備名稱或頭像（或兩者皆有）。若某篇文章的所有作者皆未提供名稱，Docusaurus 會以緊湊方式顯示其頭像。效果請參見[此測試文章](/tests/blog/2022/01/20/image-only-authors)。
...
此功能僅支援[全域作者](#global-authors)，[內嵌作者](#inline-authors)無法使用。
...
您可以定義內嵌標籤，或引用預先定義在[`標籤檔案`](api/plugins/plugin-content-blog.mdx#tags-file)（通常為`blog/tags.yml`）中的標籤。

在以下範例中：

- `docusaurus`引用的是在`blog/tags.yml`中宣告的預定義標籤鍵
...
- `frontMatter`：前言資料（鍵值對形式的記錄）
- `locale`：當前Docusaurus網站的地區設定
- `defaultReadingTime`：預設的內建閱讀時間計算函數。它會返回一個數字（閱讀時間分鐘數）或`undefined`（停用此頁面的閱讀時間顯示）。
...
通過設置 `feedOptions` 可生成 RSS/Atom/JSON 訂閱源。預設會生成 RSS 和 Atom 源，若要禁用此功能，請將 `feedOptions.type` 設為 `null`。
...
您可以將部落格文章列表頁設為首頁，無需獨立落地頁。將 `routeBasePath` 設為 `'/'` 可使部落格通過根路徑 `example.com/` 訪問（而非子路徑 `example.com/blog/`）。
...
務必刪除現有首頁文件 `./src/pages/index.js`，否則會出現兩個文件映射至相同路由！

:::

:::warning

若停用文檔插件，請同步刪除配置文件中所有相關引用，特別是導航欄中的文檔項目。

:::

:::tip

另有「純文檔模式」適用僅需文檔功能的用戶，詳見[純文檔模式](./guides/docs/docs-introduction.mdx)的詳細說明或對 `routeBasePath` 的進階解釋。

:::

### 多重部落格 {#multiple-blogs}

經典主題預設單一部落格架構，若需在同一網站建立多個部落格，可在 `docusaurus.config.js` 的 `plugins` 選項中新增部落格插件實例。

將 `routeBasePath` 設定為您希望第二個部落格被訪問的 URL 路由。請注意，這裡的 `routeBasePath` 必須與第一個部落格不同，否則可能會發生路徑衝突！同時，將 `path` 設定為包含第二個部落格條目的目錄路徑。

如 [多實例插件](./using-plugins.mdx#multi-instance-plugins-and-plugin-ids) 文件中所述，您需要為插件分配一個唯一的 ID。
...
舉例來說，我們在 [這裡](/tests/blog) 託管了第二個部落格。