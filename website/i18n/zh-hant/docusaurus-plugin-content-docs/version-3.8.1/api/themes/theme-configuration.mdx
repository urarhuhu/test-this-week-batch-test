...
當設定`respectPrefersColorScheme: true`時，`defaultMode`會被使用者系統偏好設定覆寫。
...
您可配置預設圖片用於元標籤，特別是`og:image`與`twitter:image`。
...
有時您需要在網站發布公告。為此您可添加公告欄——這是位於導覽列上方、非固定且可選擇性關閉的面板。所有配置皆位於`announcementBar`物件中。
...
標誌可放置於[靜態資料夾](static-assets.mdx)中。標誌URL預設設為您網站的基礎URL。雖然您可以指定自訂標誌URL，但若為外部連結將會在新分頁開啟。此外，您可覆寫標誌連結的target屬性值，這在您將文檔網站託管於主網站子目錄時特別有用——此情況下您可能不需要標誌連結在主網站開啟新分頁。
...
這些項目可根據 `type` 欄位具有不同的行為。以下章節將介紹所有可用的導航欄項目類型。

#### 導航欄連結 {#navbar-link}

預設情況下，導航欄項目是常規連結（內部或外部）。

React Router 應自動為連結套用活動連結樣式，但在邊緣情況下可使用 `activeBasePath`。若連結需在多個不同路徑上保持活動狀態（例如當您在同一側邊欄下有多個文檔資料夾時），可使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更靈活的替代方案，且優先級高於後者——Docusaurus 會將其解析為正則表達式並與當前 URL 進行匹配測試。
...
除了上述欄位外，您還可指定其他任意屬性套用至 HTML 連結。
...
請注意，下拉基礎項目本身也是可點擊的連結，因此該項目可接收[純導航欄連結](#navbar-link)的任何屬性。
...
若需連結至特定文檔，此特殊導航欄項目類型會渲染指向所提供 `docId` 文檔的連結。只要您瀏覽同一側邊欄的文檔，該連結就會獲得 `navbar__link--active` 類別。
...
您可將導航欄項目連結至指定側邊欄的第一個文檔連結（可以是文檔連結或自動生成的類別索引），而無需硬編碼文檔 ID。
...
#### 自訂HTML導航欄 {#navbar-with-custom-html}
...
您可以啟用此酷炫的UI功能，當使用者向下滾動頁面時自動隱藏導航欄，並在使用者向上滾動時再次顯示。
...
您可以在不影響主題切換功能的情況下，設定靜態導航欄樣式。無論使用者選擇何種主題，所選樣式都將始終生效。

目前有兩種樣式選項：`dark`（深色）和`primary`（基於`--ifm-color-primary`顏色）。您可以在[Infima文件](https://infima.dev/docs/components/navbar/)中預覽這些樣式。
...
Docusaurus使用[Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer)來突顯代碼區塊。所有配置都在`prism`物件中。
...
如果您使用行高亮 Markdown 語法，可能需要為深色模式語法高亮主題指定不同的高亮背景色。請參考[指引文件](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)。

:::

### 預設語言 {#default-language}

您可以為程式碼區塊設定預設語言（當開頭三個反引號後未指定語言時使用）。請注意必須傳入有效的[語言名稱](https://prismjs.com/#supported-languages)。
...
您可以透過 `themeConfig.footer` 在頁尾添加標誌和版權聲明。標誌可放置於[靜態資料夾](static-assets.mdx)中，其 URL 設定方式與導覽列標誌相同。
...
您可以透過 `themeConfig.footer.links` 在頁尾添加連結。頁尾配置有兩種型態：**多欄式頁尾**和**簡易頁尾**。

多欄式頁尾連結會為每個欄位設定 `title` 和一組 `FooterItem` 清單。
...
- `colorModeChoice: 'light' | 'dark' | null`: 使用者明確選擇的色彩模式。若使用者尚未做出選擇，或已將選擇重置為系統/預設值，則可能為 `null`。
- `setColorMode(colorModeChoice: 'light' | 'dark' | null, options: {persist: boolean}): void`: 當使用者明確選擇色彩模式時呼叫的函式。傳入 `null` 可將選擇重置為系統/預設值。預設情況下，選擇會持久化儲存於 `localStorage` 並在頁面重新載入時恢復，但可透過 `{persist: false}` 選擇退出此行為。

:::warning

請勿在渲染 React 元件時使用 `colorMode` 和 `colorModeChoice`。這樣做很可能會導致 [FOUC](https://en.wikipedia.org/wiki/Flash_of_unstyled_content)、版面位移以及 [React 水合](https://18.react.dev/reference/react-dom/client/hydrateRoot)不匹配的問題，特別是當您用它們來動態渲染 JSX 內容時。

然而，這些值在 **React 水合完成後** 使用是安全的，例如在 `useEffect` 和事件監聽器中，如同上方 `MyColorModeButton` 範例所示。

若您需要根據當前主題動態渲染內容，避免 FOUC、版面位移和水合不匹配的唯一方法是依賴 CSS 選擇器來動態渲染內容，基於我們在頁面顯示任何內容前設定的 `html` 資料屬性：
...
<details>
 <summary>為何在渲染時使用 `colorMode` 和 `colorModeChoice` 是不安全的？</summary>

要理解此問題，您需要了解 [React 水合](https://18.react.dev/reference/react-dom/client/hydrateRoot) 的運作原理。

在靜態網站生成階段，Docusaurus 並不知道使用者的色彩模式選擇，此時 `useColorMode()` 會回傳以下靜態值：

- `colorMode = themeConfig.colorMode.defaultMode`
- `colorModeChoice = null`

在首次 React 客戶端渲染（水合）期間，React 必須產生完全相同的 HTML 標記，並同樣使用這些靜態值。

正確的 `colorMode` 和 `colorModeChoice` 值僅會在第二次 React 渲染時提供。

典型情況下，以下元件會導致 **React 水合不匹配**。標籤可能會在 React 水合過程中從 `light` 切換為 `dark`，造成使用者體驗混亂。
