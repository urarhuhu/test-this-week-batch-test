...
靜態方法不屬於插件實例，而是附加在構造函數上。這些方法用於驗證和標準化插件選項與主題配置，這些參數隨後將作為構造函數參數用於初始化插件實例。
...
`validateOptions` 會接收傳遞給插件的 `options` 以進行驗證和標準化。
...
若您不使用 **[Joi](https://www.npmjs.com/package/joi)** 進行驗證，可以在選項無效時拋出錯誤，並在成功時返回選項。
...
`validateThemeConfig` 會接收 `docusaurus.config.js` 中提供的 `themeConfig` 以進行驗證和標準化。
...
若您不使用 **[Joi](https://www.npmjs.com/package/joi)** 進行驗證，可以在選項無效時拋出錯誤。
