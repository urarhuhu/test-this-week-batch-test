...
`options`是[使用插件時的第二個可選參數](../../using-plugins.mdx#configuring-plugins)。`options`是插件特定的，由用戶在`docusaurus.config.js`中使用時指定。如果導出了[`validateOptions`](./static-methods.mdx#validateOptions)函數，則會事先對`options`進行驗證和標準化。

或者，如果預設集包含該插件，則預設集將負責將正確的選項傳遞給插件。具體接受哪些選項由各個插件自行定義。
