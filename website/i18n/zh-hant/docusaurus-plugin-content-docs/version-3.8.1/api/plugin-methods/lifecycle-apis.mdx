...
在建置過程中，插件會平行載入以獲取各自的內容並將其渲染為路由。插件亦可配置 webpack 或對生成的文件進行後處理。
...
`loadContent` 載入的資料將在 `contentLoaded` 中被消耗。可將其渲染為路由、註冊為全域資料等。
...
宣告式回呼函式，用於建立靜態資料（通常為 JSON 或字串），後續可作為 props 提供給您的路由。接收檔案名稱及要儲存的資料，並回傳實際資料檔案的路徑。
...
這些資料可透過 [`useGlobalData`](../../docusaurus-core.mdx#useGlobalData) 和 [`usePluginData`](../../docusaurus-core.mdx#usePluginData) 鉤子在您的客戶端/主題程式碼中存取。
...
修改內部 webpack 配置。若返回值為 JavaScript 物件，將使用 [`webpack-merge`](https://github.com/survivejs/webpack-merge) 合併至最終配置中。若為函式，則會呼叫該函式並接收 `config` 作為第一個參數，`isServer` 標誌作為第二個參數。
...
`configureWebpack` 會根據客戶端/伺服器端建置傳入生成的 `config`。您可以將其視為待合併的基礎配置。

### `isServer` {#isServer}

`configureWebpack` 會在伺服器建置和客戶端建置時都被呼叫。伺服器建置會收到 `true` 作為 `isServer` 參數，而客戶端建置會收到 `false`。
...
例如，下面的插件修改 webpack 配置來轉譯 `.foo` 檔案。
...
`injectHtmlTags` 會同時被呼叫並帶入由插件載入的內容。
