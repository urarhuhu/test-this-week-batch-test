## 標籤檔案 {#tags-file}

使用 [`tags` 插件選項](#tags) 來配置 YAML 標籤檔案的路徑。

按照慣例，插件會在您的內容資料夾根目錄中尋找 `tags.yml` 檔案。

此檔案可包含預定義標籤列表。您可以在 Markdown 檔案中透過 [`tags` front matter](#markdown-front-matter) 來引用這些標籤。

:::tip[保持標籤一致性]

使用標籤檔案可確保您的標籤使用在插件內容集中保持一致。使用 [`onInlineTags: 'throw'`](#onInlineTags) 插件選項來強制執行此一致性，並防止使用臨時宣告的內聯標籤。

:::

### 類型 {#tags-file-types}

提供的標籤檔案的 YAML 內容應符合以下結構：

```tsx
type Tag = {
  label?: string; // Tag display label
  permalink?: string; // Tag URL pathname segment
  description?: string; // Tag description displayed in the tag page
};

type TagsFileInput = Record<string, Partial<Tag> | null>;
```

### 範例 {#tags-file-example}

```yml title="tags.yml"
releases:
  label: 'Product releases'
  permalink: '/product-releases'
  description: 'Content related to product releases.'

# A partial tag definition is also valid
announcements:
  label: 'Announcements'

# An empty tag definition is also valid
# Other attributes will be inferred from the key
emptyTag:
```

```md title="content.md"
---
tags: [releases, announcements, emptyTag]
---

# Title

Content
```