...
Docusaurus 的預設頁面外掛程式。經典模板預設已配置此外掛程式，提供[建立頁面](guides/creating-pages.mdx)功能。
...
若您使用 `@docusaurus/preset-classic` 預設配置，則無需額外安裝此外掛程式作為依賴項。

您可透過[預設選項](../../using-plugins.mdx#docusauruspreset-classic)配置此外掛程式。
...
您可透過預設選項或外掛程式選項配置此外掛程式。

:::tip

多數 Docusaurus 使用者透過預設選項配置此外掛程式。
...
Markdown 頁面可使用下列 Markdown [前置資料](../../guides/markdown-features/markdown-features-intro.mdx#front-matter)元資料欄位，需以 `---` 分隔線包夾。
