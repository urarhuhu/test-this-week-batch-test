...
用於透過 [XSLT](https://developer.mozilla.org/en-US/docs/Web/XSLT) 美化部落格 XML 訂閱源的顯示樣式。

- 設為 `true` 使用內建的 `.xsl` 與 `.css` 檔案
- 設為假值 (`undefined | null | false`) 停用此功能
- 設為 `string` 可指定部落格內容目錄下的自訂 `.xsl` 檔案路徑（需搭配同名 `.css` 檔案）
...
Markdown 文件可使用以下以 `---` 包圍的 [前言資料](../../guides/markdown-features/markdown-features-intro.mdx#front-matter) 欄位。
...
透過 [`authors` 外掛選項](#authors) 可指定 YAML 作者檔案路徑。

依慣例，外掛會自動尋找部落格內容根目錄下的 `authors.yml` 檔案。

此檔案可定義 [全域部落格作者](../../blog.mdx#global-authors)，並透過 Markdown 文件的 [`authors` 前言資料](#markdown-front-matter) 引用作者鍵值。
