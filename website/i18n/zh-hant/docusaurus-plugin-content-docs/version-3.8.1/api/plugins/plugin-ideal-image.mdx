...
預設情況下，該插件在開發環境中處於**非活動狀態**，以便您始終可以查看完整尺寸的圖片。如果您想調試理想圖片的行為，可以將 [`disableInDev`](#disableInDev) 選項設置為 `false`。
...
此插件註冊了一個 [Webpack loader](https://webpack.js.org/loaders/)，會改變導入/請求的圖片類型：
...
從 [pnpm 10](https://github.com/pnpm/pnpm/releases/tag/v10.0.0) 開始，運行 `pnpm install` 預設不會執行依賴項的安裝腳本。為了讓我們的 `sharp` 圖片縮放依賴項正確安裝，您需要額外的 pnpm 配置（[問題](https://github.com/lovell/sharp/issues/4343)），例如：
