...
- `fallback`: 可選的渲染回調函數，返回一個 JSX 元素。它將接收一個包含 2 個屬性的對象：`error`，捕獲到的錯誤，以及 `tryAgain`，一個用於重置組件錯誤並嘗試重新渲染的函數回調（`() => void`）。如果未提供，則會渲染 `@theme/Error` 代替。`@theme/Error` 用於包裹站點的錯誤邊界，位於佈局之上。
...
優先使用此組件而不是原生的 `<a>` 標籤，因為如果您使用 `<Link>`，Docusaurus 會進行許多優化（例如檢測無效路徑、預取、應用基礎 URL 等）。
...
當需要整合的程式碼無法在 Node.js 環境執行（例如存取 `window` 或 `document` 物件時）可使用此元件。

:::

#### Props {#browseronly-props}

- `children`: 渲染函式 prop，返回僅在瀏覽器執行的 JSX。不會在 Node.js 環境執行
...
翻譯字串會透過 [`docusaurus write-translations`](./cli.mdx#docusaurus-write-translations-sitedir) CLI 靜態提取，並在 `website/i18n/[locale]` 目錄下生成 `code.json` 翻譯檔案。
...
`<Translate>` 元件支援插值功能。您也可以透過自訂程式碼和 [`translate` 命令式 API](#translate-imperative) 實作[字串複數化](https://github.com/facebook/docusaurus/pull/i18n/i18n-tutorial.mdx#pluralization)。
...
用於存取 Docusaurus 上下文(Context)的 React Hook。該上下文包含來自 [docusaurus.config.js](api/docusaurus.config.js.mdx) 的 `siteConfig` 物件和一些額外的網站元資料。
...
客戶端首次渲染輸出（瀏覽器端）**必須與**伺服器端渲染輸出（Node.js 端）**完全一致**。違反此規則可能導致非預期的水合行為，詳見 [The Perils of Rehydration](https://www.joshwcomeau.com/react/the-perils-of-rehydration/)。
...
用於存取 Docusaurus 斷鏈檢查 API 的 React Hook，提供 Docusaurus 頁面回報與收集連結及錨點的方式。

:::warning

這是**進階** API，**多數 Docusaurus 使用者無需直接使用**。
