...
我們推薦使用 `classic` 模板，以便快速開始，它包含 Docusaurus 1 中的功能。`classic` 模板包含 `@docusaurus/preset-classic`，其中包含標準文件、部落格、自訂頁面和 CSS 框架（支援暗黑模式）。使用經典模板可以極快地啟動，並在您對 Docusaurus 更加熟悉後進行自訂。
...
假設您選擇了經典模板並將網站命名為 `my-website`，您將在 `my-website/` 目錄下看到以下生成的檔案：
...
- `/src/` - 非文件類檔案，例如頁面或自訂 React 元件。您不一定要將非文件檔案嚴格放置於此，但集中存放便於在需要進行某種 linting/處理時指定路徑
  - `/src/pages` - 此目錄下的任何 JSX/TSX/MDX 檔案都將轉換為網站頁面。詳情請參閱[頁面指南](guides/creating-pages.mdx)
- `/static/` - 靜態目錄。此處所有內容將被複製到最終 `build` 目錄的根目錄下
...
- `/sidebars.js` - 供文件系統用於指定側邊欄中文檔的排序

### 單體式儲存庫 (Monorepos) {#monorepos}

若您使用 Docusaurus 為現有專案編寫文件，單體式儲存庫可能是解決方案。此架構允許您在相似專案間共享依賴項。例如，您的網站可使用本地套件展示最新功能，而非依賴已發布版本。如此一來，貢獻者能在實作功能時同步更新文件。下方為單體式儲存庫的範例目錄結構：
...
若使用 Netlify 或 Vercel 等託管服務，需將網站的「基礎目錄」(Base directory) 變更為 Docusaurus 根目錄所在位置（本例中為 `./website`）。更多關於設定忽略指令的說明，請參閱[部署文件](./deployment.mdx#deploying-to-netlify)。

更多單體式儲存庫資訊請參閱 [Yarn 文件](https://yarnpkg.com/features/workspaces)（Yarn 並非唯一設定方式，但屬常見解決方案），或參考 [Docusaurus](https://github.com/facebook/docusaurus) 與 [Jest](https://github.com/facebook/jest) 的實際案例。
...
建置完成後，內容將生成於 `/build` 目錄內，可將其部署至任何靜態檔案託管服務，例如 [GitHub Pages](https://pages.github.com/)、[Vercel](https://vercel.com/) 或 [Netlify](https://www.netlify.com/)。詳情請參閱[部署文件](deployment.mdx)。
...
`npm install` 可能會回報數個安全性漏洞並建議執行 `npm audit` 來修補。通常這些被回報的漏洞（例如 RegExp DOS 漏洞）是無害的，可以安全忽略。另請閱讀這篇反映我們觀點的文章：[npm audit: Broken by Design](https://overreacted.io/npm-audit-broken-by-design/)。
