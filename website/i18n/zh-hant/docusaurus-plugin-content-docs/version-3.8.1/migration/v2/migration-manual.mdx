...
此手動遷移流程應在[自動遷移流程](./migration-automated.mdx)完成後執行，以補足缺失部分或除錯遷移 CLI 的輸出問題。
...
Docusaurus 1 的建置產出物存放於 `website/build/<PROJECT_NAME>`。

Docusaurus 2 則簡化為 `website/build`，請確認部署設定已更新至正確讀取新目錄。
...
原有 D1 網站的 README 文件可修改為反映 D2 變更，或直接複製預設的[Docusaurus v2 README](https://github.com/facebook/docusaurus/blob/main/packages/create-docusaurus/templates/shared/README.md)。
...
已棄用。我們為 Docusaurus 2 開發了名為 [Infima](https://infima.dev/) 的自訂 CSS 框架，採用 CSS 變數實現主題功能。相關文檔尚未完備，後續將在此更新。若要覆寫 Infima 的 CSS 變數，請建立自訂 CSS 檔案（如 `./src/css/custom.css`），並通過 `@docusaurus/preset-classic` 的選項全域導入：
...
建議使用 [ColorBox](https://www.colorbox.io/) 工具為您選擇的主色調找出合適的色階。

或者，使用以下工具生成網站所需的色階變數，並將變數複製到 `src/css/custom.css` 中：
...
網站元資訊（如資源、SEO、版權資訊）現由主題處理。若要自訂，請在 `docusaurus.config.js` 中使用 `themeConfig` 欄位：
...
您的 Algolia DocSearch v1 配置（位於[此處](https://github.com/algolia/docsearch-configs/blob/master/configs)）需針對 Docusaurus v2 進行更新（[範例](https://github.com/algolia/docsearch-configs/tree/master/configs/docusaurus-2.json)）。

可聯繫 DocSearch 團隊 (@shortcuts, @s-pace) 獲取支援。他們能協助更新配置並觸發站點重新爬取以恢復搜索功能（否則需等待最多 24 小時待下次排程爬取）
...
已棄用。請改為將其作為選項傳遞給 `@docusaurus/preset-classic` 的 docs 配置：
...
- `blogSidebarTitle`
- `cleanUrl` - 現在預設使用簡潔網址。
- `defaultVersionShown` - 版本功能尚未移植。如果您正在使用版本功能，將無法遷移至 Docusaurus 2。請持續關注。
- `disableHeaderTitle`
- `disableTitleTagline`
...
出於 SEO 考量並避免連結失效，您應在託管服務提供商處設定伺服器端重定向規則。
...
舊版本不支援巢狀側邊欄分類，且分類僅能包含文件 ID。而 v2 允許無限層級巢狀結構，並提供多種[側邊欄項目類型](../../guides/docs/sidebar/items.mdx)（不局限於文件）。
...
v2 版需在每個頁面外層套用主題佈局。Layout 元件會接收元資料 props。
...
Docusaurus 2 已將 Markdown 語法改為 [MDX](https://mdxjs.com/)，因此現有文件可能需修正不相容語法。常見範例如 HTML 有效的自閉合標籤 `<img>` 和 `<br>`，現需明確閉合為 `<img/>` 和 `<br/>`。MDX 文件中的所有標籤都必須符合 JSX 規範。
...
**提示**：您可以使用線上工具如 [HTML 轉 JSX](https://transform.tools/html-to-jsx) 來簡化遷移過程。

### 語言專屬的程式碼分頁 {#language-specific-code-tabs}

請參閱[多語言支援程式碼區塊](../../guides/markdown-features/markdown-features-code-blocks.mdx#multi-language-support-code-blocks)章節。

### Front matter {#front-matter}

Docusaurus 的部落格 front matter 欄位已從駝峰式命名改為蛇底式命名，以與文件保持一致。

欄位 `authorFBID` 和 `authorTwitter` 已被棄用。它們僅用於生成作者頭像，現在可透過 `authors` 欄位實現。

## 部署 {#deployment}

GitHub Pages 使用的 `CNAME` 檔案不再自動生成，若使用自訂網域，請確保在 `/static/CNAME` 建立該檔案。

部落格 RSS 訂閱位址變更為 `/blog/rss.xml`（原為 `/blog/feed.xml`）。建議設定伺服器端重新導向以維持訂閱功能。
