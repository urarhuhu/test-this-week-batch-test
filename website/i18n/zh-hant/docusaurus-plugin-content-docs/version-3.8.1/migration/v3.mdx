...
請查閱 [**Docusaurus v3.0.0**](https://github.com/facebook/docusaurus/releases/tag/v3.0.0) 的發布說明，並瀏覽相關的 pull-requests 以獲取更多有用資訊及本文提及的每項變更背後的原因。
...
此命令也是估算使內容相容所需工作量的好方法。請記住，大部分工作可以通過[為 Docusaurus v3 準備您的內容](/blog/preparing-your-site-for-docusaurus-v3)在升級前完成。
...
升級至 MDX 伴隨著所有在 [MDX v2](https://mdxjs.com/blog/v2/) 和 [MDX v3](https://mdxjs.com/blog/v3/) 發布部落格文章中記錄的重大變更。大多數重大變更來自 MDX v2，而 MDX v3 是一個相對較小的發布。[MDX v2 遷移指南](https://mdxjs.com/migrating/v2/) 中有一個關於如何[更新 MDX 文件](https://mdxjs.com/migrating/v2/#update-mdx-files)的章節，這對我們特別相關。同時，請確保閱讀[MDX 疑難排解](https://mdxjs.com/docs/troubleshooting-mdx/)頁面，該頁面可以幫助您解讀常見的 MDX 錯誤訊息。

請確保也閱讀我們更新的 [**MDX 與 React**](../guides/markdown-features/markdown-features-react.mdx) 文件頁面。

### 使用 MDX 遊樂場

MDX 遊樂場是您的新好朋友。它允許您理解您的內容如何**被編譯為 React 元件**，並在隔離環境中疑難排解編譯或渲染問題。
...
使用此 CLI 來估計使您的內容與 MDX v3 兼容需要多少工作。
...
> 名稱中出現意外字元 `@` (U+0040)，預期應為字母、數字、`$` 或 `_` 等名稱字元；屬性前的空白；或標籤結尾（注意：在 MDX 中建立連結請使用 `[文字](網址)`）
...
使用標準 Markdown 連結語法，或移除 `<` 和 `>`。MDX 和 GFM 已能自動識別文字連結。
...
你的 [`MDXComponent` 元件映射](../guides/markdown-features/markdown-features-react.mdx#mdx-component-scope)可能不會像之前那樣生效，自訂元件可能不再被使用。
...
您也可以將此類內容用 `{` 和 `}` 包裹起來，以避免額外的 `<p>` 標籤，如果您不打算在那裡使用 Markdown 語法。
...
Docusaurus v3 現在使用 [Markdown 指令](https://talk.commonmark.org/t/generic-directives-plugins-syntax/444)（通過 [remark-directive](https://github.com/remarkjs/remark-directive) 實現）作為一種通用方式來提供 admonitions 和其他即將推出的 Docusaurus 功能的支持。
...
指令被解析的目的是由其他 Remark 插件處理。未處理的指令將被忽略，並且不會以其原始形式渲染回來。
...
新版 MDX 解析器現已嚴格遵循 CommonMark 規範。自 v0.14 起，CommonMark 規範針對強調符號與空格及標點符號的交互制定了新規則，這尤其會影響不使用空格分隔單詞的語言。

日文和中文受此影響最為顯著，但其他某些語言（如泰文和高棉文）也可能受到波及，例如當您嘗試強調行內程式碼或連結時。使用空格分隔單詞的語言所受影響則相對較小。

以下範例中的 `**`（除了 `` `**` `` 外）在 Docusaurus 2 中會被正確解析為強調標記，但在 Docusaurus 3 中則不會。
...
MDX 生態系統中的所有官方套件（Unified、Remark、Rehype...）現已全面轉為[**僅支援 ES 模組**](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)，不再支援 [CommonJS](https://nodejs.org/api/modules.html#modules-commonjs-modules)。

這意味著您無法再使用 `require("remark-plugin")` 的寫法。

:::tip[升級指南]

Docusaurus v3 現支援[**ES 模組**](https://flaviocopes.com/es-modules/)格式的設定檔。建議您將設定檔遷移至 ES 模組，以便輕鬆導入 Remark 插件：
...
若需繼續使用 CommonJS 模組，可透過動態導入作為變通方案，實現在 CommonJS 模組中導入 ES 模組。幸運的是，[Docusaurus 設定支援非同步函式語法](/docs/configuration#syntax-to-declare-docusaurus-config)來實現此需求：
...
若您開發了自訂的 Remark 或 Rehype 插件，由於新版 AST 結構的變更，您可能需要重構甚至完全改寫這些插件。我們已建立[專屬支援討論串](https://github.com/facebook/docusaurus/discussions/9337)來協助插件開發者升級程式碼。

:::

### 程式碼格式化工具

目前最常用的格式化工具 Prettier 僅支援舊版 MDX v1（截至 Docusaurus v3.0.0 尚未支援 v3）。您可在程式碼不相容部分前添加 `{/* prettier-ignore */}` 來使其與 Prettier 相容。
...
若因需添加過多 `{/* prettier-ignore */}` 而感到繁瑣，可考慮在 Prettier 開始支援 MDX v3 前，透過在 `.prettierignore` 檔案中添加以下內容來停用 Prettier 的 MDX 格式化功能：
...
由於 Node.js 16 [已終止支援](https://nodejs.org/en/blog/announcements/nodejs16-eol)，Docusaurus v3 現要求 **Node.js >= 18.0**。
...
您也可以更新網站的 `package.json` 檔案，防止使用過時不支援的版本：
...
在升級到 Docusaurus v3 之前，先將您的 Docusaurus v2 網站升級至 Node.js 18 環境。
...
React 18 自帶了一些破壞性變更，根據您為網站自訂的 React 程式碼量，處理難度會有所不同。官方主題和外掛程式都已相容 React 18。

:::info[如何升級]

請閱讀官方 [React v18.0](https://react.dev/blog/2022/03/29/react-v18) 和 [升級至 React 18 指南](https://react.dev/blog/2022/03/08/react-18-upgrade-guide)，並檢查您自己的 React 程式碼，找出可能受此升級影響的元件。
...
這是一個包含破壞性變更的主要版本升級，我們無法保證嚴格的向後相容性。[`prism-react-renderer` v2 版本說明](https://github.com/FormidableLabs/prism-react-renderer/releases/tag/prism-react-renderer%402.0.0)並不十分詳盡，但 Docusaurus 使用者需要注意以下 3 個主要變更。
...
> **破壞性變更** 將 [node-emoji](https://www.npmjs.com/package/node-emoji) 從 v1 更新至 v2。此變更新增了許多新表情符號支援，同時移除了 GitHub 上已失效的舊快捷碼。
...
此新套件與其他 Docusaurus 核心套件同步版本號，將用於確保 TypeScript 的向下相容性及處理重大版本升級時的破壞性變更。
...
Docusaurus v3 將其內部配置載入函式庫從 [`import-fresh`](https://github.com/sindresorhus/import-fresh) 更換為 [`jiti`](https://github.com/unjs/jiti)。該載入器負責處理如 `docusaurus.config.js`、`sidebars.js` 等檔案及 Docusaurus 插件的載入作業。

:::info[如何升級]

理論上您無需任何操作，現有的設定檔應能繼續正常運作。

但這是一次重大的依賴庫更換，可能會出現細微的行為變化。

:::

### 警告提示框注意事項

由於歷史原因，我們支援未在文件中記載的紅色`:::warning`警告提示框。
...
然而其顏色與圖標始終存在錯誤。Docusaurus v3正式重新引入`:::warning`提示框，將其納入文件並修正了顏色與圖標。
...
若您曾使用未記載的`:::warning`提示框，請逐一確認黃色是否適合當前情境。如需保持紅色，請改用`:::danger`。

Docusaurus v3同時[棄用了`:::caution`](https://github.com/facebook/docusaurus/pull/9308)提示框。請將`:::caution`(黃色)重構為`:::warning`(黃色)或`:::danger`(紅色)。

如需保留「注意」標題，可考慮重構為`:::warning[caution]`(黃色)。

:::

### 版本化側邊欄

此項破壞性變更僅影響**在`v2.0.0-beta.10`版本前(2021年12月)**就開始使用文檔版本化的早期Docusaurus v2用戶。

當創建`v1.0.0`版本時，側邊欄檔案包含`version-v1.0.0/`前綴，而[Docusaurus v3不再支援此格式](https://github.com/facebook/docusaurus/pull/9310)。
...
`@docusaurus/plugin-content-blog`現在預設將RSS饋送限制為最近20篇文章。對於大型Docusaurus部落格，這是更合理的預設值以避免饋送檔案過度膨脹。
...
對於已替換(swizzle)文檔相關主題元件(如`@theme/DocPage`)的用戶，這些元件已進行重大重構以提升客製化便利性。

技術上**這不屬於破壞性變更**，因為這些元件**標記為不建議替換**，但許多Docusaurus站點仍替換了文檔元件，需注意自訂內容可能導致相容性問題。

:::info[如何升級]

刪除所有已替換的元件，重新執行替換操作，並在新版元件基礎上重新套用您的客製化內容。
...
以下變更非強制性，但了解這些資訊有助於充分發揮Docusaurus v3的優勢。

### 自動JSX運行時

Docusaurus v3 現在使用 React 18 的「自動」JSX 運行時。
...
我們建議在 Markdown 檔案中使用 JSX、`import` 或 `export`（即 MDX 功能）時，一律使用 `.mdx` 副檔名。這在語義上更為正確，並能提升與外部工具（IDE、格式化工具、linter 等）的相容性。

在未來版本的 Docusaurus 中，`.md` 檔案將被解析為標準的 [CommonMark](https://commonmark.org/)，該格式不支援上述功能。在 Docusaurus v3 中，`.md` 檔案仍會作為 MDX 檔案編譯，但未來可[選擇啟用 CommonMark](https://github.com/facebook/docusaurus/issues/3018)。
...
Docusaurus 現在透過 [Markdown 指令](https://talk.commonmark.org/t/generic-directives-plugins-syntax/444)（由 [remark-directive](https://github.com/remarkjs/remark-directive) 實現）來實作警告區塊，且提供指令標籤需使用方括號：
...
- 提供可輕鬆重現問題的**最小化範例**，建議使用[docusaurus.new](https://docusaurus.new)建立
- 若網站能構建成功，請提供展示問題的線上部署網址
- 清楚說明問題細節，避免模糊表述如「無法運作」
- 附上相關材料：代碼片段、倉庫連結、git分支網址、完整堆疊追蹤、截圖或影片
- 以清晰簡潔的方式呈現請求，展現您已盡力自助

您亦可尋找付費的[Docusaurus服務供應商](https://github.com/facebook/docusaurus/discussions/9281)代為升級。若專案為開源，也可在社群尋求[免費善意協助](https://github.com/facebook/docusaurus/discussions/9283)。