...
Docusaurus 的路由系統遵循單頁應用程式(SPA)的慣例：一個路由對應一個元件。本節將先說明三個內容外掛(docs、blog、pages)內部的路由機制，再進一步探討底層路由系統的運作原理。

## 內容外掛中的路由機制 {#routing-in-content-plugins}

每個內容外掛都提供 `routeBasePath` 選項，用於定義外掛附加路由的基礎路徑。預設情況下，docs 外掛將路由置於 `/docs` 下；blog 外掛置於 `/blog`；pages 外掛則直接使用 `/`。您可以將路由結構視為：
...
系統會將所有路由與此嵌套路由配置進行匹配，直到找到相符項。例如當處理 `/docs/configuration` 路由時，Docusaurus 會先進入 `/docs` 分支，再於 docs 外掛創建的子路由中搜尋。

修改 `routeBasePath` 可有效變更網站路由結構。例如在[純文檔模式](../guides/docs/docs-introduction.mdx#docs-only-mode)中，將 docs 外掛設為 `routeBasePath: '/'` 意味著文檔路由將不再有 `/docs` 前綴，但這不影響其他外掛(如 blog)創建的子路由(如 `/blog`)。

接下來我們分別檢視這三個外掛如何組織各自的「子路由區塊」。

### 頁面路由 {#pages-routing}

頁面路由最為直觀：檔案路徑直接映射為 URL，不提供其他自訂方式。詳見[頁面文檔](../guides/creating-pages.mdx#routing)。
...
文檔外掛是唯一會創建**嵌套路由**的外掛。頂層會註冊[版本路徑](../guides/docs/versioning.mdx)：`/`, `/next`, `/2.0.0-beta.13`... 這些路徑提供版本上下文(包括佈局和側邊欄)，確保切換文檔時能保持側邊欄狀態，並可透過導航欄下拉選單切換版本而不離開當前文檔。使用的元件是 `@theme/DocPage`。
...
個別文檔會渲染在 `DocPage` 元件提供的導航欄、頁尾、側邊欄等剩餘空間中。例如本頁面 <URLPath /> 是由 <FilePath /> 檔案生成，使用的元件是 `@theme/DocItem`。
...
- 若路徑沒有副檔名，則視為 URL 路徑。例如，在 URL 為 `/docs/advanced/routing` 的頁面上，連結 `[page](../plugins)` 會指向 `/docs/plugins`。Docusaurus 僅在建置站點時（當其知曉完整路由結構時）檢測斷開的連結，但不會對檔案存在與否做出假設。這完全等同於在 JSX 檔案中撰寫 `<a href="../plugins">page</a>`。
...
關於內容插件的說明到此為止。現在讓我們退一步，談談 Docusaurus 應用程式中路由的通用運作原理。
...
所有路由會彙整至 `.docusaurus/routes.js`，可透過除錯插件的[路由面板](/__docusaurus/debug/routes)檢視。
...
Docusaurus 建構的是[單頁應用程式](https://developer.mozilla.org/en-US/docs/Glossary/SPA)，路由轉換透過 React Router 的 `history.push()` 方法在客戶端完成。此操作前提是目標 URL 必須存在於路由系統中，否則路由會攔截路徑並顯示 404 頁面。

若將 HTML 頁面置於 `static` 資料夾，它們會被複製到建構輸出中成為網站內容，但不屬於 Docusaurus 路由系統。我們提供 `pathname://` 協定，可像外部連結般以非 SPA 方式轉跳至網域內其他路徑。
...
`pathname://` 協定適用於參照 static 資料夾內的任何內容。例如 Docusaurus 會[將所有 Markdown 靜態資源轉為 require() 呼叫](../guides/markdown-features/markdown-features-assets.mdx#static-assets)，使用此協定可保持原始連結形式，避免被 Webpack 雜湊處理。
