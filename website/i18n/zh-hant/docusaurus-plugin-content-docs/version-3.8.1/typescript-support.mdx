...
Docusaurus 支援編寫和使用 TypeScript 主題元件。若初始化模板提供 TypeScript 變體，您可直接透過 `--typescript` 標記[初始化網站](./installation.mdx#scaffold-project-website)以獲得完整 TypeScript 支援。
...
Docusaurus 不會使用此 `tsconfig.json` 編譯專案。添加它僅是為了更好的編輯器體驗，但您仍可選擇執行 `tsc` 來自行或在 CI 上進行程式碼型別檢查。
...
## 抽離 TypeScript 主題元件 {#swizzling-typescript-theme-components}

對於支援 TypeScript 主題元件的主題，您可在 `swizzle` 命令結尾添加 `--typescript` 標記以取得 TypeScript 原始碼。例如，以下命令將在 `src/theme/Footer` 中生成 `index.tsx` 和 `styles.module.css`。
