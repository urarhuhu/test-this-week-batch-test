...
當您將文件 Y 加入側邊欄 X 時，會建立雙向綁定：側邊欄 X 包含文件 Y 的連結，且瀏覽文件 Y 時會顯示側邊欄 X。但有時我們需要打破其中一種隱性綁定：
...
Front matter 選項 `displayed_sidebar` 可強制設定側邊欄關聯。在同個範例中，您仍可使用文件簡寫形式而無需特殊配置：
...
這會明確告知 Docusaurus 瀏覽 `commonDoc` 時顯示 `apiSidebar`。透過相同方法，您可讓未包含文件 Y 的側邊欄 X 出現在文件 Y 上：
...
Docusaurus 使用側邊欄在每個文件頁面底部生成「下一頁」和「上一頁」分頁連結。它嚴格使用當前顯示的側邊欄：若無關聯側邊欄，則不會生成分頁導覽。但被連結為「下一頁」和「上一頁」的文件不一定會顯示相同側邊欄：它們雖屬於當前側邊欄，但其 front matter 可能設定了不同的 `displayed_sidebar`。

若透過 `displayed_sidebar` front matter 設定的側邊欄未包含當前文件本身，則不會顯示分頁導覽。
...
「windows」頁面的下一頁連結指向「linux」並不合理：您會希望讀者在安裝後繼續閱讀「getting started」。此時可手動設定分頁：
...
您也可透過 `pagination_next: null` 或 `pagination_prev: null` 停用分頁連結顯示。
