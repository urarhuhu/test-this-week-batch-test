...
此功能僅對 [版本化文檔](./versioning.mdx) 有用。建議在閱讀本頁前先熟悉文檔版本化功能。若您只需要 [多個側邊欄](./sidebar/multiple-sidebars.mdx)，可以在單一插件內實現。
...
這種情況下，您可以為每個移動端 SDK 文檔使用獨立的插件實例。

:::warning

若每個文檔實例體量龐大，您應該考慮創建兩個獨立的 Docusaurus 站點。

當有人編輯 iOS 文檔時，重新構建包括未變更的整個 Android 文檔在內的所有內容真的有必要嗎？

:::

### 版本化與非版本化文檔 {#versioned-and-unversioned-doc}

有時您希望部分文檔進行版本控制，而其他文檔保持「全局」狀態，對其進行版本化顯得沒有必要。

我們在 Docusaurus 官方網站就採用這種模式：
...
我們將 `product` 實例視為最重要的實例，通過不分配 ID 使其成為「默認」實例。
...
這樣實例路徑會更簡潔，並與單實例配置保持向後兼容。

:::

## 標記新版本 {#tagging-new-versions}

每個插件實例都有專屬的 CLI 命令來標記新版本。運行以下命令即可查看：
