...
自動生成的標題 ID 在單一頁面中會保持唯一性，但若使用自訂 ID，請確保每個 ID 在頁面中只出現一次，否則會導致 HTML 語義錯誤，且部分標題將無法被連結。

:::

## 目錄標題層級 {#table-of-contents-heading-level}

每份 Markdown 文件右上角會顯示目錄。預設僅展示 h2 和 h3 標題，這通常足以呈現頁面結構。若需調整顯示範圍，可針對單一頁面或全局設定最小與最大標題層級。
...
`themeConfig` 選項會影響網站上所有目錄（包含[行內目錄](#inline-table-of-contents)），但 front matter 參數僅作用於右上角目錄。如需自訂每個 `<TOCInline />` 元件，需使用 `minHeadingLevel` 和 `maxHeadingLevel` 屬性。

:::

## 行內目錄 {#inline-table-of-contents}

透過 MDX 功能，您還能在 Markdown 文件內直接嵌入行內目錄。

所有 MDX 文件均可使用 `toc` 變數，該變數包含文件內所有標題。預設僅顯示 `h2` 和 `h3` 標題。可透過為個別 `TOCInline` 元件設定 `minHeadingLevel` 或 `maxHeadingLevel` 來調整顯示層級。
...
由於 `toc` 是扁平陣列結構，您可輕鬆過濾不需要的節點或插入額外節點，重新構建目錄樹。
...
非 Markdown 標題不會顯示在目錄中。這可以巧妙地用來解決上述問題。
