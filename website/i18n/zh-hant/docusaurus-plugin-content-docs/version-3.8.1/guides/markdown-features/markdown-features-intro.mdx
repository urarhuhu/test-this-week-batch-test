...
你可以透過 [`siteConfig.markdown.format`](../../api/docusaurus.config.js.mdx#markdown) 設定或 `mdx.format: md` 前置元數據**選擇使用 CommonMark**。
...
前置元數據位於文件的最頂部，由三個破折號 `---` 包圍。內容以 [YAML](https://yaml.org/spec/1.2.2/) 格式解析。
...
你可以重複使用預設解析器，並用自訂的專有邏輯進行包裝。這樣可以實現便利的 front matter 轉換、快捷方式，或是整合外部系統中 Docusaurus 插件不支援的 front matter。
...
建議將 `<summary>` 標籤保持在同一行。請注意 [MDX 會為換行符號產生額外的 HTML `<p>` 段落](https://mdxjs.com/migrating/v2/#jsx)。若有疑問，可使用 [MDX 遊樂場](https://mdxjs.com/playground/) 來排查 `<details>` 的渲染問題。
