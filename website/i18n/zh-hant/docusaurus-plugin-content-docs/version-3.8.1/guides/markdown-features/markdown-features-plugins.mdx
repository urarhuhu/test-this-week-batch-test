...
如果您試用過 [MDX 遊樂場](https://mdxjs.com/playground/)，會注意到 MDX 轉譯有兩個中間步驟：Markdown 抽象語法樹（MDAST）和超文本抽象語法樹（HAST），最終才輸出 JSX。MDX 插件也分為兩種形式：
...
這些都是 Remark 插件的典型應用場景，也可作為您開發自訂插件時的參考範例。
...
許多官方 Remark/Rehype 插件僅支援 [**ES Modules**](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)（JavaScript 模組系統），而 Docusaurus 支援此規範。建議使用 [**ES Modules**](https://flaviocopes.com/es-modules/) 設定檔以便更輕鬆導入這類套件。

:::

接著，在 `docusaurus.config.js` 中導入插件並透過插件或預設配置添加到插件選項：
...
以下說明**並非**建立外掛程式的完整指南，僅展示如何使其在 Docusaurus 中運作。如需深入了解其運作原理，請參閱 [Remark](https://github.com/remarkjs/remark/blob/main/doc/plugins.md#create-plugins) 或 [Rehype](https://github.com/rehypejs/rehype/blob/main/doc/plugins.md#create-plugins) 的官方文件。
...
Docusaurus 的預設外掛程式會先於自訂 remark 外掛程式執行，這意味著圖片或連結已被轉換為帶有 `require()` 呼叫的 JSX。例如在上例中，即使所有 `h2` 標題都加上 `第 X 節` 前綴，生成的目錄仍保持不變，因為目錄生成外掛程式先於自訂外掛程式執行。若需在預設外掛程式前處理 MDAST，請使用 `beforeDefaultRemarkPlugins` 和 `beforeDefaultRehypePlugins`：
