...
1. 安裝 `remark-math` 和 `rehype-katex` 插件：
    ...
    請確保在 Docusaurus v3 (使用 MDX v3) 中使用 `remark-math 6` 和 `rehype-katex 7` 版本。其他版本可能無法正常運作。
...
2.  這兩個插件[僅支援 ES Modules 格式](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)。建議使用[ES Modules](https://flaviocopes.com/es-modules/)格式的設定檔：
    ...
    <summary>
      ...
      格式的設定檔？
            ...
            若需使用 CommonJS 格式設定檔，可透過動態導入和異步配置函數載入這些 ES 模組插件：
...
從CDN來源載入樣式表、字體和JavaScript函式庫對於熱門函式庫和資源來說是個良好實踐，因為這能減少你需要託管的資源量。若您偏好自行託管`katex.min.css`（連同所需的KaTeX字體），可以從[KaTeX GitHub發佈頁](https://github.com/KaTeX/KaTeX/releases)下載最新版本，解壓後將`katex.min.css`和`fonts`目錄（僅需`.woff2`字體類型即可）複製到網站的`static`目錄中，並在`docusaurus.config.js`裡將樣式表的`href`從CDN網址替換為本地路徑（例如`/katex/katex.min.css`）。
