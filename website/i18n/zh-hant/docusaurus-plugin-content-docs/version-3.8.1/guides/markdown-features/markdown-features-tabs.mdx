...
預設會顯示第一個標籤頁，若要覆寫此行為，您可以透過在標籤項目中添加 `default` 來指定預設標籤。也可以將 `Tabs` 元件的 `defaultValue` 屬性設為您選擇的標籤值。例如在上方範例中，無論是為 `value="apple"` 標籤設置 `default`，或是為標籤頁設置 `defaultValue="apple"`，都會強制預設開啟「Apple」標籤頁。

若為 `Tabs` 提供的 `defaultValue` 指向不存在的值，Docusaurus 會拋出錯誤。若您希望預設不顯示任何標籤頁，請使用 `defaultValue={null}`。

## 同步標籤選擇 {#syncing-tab-choices}

您可能希望同類型的標籤選擇能相互同步。例如，您可能想為 Windows 和 macOS 用戶提供不同的操作說明，並希望透過單擊切換所有作業系統專用的說明標籤頁。要實現此功能，可為所有相關標籤頁賦予相同的 `groupId` 屬性。請注意，這會將選擇持久化到 `localStorage` 中，且所有具有相同 `groupId` 的 `<Tab>` 實例會在任一標籤值變更時自動更新。需注意群組 ID 是全域命名空間的。
...
對於所有具有相同 `groupId` 的標籤群組，其可能值不需要相同。若某個標籤群組選擇的值在另一個具有相同 `groupId` 的標籤群組中不存在，則缺少該值的標籤群組不會切換標籤頁。您可從以下範例觀察此行為：嘗試選擇 Linux 後，上方的標籤群組不會改變。
...
您可能想自訂特定標籤頁組的外觀。可透過 `className` 屬性傳入字串，指定的 CSS 類別將被添加到 `Tabs` 元件：
...
您還可透過 `attributes` 字段獨立自訂每個標籤標題。額外屬性可透過 `Tabs` 的 `values` 屬性傳遞，或像宣告 `label` 一樣透過每個 `TabItem` 的屬性傳遞。
...
可以將選取的標籤狀態持久化儲存到 URL 的搜尋參數中。這讓您能分享已預選特定標籤的頁面連結——例如從 Android 應用程式連結至已預選 Android 標籤的文件。此功能不提供錨點連結，瀏覽器不會自動滾動至該標籤位置。
...
當頁面載入時，會優先還原 URL 查詢字串中的標籤選擇（而非 `groupId` 透過 `localStorage` 儲存的選擇）。
