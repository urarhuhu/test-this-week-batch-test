...
您可以將 Docusaurus 與**您選擇的工具和 SaaS 整合**，只要將**翻譯文件放在正確的位置**。

我們記錄了 [Crowdin](https://crowdin.com/) 的使用方式，作為**一個**可能的**整合範例**。

:::warning

這**並非對 Crowdin 作為翻譯 Docusaurus 網站的唯一選擇的背書**，但 Facebook 已成功使用它來翻譯如 [Jest](https://jestjs.io/)、[Docusaurus](https://docusaurus.io/) 和 [ReasonML](https://reasonml.github.io/) 等文件專案。
...
- **上傳來源** 到 Crowdin（未翻譯的文件）
- 使用 Crowdin **翻譯內容**
- **下載翻譯** 從 Crowdin（本地化的翻譯文件）

Crowdin 提供一個 [CLI](https://support.crowdin.com/cli-tool/) 來**上傳來源**和**下載翻譯**，讓您可以自動化翻譯流程。
...
這是一個使用 Crowdin 將新初始化的英文 Docusaurus 網站翻譯成法文的逐步指南，並假設您已經遵循了 [國際化教學](./i18n-tutorial.mdx)。

最終結果可以在 [docusaurus-crowdin-example.netlify.app](https://docusaurus-crowdin-example.netlify.app/) 看到（[儲存庫](https://github.com/slorber/docusaurus-crowdin-example)）。
...
![建立一個 Crowdin 專案，英文為來源語言，法文為目標語言](/img/crowdin/crowdin-create-project.png)
...
請勿提交此權杖至版本控制，建議為公司建立專用的 **Crowdin 帳號** 而非使用個人帳戶。
...
暫時可在 `crowdin.yml` 中硬編碼個人權杖：`api_token: 'MY-TOKEN'`。
...
先使用 `Hide String` 功能，因為 Crowdin 的預先翻譯有時會過於樂觀。
...
### 使用 CI 自動化 {#automate-with-ci}

我們將設定 CI 在**建置時下載 Crowdin 翻譯**，並將其保留在 Git 之外。

將 `website/i18n` 加入 `.gitignore`。

在您的 CI 中設定 `CROWDIN_PERSONAL_TOKEN` 環境變數。

建立一個 npm 腳本來 `sync` Crowdin（提取來源、上傳來源、下載翻譯）：
...
在 CI 中呼叫 `npm run crowdin:sync` 腳本，緊接在建置 Docusaurus 網站之前。
...
Crowdin 不支援多個同時上傳/下載：最好僅在生產部署中包含翻譯，並保持部署預覽未翻譯。

:::

## 進階 Crowdin 主題 {#advanced-crowdin-topics}
...
Crowdin 會將 JSX 語法視為嵌入式 HTML，並可能在您下載翻譯時弄亂 JSX 標記，導致網站因無效的 JSX 而無法建置。

使用簡單字串屬性的簡單 JSX 片段（如 `<Username name="Sebastien"/>`）可以正常運作；而使用物件/陣列屬性的複雜 JSX 片段（如 `<User person={{name: "Sebastien"}}/>`）則更可能因語法不像 HTML 而失敗。

#### MDX 解決方案 {#mdx-solutions}

我們建議將複雜的嵌入式 JSX 程式碼提取為獨立的獨立元件。我們還新增了 `mdx-code-block` 逃逸語法：
...
當建立新版本時，原始字串通常與當前版本 (`website/docs`) 非常相似，而您不希望反覆翻譯新版本的文件。
...
如果您有一個 id 為 `ios` 的文件插件實例，您還需要配置這些原始檔案
...
當您的原始檔案被重構時，您應該使用 Crowdin UI **手動更新您的 Crowdin 檔案**：
...
不幸的是，由於技術原因目前尚無法使用，但我們有希望解決這個問題。

Crowdin 會將 Markdown 字串替換為技術性 ID（如 `crowdin:id12345`），但替換過於激進，包括隱藏字串，並會破壞 front matter、admonitions 和 JSX...

:::

### 本地化編輯網址 {#localize-edit-urls}

當用戶瀏覽 `/fr/doc1` 頁面時，預設情況下編輯按鈕會連結到未本地化的文件 `website/docs/doc1.md`。

您可能更希望編輯按鈕改為連結到 Crowdin 介面，可透過 `editUrl` 函數針對不同語系自訂編輯網址。
...
目前**無法直接連結到 Crowdin 的特定文件**。

:::

### 範例配置 {#example-configuration}

**Docusaurus 設定檔**是展示版本控制與多實例功能的良好範例：
...
截至 2023 年 12 月 7 日，官方無意變更連結處理邏輯，若計劃搭配使用 Crowdin 與 MT 引擎，請務必留意此問題。