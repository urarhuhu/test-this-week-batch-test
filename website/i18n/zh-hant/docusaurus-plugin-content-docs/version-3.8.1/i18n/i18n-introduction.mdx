...
透過其國際化 ([i18n](https://en.wikipedia.org/wiki/Internationalization_and_localization)) 支援，**翻譯 Docusaurus 網站非常容易**。

## 目標 {#goals}

理解 Docusaurus 國際化支援背後的**設計決策**非常重要。

如需更多背景資訊，您可以閱讀最初的 [RFC](https://github.com/facebook/docusaurus/issues/3317) 和 [PR](https://github.com/facebook/docusaurus/pull/3325)。
...
- **靈活的部署選項**：單一、多個網域或混合
- **模組化**：允許插件作者提供國際化支援
- **低開銷運行時**：文件主要是靜態的，不需要繁重的 JS 函式庫或 polyfills
- **可擴展的建置時間**：允許獨立建置和部署本地化網站
- **本地化資源**：您網站的圖片可能包含需要翻譯的文字
...
Markdown 和 MDX 文件作為整體進行翻譯，以完全保留翻譯上下文，而不是將每個句子拆分為單獨的字串。
...
- 透過插件選項提供的佈局標籤：文件側邊欄類別標籤、部落格側邊欄標題...
...
翻譯一個簡單的法語Docusaurus網站會產生如下目錄結構：
...
JSON檔案可通過[`docusaurus write-translations`](../cli.mdx#docusaurus-write-translations-sitedir) CLI命令初始化。每個插件在對應資料夾下提供自有翻譯內容，而`code.json`檔案則定義React代碼中使用的所有文字標籤。

每個內容插件或主題的**翻譯檔案存放位置定義各不相同**：

- [文檔i18n](../api/plugins/plugin-content-docs.mdx#i18n)
- [部落格i18n](../api/plugins/plugin-content-blog.mdx#i18n)
- [頁面i18n](../api/plugins/plugin-content-pages.mdx#i18n)
- [主題i18n](../api/themes/theme-configuration.mdx#i18n)