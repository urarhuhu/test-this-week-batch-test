...
該服務對任何開發者文件或技術部落格**免費**：請務必閱讀[檢查清單](https://docsearch.algolia.com/docs/who-can-apply/)並[申請 DocSearch 計畫](https://docsearch.algolia.com/apply)。

DocSearch 每週會爬取您的網站一次（排程可透過網頁介面設定），並將所有內容彙整至 Algolia 索引中。這些內容隨後會透過 Algolia API 直接從前端查詢。
...
在您的申請獲批並部署後，您將收到一封包含所有詳細資訊的電子郵件，以便將 DocSearch 添加到專案中。可透過[網頁介面](https://crawler.algolia.com/)編輯和管理爬取作業。索引在部署後立即可用，因此通常不需要手動配置。
...
當瀏覽 v2 文件時，若出現 v1 的搜尋結果會顯得突兀。有時 v1 與 v2 文件內容高度相似，可能導致相同查詢出現重複結果（每個版本各一筆）。

同理，瀏覽法語網站時，若出現英語文件結果也不合理。

為解決此問題，上下文搜尋會動態識別您正在瀏覽的特定文件版本與語言，並據此建立搜尋過濾條件。

- 在 `/en/docs/v1/myDoc` 頁面，搜尋結果僅包含 **英語** 的 **v1** 文件（及其他未版本控制的頁面）
- 在 `/fr/docs/v2/myDoc` 頁面，搜尋結果僅包含 **法語** 的 **v2** 文件（及其他未版本控制的頁面）
...
Algolia DocSearch 支援透過 `docusaurus.config.js` 中的 `algolia` 欄位傳遞 [多種參數](https://docsearch.algolia.com/docs/api/)。
...
若需直接編輯 Algolia 搜尋的 React 元件，可對 `@docusaurus/theme-search-algolia` 的 `SearchBar` 元件進行 [swizzle 操作](swizzling.mdx)：
...
5. 此時應能正常顯示搜尋結果（即使啟用[上下文搜尋](#contextual-search)功能）
...
請參閱[如何執行 typesense-docsearch-scraper 的逐步指南](https://typesense.org/docs/guide/docsearch.html#step-1-set-up-docsearch-scraper)，以及[在 Docusaurus 網站安裝搜尋列的教學](https://typesense.org/docs/guide/docsearch.html#option-a-docusaurus-powered-sites)。

## 👥 使用本地搜尋 {#using-local-search}

當搜尋索引規模較小且可直接下載至用戶瀏覽器時，可考慮使用本地搜尋外掛。

您可在[社群資源清單](https://docusaurus.io/community/resources#search)中找到相關的本地搜尋外掛。
...
這會在專案目錄中建立 `src/theme/SearchBar` 檔案。重新啟動開發伺服器並編輯該元件後，Docusaurus 就會改用您的自訂 `SearchBar` 元件。
