...
- 任何配置變更後重新上傳來源檔案
- 使用以 `/` 開頭的路徑（`./` 無效）
- 避免複雜的 globbing 模式如 `/docs/**/*.(md|mdx)`（無效）
...
您可以保留預設值 `CROWDIN_PERSONAL_TOKEN`，並在電腦和 CI 伺服器上將此環境變數設為生成的存取權杖。
...
本教學使用 CLI 版本 `3.5.2`，但預計 `3.x` 版本均可正常運作。
...
暫時可將個人權杖硬編碼在 `crowdin.yml` 中：`api_token: 'MY-TOKEN'`。
...
建議先使用 `Hide String` 功能，因為 Crowdin 的預先翻譯機制有時會過度樂觀地翻譯不該翻譯的內容。
...
確認您的網站現在已成功翻譯為法語版本 [`http://localhost:3000/fr/`](http://localhost:3000/fr/)。
...
建立一個 npm 腳本來 `sync` Crowdin（提取原始檔、上傳原始檔、下載翻譯檔）：
...
Crowdin 不支援多個並行上傳/下載操作：建議僅在正式環境部署中包含翻譯，保持部署預覽版本未翻譯狀態。
...
Crowdin **官方不支援 MDX**，但他們已加入**對 `.mdx` 副檔名的支援**，並將此類檔案視為 Markdown（而非純文字）處理。

#### MDX 問題 {#mdx-problems}

Crowdin 會將 JSX 語法誤認為嵌入式 HTML，在您下載翻譯時可能破壞 JSX 標記結構，導致網站因無效 JSX 而無法建置。

使用簡單字串屬性的基本 JSX 片段如 `<Username name="Sebastien"/>` 能正常運作；但使用物件/陣列屬性的複雜 JSX 片段如 `<User person={{name: "Sebastien"}}/>` 則更容易因語法不像 HTML 而失敗。
...
- Crowdin 用於匹配現有 Markdown 翻譯與源 Markdown 的啟發式方法並非 100% 可靠，同步後需要在 Crowdin UI 上驗證結果
...
不幸的是，由於技術原因它目前無法工作，但我們有希望解決這個問題。

Crowdin 將 Markdown 字符串替換為技術 ID（如 `crowdin:id12345`），但替換過於激進，包括隱藏字符串，並會破壞 front matter、admonitions、JSX...
...
**Docusaurus 設定檔**是同時使用版本控制與多實例的絕佳範例：
...
Crowdin 近期對 Markdown 檔案格式進行重大調整，連結的處理方式與過往不同。先前連結被視為標籤，現在則顯示為純文字。此變更導致純文字連結會被傳送至 MT 引擎嘗試翻譯目標網址，因而破壞翻譯正確性（例如：字串 `Allez voir [ma merveilleuse page](/ma-merveilleuse-page)` 被翻譯為 `Check out [my wonderful page](/my-wonderful-page)`，這將破壞 Docusaurus 的國際化工作流程，因頁面名稱不應被翻譯）。

截至 2023 年 12 月 7 日，Crowdin 無意變更連結處理邏輯，若您計劃搭配使用 Crowdin 與機器翻譯，請務必注意此問題。