...
- **模組化**：允許插件作者提供國際化支援
- **低運行開銷**：文件主要是靜態的，不需要繁重的 JS 函式庫或 polyfill
...
- **良好的 SEO 預設值**：我們為您設置了實用的 SEO 標頭，如 [`hreflang`](https://developers.google.com/search/docs/advanced/crawling/localized-versions)
...
- **自動語言檢測**：此功能具有主觀性，最好由[伺服器（您的託管服務提供商）](../deployment.mdx)處理
...
- 您的 React 程式碼：`src/pages` 中的獨立 React 頁面或其他組件
...
某些插件可能會讀取需整體本地化的外部資料檔案。例如部落格插件使用的[`authors.yml`](../blog.mdx#global-authors)檔案，可通過在`i18n/[locale]/docusaurus-plugin-content-blog/`下建立副本進行翻譯。
