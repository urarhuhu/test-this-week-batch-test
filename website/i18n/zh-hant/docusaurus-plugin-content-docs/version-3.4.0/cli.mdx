...
Docusaurus 提供一系列指令碼，協助您生成、運行及部署網站。

當您的網站初始化完成後，網站原始碼將包含可透過套件管理工具執行的 Docusaurus 指令碼：
...
以下是 Docusaurus CLI 指令及其用途列表：

### `docusaurus start [siteDir]` {#docusaurus-start-sitedir}

使用 [Webpack 開發伺服器](https://webpack.js.org/configuration/dev-server) 在本機建構並預覽您的網站。
...
請注意，部分功能（例如錨點連結）在開發模式下無法運作。這些功能在正式環境中將正常運作。

:::

:::info[網路開發模式]

當從遠端伺服器或虛擬機（例如 GitHub Codespaces）轉發 3000 埠時，可透過 `0.0.0.0` 參數讓開發伺服器監聽本地 IP 位址。
...
此互動式 CLI 將引導您完成完整的[元件替換流程](./swizzling.mdx)。
...
將您的網站部署至 [GitHub Pages](https://pages.github.com/)。詳情請參閱[部署文件](deployment.mdx#deploying-to-github-pages)。
...
清除 Docusaurus 網站生成的靜態資源、快取與構建產物。

建議在提交錯誤報告、升級版本後，或任何遇到 Docusaurus 網站問題時執行此指令。

### `docusaurus write-translations [siteDir]` {#docusaurus-write-translations-sitedir}

生成需翻譯的 JSON 語系檔案。

預設檔案會寫入 `website/i18n/<defaultLocale>/...` 路徑下。
...
為網站 Markdown 文件添加[顯式標題 ID](./guides/markdown-features/markdown-features-toc.mdx#heading-ids)。
