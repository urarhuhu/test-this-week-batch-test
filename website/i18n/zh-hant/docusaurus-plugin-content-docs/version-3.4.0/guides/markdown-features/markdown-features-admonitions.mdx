...
在 Markdown 之外，您可使用 `@theme/Admonition` 元件來獲得相同的輸出效果。
...
接受的類型與上述相同：`note`、`tip`、`danger`、`info`、`warning`。您亦可選擇透過傳遞 JSX 元素、字串或標題來指定圖示：
...
您可以透過 [swizzling](../../swizzling.mdx) 自訂每種提示框類型的渲染方式。通常只需簡單的包裝即可達成目標。例如，在以下範例中，我們僅替換了 `info` 提示框的圖示。
...
提示框是透過 [Remark 插件](./markdown-features-plugins.mdx) 實現的。該插件設計為可配置。若要為特定內容插件（文件、部落格、頁面）自訂 Remark 插件，請透過 `admonitions` 鍵傳遞選項。
...
預設情況下，主題不知道如何處理自訂提示框關鍵字（如 `:::my-custom-admonition`）。您需將每個提示框關鍵字映射到 React 元件，主題才能知道如何渲染它們。
