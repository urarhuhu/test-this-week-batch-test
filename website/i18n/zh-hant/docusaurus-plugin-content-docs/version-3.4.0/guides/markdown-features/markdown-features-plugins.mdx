...
若您操作過 [MDX 遊樂場](https://mdxjs.com/playground/)，會注意到 MDX 轉譯有兩個中間步驟：Markdown 抽象語法樹（MDAST）和超文本抽象語法樹（HAST），最終才輸出 JSX。MDX 插件也分為兩種形式：
...
使用插件為專案中最常用的 JSX 元素引入簡潔語法。我們提供的[警告框語法](./markdown-features-admonitions.mdx)也是由 Remark 插件生成，您可依此模式實現自訂需求。
...
MDX 插件通常是 npm 套件，因此可透過 npm 安裝。以[數學公式插件](./markdown-features-math-equations.mdx)為例。
...
許多官方 Remark/Rehype 插件僅支援 [**ES Modules**](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)（JavaScript 模組系統），而 Docusaurus 支援此規範。建議使用 [**ES Modules**](https://flaviocopes.com/es-modules/) 設定檔以便導入這類套件。

:::

接著，在 `docusaurus.config.js` 中導入插件，並透過插件或預設配置將其加入選項：
...
部分插件可接受自訂配置。此時請使用 `[plugin, pluginOptions]` 語法，如下所示：
...
以下說明**並非**建立外掛程式的完整指南，僅為示範如何使其與 Docusaurus 協同運作。如需更深入的工作原理說明，請參閱 [Remark](https://github.com/remarkjs/remark/blob/main/doc/plugins.md#create-plugins) 或 [Rehype](https://github.com/rehypejs/rehype/blob/main/doc/plugins.md#create-plugins) 的官方文件。

:::

舉例來說，我們建立一個會遍歷所有 `h2` 標題並添加 `第 X 節` 前綴的外掛程式。首先，在任何位置建立外掛程式原始檔——你甚至可以將其發布為獨立的 npm 套件並如前述方式安裝。我們會將外掛程式放在 `src/remark/section-prefix.js`。remark/rehype 外掛程式本質上是接收 `options` 並返回一個操作 AST 的 `transformer` 的函式。
...
現在你可以在 `docusaurus.config.js` 中導入此外掛程式，並如同已安裝的外掛程式般使用它！
...
`transformer` 的第二個參數 [`vfile`](https://github.com/vfile/vfile) 在需要存取當前 Markdown 檔案路徑時相當實用。
...
例如我們的 `transformImage` 外掛程式就利用此參數，將相對路徑的圖片參照轉換為 `require()` 呼叫。

:::

:::note

Docusaurus 的預設外掛程式會先於自訂 remark 外掛程式執行，這意味著圖片或連結已被轉換為帶有 `require()` 呼叫的 JSX。例如在上例中，即使所有 `h2` 標題現在都帶有 `第 X 節` 前綴，生成的目錄仍保持不變，因為目錄生成外掛程式在我們的自訂外掛程式之前執行。若你需要在預設外掛程式處理前操作 MDAST，請使用 `beforeDefaultRemarkPlugins` 和 `beforeDefaultRehypePlugins`。
