...
1. _如何在側邊欄 X 建立文件 Y 的連結，卻不讓側邊欄 X 在瀏覽 Y 時顯示？_ 例如當文件 Y 同時出現在多個側邊欄（如前例所示），而我們希望明確指定 Docusaurus 顯示特定側邊欄時。
2. _如何讓瀏覽文件 Y 時顯示側邊欄 X，但側邊欄 X 不包含 Y 的連結？_ 例如當 Y 是「文件首頁」，而側邊欄僅用於導覽時。

Front matter 選項 `displayed_sidebar` 可強制設定側邊欄關聯。沿用前例，您仍可使用簡寫格式而不需特殊配置：
...
這會明確告知 Docusaurus 在瀏覽 `commonDoc` 時顯示 `apiSidebar`。同理，您也可以讓未包含文件 Y 的側邊欄 X 出現在文件 Y 上：
...
若設定 `displayed_sidebar: null`，該頁面將不會顯示任何側邊欄，連帶也不會顯示分頁導覽。

## 生成分頁導覽 {#generating-pagination}

Docusaurus 使用側邊欄來產生每個文件頁面底部的「下一頁」和「上一頁」分頁連結。系統嚴格依據當前顯示的側邊欄生成連結：若無關聯的側邊欄，則不會產生分頁導覽。但需注意，作為「下一頁」或「上一頁」連結的文件，不一定會顯示相同的側邊欄：它們雖屬於當前側邊欄，但其 front matter 可能設定了不同的 `displayed_sidebar`。

若透過 front matter `displayed_sidebar` 設定顯示的側邊欄未包含當前文件本身，則不會顯示分頁導覽。
