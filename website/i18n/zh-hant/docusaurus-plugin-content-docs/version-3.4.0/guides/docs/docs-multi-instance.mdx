...
此功能僅對 [版本化文檔](./versioning.mdx) 有用。建議在閱讀本頁前先熟悉文檔版本控制。若您僅需 [多側邊欄](./sidebar/multiple-sidebars.mdx)，可在單一插件內實現。
...
當有人編輯 iOS 文檔時，重新構建所有內容（包括未變更的完整 Android 文檔）是否真的必要？
...
舉例來說，若您想為每個行動SDK（iOS和Android）設置獨立的版本下拉選單，可以這樣配置：
