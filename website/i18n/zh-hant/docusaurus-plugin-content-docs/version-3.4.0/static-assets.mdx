...
在 JSX 中，您可以使用絕對 URL 從 `static` 資料夾引用資源，但這並非理想做法，因為變更網站 `baseUrl` 會**導致這些連結失效**。對於 `<img src="/img/docusaurus.png" />` 這張圖片，若網站服務於 `https://example.com/test`，瀏覽器會嘗試從 URL 根目錄解析，即 `https://example.com/img/docusaurus.png`，這將失敗因為實際服務路徑是 `https://example.com/test/img/docusaurus.png`。
...
在 Markdown 中，您可以使用絕對路徑撰寫連結或圖片（使用 Markdown 語法），因為 Docusaurus 在解析 Markdown 時會將其視為 `require` 調用而非 URL。詳見 [Markdown 靜態資源](./guides/markdown-features/markdown-features-assets.mdx)。
...
關鍵結論：**切勿硬編碼您的基礎 URL！** 基礎 URL 被視為實作細節，應可輕鬆變更。所有路徑（即使看似 URL 片段）實際都是檔案路徑。
...
- 若透過硬編碼的絕對路徑引用缺失檔案，將無法在編譯時被偵測，並會導致 404 錯誤。
- GitHub Pages 預設會使用 [Jekyll](https://jekyllrb.com/) 處理發布檔案。由於 Jekyll 會忽略所有以 `_` 開頭的檔案，若您使用 GitHub Pages 託管，建議在 `static` 目錄中添加名為 `.nojekyll` 的空檔案來停用 Jekyll。