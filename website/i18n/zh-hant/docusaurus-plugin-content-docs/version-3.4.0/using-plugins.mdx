...
**Docusaurus 核心本身不提供任何功能。** 所有功能都委託給個別插件實現：[文檔功能](./guides/docs/docs-introduction.mdx)由[文檔插件](./api/plugins/plugin-content-docs.mdx)提供；[部落格功能](./blog.mdx)由[部落格插件](./api/plugins/plugin-content-blog.mdx)提供；或是透過[頁面插件](./api/plugins/plugin-content-pages.mdx)建立的獨立[頁面](./guides/creating-pages.mdx)。若未安裝任何插件，網站將不會包含任何路由。
...
若您精力充沛，也可以閱讀[插件開發指南](./advanced/plugins.mdx)或[插件方法參考文檔](./api/plugin-methods/README.mdx)來了解如何自行開發插件。

## 安裝插件 {#installing-a-plugin}

插件通常是 npm 套件，因此您可以使用 npm 像安裝其他套件一樣安裝它們。
...
Docusaurus 也可以從本地目錄載入插件，配置方式如下：
...
但插件可以透過在配置中將名稱和選項物件包裝成雙成員元組來指定選項。此風格通常稱為「Babel 風格」。
...
所有 Docusaurus 內容插件都支援多個插件實例。例如，可能需要使用[多個文檔插件實例](./guides/docs/docs-multi-instance.mdx)或[多個部落格](./blog.mdx#multiple-blogs)。必須為每個插件實例分配唯一 ID，預設情況下插件 ID 為 `default`。
...
主題的載入方式與插件完全相同。從使用者角度來看，在安裝和配置插件時，`themes` 和 `plugins` 條目可以互換使用。唯一的細微差別在於主題是在插件之後載入的，且[主題可以覆蓋插件的預設主題組件](./advanced/client.mdx#theme-aliases)。

:::tip

`themes` 和 `plugins` 選項會導致不同的[模組簡寫解析](#module-shorthands)，因此若您想利用簡寫功能，請確保使用正確的條目！
...
接著，將其加入您網站的 `docusaurus.config.js` 的 `presets` 選項中：
...
若名稱不存在（例如 `@jquery`），則會載入 `[scope]/docusaurus-[moduleType]`（即 `@jquery/docusaurus-plugin`）。否則，系統將嘗試以下方式：

- `[scope]/[name]`（例如 `@jquery/content-docs`）
- `[scope]/docusaurus-[moduleType]-[name]`（例如 `@jquery/docusaurus-plugin-content-docs`）

以下是一些在 `plugins` 欄位中註冊的插件範例。請注意，與 [ESLint](https://eslint.org/docs/user-guide/configuring/plugins#configuring-plugins) 或 [Babel](https://babeljs.io/docs/en/options#name-normalization) 不同，這些工具強制要求插件遵循一致的命名慣例，而 Docusaurus 允許更大的命名自由，因此解析結果並非絕對，但會遵循上述定義的優先順序。
