...
主要的重大變更是從 MDX v1 升級至 MDX v3。詳情請參閱 [**MDX v2**](https://mdxjs.com/blog/v2/) 和 [**MDX v3**](https://mdxjs.com/blog/v3/) 的版本說明。MDX 現在會以**更嚴格**的方式和**細微差異**編譯您的 Markdown 內容。
...
對於複雜的網站，我們也建議設定[**視覺回歸測試**](/blog/upgrading-frontend-dependencies-with-confidence-using-visual-regression-testing)，這是確保網站視覺效果保持不變的好方法。Docusaurus v3 主要升級相依項目，預計不會產生任何視覺變更。

:::

:::note

請查看 [**Docusaurus v3.0.0**](https://github.com/facebook/docusaurus/releases/tag/v3.0.0) 的版本說明，並瀏覽 pull-requests 以獲取更多實用資訊及此處提到的每個變更背後的動機。
...
對於大型網站而言，手動審查所有頁面相當耗時，建議您設置[視覺回歸測試](https://docusaurus.io/blog/upgrading-frontend-dependencies-with-confidence-using-visual-regression-testing)。

:::

升級至 MDX 將伴隨 [MDX v2](https://mdxjs.com/blog/v2/) 與 [MDX v3](https://mdxjs.com/blog/v3/) 版本公告中記載的所有重大變更。多數變更源自 MDX v2，而 MDX v3 屬於相對小幅的更新。[MDX v2 遷移指南](https://mdxjs.com/migrating/v2/) 中關於 [更新 MDX 檔案](https://mdxjs.com/migrating/v2/#update-mdx-files) 的章節與我們密切相關。同時請務必閱讀 [MDX 疑難排解](https://mdxjs.com/docs/troubleshooting-mdx/) 頁面，該頁面能協助您解讀常見的 MDX 錯誤訊息。

請同時參閱我們更新後的 [**MDX 與 React**](../guides/markdown-features/markdown-features-react.mdx) 文件頁面。

### 使用 MDX 遊樂場

MDX 遊樂場將成為您的最佳夥伴。它能幫助您理解內容如何**被編譯為 React 元件**，並隔離排查編譯或渲染問題。
...
並排使用兩個 MDX 遊樂場時，您會迅速注意到某些內容在 v2 中的編譯結果不同或無法編譯。

:::tip[讓您的內容具備未來兼容性]

目標是重構有問題的內容，使其**能同時兼容兩個 MDX 版本**。如此一來，當您升級至 Docusaurus v3 時，這些內容將可直接運作。

:::

### 使用 MDX 檢查器 CLI

我們提供 [docusaurus-mdx-checker](https://github.com/slorber/docusaurus-mdx-checker) CLI 工具，可輕鬆標記有問題的內容。在您的網站上執行此命令，即可獲取無法在 MDX v3 下編譯的檔案清單。

```bash
npx docusaurus-mdx-checker
```

針對每個編譯問題，CLI 會記錄檔案路徑與對應的行號供您查閱。

![終端機截圖顯示 MDX 檢查器 CLI 輸出範例，包含數個錯誤訊息](@site/blog/2023/09-29-preparing-your-site-for-docusaurus-v3/img/mdx-checker-output.png)

:::tip

使用此 CLI 評估讓您的內容兼容 MDX v3 所需的工作量。

:::

:::warning

此 CLI 僅盡力而為，且**僅回報編譯錯誤**。

對於不產生錯誤但可能影響內容顯示的細微編譯變更，它不會回報。為捕捉這類問題，建議採用[視覺回歸測試](/blog/upgrading-frontend-dependencies-with-confidence-using-visual-regression-testing)。

:::

### 常見 MDX 問題

Docusaurus 無法詳盡記錄 MDX 帶來的所有變更。這屬於 [MDX v2](https://mdxjs.com/migrating/v2/) 與 [MDX v3](https://mdxjs.com/migrating/v3/) 遷移指南的範疇。

然而，透過升級數個 Docusaurus 網站，我們注意到多數問題可歸納為幾種常見案例，已為您記錄於此。

#### 錯誤使用 `{`

`{` 字元用於開啟 [JavaScript 表達式](https://mdxjs.com/docs/what-is-mdx/#expressions)。若您置於 `{expression}` 中的內容非有效表達式，MDX 現在將報錯。
...
使用標準 Markdown 連結語法，或移除 `<` 和 `>`。MDX 和 GFM 已能自動將字面值轉為連結。
...
您的 [`MDXComponent` 元件映射](../guides/markdown-features/markdown-features-react.mdx#mdx-component-scope)可能無法如預期生效，自訂元件可能不再被使用。
...
指令會被解析，目的是由其他 Remark 插件處理。未處理的指令將被忽略，並且不會以其原始形式渲染回來。
...
日文和中文受此影響最大，但其他某些語言（如泰文和高棉文）也可能受到影響，例如當您嘗試強調行內代碼或連結時。使用空格分隔單詞的語言受影響程度較小。
...
如果您想繼續使用 CommonJS 模組，可以使用動態導入作為變通方法，這讓您能在 CommonJS 模組中導入 ES 模組。幸運的是，[Docusaurus 設定支援使用異步函數](/docs/configuration#syntax-to-declare-docusaurus-config)來實現這一點。
...
如果您創建了自定義的 Remark 或 Rehype 插件，由於新的 AST 結構，您可能需要重構這些插件，甚至完全重寫。我們創建了一個[專屬支援討論串](https://github.com/facebook/docusaurus/discussions/9337)來幫助插件開發者升級他們的代碼。

:::

### 格式化工具

最常見的格式化工具 Prettier 目前僅支援舊版 MDX v1，截至 Docusaurus v3.0.0 尚未支援 v3。您可以在代碼的不相容部分前添加 `{/* prettier-ignore */}` 來讓 Prettier 正常工作。
...
如果您厭倦了插入太多 `{/* prettier-ignore */}`，可以考慮在 Prettier 開始支援 MDX v3 之前，通過在 `.prettierignore` 文件中添加以下內容來禁用 Prettier 對 MDX 的格式化：
...
您也可以更新網站的 `package.json` 檔案，防止使用過時的不支援版本：
...
在升級至 Docusaurus v3 之前，先將您的 Docusaurus v2 網站升級至 Node.js 18 環境。
...
React 18 自帶的破壞性變更相對容易處理，具體取決於您為網站編寫的自訂 React 程式碼量。官方主題和外掛程式已相容 React 18。

:::info[如何升級]

閱讀官方 [React v18.0](https://react.dev/blog/2022/03/29/react-v18) 和 [升級至 React 18 指南](https://react.dev/blog/2022/03/08/react-18-upgrade-guide)，檢查您的 React 程式碼以確認哪些元件可能受此升級影響。
...
Docusaurus 對這些功能的支援尚屬實驗性質。未來可能需要調整整合方式，導致運行時行為變化。
...
這是包含破壞性變更的主要版本更新，無法保證完全向下相容。雖然 [`prism-react-renderer` v2 發布說明](https://github.com/FormidableLabs/prism-react-renderer/releases/tag/prism-react-renderer%402.0.0)不夠詳盡，但 Docusaurus 使用者需注意 3 項重大變更。
...
> **破壞性變更** 將 [node-emoji](https://www.npmjs.com/package/node-emoji) 從 v1 更新至 v2。此變更新增了對許多新表情符號的支援，同時移除了 GitHub 上已失效的舊表情符號快捷碼。
...
此新套件與其他 Docusaurus 核心套件同步版本，將用於確保 TypeScript 的向下相容性及處理重大版本升級時的破壞性變更。
...
理論上，您無需進行任何操作，現有的設定檔案應能繼續正常運作。

但這是一次主要的依賴庫更換，可能會出現細微的行為變化。

:::

### 警告提示框注意事項

由於歷史原因，我們支援一個未公開的 `:::warning` 提示框，其會呈現紅色樣式。
...
然而其顏色與圖示始終存在錯誤。Docusaurus v3 正式重新引入 `:::warning` 提示框，將其列入文件並修正了顏色與圖示。
...
若您先前使用過未公開的 `:::warning` 提示框，請逐一確認黃色是否適合替代原有紅色樣式。若需維持紅色警示，請改用 `:::danger`。

Docusaurus v3 同時[棄用了 `:::caution`](https://github.com/facebook/docusaurus/pull/9308) 提示框。請將 `:::caution`（黃色）重構為 `:::warning`（黃色）或 `:::danger`（紅色）。

若需保留「caution」標題，可考慮重構為 `:::warning[caution]`（黃色）。

:::

### 版本化側邊欄

此項重大變更僅影響 **在 `v2.0.0-beta.10` 版本（2021年12月）前啟用文檔版本化功能** 的早期 Docusaurus v2 使用者。
...
技術上**這不屬於重大變更**，因這些元件**標記為不建議替換**，但許多 Docusaurus 站點已替換文檔相關元件，需注意其客製化可能導致相容性問題。

:::info[如何升級]

刪除所有已替換的元件，重新執行替換操作，並在新的元件基礎上重新套用您的客製化設定。
...
部分變更非強制性，但了解後有助於充分發揮 Docusaurus v3 的效能。
...
在未來版本的 Docusaurus 中，`.md` 檔案將被解析為標準的 [CommonMark](https://commonmark.org/)，該標準不支援這些功能。在 Docusaurus v3 中，`.md` 檔案仍被編譯為 MDX 檔案，但可以[選擇使用 CommonMark](https://github.com/facebook/docusaurus/issues/3018)。
...
Docusaurus 現在使用 [Markdown 指令](https://talk.commonmark.org/t/generic-directives-plugins-syntax/444)（通過 [remark-directive](https://github.com/remarkjs/remark-directive) 實現）來實現警告，且提供指令標籤的語法需要使用方括號：
...
- 確保所有文檔能在[MDX遊樂場](https://mdxjs.com/playground/)編譯通過，或使用[`npx docusaurus-mdx-checker`](https://github.com/slorber/docusaurus-mdx-checker)檢查
- 刪除`node_modules`和`package-lock.json`後重新執行`npm install`
- 執行`docusaurus clear`清除緩存
- 移除可能不支援Docusaurus v3的第三方插件
- 刪除所有自行替換的組件

若問題仍未解決，可透過以下支援管道尋求協助：
...
- 清楚說明問題細節，避免僅描述模糊的「無法運作」
- 提供完整相關材料：程式碼片段、儲存庫網址、git分支連結、完整堆疊追蹤、截圖與影片
- 以清晰簡潔的方式呈現請求，展現您已盡力協助我們幫助您

您也可以考慮付費請[Docusaurus服務供應商](https://github.com/facebook/docusaurus/discussions/9281)代為執行升級。若您的專案是開源的，亦可向社群尋求[免費的善意協助](https://github.com/facebook/docusaurus/discussions/9283)。