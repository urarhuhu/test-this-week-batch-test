...
`write-translations` CLI 工具仍可用於從程式碼中提取翻譯。

程式碼翻譯現在會以 Chrome 國際化 JSON 格式存入 `i18n/[locale]/code.json` 檔案。
...
這對國際化特別重要，因為若 Crowdin 上的翻譯品質不佳且使用了無效的標記語言，您的 v2 翻譯網站可能會建置失敗：您可能需要進行一些翻譯清理來修復錯誤。
...
- 使用全新且未翻譯的 Crowdin 專案和 [Crowdin 教學](../../i18n/i18n-crowdin.mdx)讓 Crowdin 適用於您的 v2 網站
...
為避免任何可能導致生產環境中 v1 網站中斷的風險，一種可能的策略是複製原始的 v1 Crowdin 專案。
...
- 使用Crowdin CLI將Docusaurus v2原始檔上傳至新專案
...
您需要修正所有錯誤直到網站能成功建置。可以透過修改本地端的翻譯MD檔案，並使用`docusaurus build --locale fr`一次針對一個語系修正網站問題。

我們無法提供解決這些錯誤的終極指南，但常見錯誤原因包括：

- 未在Crowdin標記足夠的「隱藏字串」，導致預翻譯嘗試翻譯這些字串。
- 存在不良的v1翻譯，導致v2出現無效標記：翻譯內容中的錯誤HTML元素和未閉合標籤
...
Crowdin的Markdown解析器會隨時間演進，且每個Crowdin專案的解析器版本可能不同，這會導致預翻譯無法完整預翻譯所有字串。
...
若您不介意修改現有Crowdin專案並承擔可能出錯的風險，可以嘗試使用Crowdin的分支系統。
...
如此一來，您無需建立新Crowdin專案、轉移翻譯記憶檔、套用預翻譯，也無需修正預翻譯錯誤。

您可以為Docusaurus v2建立Crowdin分支，上傳v2原始檔至該分支，待準備完成後再將分支合併至主線。

### 改用Git而非Crowdin {#use-git-instead-of-crowdin}

可以選擇遷移離開Crowdin，改將翻譯檔案加入Git版本控制。
