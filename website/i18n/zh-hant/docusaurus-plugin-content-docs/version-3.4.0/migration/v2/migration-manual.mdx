...
此變更能明確區分官方套件與社群維護套件。換言之，所有官方套件皆歸類於 `@docusaurus/` 命名空間下。
...
D1 專案可能已有 README 文件，可修改內容反映 D2 變更，或直接套用預設的 [Docusaurus v2 README](https://github.com/facebook/docusaurus/blob/main/packages/create-docusaurus/templates/shared/README.md)。
...
Docusaurus 2 將各功能（部落格、文件、頁面）拆分為插件以實現模組化。預設集為插件組合包，為保持相容性，我們提供 `@docusaurus/preset-classic` 預設集，包含 Docusaurus 1 的核心插件。
...
我們建議將 `docs` 資料夾移至 `website` 資料夾內，這也是 v2 的預設目錄結構。[Vercel](https://vercel.com) 支援[開箱即用的 Docusaurus 專案部署](https://github.com/vercel/vercel/tree/main/examples/docusaurus)，前提是 `docs` 目錄必須位於 `website` 內。將文檔放在網站目錄中也更為理想，這樣文檔和網站程式碼就能集中在同一個 `website` 目錄下。

如果您正在遷移 Docusaurus v1 網站，且有待處理的文檔拉取請求(pull requests)，可以暫時保留 `/docs` 資料夾在原位以避免產生衝突。

請參閱下方遷移指南中針對 `siteConfig.js` 每個欄位的說明。
...
已棄用。我們為 Docusaurus 2 開發了名為 [Infima](https://infima.dev/) 的自訂 CSS 框架，使用 CSS 變數進行主題設定。相關文檔尚未完備，我們將在準備好後更新此處。若要覆寫 Infima 的 CSS 變數，請建立自己的 CSS 檔案（例如 `./src/css/custom.css`），並通過將其作為選項傳遞給 `@docusaurus/preset-classic` 來全局導入：
...
- `blogSidebarTitle`
- `cleanUrl` - 現在預設使用簡潔網址。
- `defaultVersionShown` - 版本功能尚未移植。如果您正在使用版本功能，將無法遷移至 Docusaurus 2。敬請期待。
- `disableHeaderTitle`
- `disableTitleTagline`
- `docsSideNavCollapsible` 現在可在 `docsPluginOptions.sidebarCollapsible` 中設定，且預設為啟用狀態。
- `facebookAppId`
- `facebookComments`
- `facebookPixelId`
- `fonts`
- `highlight` - 我們現在使用 [Prism](https://prismjs.com/) 而非 [highlight.js](https://highlightjs.org/)。
- `markdownOptions` - v2 改用 MDX 而非 Remarkable。您的 Markdown 選項需轉換為 Remark/Rehype 插件。
- `markdownPlugins` - v2 改用 MDX 而非 Remarkable。您的 Markdown 插件需轉換為 Remark/Rehype 插件。
- `manifest`
- `onPageNav` - 此功能現在預設為啟用。
...
在 v2 中，預設情況下，規範頁面網址為 `/installation`，而非 `/installation.html`。

如果您在 v1 中設定 `cleanUrl: false`，可能有人已發布了指向 `/installation.html` 的連結。
...
舊版本不允許嵌套側邊欄分類，且分類只能包含文件 ID。而 v2 允許無限嵌套側邊欄，並提供多種[側邊欄項目類型](../../guides/docs/sidebar/items.mdx)（不局限於文件）。
...
請勿僅為在左側添加標誌而替換頁尾元件。v2 版刻意移除左側標誌並移至底部，請直接在 `docusaurus.config.js` 中透過 `themeConfig.footer` 配置頁尾：
...
請參閱[建立頁面](guides/creating-pages.mdx)了解 Docusaurus 2 的頁面運作機制。閱讀後請注意：需將 v1 版中的 `pages/en` 檔案移至 `src/pages` 目錄。

Docusaurus v1 的頁面會接收 `siteConfig` 物件作為 props。

Docusaurus v2 需改從 `useDocusaurusContext` 獲取 `siteConfig` 物件。

v2 版需在每個頁面套用主題佈局。Layout 元件會接收 metadata props。
...
Docusaurus 2 已將 Markdown 語法改為 [MDX](https://mdxjs.com/)，因此現有文件可能需要修正語法。常見範例如 HTML 中有效的自閉合標籤 `<img>` 和 `<br>`，現在需明確閉合為 `<img/>` 和 `<br/>`。MDX 文件中的所有標籤都必須是有效的 JSX。

Front matter 由 [gray-matter](https://github.com/jonschlinkert/gray-matter) 解析。若您的 front matter 包含特殊字符如 `:`，現在需加上引號：`title: Part 1: my part1 title` → `title: "Part 1: my part1 title"`。

**提示**：您可使用線上工具如 [HTML 轉 JSX](https://transform.tools/html-to-jsx) 來簡化遷移流程。
...
Docusaurus 部落格的 front matter 欄位已從駝峰式命名改為蛇形命名，以與文件保持一致。

欄位 `authorFBID` 和 `authorTwitter` 已被棄用。這些欄位僅用於產生作者頭像，現可透過 `authors` 欄位實現相同功能。

## 部署 {#deployment}

GitHub Pages 使用的 `CNAME` 檔案不再自動生成，若使用自訂網域，請確保在 `/static/CNAME` 手動建立該檔案。

部落格 RSS 訂閱網址變更為 `/blog/rss.xml`（原為 `/blog/feed.xml`）。建議設定伺服器端重新導向，以維持使用者訂閱的有效性。
