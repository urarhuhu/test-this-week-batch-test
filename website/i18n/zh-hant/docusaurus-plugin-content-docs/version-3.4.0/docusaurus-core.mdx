...
用它來包裹可能拋出錯誤的組件，當錯誤發生時顯示備用內容，而不是導致整個應用崩潰。
...
點擊此處查看實際效果：<ErrorBoundaryTestButton/>
...
此組件不會捕獲構建時錯誤，僅保護客戶端渲染時可能發生的錯誤，這些錯誤在使用有狀態的 React 組件時可能出現。

:::

#### 屬性 {#errorboundary-props}

- `fallback`: 可選的渲染回調函數，返回一個 JSX 元素。它會接收一個包含兩個屬性的對象：`error`，捕獲到的錯誤；以及 `tryAgain`，一個用於重置組件錯誤並嘗試重新渲染的函數回調（`() => void`）。如果未提供，則會渲染 `@theme/Error` 作為替代。`@theme/Error` 用於包裹整個網站的錯誤邊界，位於佈局之上。

:::warning

`fallback` 屬性是一個回調函數，**而不是一個 React 函數組件**。您不能在這個回調中使用 React 鉤子。
...
此組件支持鏈接到內部頁面，並提供了一個強大的性能特性稱為預加載。預加載用於預取資源，以便用戶通過此組件導航時資源已經加載完成。我們使用 `IntersectionObserver` 在 `<Link>` 進入視口時發起低優先級請求，然後在用戶可能導航到該資源時使用 `onMouseOver` 事件觸發高優先級請求。

該組件是 react-router 的 `<Link>` 組件的封裝，增加了 Docusaurus 特有的增強功能。所有屬性都會傳遞給 react-router 的 `<Link>` 組件。
...
渲染 `<Redirect>` 將導航到新位置。新位置將像服務器端重定向（HTTP 3xx）一樣覆蓋歷史堆棧中的當前位置。您可以參考 [React Router 的 Redirect 文檔](https://reacttraining.com/react-router/web/api/Redirect) 獲取更多關於可用屬性的信息。
...
當程式碼無法在 Node.js 環境執行（例如需要存取 `window` 或 `document` 物件時）可使用此元件進行整合。
...
佔位符將被替換為您提供的動態值和選擇的 JSX 元素（字串、連結、樣式化元素等）。
...
當[本地化您的網站](./i18n/i18n-introduction.mdx)時，`<Translate/>` 元件可為 React 元件（例如首頁）提供翻譯支援。此元件支援[插值功能](#interpolate)。

翻譯字串會透過 [`docusaurus write-translations`](./cli.mdx#docusaurus-write-translations-sitedir) CLI 從程式碼中靜態提取，並在 `website/i18n/[locale]` 目錄下產生 `code.json` 翻譯檔案。
...
您甚至可以省略 children prop，在執行 `docusaurus write-translations` CLI 指令後，手動在 `code.json` 檔案中指定翻譯字串。
...
`<Translate>` 元件支援插值功能。您也可以透過自訂程式碼和[命令式 `translate` API](#translate-imperative) 實作[字串複數化](https://github.com/facebook/docusaurus/pull/i18n/i18n-tutorial.mdx#pluralization)。
...
客戶端首次渲染輸出（瀏覽器中）**必須與**伺服器端渲染輸出（Node.js）**完全一致**。違反此規則可能導致意外的水合行為，詳見 [The Perils of Rehydration](https://www.joshwcomeau.com/react/the-perils-of-rehydration/)。
...
多數情況下，您不需要使用 `useBaseUrl`。

對於[資源文件](./guides/markdown-features/markdown-features-assets.mdx)，建議使用 `require()`：
...
當 `useBaseUrl` 無法滿足需求時，此鉤子提供與站點基礎 URL 相關的額外工具函數。

- `withBaseUrl`：若需一次性為多個 URL 添加基礎 URL 時非常實用。
...
插件 ID 僅在單一插件於同站點被多次使用時才有意義。每個插件實例均可建立自己的全域資料。
...
存取特定插件建立的全域資料。給定插件名稱後，會按插件 ID 返回該名稱下所有插件實例的資料。
...
React hook 用於存取 Docusaurus 的壞鏈檢查 API，提供一種方式讓 Docusaurus 頁面回報並收集其連結與錨點。
