...
將潛在敏感信息放在環境中是常見的做法。然而，在典型的 Docusaurus 網站中，`docusaurus.config.js` 檔案是與 Node.js 環境的唯一接口（請參閱 [我們的架構概述](advanced/architecture.mdx)），而其他所有內容（MDX 頁面、React 組件等）都是客戶端，無法直接訪問 `process` 全域變數。在這種情況下，您可以考慮使用 [`customFields`](api/docusaurus.config.js.mdx#customFields) 將環境變數傳遞到客戶端。
...
沒有一個萬能的解決方案。您需要根據需求和資源來權衡選擇。

## 自架主機 {#self-hosting}

Docusaurus 可使用 [`docusaurus serve`](cli.mdx#docusaurus-serve-sitedir) 指令自架主機，透過 `--port` 變更連接埠，`--host` 變更主機位址。
...
後續章節將介紹常見託管服務商及其高效部署 Docusaurus 網站的配置方式。Docusaurus 與這些服務無商業合作，此處資訊僅供參考。部分說明由第三方提供，最新API變動可能未即時更新。若發現內容過時，歡迎提交PR修正。

由於我們僅能提供最佳努力(best-effort)的維護，目前已停止接受新增託管選項的PR。但您可將部署指南發表於其他平台（如個人部落格或服務商官網），並請我們加入相關連結。

:::

## 部署至 Netlify {#deploying-to-netlify}

部署 Docusaurus 網站至 [Netlify](https://www.netlify.com/) 前，請確認以下設定正確：
...
若未預先設定建置選項，仍可於站點建立後至「Site settings」→「Build & deploy」補設。

完成上述配置後，網站將自動部署，並在程式碼合併至部署分支（預設為 `main`）時觸發重新部署。

:::warning

部分 Docusaurus 網站將 `docs` 資料夾置於 `website` 外（常見於 Docusaurus v1 舊版結構）：
...
若選擇將 `website` 設為 Netlify 基礎目錄，當 `docs` 資料夾更新時 Netlify 不會觸發建置，此時需配置 [自訂忽略指令](https://docs.netlify.com/configure-builds/common-configurations/ignore-builds/)：
...
**請特別注意**：全局選項「Disable asset optimization」實際無法停用「Pretty URLs」設定，務必獨立取消勾選該項目。
...
透過 [導入流程](https://vercel.com/import/git) 將專案導入 Vercel。在導入過程中，您會發現所有相關選項都已預先配置完成；不過，您仍可選擇變更其中任何一項 [設定](https://vercel.com/docs/build-step#build-&-development-settings)。

專案導入後，所有後續推送至分支的動作都會產生 [預覽部署](https://vercel.com/docs/platform/deployments#preview)，而對 [生產分支](https://vercel.com/docs/git-integrations#production-branch)（通常為 "main" 或 "master"）所做的任何變更，則會觸發 [生產環境部署](https://vercel.com/docs/platform/deployments#production)。

## 部署至 GitHub Pages {#deploying-to-github-pages}

Docusaurus 提供簡便的方式來發布至 [GitHub Pages](https://pages.github.com/)，此服務免費附屬於每個 GitHub 儲存庫。

### 概覽 {#github-pages-overview}

通常發布流程會涉及兩個儲存庫（或至少兩個分支）：包含原始碼檔案的分支，以及包含需透過 GitHub Pages 服務之建置輸出的分支。以下教學中，將分別稱其為「來源」與「部署」分支。

每個 GitHub 儲存庫都關聯著一項 GitHub Pages 服務。若部署儲存庫名為 `my-org/my-project`（其中 `my-org` 為組織名稱或使用者名稱），部署後的網站將出現在 `https://my-org.github.io/my-project/`。若部署儲存庫名為 `my-org/my-org.github.io`（即 _組織 GitHub Pages 儲存庫_），網站則會出現在 `https://my-org.github.io/`。

:::info

若想為 GitHub Pages 使用自訂網域，請在 `static` 目錄中建立 `CNAME` 檔案。`static` 目錄內的所有內容都會被複製到部署用的 `build` 目錄根層。使用自訂網域時，您應可將 `baseUrl: '/projectName/'` 改回 `baseUrl: '/'`，並將 `url` 設為您的自訂網域。

更多細節可參考 GitHub Pages 文件 [使用者、組織與專案頁面](https://help.github.com/en/articles/user-organization-and-project-pages)。

:::

GitHub Pages 會從預設分支（通常為 `master` 或 `main`）或 `gh-pages` 分支，並從根目錄或 `/docs` 資料夾中擷取已準備部署的檔案（即 `docusaurus build` 的輸出）。您可透過儲存庫中的 `設定 > Pages` 來配置此選項。該分支將稱為「部署分支」。

我們提供 `docusaurus deploy` 指令，協助您透過單一指令將網站從來源分支部署至部署分支：包含克隆、建置與提交。

### `docusaurus.config.js` 設定 {#docusaurusconfigjs-settings}

首先，修改您的 `docusaurus.config.js` 並新增以下參數：
...
這些欄位也有對應的環境變數版本（優先權更高）：`ORGANIZATION_NAME`、`PROJECT_NAME` 與 `DEPLOYMENT_BRANCH`。

:::warning

GitHub Pages 預設會為 Docusaurus 網址加上尾部斜線。建議設定 `trailingSlash` 配置（設為 `true` 或 `false`，而非 `undefined`）。
...
預設情況下，GitHub Pages 會透過 [Jekyll](https://jekyllrb.com/) 處理發布的檔案。由於 Jekyll 會捨棄任何以 `_` 開頭的檔案，建議您在 `static` 目錄中加入名為 `.nojekyll` 的空檔案來停用 Jekyll。
...
GitHub 企業版安裝應與 github.com 的運作方式相同；您只需將組織的 GitHub Enterprise 主機設為環境變數：
...
自 2021 年 8 月起，GitHub 要求所有命令行登入必須使用 **個人存取權杖 (PAT)** 而非密碼。當 GitHub 提示輸入密碼時，請改為輸入 PAT。詳情請參閱 [GitHub 文件](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/creating-a-personal-access-token)。
...
以下工作流程範例假設您的網站原始碼位於儲存庫的 `main` 分支（即 _原始分支_ 為 `main`），且您的 [發布來源](https://help.github.com/en/github/working-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site) 已設定為 [使用自訂 GitHub Actions 工作流程發布](https://docs.github.com/en/pages/getting-started-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site#publishing-with-a-custom-github-actions-workflow)。

我們的目標是：

1. 當有新的拉取請求提交至 `main` 分支時，執行一個確保網站能成功建構（但不實際部署）的動作。此工作稱為 `test-deploy`。
2. 當拉取請求合併至 `main` 分支或有人直接推送至 `main` 分支時，將建構並部署至 GitHub Pages。此工作稱為 `deploy`。

以下是兩種使用 GitHub Actions 部署文件的方法。根據您的部署儲存庫位置，選擇下方對應的標籤：

- 原始儲存庫與部署儲存庫為 **同一個** 儲存庫。
- 部署儲存庫是一個 **遠端** 儲存庫，與原始儲存庫不同。此情境的說明假設 [發布來源](https://help.github.com/en/github/working-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site) 為 `gh-pages` 分支。
...
雖然您可以將兩個工作定義在同一個工作流程檔案中，但原始的 `deploy` 工作流程在 PR 檢查套件狀態中會始終顯示為「已跳過」，這無法反映實際狀態且對審查流程無益。因此，我們建議改為將它們分開管理為獨立的工作流程。
...
1. 生成一組新的 [SSH 金鑰](https://help.github.com/en/github/authenticating-to-github/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent)。由於此 SSH 金鑰將用於 CI 環境，請確保不要設定任何密碼。
2. 預設情況下，您的公鑰應建立於 `~/.ssh/id_rsa.pub`；若使用其他名稱，請在[GitHub 部署金鑰](https://developer.github.com/v3/guides/managing-deploy-keys/)中添加您的金鑰。
3. 使用 `pbcopy < ~/.ssh/id_rsa.pub` 將金鑰複製到剪貼簿，並將其貼上至部署儲存庫的[部署金鑰](https://developer.github.com/v3/guides/managing-deploy-keys/#deploy-keys)中。若命令列無法使用，請直接複製檔案內容。在儲存部署金鑰前，勾選 `允許寫入存取` 選項。
...
- 來源儲存庫：已設定 GitHub 工作流程，並將私鑰 SSH 金鑰設為 GitHub 秘密
- 部署儲存庫：已在 GitHub 部署金鑰中設定公鑰 SSH 金鑰
...
持續整合 (CI) 服務通常用於在提交新程式碼至版本控制時執行例行任務。這些任務可包括執行單元測試與整合測試、自動化建置、發布套件至 npm，以及部署變更至您的網站。要自動化部署您的網站，只需在網站更新時呼叫 `yarn deploy` 指令碼即可。以下章節將說明如何使用熱門的持續整合服務供應商 [Travis CI](https://travis-ci.com/) 來達成此目的。

1. 前往 https://github.com/settings/tokens 並生成一個新的[個人存取權杖](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/)。建立權杖時，請授予其 `repo` 權限範圍以取得所需權限。
2. 使用您的 GitHub 帳戶，[新增 Travis CI 應用程式](https://github.com/marketplace/travis-ci)至您要啟用的儲存庫。
3. 開啟您的 Travis CI 儀表板（網址類似 `https://travis-ci.com/USERNAME/REPO`），並導航至儲存庫的 `更多選項 > 設定 > 環境變數` 區段。
4. 建立一個名為 `GH_TOKEN` 的新環境變數，其值為您新生成的權杖，接著設定 `GH_EMAIL`（您的電子郵件地址）和 `GH_NAME`（您的 GitHub 使用者名稱）。
5. 在儲存庫根目錄建立一個包含以下內容的 `.travis.yml` 檔案：
...
現在，每當有新的提交進入 `main` 分支時，Travis CI 將執行您的測試套件，若一切通過，您的網站將透過 `yarn deploy` 指令碼進行部署。

### 使用 Buddy 觸發部署 {#triggering-deployment-with-buddy}

[Buddy](https://buddy.works/) 是一款易於使用的 CI/CD 工具，可讓您自動將入口網站部署至不同環境，包括 GitHub Pages。

請按照以下步驟建立一個管道，以便在您推送變更至專案的選定分支時，自動部署網站的新版本：

1. 前往 https://github.com/settings/tokens 生成新的[個人存取權杖](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/)。建立權杖時，請授予其 `repo` 範圍以確保其具備所需權限。
...
建立此簡單管線後，每次推送至您選定分支的新提交都會使用 `yarn deploy` 將您的網站部署至 GitHub Pages。閱讀[本指南](https://buddy.works/guides/react-docusaurus)以瞭解更多關於為 Docusaurus 設定 CI/CD 管線的資訊。
...
現在，每當您推送新標籤至 GitHub 時，此觸發器將啟動 drone CI 作業以發布您的網站。

## 部署至 Flightcontrol {#deploying-to-flightcontrol}

[Flightcontrol](https://www.flightcontrol.dev/?ref=docusaurus) 是一項服務，可自動從您的 Git 儲存庫將您的網頁應用程式構建並部署至 AWS Fargate。它讓您能夠完整檢視並進行基礎架構變更，不受傳統 PaaS 的限制。
...
1. 創建 Qovery 帳戶。如果您還沒有帳戶，請訪問 [Qovery 儀表板](https://console.qovery.com) 創建一個帳戶。
2. 創建專案。
   - 點擊 **創建專案** 並為您的專案命名。
   - 點擊 **下一步**。
3. 創建新環境。
...
4. 新增應用程式。
   - 點擊 **創建應用程式**，命名並選擇您的 GitHub 或 GitLab 儲存庫，其中包含您的 Docusaurus 應用程式。
   - 定義主分支名稱和應用程式的根路徑。
   - 點擊 **創建**。應用程式創建完成後：
   - 導航至您的應用程式 **設定**
   - 選擇 **端口**
   - 新增您的 Docusaurus 應用程式使用的端口
5. 部署
...
2. 選擇要部署的專案。
   ...
   - 如果您使用 GitHub、GitLab 或 Bitbucket 帳號登入 Hostman，您將看到包含您專案的儲存庫，包括私有專案。

   - 選擇您要部署的專案。該專案必須包含專案檔案的目錄（例如 `website`）。

   - 若要存取其他儲存庫，請點擊**連接其他儲存庫**。

   - 如果您未使用 Git 帳號憑證登入，現在可以存取所需帳號，然後選擇專案。
...
3. 設定建置選項。
   ...
   - 如有需要，您可以修改建置指令。您可以輸入多個指令，以 `&&` 分隔。

4. 部署。

   - 點擊**部署**以開始建置流程。

   - 開始後，您將進入部署日誌。如果程式碼有任何問題，日誌中會顯示警告或錯誤訊息，指出問題原因。通常日誌會包含所有您需要的除錯資料。
...
1. 首先，透過執行以下指令使用 npm 安裝 Surge：
   ```bash npm2yarn
   npm install -g surge
   ```
2. 在專案的根目錄中執行以下指令，為生產環境建置網站的靜態檔案：
   ```bash npm2yarn
   npm run build
   ```
3. 接著，在專案的根目錄中執行以下指令：
...
確認要發佈的網站位於 `build` 目錄中。系統會自動產生一個隨機子網域 `*.surge.sh`（可編輯）。
...
您可以將 Docusaurus 專案部署至 [Stormkit](https://www.stormkit.io)，這是一個用於靜態網站、單頁應用程式 (SPA) 和無伺服器函數的部署平台。詳細說明請參閱此[指南](https://www.stormkit.io/blog/how-to-deploy-docusarous)。
...
[Layer0](https://www.layer0.co) 是一個全端平台，用於開發、部署、預覽、實驗、監控和運行您的無頭前端。它專注於大型動態網站，並透過 EdgeJS（基於 JavaScript 的內容交付網絡）、預測性預取和性能監控提供一流的性能。Layer0 提供免費方案。只需幾分鐘即可開始，請遵循 [Layer0 的 Docusaurus 部署指南](https://docs.layer0.co/guides/docusaurus)。

## 部署至 Cloudflare Pages {#deploying-to-cloudflare-pages}

[Cloudflare Pages](https://pages.cloudflare.com/) 是一個 Jamstack 平台，供前端開發者協作和部署網站。幾分鐘內即可開始，請遵循[這篇文章](https://dev.to/apidev234/deploying-docusaurus-to-cloudflare-pages-565g)。

## 部署至 Azure Static Web Apps {#deploying-to-azure-static-web-apps}

[Azure Static Web Apps](https://docs.microsoft.com/en-us/azure/static-web-apps/overview) 是一項服務，可直接從代碼存儲庫自動構建和部署全端 Web 應用程序到 Azure，簡化了 CI/CD 的開發者體驗。Static Web Apps 將 Web 應用程序的靜態資源與其動態（API）端點分開。靜態資源從全球分佈的內容服務器提供，使客戶端能夠使用附近的服務器更快地檢索文件。動態 API 使用基於事件的無服務器架構進行擴展，這種方法更具成本效益並按需擴展。幾分鐘內即可開始，請遵循[這份逐步指南](https://dev.to/azure/11-share-content-with-docusaurus-azure-static-web-apps-30hc)。

## 部署至 Kinsta {#deploying-to-kinsta}

[Kinsta 靜態網站託管](https://kinsta.com/static-site-hosting) 讓您可以免費部署最多 100 個靜態網站，包括自定義域名與 SSL、每月 100 GB 頻寬，以及 260+ 個 Cloudflare CDN 位置。

只需點擊幾下即可開始，請遵循我們的 [Kinsta 上的 Docusaurus](https://kinsta.com/docs/docusaurus-example/) 文章。