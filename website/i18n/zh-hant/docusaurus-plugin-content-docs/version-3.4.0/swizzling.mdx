...
本節與[樣式與版面](./styling-layout.mdx)類似，但這次我們將自訂 React 元件本身，而非其外觀。我們將討論 Docusaurus 的核心概念：**元件替換 (swizzling)**，這項技術允許進行**更深層的網站客製化**。

實務上，元件替換可透過兩種模式**替換主題元件為您的實作版本**：

- [**彈出 (Ejecting)**](#ejecting)：建立原始主題元件的**副本**，您可完全**自訂**
- [**包裝 (Wrapping)**](#wrapping)：建立原始主題元件的**包裝器**，您可進行**功能增強**
...
`docusaurus swizzle` 僅是協助自動化替換元件的工具。您亦可手動建立 `src/theme/SomeComponent.js` 檔案，Docusaurus 會[自動解析](./advanced/client.mdx#theme-aliases)。此命令背後並無特殊魔法！

:::

### 彈出 (Ejecting) {#ejecting}

彈出主題元件是指**建立原始主題元件的副本**，您可**完全自訂與覆寫**該副本。
...
此操作會將當前 `<Footer />` 元件的實作複製到您網站的 `src/theme` 目錄。Docusaurus 將使用此 `<Footer>` 元件副本而非原始版本。您現在可完全重新實作 `<Footer>` 元件。
...
彈出[**不安全**](#what-is-safe-to-swizzle)元件時，有時會導致複製大量內部程式碼，這些程式碼現在需由您自行維護。這可能使 Docusaurus 升級更加困難，因為若接收的 props 或使用的內部主題 API 發生變更，您需自行遷移客製化內容。

**請盡可能優先採用[包裝](#wrapping)方式**：需維護的程式碼量較少。
...
對於每個主題元件，混寫 CLI 會標示主題作者宣告的**三種不同安全等級**：
...
要達成預期效果時，往往不清楚究竟該混寫哪個元件。提供大多數主題元件的 `@docusaurus/theme-classic` 擁有約 [100 個元件](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-theme-classic/src/theme)！

:::tip

若要列印 `@docusaurus/theme-classic` 所有元件的概覽：
...
2. **元件名稱**。官方主題元件採用語義化命名，您可從名稱推斷其功能。Swizzle CLI 允許輸入元件名稱部分字串來篩選選項。例如執行 `yarn swizzle @docusaurus/theme-classic` 後輸入 `Doc`，將只列出與文檔相關的元件。
3. **從高層級元件開始**。元件形成樹狀結構，某些元件會導入其他元件。每條路由都會關聯一個頂層元件（多數列於[內容插件路由](./advanced/routing.mdx#routing-in-content-plugins)）。例如所有部落格文章頁面的最頂層元件都是 `@theme/BlogPostPage`。您可以先swizzle此元件，再向下遍歷元件樹定位目標元件。找到正確元件後，請刪除其他swizzle的檔案以避免維護過多元件。
...
`<Root>`元件會渲染在React樹的**最頂層**，位於主題`<Layout>`之上且**永不卸載**。非常適合用於添加不應因導航而重新初始化的狀態邏輯（使用者認證狀態、購物車狀態等）。
