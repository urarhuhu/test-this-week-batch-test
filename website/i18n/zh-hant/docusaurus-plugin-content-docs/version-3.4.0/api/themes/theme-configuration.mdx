...
當需要在網站發布公告時，可添加公告欄。這是位於導覽列上方、非固定且可選擇性關閉的面板。所有配置皆位於`announcementBar`物件中。
...
標誌可置於[靜態資料夾](static-assets.mdx)中。標誌URL預設為網站根路徑。您可自訂標誌URL，若為外部連結將在新分頁開啟。此外，可覆寫標誌連結的target屬性值，當文件網站位於主網站子目錄時特別有用（此時可能不需讓標誌連結在主網站新分頁開啟）。
...
項目行為會根據`type`欄位而異。以下章節將介紹所有可用的導覽列項目類型。

#### 導覽列連結 {#navbar-link}

預設情況下，導航欄項目是常規連結（內部或外部）。

React Router 應會自動為連結套用活動連結樣式，但在邊緣情況下可使用 `activeBasePath`。若某連結需在多個路徑下保持活動狀態（例如同一側邊欄下有多個文檔資料夾），可使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更靈活的替代方案且優先級更高——Docusaurus 會將其解析為正則表達式並與當前 URL 進行匹配測試。
...
若使用版本化文檔，此特殊導航欄項目類型會渲染包含所有可用版本的下拉選單。
...
若您使用版本化文件，此特殊導航欄項目類型將連結至當前瀏覽的文件版本（取決於當前URL），並在無匹配版本時自動回退至最新版本。
...
可啟用此UI特性：當用戶向下滾動頁面時自動隱藏導航欄，向上滾動時重新顯示。
...
您可設定靜態導航欄樣式而不影響主題切換功能。所選樣式將始終生效，不受用戶選擇的主題影響。

目前提供兩種樣式選項：`dark`（深色）與`primary`（基於`--ifm-color-primary`主色）。樣式預覽請參閱[Infima文檔](https://infima.dev/docs/components/navbar/)。
...
預設採用[Palenight](https://github.com/FormidableLabs/prism-react-renderer/blob/master/packages/prism-react-renderer/src/themes/palenight.ts)語法高亮主題。您可從[可用主題列表](https://github.com/FormidableLabs/prism-react-renderer/tree/master/packages/prism-react-renderer/src/themes)選擇自定義主題，並為深色模式指定不同的高亮主題。
...
若您使用行標記的 Markdown 語法，可能需要為深色模式的語法高亮主題指定不同的標記背景色。請參閱[行標記指南文件](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)以獲取指引。

:::

### 預設語言 {#default-language}

您可以為未標註語言的三重反引號代碼塊（即 ```）設定預設語言。請注意，必須傳入有效的[語言名稱](https://prismjs.com/#supported-languages)。
...
可透過 `themeConfig.footer` 添加標誌和版權聲明。標誌可置於[靜態資源目錄](static-assets.mdx)，其 URL 設定方式與導航欄標誌相同。
...
可透過 `themeConfig.footer.links` 添加頁尾連結。提供兩種配置類型：**多欄式頁尾**與**簡易式頁尾**。

多欄式頁尾連結需為每欄設定 `title` 及一組 `FooterItem` 列表。
...
簡易式頁尾僅需將一組 `FooterItem` 以水平排列方式顯示。
...
可透過 `themeConfig.tableOfContents` 調整預設目錄設定。
...
此 React 鉤子用於存取色彩模式上下文。該上下文包含設定淺色/深色模式的函數，並暴露當前使用模式的布林值變量。
...
- **多實例路徑**: 不適用
- **JSON 文件**: 透過 [`docusaurus write-translations`](../../cli.mdx#docusaurus-write-translations-sitedir) 指令提取
- **Markdown 文件**: 不適用
