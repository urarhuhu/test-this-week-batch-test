...
請注意，`error` 訊息即使不會使程序掛起，也可能會引起混淆。當用戶檢查日誌並發現 `[ERROR]` 時，即使構建成功，他們也會認為出了問題。請謹慎使用。

Docusaurus 僅在拋出錯誤前打印訊息時，或當用戶將 `onBrokenLink` 等的報告嚴重性設置為 `"error"` 時使用 `logger.error`。
...
模板字面量標籤評估模板和嵌入的表達式。`interpolate` 返回一個新字符串，而其他日誌函數則打印它。以下是一個典型用法：
...
嵌入的表達式可以選擇在前面加上一個標誌，形式為 `[a-z]+=`（幾個小寫字母，後跟等號，直接位於嵌入表達式之前）。如果表達式前面沒有任何標誌，則按原樣打印。否則，它會用以下格式化器之一進行格式化：
...
如果表達式是一個數組，則由 `` `\n- ${array.join('\n- ')}\n` `` 格式化（請注意它會自動獲得一個前導行結束符）。每個成員自行格式化，項目符號不格式化。因此，您會看到上述訊息打印為：
