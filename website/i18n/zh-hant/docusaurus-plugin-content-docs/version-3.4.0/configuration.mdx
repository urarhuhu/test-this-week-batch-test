...
Docusaurus 對於配置有獨特的設計理念。我們鼓勵您將網站相關的所有信息集中到一個地方。我們會保護此文件的字段，並確保整個網站都能方便地訪問這個數據對象。

維護良好的 `docusaurus.config.js` 可以幫助您、您的協作者以及開源貢獻者專注於文檔撰寫，同時仍能自定義網站。

## 聲明 `docusaurus.config.js` 的語法 {#syntax-to-declare-docusaurus-config}

`docusaurus.config.js` 文件在 Node.js 環境中運行，應該導出以下兩種之一：
...
- **必須**使用 `export default /* 您的配置 */`（或 `module.exports`）導出 Docusaurus 配置
...
即使您正在開發網站，也不應該從頭開始編寫 `docusaurus.config.js`。所有模板都帶有一個 `docusaurus.config.js`，其中包含常見選項的默認值。

然而，了解配置的設計和實現方式對您會有幫助。

Docusaurus 配置的高層次概述可分為以下幾類：
...
這些元數據用於多個地方，例如網站的標題和標題、瀏覽器標籤圖標、社交分享（Facebook、X）信息，甚至用於生成提供靜態文件的正確路徑。
...
大多數情況下此配置即可滿足需求。若要自訂 Babel 配置（例如添加 Flow 支援），可直接編輯此檔案。變更生效需重新啟動 Docusaurus 開發伺服器。