...
Docusaurus 使用 MDX 編譯器將 `.md` 和 `.mdx` 文件都編譯為 React 組件，但**根據你的設置，語法可能會被不同解讀**。

MDX 編譯器支援 [2 種格式](https://mdxjs.com/packages/mdx/#optionsformat)：

- [MDX 格式](https://mdxjs.com/docs/what-is-mdx/)：強大的解析器，允許使用 JSX
- [CommonMark 格式](https://commonmark.org/)：標準合規的 Markdown 解析器，不允許使用 JSX
...
Markdown 是一種能讓你以可讀性高的語法撰寫格式化內容的語法。
...
前置元數據用於為你的 Markdown 文件添加元數據。所有內容插件都有自己的前置元數據結構，並使用前置元數據來豐富從內容或其他配置推斷出的預設元數據。

前置元數據位於文件最頂部，由三個破折號 `---` 包圍。內容以 [YAML](https://yaml.org/spec/1.2.2/) 格式解析。
...
使用 [Markdown 設定中的 `parseFrontMatter` 函數](../../api/docusaurus.config.js.mdx#markdown) 來提供自訂的前置資料解析器，或增強預設解析器。

你可以重複使用預設解析器，並用自有的專屬邏輯進行包裝。這讓你能實作便利的前置資料轉換、快捷方式，或是透過前置資料與 Docusaurus 外掛不支援的外部系統整合。
...
你可能會希望將 `<summary>` 保持在一行內。請注意 [MDX 會為換行建立額外的 HTML `<p>` 段落](https://mdxjs.com/migrating/v2/#jsx)。如有疑問，可使用 [MDX 遊樂場](https://mdxjs.com/playground/) 來排查 `<details>` 的渲染問題。
