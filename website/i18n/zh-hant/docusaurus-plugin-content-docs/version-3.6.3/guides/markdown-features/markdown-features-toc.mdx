...
每個標題都有一個可自動生成或手動指定的 ID。標題 ID 允許您透過 Markdown 或 JSX 連結到特定段落：
...
自動生成的標題 ID 在單一頁面中會保持唯一性，但若使用自訂 ID，請確保同一頁面中每個 ID 只出現一次，否則會導致 HTML 語意錯誤，且部分標題將無法被連結。

:::

## 目錄標題層級 {#table-of-contents-heading-level}

每份 Markdown 文件右上角會顯示目錄。預設僅顯示 h2 和 h3 標題，這通常足以呈現頁面結構。如需調整顯示範圍，可針對單一頁面或全域設定標題層級範圍。
...
`themeConfig` 選項會影響網站上所有目錄（包含[行內目錄](#inline-table-of-contents)），而 front matter 參數僅影響右上角的目錄。如需自訂每個 `<TOCInline />` 元件，需使用 `minHeadingLevel` 和 `maxHeadingLevel` 屬性。

:::

## 行內目錄 {#inline-table-of-contents}

透過 MDX 功能，您還可以在 Markdown 文件中直接嵌入行內目錄。

所有 MDX 文件都可使用 `toc` 變數，其中包含該文件的所有標題。預設僅顯示 `h2` 和 `h3` 標題。透過為個別 `TOCInline` 元件設定 `minHeadingLevel` 或 `maxHeadingLevel` 可調整顯示範圍。
...
請注意 `toc` 是扁平化陣列，因此可輕鬆過濾不需要的節點或插入額外節點，重新組合成新的目錄樹狀結構。
...
目前尚無法方便地插入額外標題或忽略特定標題。如果您需要此功能，請在 [此問題](https://github.com/facebook/docusaurus/issues/6201) 中報告您的使用案例。
