...
您可能希望同類型的標籤選擇能相互同步。例如，針對 Windows 與 macOS 使用者提供不同操作指引時，希望透過單次點擊即可切換所有作業系統專屬的指引標籤頁。為實現此功能，可為所有相關標籤頁設定相同的 `groupId` 屬性。請注意，此設定會將選擇狀態持久化至 `localStorage`，且所有具有相同 `groupId` 的 `<Tab>` 實例會在任一標籤值變更時自動更新。需注意群組 ID 為全域命名空間。
...
對於相同 `groupId` 的所有標籤群組，其可能值不需一致。若某標籤群組選擇的值在另一相同 `groupId` 的標籤群組中不存在，則該標籤群組不會切換標籤頁。以下範例可說明此行為：嘗試選擇 Linux 時，上方標籤群組不會變動。
...
可以將選取的標籤狀態持久化儲存至網址的搜尋參數中。這讓您能分享已預選特定標籤的頁面連結——例如從您的 Android 應用程式連結至已預選 Android 標籤的文件。此功能不提供錨點連結，瀏覽器不會自動滾動至該標籤位置。
...
當點擊標籤時，網址末端會立即加入搜尋參數：`?current-os=android` 或 `?current-os=ios`。
...
當頁面載入時，會優先還原網址查詢字串中的標籤選擇（而非使用 `localStorage` 儲存的 `groupId` 選擇）。
