...
如果您試用 [MDX 遊樂場](https://mdxjs.com/playground/)，會注意到 MDX 轉譯有兩個中間步驟：Markdown 抽象語法樹（MDAST）和超文本抽象語法樹（HAST），最終才輸出 JSX。MDX 插件也有兩種形式：
...
使用插件為專案中最常用的 JSX 元素引入簡潔語法。我們提供的[警示語法](./markdown-features-admonitions.mdx)也是由 Remark 插件生成，您可依此模式實作自己的用例。
...
這些都是 Remark 插件的典型用例，也可作為您實作自訂插件的參考。
...
許多官方 Remark/Rehype 插件僅支援 [**ES Modules**](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)（一種 JavaScript 模組系統），而 Docusaurus 支援此格式。建議使用 [**ES Modules**](https://flaviocopes.com/es-modules/) 設定檔，以便更輕鬆導入這類套件。
...
部分插件可接受自訂配置選項。此時，請使用 `[plugin, pluginOptions]` 語法，如下所示：
...
以下說明**並非**建立外掛程式的完整指南，僅展示如何使其在 Docusaurus 中運作。如需深入了解運作原理，請參閱 [Remark](https://github.com/remarkjs/remark/blob/main/doc/plugins.md#create-plugins) 或 [Rehype](https://github.com/rehypejs/rehype/blob/main/doc/plugins.md#create-plugins) 的官方文件。

:::

舉例來說，我們建立一個會為每個 `h2` 標題添加 `Section X. ` 前綴的外掛程式。首先，在任何位置建立外掛原始檔（您甚至可以將其發佈為獨立的 npm 套件並如前述方式安裝）。我們將外掛程式置於 `src/remark/section-prefix.js`。remark/rehype 外掛程式本質上是接收 `options` 並返回操作 AST 的 `transformer` 函式。
...
現在您可以在 `docusaurus.config.js` 中導入此外掛程式，並如同已安裝的外掛程式般使用！
...
`transformer` 函式的第二個參數 [`vfile`](https://github.com/vfile/vfile) 可用於存取當前 Markdown 檔案的路徑：
...
例如我們的 `transformImage` 外掛程式就利用此參數將相對圖片參照轉換為 `require()` 呼叫。

:::

:::note

Docusaurus 的預設外掛程式會先於自訂 remark 外掛程式執行，這意味著圖片或連結已被轉換為帶有 `require()` 呼叫的 JSX。例如在上例中，即使所有 `h2` 標題都添加了 `Section X.` 前綴，生成的目錄仍保持原樣，因為目錄生成外掛程式先於自訂外掛程式執行。若需在預設外掛程式處理前操作 MDAST，請使用 `beforeDefaultRemarkPlugins` 和 `beforeDefaultRehypePlugins`：
