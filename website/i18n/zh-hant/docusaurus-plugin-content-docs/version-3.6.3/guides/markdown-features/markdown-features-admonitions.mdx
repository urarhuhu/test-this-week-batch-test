...
若您使用 [Prettier](https://prettier.io) 格式化 Markdown 文件，Prettier 可能會將提示框語法自動轉換為無效格式。為避免此問題，請在起始與結束指令周圍保留空行。這也是為何此處所有範例都在內容周圍留有空行。
...
接受的類型與上述相同：`note`、`tip`、`danger`、`info`、`warning`。您可選擇性地透過傳遞 JSX 元素、字串或標題來指定圖標：
...
您可透過 [swizzling](../../swizzling.mdx) 自訂每種提示框類型的渲染方式。通常只需簡單的包裝組件即可達成目標。例如下方範例中，我們僅替換了 `info` 類型提示框的圖標。
...
預設情況下，主題無法處理自訂提示框關鍵字（如 `:::my-custom-admonition`）。您需自行將每個提示框關鍵字映射到 React 組件，主題才能正確渲染。
...
您可以為 `:::my-custom-admonition` 提供對應的 React 組件，方法是創建以下文件（遺憾的是，由於它不是 React 組件文件，因此無法透過 swizzle 功能生成）：
...
現在您可以在 Markdown 文件中使用新的 admonition 關鍵字，它將被解析並使用您的自定義邏輯渲染：
