...
`@docusaurus/plugin-content-docs` 插件可支援 [多實例功能](../../using-plugins.mdx#multi-instance-plugins-and-plugin-ids)。

:::note

此功能僅對 [版本化文檔](./versioning.mdx) 有用。建議在閱讀本頁前先熟悉文檔版本控制。若您僅需要 [多側邊欄](./sidebar/multiple-sidebars.mdx)，可在單一插件內實現。

:::

## 使用場景 {#use-cases}

有時您會希望一個 Docusaurus 站點托管兩套（或更多）獨立的文檔集。

這些文檔集可能還具有不同的版本控制/發布週期。

### 移動端 SDK 文檔 {#mobile-sdks-documentation}

如果您開發跨平台移動端 SDK，可能會擁有兩套文檔：
...
若每個文檔實例體量龐大，您應該建立兩個獨立的 Docusaurus 站點。

當有人編輯 iOS 文檔時，重新構建包括未變更的完整 Android 文檔在內的所有內容是否真的必要？

:::

### 版本化與非版本化文檔 {#versioned-and-unversioned-doc}

有時您會希望部分文檔進行版本控制，而其他文檔則保持「全局性」，對其進行版本控制顯得無意義。

我們在 Docusaurus 官方網站上就採用此模式：
...
您可為其中一個文檔插件實例省略 `id` 屬性（默認為 `default`）。

該實例路徑會更簡潔，並與單實例配置保持回溯兼容性。
