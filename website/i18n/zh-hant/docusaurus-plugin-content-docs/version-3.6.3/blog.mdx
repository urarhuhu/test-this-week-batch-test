...
部落格側邊欄會顯示最近的部落格文章。預設顯示的項目數量為 5，但您可以透過插件配置中的 `blogSidebarCount` 選項進行自訂。設定 `blogSidebarCount: 0` 時，側邊欄將完全停用，容器也會被移除，這將增加主容器的寬度。特別地，如果設定 `blogSidebarCount: 'ALL'`，則會顯示 _所有_ 文章。

您還可以透過 `blogSidebarTitle` 選項更改側邊欄的標題文字。例如，如果設定 `blogSidebarCount: 'ALL'`，您可以將預設的「最近文章」改為「所有文章」：
...
此命名慣例為可選項，您也可以透過 front matter 提供日期。由於 front matter 遵循支援日期時間標記法的 YAML 語法，若您需要更精確的發布時間，可使用 front matter。例如，若您有多篇同一天發布的文章，可依照當天時間排序：
...
使用 `authors` front matter 欄位來宣告部落格文章作者。作者至少需具備 `name` 或 `image_url`。Docusaurus 會使用 `url`、`email` 和 `title` 等資訊，但其他任何資訊也允許使用。
...
對於固定發布部落格文章的作者來說，在每篇文章中維護行內作者資訊可能相當繁瑣。
...
使用 `authorsMapPath` 外掛選項來設定路徑。JSON 格式也受支援。
...
無論是透過 front matter 或作者對映宣告的作者，都需具備名稱或頭像，或兩者兼具。若文章的所有作者皆無名稱，Docusaurus 會以緊湊方式顯示其頭像。效果請參見[此測試文章](/tests/blog/2022/01/20/image-only-authors)。

:::warning[訂閱源生成]

[RSS 訂閱源](#feed)需設定作者的電子郵件，該作者才會出現在訂閱源中。

:::

### 作者頁面 {#authors-pages}

作者頁面功能是選用的，主要適用於多位作者共同撰寫的部落格。
...
只有[全域作者](#global-authors)能啟用此功能。[內聯作者](#inline-authors)不支援此功能。

:::

## 部落格文章標籤 {#blog-post-tags}

標籤在前言資料中宣告，為分類提供了另一個維度。

可以內聯定義標籤，或引用預先在[`標籤檔案`](api/plugins/plugin-content-blog.mdx#tags-file)（通常為`blog/tags.yml`）中定義的標籤。

在以下範例中：

- `docusaurus`引用自`blog/tags.yml`中預定義的標籤鍵
- `Releases`是內聯標籤，因為它不存在於`blog/tags.yml`中
...
`readingTime`回調函數接收三個參數：部落格內容文字（字串形式）、前言資料（鍵值對記錄）以及預設閱讀時間函數。它返回一個數字（閱讀時間分鐘數）或`undefined`（停用該頁面的閱讀時間顯示）。

預設閱讀時間函數可接受額外選項：`wordsPerMinute`（數字，預設值：300）和`wordBound`（從字串到布林值的函數）。若傳遞給`wordBound`的字串應被視為單詞邊界（預設為空格、製表符和換行符），則該函數應返回`true`。
...
```mdx-code-block
<Tabs>
<TabItem value="disable-per-post" label="每篇文章獨立設定">
```
...
<TabItem value="using-custom-algo" label="使用自訂演算法">
```
...
透過設定 `feedOptions` 可生成 RSS/Atom/JSON 訂閱源。預設會生成 RSS 和 Atom 源，若需停用請將 `feedOptions.type` 設為 `null`。
...
可將部落格文章列表設為首頁，無需獨立落地頁。將 `routeBasePath` 設為 `'/'` 即可透過根路徑 `example.com/` 存取部落格（而非子路徑 `example.com/blog/`）。
...
務必刪除現有首頁檔案 `./src/pages/index.js`，否則會出現兩個檔案映射至相同路由！

:::

:::warning

若停用文件插件，請同步移除設定檔中所有相關參照，特別是導航欄的文件相關項目。

:::

:::tip

另有「純文件模式」供僅需文件功能的用戶，詳見[純文件模式](./guides/docs/docs-introduction.mdx)了解 `routeBasePath` 進階設定。

:::

### 多重部落格 {#multiple-blogs}

經典主題預設僅支援單一部落格，若需在同一網站建立多重部落格，可於 `docusaurus.config.js` 的 `plugins` 選項中新增部落格插件實例。

將 `routeBasePath` 設定為您希望第二個部落格被訪問的 URL 路由。請注意，此處的 `routeBasePath` 必須與第一個部落格不同，否則可能發生路徑衝突！同時，將 `path` 設定為包含第二個部落格條目的目錄路徑。

如 [多實例插件](./using-plugins.mdx#multi-instance-plugins-and-plugin-ids) 文件中所述，您需要為插件分配唯一 ID。
...
舉例來說，我們在 [此處](/tests/blog) 託管了第二個部落格。