...
本節將介紹如何在 Docusaurus 中進行佈局客製化。

> 似曾相識...？

本節與[樣式與佈局](./styling-layout.mdx)類似，但這次我們將客製化 React 元件本身，而非其外觀。我們將討論 Docusaurus 的核心概念：**元件替換(Swizzling)**，這項功能允許進行**更深層的網站客製化**。

實務上，元件替換可讓您**用自己實作的元件替換主題元件**，並提供兩種模式：

- [**彈出(Ejecting)**](#ejecting)：建立原始主題元件的**副本**，您可完全**自訂**該副本
- [**包裝(Wrapping)**](#wrapping)：建立原始主題元件的**包裝器**，您可**增強**其功能
...
該指令會在您的 `src/theme` 目錄中生成新元件，如下例所示：
...
請務必了解[哪些元件可安全替換](#what-is-safe-to-swizzle)。某些元件是主題的**內部實作細節**。

:::

:::info

`docusaurus swizzle` 僅是協助您替換元件的自動化工具。您也可以手動建立 `src/theme/SomeComponent.js` 文件，Docusaurus 會[解析它](./advanced/client.mdx#theme-aliases)。此命令背後並無特殊魔法！

:::

### 彈出 (Ejecting) {#ejecting}

彈出主題元件是指**建立原始主題元件的副本**，您可**完全自訂和覆寫**該副本。

若要彈出主題元件，請使用互動式元件替換 CLI 或搭配 `--eject` 選項：
...
此操作會將當前 `<Footer />` 元件的實作複製到您網站的 `src/theme` 目錄。Docusaurus 現在將使用此 `<Footer>` 元件副本而非原始元件。您現在可完全重新實作 `<Footer>` 元件。
...
彈出[**不安全**](#what-is-safe-to-swizzle)元件有時會導致複製大量內部程式碼，這些程式碼現在需要您自行維護。這可能使 Docusaurus 升級更加困難，因為若接收的屬性或使用的內部主題 API 發生變更，您需要遷移自訂內容。

**盡可能優先選擇[包裝](#wrapping)**：需維護的程式碼量較少。
...
封裝主題是**在現有元件周圍添加額外元件**的絕佳方式，而無需[抽出](#ejecting)它。例如，您可以輕鬆在每篇部落格文章下方加入自訂評論系統：
...
- **禁止**：混寫 CLI 會阻止您混寫此元件，因為它從設計上就不該被混寫
...
不必過於**害怕混寫不安全元件**：只需記住可能發生**破壞性變更**，且在次要版本升級時可能需要手動更新您的自訂內容。
...
要實現預期效果時，往往不清楚究竟該混寫哪個元件。提供大多數主題元件的 `@docusaurus/theme-classic` 約有 [100 個元件](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-theme-classic/src/theme)！
...
2. **元件名稱**。官方主題元件採用語義化命名，您可從名稱推斷其功能。swizzle CLI 支援輸入元件名稱片段來篩選選項。例如執行 `yarn swizzle @docusaurus/theme-classic` 後輸入 `Doc`，將只列出與文檔相關的元件。
3. **從高層級元件開始**。元件會形成樹狀結構，某些元件會導入其他元件。每條路由都會關聯一個頂層元件（多數列於[內容外掛路由](./advanced/routing.mdx#routing-in-content-plugins)）。例如所有部落格文章頁面的最頂層元件都是 `@theme/BlogPostPage`。您可以先 swizzle 此元件，再沿元件樹向下定位目標元件。找到正確元件後，請記得刪除其他已 swizzle 的元件檔案以避免維護過多元件。
...
Swizzle 意味著您需維護與 Docusaurus 內部 API 互動的 React 程式碼。在客製化網站時，請優先考慮以下替代方案：

1. **使用 CSS**。CSS 規則與選擇器通常能滿足多數客製化需求，詳見[樣式與版面](./styling-layout.mdx)。
2. **使用翻譯**。雖然看似意外，但翻譯本質上也是文字標籤的客製化方式。例如若網站預設語言為 `en`，仍可執行 `yarn write-translations -l en` 後編輯生成的 `code.json`，詳見[i18n 教學](./i18n/i18n-tutorial.mdx)。

:::tip

**範圍越小越好**。若必須 swizzle，請僅針對相關部分進行，並盡量減少自行維護的程式碼量。swizzle 小型元件通常能降低升級時的**破壞性變更**風險。

相較於[完整抽離](#ejecting)，[包裝元件](#wrapping)是更安全的選擇。

:::

## 使用 `<Root>` 包裝您的網站 {#wrapper-your-site-with-root}

`<Root>` 元件會渲染在 React 樹的最頂層（高於主題 `<Layout>`），且**永不卸載**。非常適合用來添加不該因導航而重新初始化的狀態邏輯（使用者認證狀態、購物車狀態等）。

請手動建立 `src/theme/Root.js` 檔案來 swizzle 此元件：
...
建議在此元件中渲染 React Context providers。
