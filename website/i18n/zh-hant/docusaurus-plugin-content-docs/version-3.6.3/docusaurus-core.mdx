...
此組件不會捕獲構建時錯誤，僅保護客戶端渲染時可能發生的錯誤，這些錯誤通常在使用有狀態的 React 組件時出現。

:::

#### 屬性 {#errorboundary-props}

- `fallback`: 可選的渲染回調函數，返回一個 JSX 元素。它會接收一個包含兩個屬性的對象：`error`，捕獲到的錯誤；`tryAgain`，一個用於重置組件錯誤並嘗試重新渲染的函數回調（`() => void`）。如果未提供，則會渲染 `@theme/Error` 作為替代。`@theme/Error` 用於包裹整個站點的錯誤邊界，位於佈局之上。
...
相比普通的 `<a>` 標籤，更推薦使用此組件，因為 Docusaurus 在使用 `<Link>` 時會進行許多優化（例如檢測無效路徑、預取、應用基礎 URL 等）。

:::

### `<Redirect/>` {#redirect}

渲染 `<Redirect>` 將導航到一個新位置。新位置會像服務器端重定向（HTTP 3xx）一樣覆蓋歷史堆棧中的當前位置。您可以參考 [React Router 的 Redirect 文檔](https://reacttraining.com/react-router/web/api/Redirect) 獲取更多關於可用屬性的信息。
...
`<Translate>` 元件支援插值功能。您也可以透過自訂程式碼和[命令式 `translate` API](#translate-imperative) 實作[字串複數形式](https://github.com/facebook/docusaurus/pull/i18n/i18n-tutorial.mdx#pluralization)。
...
插件 ID 僅在單一插件被多次用於同個站點時有意義。每個插件實例可建立獨立的全域資料。
...
存取特定插件建立的全局資料。給定插件名稱後，將按插件 ID 返回該名稱下所有插件實例的資料。
...
React Hook 用於存取 Docusaurus 的壞鏈結檢查 API，提供 Docusaurus 頁面回報與收集其鏈結與錨點的方式。
