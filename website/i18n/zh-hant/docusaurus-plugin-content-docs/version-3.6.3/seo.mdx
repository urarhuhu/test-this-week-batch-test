...
官方插件均支援以下[前置資料](./guides/markdown-features/markdown-features-intro.mdx#front-matter)：`title`、`description`、`keywords` 和 `image`。請參考各自的 API 文件以獲取其他[前置資料](./guides/markdown-features/markdown-features-intro.mdx#front-matter)支援：
...
Docusaurus 部落格預設支援[豐富搜尋結果](https://search.google.com/test/rich-results)，可提供最佳搜尋引擎體驗。這些資訊會根據您在部落格/全域設定中的元資訊自動生成。要獲得豐富搜尋資訊的優勢，請完整填寫文章發布日期、作者、圖片等資訊。更多關於元資訊的說明請參閱[此文件](./blog.mdx)。
...
Docusaurus 提供預設隨 `preset-classic` 內建的 [`@docusaurus/plugin-sitemap`](./api/plugins/plugin-sitemap.mdx) 外掛程式。它會自動生成 `sitemap.xml` 檔案，在正式環境建置後可透過 `https://example.com/[baseUrl]/sitemap.xml` 存取。此網站地圖元資料能幫助搜尋引擎爬蟲更精確地抓取您的網站。
