...
- **模組化**：允許插件作者提供國際化支援
- **低運行負擔**：文件主要是靜態的，不需要繁重的 JS 函式庫或 polyfills
- **可擴展的建置時間**：允許獨立建置和部署本地化網站
- **本地化資源**：網站圖片可能包含需要翻譯的文字
- **無耦合**：不強制使用任何 SaaS，但可整合
- **易於與 [Crowdin](https://crowdin.com/) 整合**：許多 Docusaurus v1 網站使用 Crowdin，應能遷移至 v2
...
部分插件會從外部資料檔案讀取內容，這些檔案需整體本地化。例如部落格插件使用的[`authors.yml`](../blog.mdx#global-authors)檔案，可通過在`i18n/[locale]/docusaurus-plugin-content-blog/`路徑下建立副本進行翻譯。
...
為簡易的Docusaurus網站新增法文翻譯後，目錄結構如下：
...
JSON檔案可透過[`docusaurus write-translations`](../cli.mdx#docusaurus-write-translations-sitedir)指令初始化。各插件在對應目錄下管理自身翻譯內容，而`code.json`則定義React程式碼中使用的文字標籤。

不同內容插件與主題**有各自的翻譯檔案存放規則**：
