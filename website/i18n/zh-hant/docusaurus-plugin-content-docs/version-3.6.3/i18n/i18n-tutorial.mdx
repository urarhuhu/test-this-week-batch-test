...
所有法文語言環境的翻譯資料都儲存在 `website/i18n/fr` 中。每個插件在相應的資料夾下提供自己的翻譯內容，而 `code.json` 檔案則定義 React 程式碼中使用的所有文字標籤。

:::note

複製檔案後，使用 `npm run start -- --locale fr` 重新啟動您的網站。編輯現有檔案時，熱重載會更順暢。
...
根據語境語義選擇更適合的用法。例如，`<Translate>` 可作為 React 子元素使用，而對於需要字符串的 props 則可使用回調函數。

:::warning

JSX 元素是_物件_而非字符串。在預期字符串的上下文中使用（例如 [`<option>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/option) 的子元素）會[強制轉換為字符串](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/toString)，導致返回 `"[object Object]"`。雖然我們鼓勵將 `<Translate>` 作為 JSX 子元素使用，但請僅在實際可行的情況下使用元素形式。
...
`docusaurus write-translations` 指令會靜態分析您網站中使用的所有 React 程式碼文件，提取這些 API 的調用，並將其匯總到 `code.json` 文件中。翻譯文件將以從 ID 到翻譯訊息物件（包含翻譯後的標籤和標籤描述）的映射形式存儲。在調用翻譯 API（`<Translate>` 或 `translate()`）時，您需要指定未翻譯的默認訊息或 ID，以便 Docusaurus 正確將每個翻譯條目與 API 調用關聯起來。

:::warning[文字標籤必須是靜態的]

`docusaurus write-translations` 指令僅對您的程式碼進行**靜態分析**，不會實際運行您的網站。因此，無法提取動態訊息，因為訊息是_表達式_而非_字符串_：
...
這在運行時仍能正常運作。但未來我們可能會提供「無運行時」機制，通過 Babel 轉換直接將翻譯內聯到 React 程式碼中，而非在運行時調用 API。因此，為確保未來相容性，應始終優先使用可靜態分析的訊息。例如，我們可以將上述程式碼重構為：
...
每種語言都有一組[可能的複數類別](https://unicode-org.github.io/cldr-staging/charts/37/supplemental/language_plural_rules.html)。Docusaurus 會按照 `["zero", "one", "two", "few", "many", "other"]` 的順序排列這些類別。例如，英語 (`en`) 有兩種複數形式（"one" 和 "other"），因此翻譯訊息會用管道符 (`|`) 分隔兩個標籤。而波蘭語 (`pl`) 有三種複數形式（"one"、"few" 和 "many"），您需要按順序提供三個用管道符連接的標籤。
...
該命令會提取並初始化需要翻譯的 JSON 翻譯文件。根目錄下的 `code.json` 包含從原始碼中提取的所有翻譯 API 調用，這些可能是您編寫的或由主題提供的，其中部分可能已預設翻譯完成。
...
預設情況下，Markdown 標題 `### Hello World` 會生成一個 ID `hello-world`。其他文件可以透過 `[連結](#hello-world)` 來連結它。然而，翻譯後標題變為 `### Bonjour le Monde`，ID 也變為 `bonjour-le-monde`。

生成的 ID 並不總是適合本地化網站，因為這要求你本地化所有的錨點連結：
...
**本地化你的 404 頁面** 的方法是設定你的託管服務將 `/fr/*` 重新導向至 `/fr/404.html`。

這並非總是可行，且取決於你的託管服務：GitHub Pages 無法做到，但 [Netlify](https://docs.netlify.com/routing/redirects/redirect-options/#custom-404-page-handling) 可以。
...
也可以將每個語言版本部署為獨立的子網域，並在 CDN 層級將這些子網域組合成一個統一的網域：
...
Docusaurus 不關心你如何管理翻譯：它只需要所有翻譯檔案（JSON、Markdown 或其他資料檔案）在建置時存在於檔案系統中。然而，作為網站創建者，你需要考慮如何管理翻譯，以便翻譯貢獻者能夠良好協作。
