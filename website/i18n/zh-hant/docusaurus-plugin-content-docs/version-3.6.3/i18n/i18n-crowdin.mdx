...
使用這個 **[社群驅動的 GitHub 討論](https://github.com/facebook/docusaurus/discussions/4052)** 來討論任何與 Docusaurus + Crowdin 相關的事宜。
...
- **上傳來源**至 Crowdin（未翻譯的文件）
- 使用 Crowdin **翻譯內容**
- **下載翻譯**自 Crowdin（本地化的翻譯文件）

Crowdin 提供 [CLI](https://support.crowdin.com/cli-tool/) 來**上傳來源**和**下載翻譯**，讓您能自動化翻譯流程。

[`crowdin.yml` 設定檔](https://support.crowdin.com/configuration-file/) 對 Docusaurus 來說很方便，並允許**將本地化的翻譯文件下載到預期的位置**（在 `i18n/[locale]/..` 中）。

閱讀 **[官方文件](https://support.crowdin.com/)** 以了解更多關於進階功能和不同的翻譯工作流程。
...
可暫時在 `crowdin.yml` 中硬編碼個人權杖：`api_token: 'MY-TOKEN'`。
...
建議先使用 `隱藏字串` 功能，因為 Crowdin 的預先翻譯有時會過度樂觀。
...
在您的 CI 環境中設定 `CROWDIN_PERSONAL_TOKEN` 環境變數。

建立一個 npm 指令來 `同步` Crowdin（提取原始檔、上傳原始檔、下載翻譯）：
...
在 CI 流程中呼叫 `npm run crowdin:sync` 指令，緊接在建置 Docusaurus 網站之前。

:::tip

保持部署預覽速度：不要下載翻譯檔案，對功能分支使用 `npm run build -- --locale en`。

:::

:::warning

Crowdin 不支援多個同時進行的上傳/下載作業：建議僅在正式部署時包含翻譯，保持部署預覽為未翻譯狀態。
...
- `website/ios`
- `website/ios_versioned_docs` (如果版本化)
...
當您的源代碼重構時，應使用 Crowdin UI **手動更新 Crowdin 文件**：
...
理論上這本可以幫助在 Git 和 Crowdin 中編輯翻譯，並實現兩個系統之間的**雙向同步**。

實際上，**它的可靠性不佳**，原因如下：

- Crowdin -> Git 同步正常（通過拉取請求）
- Git -> Crowdin 同步是手動的（需要按按鈕）
- Crowdin 用於匹配現有 Markdown 翻譯與源文件的啟發式方法並非 100% 可靠，每次從 Git 同步後都需在 Crowdin UI 上驗證結果
- 用戶同時在 Git 和 Crowdin 上編輯可能導致翻譯丟失
- 需要將 `crowdin.yml` 文件放在存儲庫根目錄
...
Crowdin 會用技術性 ID（如 `crowdin:id12345`）替換 Markdown 字符串，但替換過於激進，包括隱藏字符串，並會破壞 front matter、admonitions、JSX...

:::

### 本地化編輯網址 {#localize-edit-urls}

當用戶瀏覽 `/fr/doc1` 頁面時，預設情況下編輯按鈕會連結至未本地化的文件 `website/docs/doc1.md`。

您可能更希望編輯按鈕連結至 Crowdin 介面，可透過 `editUrl` 函數依據不同語系自訂編輯網址。
...
Crowdin 近期對 Markdown 檔案格式進行重大變更，連結的處理方式與以往不同。過去連結被視為標籤，但現在會以純文字形式呈現。由於這些變更，純文字連結會被傳送至 MT 引擎嘗試翻譯目標網址，因而導致翻譯錯誤（例如：字串 `Allez voir [ma merveilleuse page](/ma-merveilleuse-page)` 被翻譯為 `Check out [my wonderful page](/my-wonderful-page)`，這會破壞 Docusaurus 的國際化工作流程，因為頁面名稱不應被翻譯）。
