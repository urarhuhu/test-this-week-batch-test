...
Docusaurus 不會使用此 `tsconfig.json` 來編譯您的專案。其僅用於提升編輯器體驗，但您仍可選擇執行 `tsc` 來自行或於 CI 環境中進行型別檢查。
...
對於支援 TypeScript 主題元件的主題，您可在 `swizzle` 指令末端加入 `--typescript` 標記以取得 TypeScript 原始碼。例如，以下指令將於 `src/theme/Footer` 中生成 `index.tsx` 和 `styles.module.css`。

```bash npm2yarn
npm run swizzle @docusaurus/theme-classic Footer -- --typescript
```

所有官方 Docusaurus 主題均支援 TypeScript 主題元件，包含 [`theme-classic`](./api/themes/theme-classic.mdx)、[`theme-live-codeblock`](./api/themes/theme-live-codeblock.mdx) 及 [`theme-search-algolia`](./api/themes/theme-search-algolia.mdx)。若您為欲新增 TypeScript 支援的 Docusaurus 主題套件作者，請參閱[生命週期 API 文件](./api/plugin-methods/extend-infrastructure.mdx#getTypeScriptThemePath)。