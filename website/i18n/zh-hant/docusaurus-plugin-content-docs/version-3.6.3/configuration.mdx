...
- **必填：** 使用 `export default /* 你的配置 */` (或 `module.exports`) 來導出 Docusaurus 配置
- **選填：** 使用 `import Lib from 'lib'` (或 `require('lib')`) 來導入 Node.js 套件

:::

Docusaurus 讓我們能夠以多種**等效方式**宣告配置，以下所有配置範例都會產生完全相同的結果：
...
使用非同步配置建立器對於導入僅限 ESM 的模組（特別是大多數 Remark 插件）很有用。可以透過動態導入來導入此類模組：
...
它們會在多處使用，例如網站標題與標題、瀏覽器分頁圖示、社交分享（Facebook、X）資訊，甚至用於生成提供靜態檔案的正確路徑。
...
大多數情況下，預設的預設集配置都能正常運作。若要自訂 Babel 配置（例如新增對 Flow 的支援），可以直接編輯此檔案。為了讓變更生效，您需要重新啟動 Docusaurus 開發伺服器。