...
靜態資源是指直接複製到建置輸出中的非程式碼檔案，包括圖片、樣式表、網站圖示、字型等。

預設情況下，建議將這些資源放在 `static` 資料夾中。您放入該目錄的每個檔案都會被複製到生成的 `build` 資料夾的根目錄中，並保留目錄層級結構。例如，如果您將名為 `sun.jpg` 的檔案添加到靜態資料夾，它將被複製到 `build/sun.jpg`。
...
在 JSX 中，您可以使用絕對 URL 從 `static` 資料夾引用資源，但這並不是理想的做法，因為更改網站 `baseUrl` 會導致這些連結失效。對於圖片 `<img src="/img/docusaurus.png" />` 在 `https://example.com/test` 提供時，瀏覽器會嘗試從 URL 根目錄解析它，即 `https://example.com/img/docusaurus.png`，這將失敗，因為它實際上是在 `https://example.com/test/img/docusaurus.png` 提供。
...
- 預設情況下，`static` 資料夾中的檔案都不會經過後處理、雜湊或最小化。
  - 但如先前示範，我們通常能將其轉換為 `require` 調用以便進行處理。這有利於實現積極緩存並提升使用者體驗。
