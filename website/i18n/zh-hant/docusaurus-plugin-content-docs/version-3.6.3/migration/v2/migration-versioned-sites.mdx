...
由於我們採用快照方式並允許人們在版本內輕鬆移動（和編輯）文件，`id` 前置資料不再被修改並保持不變。在內部，它被設置為 `version-${version}/${id}`。
...
由於我們在 v1 和 v2 中不允許前置資料中使用 `/`，因此衝突的可能性較小。
...
在 v2 中，我們使用快照方式進行文件版本化。**每個版本化的文件不依賴於其他版本**。有可能在 `version-1.0.0` 中有 `foo.md`，但在 `version-1.2.0` 中不存在。這在之前的版本中是不可能的，因為 Docusaurus v1 有回退功能 (https://v1.docusaurus.io/docs/en/versioning#fallback-functionality)。
...
Docusaurus v1 只有在文件內容不同時才會創建版本化文件。如果你的文件從 v1.0.0 到 v1.1.0 只有 `hello.md` 被更改，你的文件結構可能看起來像這樣。
...
在 v2 中，你必須填充缺失的 `versioned_docs` 和 `versioned_sidebars`（並使用正確的前置資料和 ID 引用）。
