...
此手動遷移流程應在[自動遷移流程](./migration-automated.mdx)後執行，以補齊缺失部分或除錯遷移 CLI 輸出的問題。

## 專案設定 {#project-setup}

### `package.json` {#packagejson}

#### 套件範圍命名 {#scoped-package-names}

在 Docusaurus 2 中，我們採用範圍化套件命名：

- `docusaurus` → `@docusaurus/core`

此舉能明確區分官方套件與社群維護套件。換言之，所有官方套件皆歸類於 `@docusaurus/` 命名空間下。
...
請使用最新版 Docusaurus 2，版本號可於[此處](https://www.npmjs.com/package/@docusaurus/core)查閱（使用 `latest` 標籤）。
...
Docusaurus 1 的建置產出皆位於 `website/build/<PROJECT_NAME>`。

Docusaurus 2 則簡化為 `website/build`，請確認部署配置已更新至正確讀取建置目錄。
...
D1 專案可能已有 README 文件，可修改其內容以反映 D2 變更，或直接複製預設的 [Docusaurus v2 README](https://github.com/facebook/docusaurus/blob/main/packages/create-docusaurus/templates/shared/README.md)。
...
Docusaurus 2 將各功能（部落格、文件、頁面）拆分為模組化插件。預設集為插件組合包，為保持相容性，我們提供 `@docusaurus/preset-classic` 預設集，包含 Docusaurus 1 的核心插件。
...
我們建議將 `docs` 資料夾移至 `website` 資料夾內，這也是 v2 版本的預設目錄結構。[Vercel](https://vercel.com) 支援[開箱即用的 Docusaurus 專案部署](https://github.com/vercel/vercel/tree/main/examples/docusaurus)，前提是 `docs` 目錄需位於 `website` 內。將文檔與網站程式碼集中存放在單一 `website` 目錄下，通常能帶來更好的管理效率。

若您正在遷移 Docusaurus v1 網站，且存在待處理的文檔拉取請求(pull requests)，可暫時保留 `/docs` 資料夾於原位置，以避免產生衝突。

請參閱下方遷移指南，了解 `siteConfig.js` 中各欄位的調整方式。

### 已更新欄位 {#updated-fields}
...
已棄用。我們為 Docusaurus 2 開發了名為 [Infima](https://infima.dev/) 的自訂 CSS 框架，採用 CSS 變數實現主題化。相關文檔尚未完備，後續將更新說明。若要覆寫 Infima 的 CSS 變數，請建立自訂 CSS 檔案（如 `./src/css/custom.css`）並透過 `@docusaurus/preset-classic` 選項全域引入：
...
或使用以下工具生成網站所需的色階變數，並將變數複製到 `src/css/custom.css`：
...
網站元資訊（如資源、SEO、版權資訊）現由主題處理。請透過 `docusaurus.config.js` 中的 `themeConfig` 欄位進行客製化：
...
在 Docusaurus 1 中，標題圖示與導覽連結為 `siteConfig` 的根欄位：
...
您的 Algolia DocSearch v1 配置（存放於[此處](https://github.com/algolia/docsearch-configs/blob/master/configs)）需針對 Docusaurus v2 更新（參見[範例](https://github.com/algolia/docsearch-configs/tree/master/configs/docusaurus-2.json)）。

可聯繫 DocSearch 團隊 (@shortcuts, @s-pace) 獲取支援。他們能協助更新配置並觸發站點重新爬取以恢復搜索功能（否則需等待最多 24 小時的定期爬取週期）

:::

#### `blogSidebarCount` {#blogsidebarcount}

已棄用。請改為透過 `@docusaurus/preset-classic` 的部落格選項傳遞：
...
已棄用。請改為在 `static` 資料夾中創建一個 `CNAME` 檔案，並填入您的自訂網域。`static` 資料夾中的檔案將在執行建置命令時被複製到 `build` 資料夾的根目錄。
...
以下欄位均已棄用，您可以從配置檔案中移除它們。

- `blogSidebarTitle`
- `cleanUrl` - 現在預設使用簡潔 URL。
- `defaultVersionShown` - 版本控制功能尚未移植。如果您正在使用版本控制，將無法遷移到 Docusaurus 2。請持續關注。
- `disableHeaderTitle`
- `disableTitleTagline`
...
如果您在 v1 中設定了 `cleanUrl: false`，可能有人發布了指向 `/installation.html` 的連結。

為了 SEO 和避免連結失效，您應在託管服務提供商處配置伺服器端重定向規則。
...
若想保留 `.html` 副檔名作為頁面的標準網址，可在文件的 front matter 中宣告 `slug: installation.html`。

## 元件 {#components}

### 側邊欄 {#sidebar}

舊版本不支援巢狀側邊欄分類，且分類僅能包含文件 ID。而 v2 允許無限層級巢狀結構，並提供多種[側邊欄項目類型](../../guides/docs/sidebar/items.mdx)（不限於文件）。
...
請勿僅為在左側添加標誌而替換頁尾元件。v2 版刻意移除標誌並移至底部，請直接在 `docusaurus.config.js` 中透過 `themeConfig.footer` 配置頁尾：
...
請參閱[建立頁面](guides/creating-pages.mdx)了解 Docusaurus 2 的頁面運作機制。閱讀後請注意：需將 v1 的 `pages/en` 檔案移至 `src/pages`。
...
v2 已棄用 `CompLibrary`，需自行編寫 React 元件或使用 Infima 樣式（相關文件即將推出，敬請見諒！期間可檢查 V2 網站或參閱 https://infima.dev/ 查看可用樣式）。
...
Docusaurus 2 的 Markdown 語法已改為 [MDX](https://mdxjs.com/)，因此現有文件可能需修正部分語法。常見範例如：HTML 中有效的自閉合標籤 `<img>` 和 `<br>`，現在需明確閉合為 `<img/>` 和 `<br/>`。MDX 文件中的所有標籤都必須是有效的 JSX。
...
部落格 RSS 訂閱位址變更為 `/blog/rss.xml`（原為 `/blog/feed.xml`）。建議設定伺服器端重新導向以維持用戶訂閱有效性。
