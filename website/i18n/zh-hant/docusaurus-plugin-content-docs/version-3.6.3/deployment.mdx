...
將潛在敏感資訊放在環境中是常見的做法。然而，在典型的 Docusaurus 網站中，`docusaurus.config.js` 檔案是與 Node.js 環境的唯一介面（請參閱 [我們的架構概述](advanced/architecture.mdx)），而其他所有內容（MDX 頁面、React 組件等）都是客戶端，無法直接存取 `process` 全域變數。在這種情況下，您可以考慮使用 [`customFields`](api/docusaurus.config.js.mdx#customFields) 將環境變數傳遞到客戶端。
...
沒有一個萬全之策。您需要根據需求和資源權衡後做出選擇。

## 自行託管 {#self-hosting}

Docusaurus 可使用 [`docusaurus serve`](cli.mdx#docusaurus-serve-sitedir) 指令進行自行託管。透過 `--port` 參數變更連接埠，或使用 `--host` 變更主機位址。
...
以下章節將介紹常見託管服務商及其配置方式，以最有效率地部署 Docusaurus 網站。Docusaurus 與這些服務無商業合作關係，此處資訊僅供便利參考。部分說明由第三方提供，最新 API 變動可能未即時反映。若發現內容過時，歡迎提交 PR 修正。

由於我們僅能提供盡力而為的維護，目前已停止接受新增託管選項的 PR。您可將教學文章發表於其他平台（如個人部落格或服務商官網），並請我們加入相關連結。
...
設定站點時，請指定以下建置指令與發佈目錄：

- 建置指令：`npm run build`
- 發佈目錄：`build`

若未在初始設定中配置這些選項，仍可於站點建立後，至「Site settings」→「Build & deploy」中調整。

正確配置後，您的網站將自動部署，並在程式碼合併至部署分支（預設為 `main`）時觸發重新部署。

:::warning

部分 Docusaurus 網站將 `docs` 目錄置於 `website` 外（多為舊版 Docusaurus v1 結構）：
...
建議停用 Netlify 設定「Post Processing > Asset Optimization > Pretty Urls」，以避免網址強制小寫、非必要重新導向及 404 錯誤。

**請特別注意**：全局選項「Disable asset optimization」實際無法停用「Pretty URLs」設定，務必手動獨立取消勾選。
...
要透過 [Vercel Git 整合功能](https://vercel.com/docs/concepts/git)部署，請先將專案推送至 Git 儲存庫。

透過 [Vercel 導入流程](https://vercel.com/import/git) 將專案導入 Vercel。在導入過程中，您會發現所有相關選項都已預先配置好；不過，您仍可選擇更改其中任何 [選項](https://vercel.com/docs/build-step#build-&-development-settings)。

專案導入後，所有後續推送至分支的動作都會產生 [預覽部署](https://vercel.com/docs/platform/deployments#preview)，而對 [生產分支](https://vercel.com/docs/git-integrations#production-branch)（通常是 "main" 或 "master"）所做的任何變更，都會觸發 [生產環境部署](https://vercel.com/docs/platform/deployments#production)。
...
通常發布流程會涉及兩個儲存庫（或至少兩個分支）：包含原始檔的分支，以及包含要透過 GitHub Pages 服務的建置輸出的分支。在接下來的教學中，它們將分別被稱為「來源」和「部署」。

每個 GitHub 儲存庫都關聯著一個 GitHub Pages 服務。如果部署儲存庫名為 `my-org/my-project`（其中 `my-org` 是組織名稱或使用者名稱），部署後的網站將出現在 `https://my-org.github.io/my-project/`。若部署儲存庫名為 `my-org/my-org.github.io`（即 _組織 GitHub Pages 儲存庫_），網站則會出現在 `https://my-org.github.io/`。

:::info

若您想為 GitHub Pages 使用自訂網域，請在 `static` 目錄中建立一個 `CNAME` 檔案。`static` 目錄中的任何內容都會被複製到部署用的 `build` 目錄根目錄。使用自訂網域時，您應能將 `baseUrl: '/projectName/'` 改回 `baseUrl: '/'`，並將 `url` 設為您的自訂網域。

更多細節可參考 GitHub Pages 的文件 [使用者、組織與專案頁面](https://help.github.com/en/articles/user-organization-and-project-pages)。

:::

GitHub Pages 會從預設分支（通常是 `master` 或 `main`）或 `gh-pages` 分支，以及根目錄或 `/docs` 資料夾中擷取準備好部署的檔案（即 `docusaurus build` 的輸出）。您可透過儲存庫中的 `Settings > Pages` 進行配置。此分支將被稱為「部署分支」。

我們提供 `docusaurus deploy` 指令，協助您透過單一指令從來源分支部署至部署分支：包含克隆、建置和提交。
...
預設情況下，GitHub Pages 會透過 [Jekyll](https://jekyllrb.com/) 處理發布的檔案。由於 Jekyll 會忽略任何以 `_` 開頭的檔案，建議您在 `static` 目錄中加入一個名為 `.nojekyll` 的空檔案來停用 Jekyll。
...
1. 當有新的拉取請求提交至 `main` 分支時，會有一個動作確保網站能成功建置，但不會實際部署。此工作將稱為 `test-deploy`。
2. 當拉取請求合併至 `main` 分支或有人直接推送至 `main` 分支時，將會建置並部署至 GitHub Pages。此工作將稱為 `deploy`。

以下是兩種使用 GitHub Actions 部署文件的方法。根據您的部署儲存庫位置，選擇下方相關的標籤：

- 原始碼儲存庫與部署儲存庫為 **同一個** 儲存庫。
- 部署儲存庫為 **遠端** 儲存庫，與原始碼不同。此情境的指示假設 [發布來源](https://help.github.com/en/github/working-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site) 為 `gh-pages` 分支。
...
雖然您可以將兩個工作定義在同一個工作流程檔案中，但原始的 `deploy` 工作流程在 PR 檢查套件狀態中會始終顯示為已跳過，這無法反映實際狀態且對審查流程毫無幫助。因此，我們建議將它們作為獨立的工作流程來管理。
...
2. 預設情況下，您的公鑰應建立於 `~/.ssh/id_rsa.pub`；若使用自訂名稱，請將金鑰新增至 [GitHub 部署金鑰](https://developer.github.com/v3/guides/managing-deploy-keys/)。
3. 使用 `pbcopy < ~/.ssh/id_rsa.pub` 將金鑰複製到剪貼簿，並貼上作為部署儲存庫的 [部署金鑰](https://developer.github.com/v3/guides/managing-deploy-keys/#deploy-keys)。若命令列無法運作，請直接複製檔案內容。儲存部署金鑰前，請勾選 `允許寫入存取` 選項。
4. 您需要將私鑰設為 [GitHub 秘密](https://help.github.com/en/actions/configuring-and-managing-workflows/creating-and-storing-encrypted-secrets)，以允許 Docusaurus 執行部署。
5. 使用 `pbcopy < ~/.ssh/id_rsa` 複製私鑰，並在原始碼儲存庫建立名為 `GH_PAGES_DEPLOY` 的 GitHub 秘密。若命令列無法運作，請直接複製檔案內容並儲存秘密。
...
持續整合 (CI) 服務通常用於在原始碼控制提交新變更時執行例行任務。這些任務可包含執行單元測試、整合測試、自動化建置、發布套件至 npm，以及部署網站變更。要自動化網站部署，您只需在網站更新時呼叫 `yarn deploy` 指令碼即可。以下章節將說明如何使用熱門的持續整合服務供應商 [Travis CI](https://travis-ci.com/) 達成此目的。

1. 前往 https://github.com/settings/tokens 並建立新的 [個人存取權杖](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/)。建立權杖時，請授予 `repo` 權限範圍以確保具備所需權限。
2. 使用您的 GitHub 帳戶，[新增 Travis CI 應用程式](https://github.com/marketplace/travis-ci) 至要啟用的儲存庫。
3. 開啟 Travis CI 儀表板 (網址格式為 `https://travis-ci.com/USERNAME/REPO`)，並導覽至儲存庫的 `更多選項 > 設定 > 環境變數` 區段。
4. 建立名為 `GH_TOKEN` 的新環境變數，其值為您剛建立的權杖，接著設定 `GH_EMAIL` (您的電子郵件地址) 和 `GH_NAME` (您的 GitHub 使用者名稱)。
...
3. 選擇 GitHub 作為 git 託管供應商，並選取包含網站代碼的儲存庫。
...
[Koyeb](https://www.koyeb.com) 是一個對開發者友善的無伺服器平台，可全球部署應用程式。該平台讓您能無縫運行 Docker 容器、網路應用程式和 API，具備基於 Git 的部署、原生自動擴展、全球邊緣網路，以及內建的服務網格和服務發現功能。查看 [Koyeb 的 Docusaurus 部署指南](https://www.koyeb.com/tutorials/deploy-docusaurus-on-koyeb) 以開始使用。
...
2. 創建專案。
   ...
   - 點擊 **Next**。
3. 創建新環境。
   - 點擊 **Create environment** 並命名（例如 staging、production）。
4. 新增應用程式。
   - 點擊 **Create an application**，命名並選擇存放 Docusaurus 應用程式的 GitHub 或 GitLab 儲存庫。
   - 定義主分支名稱和應用程式根路徑。
   - 點擊 **Create**。應用程式創建後：
   - 導航至您的應用程式 **Settings**
   - 選擇 **Port**
...
就是這樣。觀察狀態並等待應用程式部署完成。要在瀏覽器中開啟應用程式，請在應用程式概覽中點擊 **Action** 和 **Open**。
...
3. 配置建置設定。
   ...
   - 如果需要，您可以修改建置指令。您可以輸入多個指令，並以 `&&` 分隔。
...
Surge 是一個[靜態網站託管平台](https://surge.sh/help/getting-started-with-surge)，您可以用來在幾秒內從命令行部署您的 Docusaurus 專案。將您的專案部署至 Surge 既簡單又免費（包括自訂網域和 SSL 憑證）。
...
1. 首先，使用 npm 安裝 Surge，執行以下指令：
   ...
   npm install -g surge
   ```
2. 在您的專案根目錄中，為生產環境建置靜態檔案，執行：
   ```bash npm2yarn
   npm run build
   ```
3. 然後，在您的專案根目錄中執行以下指令：
   ```bash
   surge build/
   ```
...
確認您要發佈的網站位於 `build` 目錄中。系統會自動生成一個隨機子網域 `*.surge.sh`（可編輯）。
...
未來您可以透過指令 `surge` 部署任何其他變更。

## 部署至 Stormkit {#deploying-to-stormkit}

您可以將您的 Docusaurus 專案部署至 [Stormkit](https://www.stormkit.io)，這是一個用於靜態網站、單頁應用程式（SPA）和無伺服器功能的部署平台。詳細說明請參閱此[指南](https://www.stormkit.io/blog/how-to-deploy-docusarous)。
...
[Cloudflare Pages](https://pages.cloudflare.com/) 是一個 Jamstack 平台，供前端開發者協作和部署網站。只需幾分鐘即可開始，請遵循[這篇文章](https://dev.to/apidev234/deploying-docusaurus-to-cloudflare-pages-565g)。

## 部署至 Azure Static Web Apps {#deploying-to-azure-static-web-apps}

[Azure Static Web Apps](https://docs.microsoft.com/en-us/azure/static-web-apps/overview) 是一項服務，可直接從代碼存儲庫自動構建和部署全端 Web 應用程序到 Azure，簡化了 CI/CD 的開發者體驗。Static Web Apps 將 Web 應用程序的靜態資源與其動態（API）端點分開。靜態資源從全球分佈的內容服務器提供，使客戶端能夠使用附近的服務器更快地檢索文件。動態 API 使用基於無服務器架構的事件驅動函數方法進行擴展，這種方法更具成本效益並按需擴展。只需幾分鐘即可開始，請遵循[這份逐步指南](https://dev.to/azure/11-share-content-with-docusaurus-azure-static-web-apps-30hc)。
