...
本插件會讀取 [`siteConfig.onDuplicateRoutes`](../docusaurus.config.js.mdx#onDuplicateRoutes) 配置項，據此調整當多個文件被生成至相同路徑時的日誌級別。
...
- "來源"指您希望創建的路徑（即待寫入的額外HTML文件）
- "目標"指您希望重定向至的路徑（通常是Docusaurus已知的路由）

因此可以為同個"目標"配置多個"來源"（生成多個重定向至相同目標的HTML文件），但每個"來源"只能對應一個"目標"（生成的HTML文件必須有確定的重定向目標）。
