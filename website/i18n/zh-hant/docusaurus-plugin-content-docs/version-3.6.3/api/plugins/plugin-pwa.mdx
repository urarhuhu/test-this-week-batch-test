...
Docusaurus 插件，使用 [Workbox](https://developers.google.com/web/tools/workbox) 添加 PWA 支援。此插件僅在生產環境建置時生成 [Service Worker](https://developers.google.com/web/fundamentals/primers/service-workers)，讓您能建立具備離線與安裝功能的完整 PWA 相容文件網站。
...
![應用安裝流程畫面錄製。瀏覽器網址列出現按鈕，點擊後顯示「安裝此應用程式？」對話框。點擊「安裝」按鈕後，作業系統會開啟新應用程式並顯示 Docusaurus 首頁。](/img/pwa_install.gif)

:::note

應用安裝需使用 HTTPS 通訊協定與有效清單文件。
...
> Service worker 的特性之一是在安裝時能將一組文件保存至快取。這通常稱為「預緩存」，因為您是在 service worker 被使用前就進行快取。
>
> 主要目的是讓開發者能控制快取，決定文件何時被快取、快取多久，以及不透過網路直接提供給瀏覽器，這可用來建立可離線運作的網路應用。
>
> Workbox 透過簡化 API 並確保資源有效下載，大幅減輕了預緩存的繁瑣工作。

預設情況下，當網站以應用程式形式安裝時會啟用離線模式。詳見 `offlineModeActivationStrategies` 選項說明。

網站完成預緩存後，service worker 會為後續訪問提供快取回應。當新版本部署並伴隨新 service worker 時，新版本會開始安裝並最終進入等待狀態。在此等待狀態期間，會顯示重新載入彈窗，要求使用者刷新頁面以取得新內容。在使用者清除應用快取或點擊彈窗中的 `reload` 按鈕前，service worker 會持續提供舊內容。
...
包含 `tagName` 和屬性鍵值對的物件陣列，用於注入到 `<head>` 標籤。技術上您可透過此方式注入任何 head 標籤，但理想情況下用於使網站符合 PWA 規範的標籤。以下是使應用完全合規的標籤列表：
...
適用於額外的 Workbox 規則。您可在此執行任何 service worker 能實現的功能，並充分利用 workbox 函式庫。程式碼會被轉譯，因此可使用現代 ES6+ 語法。
...
在 Docusaurus 應用前添加條目，使註冊能在應用運行前完成。預設的 `registerSW.js` 檔案足以應付簡單註冊需求。
...
當新服務工作者等待安裝時，會渲染 `@theme/PwaReloadPopup` 元件並向使用者建議重新載入。您可以[替換](../../swizzling.mdx)此元件並實作自己的介面。該元件會接收 `onReload` 回調函式作為 props，應在使用者點擊「重新載入」按鈕時呼叫此函式，這將指示服務工作者安裝等待中的服務工作者並重新載入頁面。

預設主題包含重新載入彈出視窗的實作，並使用 [Infima 警示框](https://infima.dev/docs/components/alert)。

![重新載入流程的螢幕錄影畫面。視窗右下角顯示警示框，內容為「有新內容可用」。點擊「重新整理」按鈕後，頁面主標題從「介紹」變更為「PWA :))」。](/img/pwa_reload.gif)
