...
:::warning[部分功能僅在生產環境生效]

[訂閱功能](../../blog.mdx#feed) 透過解析建置輸出運作，且 **僅在生產環境生效**。
...
允許透過 [XSLT](https://developer.mozilla.org/en-US/docs/Web/XSLT) 樣式化部落格 XML 訂閱源，使瀏覽器能美觀渲染。

- 使用 `true` 讓部落格使用內建的 `.xsl` 和 `.css` 檔案來樣式化訂閱源
- 使用假值（`undefined | null | false`）停用此功能
- 使用 `string` 提供相對於部落格內容目錄的自訂 `.xsl` 檔案路徑。依慣例，需提供同名 `.css` 檔案。
...
Markdown 文件可使用以下 Markdown [前置元數據](../../guides/markdown-features/markdown-features-intro.mdx#front-matter) 欄位，需以 `---` 分行包圍。
...
此檔案可包含預定義的 [全域部落格作者](../../blog.mdx#global-authors) 清單。可透過 Markdown 檔案中的 [`authors` 前置元數據](#markdown-front-matter) 以作者鍵值參照這些作者。

### 類型 {#authors-file-types}

提供的作者檔案 YAML 內容應符合以下結構：
