...
當`respectPrefersColorScheme: true`時，`defaultMode`會被使用者系統偏好設定覆蓋。
...
有時您需要在網站發布公告。為此您可添加公告欄——這是位於導覽列上方非固定且可關閉的面板。所有配置皆位於`announcementBar`物件中。
...
標誌可置於[靜態資料夾](static-assets.mdx)中。標誌URL預設設為您網站的基礎URL。雖然您可以指定自己的標誌URL，但若為外部連結將會在新分頁開啟。此外，您可覆寫標誌連結的target屬性值，這在您將文檔網站託管於主網站子目錄時特別有用——此情況下您可能不需要標誌連結在主網站開啟新分頁。

為加強深色模式支援，您亦可為此模式設定不同標誌。
...
您可以透過 `themeConfig.navbar.items` 添加項目至導航欄。
...
根據 `type` 欄位的不同，項目可具備不同行為。以下章節將介紹所有可用的導航欄項目類型。

#### 導航欄連結 {#navbar-link}

預設情況下，導航欄項目為常規連結（內部或外部）。

React Router 應會自動為連結套用活動連結樣式，但在邊緣情況下可使用 `activeBasePath`。若需讓連結在多個路徑下保持活動狀態（例如同一側邊欄下有多個文件夾），可使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更靈活的替代方案，且優先級高於後者——Docusaurus 會將其解析為正則表達式並與當前 URL 進行匹配。
...
類型為 `dropdown` 的導航欄項目具備額外的 `items` 字段，即導航欄項目的內部陣列。
...
請注意，下拉選單基底項目本身也是可點擊的連結，因此該項目可接受[普通導航欄連結](#navbar-link)的任何屬性。
...
若需連結至特定文件，此特殊導航欄項目類型會渲染指向 `docId` 所提供文件的連結。只要您瀏覽同一側邊欄的文件，該連結就會獲得 `navbar__link--active` 類別。
...
若您使用具版本控制的文檔，此特殊導航欄項目類型將渲染一個下拉選單，顯示您站點所有可用的版本。

用戶可在不同版本間切換，同時停留在同一文檔上（只要文檔ID在各版本間保持不變）。
...
若您使用具版本控制的文檔，此特殊導航欄項目類型將鏈接到當前瀏覽的文檔版本（取決於當前URL），並回退至最新版本。
...
若您使用[國際化功能](../../i18n/i18n-introduction.mdx)，此特殊導航欄項目類型將渲染一個下拉選單，顯示您站點所有可用的語言。

用戶可在不同語言間切換，同時停留在同一頁面。
...
若您使用[搜索功能](../../search.mdx)，搜索欄默認會出現在導航欄的最右側。

但通過此特殊導航欄項目類型，您可以更改其默認位置。
...
#### 自定義HTML的導航欄項目 {#navbar-with-custom-html}

您也可以通過此導航欄項目類型，在導航欄項目中渲染自定義的HTML標記。
...
### 自動隱藏粘性導航欄 {#auto-hide-sticky-navbar}

您可以啟用此酷炫的UI功能：當用戶向下滾動頁面時自動隱藏導航欄，向上滾動時重新顯示。
...
您可以在不影響主題切換功能的前提下，設置靜態的導航欄樣式。所選樣式將始終應用，無論用戶選擇何種主題。

目前有兩種樣式選項：`dark`（暗色）和`primary`（基於`--ifm-color-primary`顏色）。您可以在[Infima文檔](https://infima.dev/docs/components/navbar/)中查看樣式預覽。
...
Docusaurus使用[Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer)來高亮代碼塊。所有配置均在`prism`對象中設置。
...
如果您使用行高亮標記語法，可能需要為深色模式語法高亮主題指定不同的高亮背景色。請參考[文件指引](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)。

:::

### 預設語言 {#default-language}

您可以為程式碼區塊設定預設語言，當開頭三個反引號（即```）後未指定語言時使用。請注意必須傳入有效的[語言名稱](https://prismjs.com/#supported-languages)。
...
您可以透過`themeConfig.footer`新增標誌和版權資訊。標誌可放置於[靜態資料夾](static-assets.mdx)中，其URL運作方式與導覽列標誌相同。
...
您可以透過`themeConfig.footer.links`新增頁尾連結。頁尾配置有兩種型態：**多欄式頁尾**和**簡易頁尾**。

多欄式頁尾連結每欄包含一個`title`和一組`FooterItem`清單。
...
用於存取色彩模式的React鉤子。此上下文包含設定淺色與深色模式的函式，並公開當前使用模式的布林變數。
