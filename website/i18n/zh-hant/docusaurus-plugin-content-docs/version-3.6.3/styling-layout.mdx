...
根據您的偏好和您試圖構建的網站類型，有幾種可行的方法/框架。對於行為更像網路應用程式且具有高度互動性的網站，將樣式與元件放在一起的現代樣式設定方法會更有優勢。當您希望自定義或替換元件時，元件樣式設定也特別有用。

## 全域樣式 {#global-styles}

這是最傳統的樣式設定方法，大多數開發人員（包括非前端開發人員）都熟悉這種方法。對於沒有太多自定義的小型網站來說，這種方法效果很好。
...
您在該文件中編寫的任何 CSS 都將在全域範圍內可用，並可以直接使用字符串文字引用。
...
如果要為任何元素添加 CSS，您可以在瀏覽器中打開開發者工具來檢查其類名。類名有以下幾種：

- **主題類名**。這些類名在[下一小節](#theme-class-names)中有詳盡列出。它們沒有任何默認屬性。在您的自定義 CSS 中，應始終優先針對這些穩定的類名。
...
我們提供了一些穩定的 CSS 類名，用於強大且可維護的全域佈局樣式設定。這些名稱與主題無關，旨在供自定義 CSS 使用。
...
當您使用 `create-docusaurus` 搭建 Docusaurus 項目時，網站將生成基本的 Infima 樣式表和默認樣式。您可以全域覆蓋 Infima CSS 變量。
...
在淺色模式下，`<html>` 元素會帶有 `data-theme="light"` 屬性；在深色模式下則為 `data-theme="dark"`。因此，您可以透過針對特定屬性的 `html` 元素來限定 CSS 僅在深色模式下生效。
...
若要使用 [CSS 模組](https://github.com/css-modules/css-modules)來為元件設定樣式，請將樣式表檔案命名為 `.module.css` 後綴（例如 `welcome.module.css`）。Webpack 會將此類 CSS 檔案視為 CSS 模組載入，您必須將類別名稱作為導入的 CSS 模組屬性來引用（而非使用純字串）。這與 [Create React App](https://facebook.github.io/create-react-app/docs/adding-a-css-modules-stylesheet) 中使用的慣例類似。
...
類別名稱會在建置時由 webpack 處理為全域唯一的類別名稱。
