...
使用 [Webpack Dev Server](https://webpack.js.org/configuration/dev-server) 在本機建置並預覽您的網站。
...
為實現 CSS 套件的進階壓縮，我們使用 [cssnano 進階預設](https://github.com/cssnano/cssnano/tree/master/packages/cssnano-preset-advanced)（搭配多個 PostCSS 外掛）與 [clean-css 第 2 級優化](https://github.com/jakubpawlowicz/clean-css#level-2-optimizations)。若此進階 CSS 壓縮導致樣式異常，可透過設定環境變數 `USE_SIMPLE_CSS_MINIFIER=true` 改用 [cssnano 預設配置](https://github.com/cssnano/cssnano/tree/master/packages/cssnano-preset-default) 壓縮 CSS。**若遭遇 CSS 壓縮錯誤，請[提交問題報告](https://github.com/facebook/docusaurus/issues/new?labels=bug%2C+needs+triage&template=bug.md)。**
...
為網站中的 Markdown 文件添加 [明確的標題 ID](./guides/markdown-features/markdown-features-toc.mdx#heading-ids)。
