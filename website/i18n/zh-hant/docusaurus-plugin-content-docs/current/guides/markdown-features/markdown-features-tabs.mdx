...
預設會顯示第一個標籤頁。若要覆寫此行為，可在任一標籤項目添加 `default` 屬性，或為 `Tabs` 元件的 `defaultValue` 屬性指定您選擇的標籤值。例如在上方範例中，無論是為 `value="apple"` 的標籤設定 `default`，或是為標籤頁設定 `defaultValue="apple"`，都會強制預設開啟「Apple」標籤頁。

若為 `Tabs` 提供的 `defaultValue` 指向不存在的值，Docusaurus 會拋出錯誤。若您希望預設不顯示任何標籤頁，請使用 `defaultValue={null}`。

## 同步標籤選擇 {#syncing-tab-choices}

您可能希望同類型的標籤選擇能彼此同步。例如，您可能想為 Windows 和 macOS 使用者提供不同操作說明，並希望透過單擊切換所有作業系統專屬的說明標籤頁。為實現此功能，可為所有相關標籤頁設定相同的 `groupId` 屬性。請注意，此操作會將選擇持久化儲存在 `localStorage` 中，且所有相同 `groupId` 的 `<Tab>` 實例會在任一標籤值變更時自動更新。需注意群組 ID 是全域命名空間的。
...
對於所有具有相同 `groupId` 的標籤群組，其可能值不需相同。若某標籤群組選擇的值在另一相同 `groupId` 的標籤群組中不存在，則該標籤群組不會切換標籤頁。您可從以下範例觀察此行為：嘗試選擇 Linux 後，上方標籤群組不會改變。
...
可以將選取的標籤狀態持久化到 URL 的搜尋參數中。這讓您能分享已預選標籤的頁面連結——例如從 Android 應用程式連結到已預選 Android 標籤的文件。此功能不提供錨點連結，瀏覽器不會自動滾動至該標籤。
...
當標籤被點選時，URL 末端會加入搜尋參數：`?current-os=android` 或 `?current-os=ios`。
...
當頁面載入時，會優先從標籤的查詢字串恢復選擇狀態（其次才從 `groupId` 的 `localStorage` 記錄恢復）。
