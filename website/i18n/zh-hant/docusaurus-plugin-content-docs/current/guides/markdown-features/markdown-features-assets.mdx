...
同樣地，您可以通過 `require` 引用現有資源，並將返回的 URL 用於 `video`、`a` 錨點連結等。
...
若使用 Markdown 圖片或連結語法，所有資源路徑都會被 Docusaurus 解析為檔案路徑並自動轉換為 `require()` 調用。除非使用 JSX 語法（此時需自行處理），否則無需在 Markdown 中使用 `require()`。
...
要使用路徑片段（GitHub 採用 `#gh-dark-mode-only` 和 `#gh-light-mode-only`）切換圖片可見性，請將以下內容加入自訂 CSS（若不想與 GitHub 耦合，也可使用自訂後綴）：
...
若 Markdown 連結或圖片使用絕對路徑，該路徑將被視為檔案路徑，並從靜態目錄解析。例如，若您已將[靜態目錄](../../static-assets.mdx)配置為 `['public', 'static']`，則對於以下圖片：

```md title="my-doc.md"
![An image from the static](/img/docusaurus.png)
```

Docusaurus 會嘗試同時在 `static/img/docusaurus.png` 和 `public/img/docusaurus.png` 路徑下尋找該檔案。該連結最終會被轉換為 `require()` 呼叫而非保留為 URL 格式，此設計具有兩項優勢：

1. 您無需考慮基礎 URL 問題，Docusaurus 在提供資源時會自動處理；
