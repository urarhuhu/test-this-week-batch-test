...
Docusaurus 插件，使用 [Workbox](https://developers.google.com/web/tools/workbox) 添加 PWA 支援。此插件僅在生產環境構建時生成 [Service Worker](https://developers.google.com/web/fundamentals/primers/service-workers)，讓您能創建具備離線與安裝功能的完整 PWA 合規文檔網站。
...
![應用安裝流程畫面錄製。瀏覽器網址列出現安裝按鈕，點擊後顯示「安裝此應用程式？」對話框。點擊「安裝」按鈕後，作業系統會開啟新應用程式窗口並載入 Docusaurus 首頁。](/img/pwa_install.gif)
...
我們透過 service-worker 預緩存機制，讓用戶能在離線狀態瀏覽 Docusaurus 網站。

[workbox-precaching](https://developers.google.com/web/tools/workbox/modules/workbox-precaching) 頁面解釋其原理：

> Service worker 的特性之一是在安裝時能將文件組保存至緩存。此過程常稱為「預緩存」，因為您是在 service worker 啟用前預先緩存內容。
>
...
網站完成預緩存後，service worker 會為後續訪問提供緩存響應。當新版本部署並伴隨新 service worker 時，新 worker 會開始安裝並最終進入等待狀態。在此等待狀態期間，將顯示重新載入彈窗提示用戶更新內容。在用戶清除應用緩存或點擊彈窗的 `reload` 按鈕前，service worker 會持續提供舊內容。
...
包含 `tagName` 和屬性鍵值對的物件陣列，用於注入 `<head>` 標籤。技術上您可透過此方式注入任何 head 標籤，但理想情況下用於使網站符合 PWA 規範的標籤。以下是使應用完全合規的標籤清單：
...
適用於額外的 Workbox 規則。您可在此執行任何 service worker 能做的操作，並充分利用 workbox 函式庫的功能。程式碼會經過轉譯，因此可使用現代 ES6+ 語法。
...
在 Docusaurus 應用前添加條目，以便註冊可在應用運行前完成。預設的 `registerSW.js` 檔案足以進行簡單註冊。
...
當新的 service worker 等待安裝時，會渲染 `@theme/PwaReloadPopup` 元件並向使用者建議重新載入。您可以[替換](../../swizzling.mdx)此元件並實作自己的 UI。該元件會接收 `onReload` 回調函式作為 props，應在使用者點擊「重新載入」按鈕時呼叫此函式。這將指示 service worker 安裝等待中的 service worker 並重新載入頁面。

預設主題包含重新載入彈出視窗的實作，並使用 [Infima 警示元件](https://infima.dev/docs/components/alert)。

![重新載入流程的螢幕錄影。視窗右下角顯示警示框，內容為「有新內容可用」。點擊「重新整理」按鈕後，頁面主標題從「簡介」變更為「PWA :))」。](/img/pwa_reload.gif)
