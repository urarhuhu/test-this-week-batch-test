...
[訂閱功能](../../blog.mdx#feed) 透過解析建置輸出運作，且**僅在正式環境生效**。
...
若使用 `@docusaurus/preset-classic` 預設套件，無需額外安裝此外掛。
...
允許透過 [XSLT](https://developer.mozilla.org/en-US/docs/Web/XSLT) 美化部落格 XML 訂閱源在瀏覽器的呈現樣式。

- 設為 `true` 使用外掛內建的 `.xsl` 和 `.css` 檔案樣式化訂閱源
- 設為假值 (`undefined | null | false`) 停用此功能
- 設為 `string` 可指定部落格內容目錄下的自訂 `.xsl` 檔案路徑。依慣例需提供同名 `.css` 檔案。
...
Markdown 文件可使用以下 [前言資料](../../guides/markdown-features/markdown-features-intro.mdx#front-matter) 元資料欄位，需以 `---` 分隔線包覆。
...
依慣例，外掛會在部落格內容目錄根層級尋找 `authors.yml` 檔案。

此檔案可包含預定義的 [全域部落格作者](../../blog.mdx#global-authors) 清單。可透過 Markdown 文件中的 [`authors` 前言資料](#markdown-front-matter) 以作者鍵值進行參照。
...
- **JSON 檔案**: 透過 [`docusaurus write-translations`](../../cli.mdx#docusaurus-write-translations-sitedir) 命令提取
