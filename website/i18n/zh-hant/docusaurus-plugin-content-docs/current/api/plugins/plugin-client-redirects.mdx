...
此插件會為您的靜態站點寫入額外的 HTML 頁面，這些頁面會通過 JavaScript 將用戶重定向至現有的 Docusaurus 頁面。

:::warning[僅在生產環境生效]

此插件在開發環境始終處於非活躍狀態，**僅在生產環境生效**，因為其運作依賴於構建輸出。
...
本插件會同時讀取 [`siteConfig.onDuplicateRoutes`](../docusaurus.config.js.mdx#onDuplicateRoutes) 配置項，以調整當多個文件被生成至相同位置時的日誌級別。
...
"來源"與"目標"是本插件的核心概念。"來源"指您希望創建的路徑（即待寫入的額外HTML文件）；"目標"指您希望重定向至的路徑（通常是Docusaurus已識別的路由）。

因此同一個"目標"可對應多個"來源"（我們會創建多個指向相同目的地的HTML文件），但單個"來源"不能對應多個"目標"（生成的HTML文件必須有確定的重定向目的地）。
