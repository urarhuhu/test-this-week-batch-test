...
此檔案**在 Node.js 環境中執行**，應導出一個網站配置物件，或是一個創建該物件的函式。
...
您網站的基礎 URL。可視為主機名後的路徑。例如，`/metro/` 是 https://facebook.github.io/metro/ 的基礎 URL。對於沒有路徑的 URL，應將 `baseUrl` 設為 `/`。此欄位與 [`url`](#url) 欄位相關。始終包含前導和尾隨斜線。
...
您網站的 favicon 路徑；必須是一個可用於連結 `href` 的 URL。例如，若您的 favicon 位於 `static/img/favicon.ico`：
...
請參閱[部署指南](../deployment.mdx)與[slorber/trailing-slash-guide](https://github.com/slorber/trailing-slash-guide)來選擇合適的設定。
...
- `path`：所有語言環境資料夾的根目錄。可為絕對路徑或相對於配置檔案的路徑。預設為`i18n`
- `localeConfigs`：各語言環境的獨立配置
  - `label`：在語言切換下拉選單中顯示的標籤
  - `direction`：`ltr`（預設）或`rtl`（適用於[從右至左書寫的語言](https://developer.mozilla.org/en-US/docs/Glossary/rtl)如波斯語、阿拉伯語、希伯來語等）。用於設定該語言的CSS與HTML元屬性
  - `htmlLang`：用於`<html lang="...">`（或其他DOM標籤）和`<link ... hreflang="...">`的[BCP 47語言標籤](https://developer.mozilla.org/en-US/docs/Glossary/rtl)
  - `calendar`：計算日期紀元的[曆法](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/Locale/calendar)。注意這不控制實際顯示的日期格式：`MM/DD/YYYY`和`DD/MM/YYYY`都屬於`gregory`曆法。要選擇格式需設定語言環境名稱為`en-GB`（英式）或`en-US`（美式）（`en`預設為`en-US`）
  - `path`：該語言環境所有插件本地化資料夾的根目錄。會基於`i18n.path`解析。預設為語言環境名稱。注意：這不影響語言環境的`baseUrl`——基礎網址客製化功能仍在開發中

### `future` {#future}

- 類型：`Object`

`future`配置物件允許啟用尚未穩定/實驗性的Docusaurus功能，這些功能尚未準備好正式發布。

此配置也可用於提前啟用下個主要版本的破壞性變更，讓您能在當前版本中為升級做準備。[Remix未來標誌部落格文章](https://remix.run/blog/future-flags)對此概念有詳細說明。

:::danger[次要版本中的破壞性變更]

標記為`experimental_`或`unstable_`前綴的功能可能在**次要版本**中變更，不屬於[語意化版本](https://semver.org/)的破壞性變更範疇。

以`v<主版本號>`（如`v6`、`v7`等）命名空間的功能是未來標誌，預計在下個主版本中默認啟用。這些功能較少變動，但我們保留修改權利。

`future`API的破壞性變更應易於處理，並會在次要/主要版本發布公告中說明。
...
- `v4`: 允許預先啟用即將到來的 Docusaurus v4 重大變更與功能，為新版本提前做好準備。使用 `true` 作為快捷方式啟用所有標記。
  ...
  - [`useCssCascadeLayers`](https://github.com/facebook/docusaurus/pull/11142): 啟用 [Docusaurus CSS 層疊層級插件](./plugins/plugin-css-cascade-layers.mdx)，並預先配置我們計劃在 Docusaurus v4 中預設應用的層級。
- `experimental_faster`: 包含功能標記的物件，用於加速 Docusaurus 建置流程。需將 `@docusaurus/faster` 套件加入專案依賴。使用 `true` 作為快捷方式啟用所有標記。詳見 [Docusaurus 加速議題](https://github.com/facebook/docusaurus/issues/10556)。可用功能標記：
...
- `experimental_storage`: 全站瀏覽器儲存選項，主題開發者應盡量遵循。
  ...
  - `namespace`: 是否為瀏覽器儲存鍵添加命名空間，避免當多個 Docusaurus 網站托管於相同網域或 localhost 時發生鍵名衝突。可選值為 `string | boolean`。命名空間會附加在儲存鍵末尾形成 `key-namespace`。使用 `true` 會根據網站 `url + baseUrl` 自動生成隨機命名空間。預設為 `false`（無命名空間，歷史行為）。
- `experimental_router`: 使用的路由類型。可選值為 `browser` 和 `hash`。預設為 `browser`。`hash` 路由僅適用於特殊情況，例如需完全退出靜態網站生成、建立純客戶端應用且僅有單一 `index.html` 入口檔案。這對於將 Docusaurus 網站打包為 [無需網頁伺服器即可本地瀏覽的 .zip 壓縮檔](https://github.com/facebook/docusaurus/issues/3825) 很有用。
...
用於自訂網站 UI（如導覽列與頁尾）的[主題設定物件](./themes/theme-configuration.mdx)。
...
參閱[外掛方法參考文件](./plugin-methods/README.mdx)了解 `PluginModule` 的結構定義。
...
將插入 HTML `<head>` 的標籤陣列。值必須為包含兩個屬性的物件：`tagName` 與 `attributes`。`tagName` 為決定標籤類型的字串（例如 `"link"`），`attributes` 為屬性-值的對應物件。
...
需載入的腳本陣列。值可為字串或屬性-值對應的純物件。`<script>` 標籤將插入 HTML `<head>`。若使用純物件，唯一必要屬性為 `src`，其他屬性皆可選用（值可為布林值/字串）。
...
一個用於載入 CSS 來源的陣列。值可以是字串或屬性-值映射的純物件。`<link>` 標籤會被插入到 HTML `<head>` 中。若使用物件，唯一必需的屬性是 `href`，其他屬性皆可允許（每個屬性應為布林值/字串值）。
...
一個使用 [Eta 語法](https://eta.js.org/docs/syntax#syntax-overview)編寫的 HTML 模板，將用於渲染您的應用程式。這可用於在 `body` 標籤上設定自訂屬性、添加額外的 `meta` 標籤、自訂 `viewport` 等。請注意，Docusaurus 依賴模板的正確結構才能正常運作，一旦您進行自訂，必須確保您的模板符合上游要求。
...
啟用時，當網站無法載入其 CSS 或 JavaScript 檔案時會顯示橫幅，這是一個非常常見的問題，通常與網站配置中的錯誤 `baseUrl` 有關。
...
如果您有嚴格的[內容安全政策](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP)，應將其停用。
