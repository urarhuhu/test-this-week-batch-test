...
- 一些有用的顏色。
  ...
  - `bold`
  - `dim`
- 格式化器。這些函數都具有簽名 `(msg: unknown) => string`。請注意，它們的實現不保證穩定。您只需關心它們的語義。
  - `path`：格式化文件路徑。
  - `url`：格式化 URL。
  - `name`：格式化標識符。
  - `code`：格式化代碼片段。
...
請注意，`error` 訊息即使不會導致程序掛起，也可能引起混淆。當用戶檢查日誌並發現 `[ERROR]` 時，即使構建成功，他們也會認為出現了問題。請謹慎使用。

Docusaurus 僅在打印即將拋出錯誤的訊息時，或當用戶將 `onBrokenLink` 等的報告嚴重性設置為 `"error"` 時，才使用 `logger.error`。

此外，`warn` 和 `error` 會為**整個**訊息著色以引起注意。如果您要打印關於錯誤的大段幫助文本，最好使用 `logger.info`。

:::

### 使用模板字面量標籤

模板字面量標籤會評估模板和嵌入的表達式。`interpolate` 返回一個新字符串，而其他日誌函數則打印它。以下是一個典型用法：
