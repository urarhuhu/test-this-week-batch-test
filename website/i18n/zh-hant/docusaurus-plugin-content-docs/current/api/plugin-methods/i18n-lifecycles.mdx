...
- `path`：相對於插件本地化資料夾 `i18n/[locale]/[pluginName]` 的路徑。應省略副檔名 `.json` 以保持通用性。
- `content`：使用 Chrome i18n JSON 格式。

這些檔案將由 [`write-translations` CLI](../../cli.mdx#docusaurus-write-translations-sitedir) 寫入插件的 i18n 子資料夾，並會在呼叫 [`translateContent()`](#translateContent) 和 [`translateThemeConfig()`](#translateThemeConfig) 之前，以適當的語言環境讀取。
