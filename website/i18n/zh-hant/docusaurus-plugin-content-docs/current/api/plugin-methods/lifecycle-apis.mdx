...
`loadContent` 載入的資料將在 `contentLoaded` 中被消耗。可渲染至路由、註冊為全域資料等。
...
宣告式回呼函式，用於建立靜態資料（通常為 JSON 或字串），後續可作為 props 提供給路由。接收檔案名稱與要儲存的資料，回傳實際資料檔案路徑。

例如，下方插件建立了一個顯示「你的朋友是：Yangshun、Sebastien」的 `/friends` 頁面：
...
修改內部 webpack 配置。若返回值為 JavaScript 物件，將使用 [`webpack-merge`](https://github.com/survivejs/webpack-merge) 合併至最終配置；若為函式，則會呼叫該函式並接收 `config` 作為第一參數、`isServer` 標誌作為第二參數。

:::warning

`configureWebpack` 的 API 未來將調整為接收物件形式 (`configureWebpack({config, isServer, utils, content})`)

:::

### `config` {#config}

`configureWebpack` 呼叫時會傳入根據客戶端/伺服器建置生成的 `config`。可將其視為待合併的基礎配置。

### `isServer` {#isServer}

`configureWebpack` 會在伺服器建置和客戶端建置時都被呼叫。伺服器建置會收到 `true` 作為 `isServer` 參數，而客戶端建置則會收到 `false`。
