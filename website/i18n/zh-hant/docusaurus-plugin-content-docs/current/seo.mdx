...
透過[網站設定](./configuration.mdx#site-metadata)為整個網站提供全域元屬性。這些元數據將以鍵值對的形式作為屬性名稱和值，全部渲染在 HTML 的 `<head>` 中。`metadata` 屬性是宣告 `<meta>` 標籤的便捷方式，但也可以透過 `headTags` 屬性在 `<head>` 中注入任意標籤。
...
Docusaurus 是一個靜態網站生成器——每個 URL 路由都會生成靜態 HTML 文件，這有助於搜尋引擎更容易發現您的內容。

## 圖片元描述 {#image-meta-description}

圖片的 alt 標籤告訴搜尋引擎圖片的內容，並在圖片無法被視覺看到時使用，例如使用螢幕閱讀器時或圖片損壞時。Alt 標籤在 Markdown 中普遍支援。
...
Docusaurus 部落格預設支援[豐富搜尋結果](https://search.google.com/test/rich-results)，以獲得最佳搜尋引擎體驗。這些資訊會根據您在部落格/全域設定中的元資訊自動生成。要獲得豐富搜尋資訊的優勢，請填寫文章發布日期、作者和圖片等資訊。更多關於元資訊的說明請參閱[此處](./blog.mdx)。

## Robots 檔案 {#robots-file}

`robots.txt` 檔案用於規範搜尋引擎的行為，決定哪些內容應該被顯示或排除。您可以將其作為[靜態資源](./static-assets.mdx)提供。以下設定允許所有請求存取所有子頁面：
...
若要阻止整個 Docusaurus 網站被索引，請使用 [`noIndex`](./api/docusaurus.config.js.mdx#noIndex) 網站設定。某些[託管服務提供商](./deployment.mdx)可能也允許您配置 `X-Robots-Tag: noindex` HTTP 標頭（GitHub Pages 不支援此功能）。
...
Docusaurus 提供預設隨 `preset-classic` 內建的 [`@docusaurus/plugin-sitemap`](./api/plugins/plugin-sitemap.mdx) 外掛程式。它會自動生成 `sitemap.xml` 檔案，在正式環境建置後可透過 `https://example.com/[baseUrl]/sitemap.xml` 存取。此網站地圖元資料有助於搜尋引擎爬蟲更準確地抓取您的網站。
...
Docusaurus 使用檔案名稱作為連結，但您始終可以使用 slug 來更改連結，詳情請參閱此[教學](./guides/docs/docs-create-doc.mdx#document-id)。
