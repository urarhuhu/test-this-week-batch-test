...
- [Node.js](https://nodejs.org/en/download/) 版本 18.0 或更高（可通過運行 `node -v` 檢查）。您可以使用 [nvm](https://github.com/nvm-sh/nvm) 來管理單台機器上的多個 Node.js 版本。
  - 安裝 Node.js 時，建議勾選所有與依賴相關的選項。

## 建立專案網站架構 {#scaffold-project-website}

安裝 Docusaurus 最簡單的方法是使用 [`create-docusaurus`](./api/misc/create-docusaurus.mdx) 命令行工具，它可以幫助您建立一個 Docusaurus 網站的基本架構。您可以在新的空存儲庫或現有存儲庫中的任何位置運行此命令，它將創建一個包含生成文件的新目錄。
...
- `/docs/` - 存放文件的 Markdown 檔案。可在 `sidebars.js` 中自訂文件側邊欄的排序。若您已停用文件外掛程式，可刪除此目錄；或透過設定 `path` 選項後更改其名稱。詳情請參閱[文件指南](./guides/docs/docs-introduction.mdx)
- `/src/` - 非文件類檔案，例如頁面或自訂 React 元件。您不一定要將非文件檔案嚴格放置於此，但集中存放便於在需要進行某種 linting/處理時指定
  - `/src/pages` - 此目錄內的任何 JSX/TSX/MDX 檔案都將轉換為網站頁面。詳情請參閱[頁面指南](guides/creating-pages.mdx)
- `/static/` - 靜態目錄。此處內容將直接複製到最終 `build` 目錄的根目錄下
- `/docusaurus.config.js` - 包含網站設定的設定檔，相當於 Docusaurus v1 的 `siteConfig.js`
- `/package.json` - Docusaurus 網站本質上是 React 應用程式，您可安裝並使用任何 npm 套件
- `/sidebars.js` - 供文件功能使用，用於指定側邊欄中文檔的排序

### 單體倉儲 (Monorepos) {#monorepos}

若您使用 Docusaurus 為現有專案編寫文件，單體倉儲可能是適合的解決方案。單體倉儲允許您在相似專案間共享依賴項。例如，您的網站可使用本地套件來展示最新功能，而無需依賴已發布版本。如此一來，貢獻者能在實作功能時同步更新文件。下方是單體倉儲的範例目錄結構：
...
若使用 Netlify 或 Vercel 等託管服務，需將網站的「基礎目錄」(Base directory) 變更為 Docusaurus 根目錄所在位置。此例中應設為 `./website`。更多關於設定忽略指令的資訊，請參閱[部署文件](./deployment.mdx#deploying-to-netlify)。

更多單體倉儲相關資訊，請參閱 [Yarn 文件](https://yarnpkg.com/features/workspaces)（Yarn 並非設定單體倉儲的唯一方式，但屬常見解決方案），或參考 [Docusaurus](https://github.com/facebook/docusaurus) 與 [Jest](https://github.com/facebook/jest) 的實際案例。
...
恭喜！您已成功建立第一個 Docusaurus 網站！請瀏覽網站查看現有功能。

## 建置 {#build}

Docusaurus 是現代靜態網站生成器，因此需將網站建置為靜態內容目錄後部署至網頁伺服器才能檢視。建置網站指令如下：
...
有多種方式可以更新您的 Docusaurus 版本。其中一種可靠的方法是手動修改 `package.json` 中的版本號至目標版本。請注意所有 `@docusaurus/` 命名空間的套件都應使用相同版本。

import UpgradeGuide from '@site/src/components/UpgradeGuide';

<UpgradeGuide />

接著，在包含 `package.json` 的目錄中執行您的套件管理器的安裝指令：
...
`npm install` 可能會報告數個漏洞並建議執行 `npm audit` 來修復。通常這些被報告的漏洞（例如 RegExp DOS 漏洞）是無害的，可以安全忽略。另請閱讀這篇反映我們想法的文章：[npm audit: Broken by Design](https://overreacted.io/npm-audit-broken-by-design/)。
