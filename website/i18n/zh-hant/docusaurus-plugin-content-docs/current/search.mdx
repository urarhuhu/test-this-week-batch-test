...
該服務對所有開發者文件或技術部落格**免費開放**：請務必閱讀[檢查清單](https://docsearch.algolia.com/docs/who-can-apply/)並[申請 DocSearch 計畫](https://docsearch.algolia.com/apply)。

DocSearch 每週會爬取您的網站一次（排程可透過網頁介面設定），並將所有內容彙總至 Algolia 索引中。這些內容隨後會直接透過 Algolia API 從前端查詢。

若您的網站[不符合資格](https://docsearch.algolia.com/docs/who-can-apply)使用免費託管版 DocSearch，或網站位於防火牆後且非公開狀態，您可以[自行運行](https://docsearch.algolia.com/docs/run-your-own/) DocSearch 爬蟲。
...
當您的申請通過並部署後，您將收到一封包含所有詳細資訊的電子郵件，以便將 DocSearch 加入專案。您可以透過[網頁介面](https://crawler.algolia.com/)編輯和管理爬取作業。索引在部署後即可立即使用，通常無需手動配置。
...
爬蟲配置中包含 `initialIndexSettings`，該設定僅在 Algolia 索引不存在時用於初始化。

若您更新 `initialIndexSettings` 爬蟲設定，可以透過介面手動更新索引，但 [Algolia 團隊建議刪除索引後重新啟動爬取](https://github.com/facebook/docusaurus/issues/9200#issuecomment-1667338492)，以完全使用新設定重新初始化。

:::

### 連接 Algolia {#connecting-algolia}

Docusaurus 自身的 `@docusaurus/preset-classic` 支援 Algolia DocSearch 整合。若您使用經典預設配置，則無需額外安裝。
...
若重大變更後搜尋失效，請透過 Algolia 儀表板 **手動觸發重新爬取**。
...
為解決此問題，情境化搜尋會動態識別您當前瀏覽的文件版本與語言，自動建立對應的搜尋篩選條件。

- 在 `/en/docs/v1/myDoc` 路徑下，搜尋結果僅包含 **v1 英文版**文件（與其他未版本化頁面）
- 在 `/fr/docs/v2/myDoc` 路徑下，搜尋結果僅包含 **v2 法文版**文件（與其他未版本化頁面）

:::info

當啟用 `contextualSearch: true`（預設值）時，情境化篩選條件會與 `algolia.searchParameters.facetFilters` 提供的條件合併。
...
詳見 [Algolia 篩選條件文件](https://www.algolia.com/doc/guides/managing-results/refine-results/faceting/)。

:::

:::warning[情境化搜尋失效？]

若關閉情境化搜尋才能獲取結果，極可能是 [索引設定問題](#algolia-no-search-results) 所致。

:::

### 自訂 Algolia 搜尋樣式 {#styling-your-algolia-search}

DocSearch 預載符合無障礙標準的主題，嚴格遵守色彩對比規範。

您仍可透過修改 `/src/css/custom.css` 檔案，套用 Docusaurus 的 [Infima CSS 變數](styling-layout.mdx#styling-your-site-with-infima) 來調整樣式。
...
若要直接編輯 Algolia 搜尋 React 元件，請對 `@docusaurus/theme-search-algolia` 的 `SearchBar` 元件進行 [swizzle 操作](swizzling.mdx)：
...
2. 透過介面刪除現有索引
3. 透過介面觸發重新爬取
4. 確認索引重建時包含以下分面字段：`docusaurus_tag`、`language`、`lang`、`version`、`type`
5. 確認現在即使啟用[上下文搜尋](#contextual-search)也能獲得結果
...
- [typesense-docsearch-scraper](https://github.com/typesense/typesense-docsearch-scraper) - 用於爬取網站並將資料索引至 Typesense 叢集
...
當搜尋索引規模較小且可被使用者瀏覽器下載時，您可使用本地搜尋外掛程式。

您可在[社群資源清單](https://docusaurus.io/community/resources#search)找到社群維護的本地搜尋外掛程式。
...
這會在專案目錄建立 `src/theme/SearchBar` 檔案。重新啟動開發伺服器並編輯元件後，您會發現 Docusaurus 已改用您的自訂 `SearchBar` 元件。

**注意**：您也可以[從 Algolia SearchBar 開始替換](#editing-the-algolia-search-component)，並以此為基礎建立自己的搜尋元件。