...
在部落格文章中使用 `<!--truncate-->` 標記來表示在查看所有已發佈部落格文章時將顯示的摘要。`<!--truncate-->` 上方的任何內容都將成為摘要的一部分。請注意，截斷標記上方的部分必須是獨立可渲染的 Markdown。例如：
...
預設情況下，每個部落格列表頁面會顯示 10 篇文章，但您可以透過插件配置中的 `postsPerPage` 選項控制分頁。如果設定 `postsPerPage: 'ALL'`，則會停用分頁，所有文章將顯示在第一頁。您還可以為部落格列表頁面添加元描述以提升 SEO：
...
您還可以透過 `blogSidebarTitle` 選項修改側邊欄的標題文字。例如，如果設定 `blogSidebarCount: 'ALL'`，您可以將預設的「近期文章」改為「所有文章」：
...
您可以讓 Docusaurus 網站不顯示專用首頁，而是將部落格文章列表頁設為首頁。將 `routeBasePath` 設為 `'/'`，即可讓部落格透過根路徑 `example.com/` 而非子路徑 `example.com/blog/` 來提供服務。
...
若您停用了文檔插件，請記得同時刪除設定檔中其他對文檔插件的引用。特別注意需移除導航欄中與文檔相關的項目。

:::

:::tip

另有「純文檔模式」可供僅需使用文檔功能的用戶參考。詳見[純文檔模式](./guides/docs/docs-introduction.mdx)獲取詳細說明或關於 `routeBasePath` 的更完整解釋。

:::

### 多個部落格 {#multiple-blogs}

預設情況下，經典主題假設每個網站只有一個部落格，因此僅包含一個部落格插件實例。若您希望在單一網站上運行多個部落格，這同樣可行！您可以在 `docusaurus.config.js` 的 `plugins` 選項中指定另一個部落格插件來新增部落格。
