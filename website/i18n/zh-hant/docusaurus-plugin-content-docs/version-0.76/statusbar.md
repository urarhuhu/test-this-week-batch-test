...
控制應用程式狀態列的元件。狀態列通常是螢幕頂部的區域，顯示當前時間、Wi-Fi和行動網路資訊、電量等級和其他狀態圖標。
...
:::warning
由於 Android 15 引入了邊到邊強制執行，在 API 級別 35 中設置狀態列背景顏色已被棄用，設置它將不會產生效果。您可以在此處閱讀更多關於我們的[邊到邊建議](https://github.com/react-native-community/discussions-and-proposals/discussions/827)。
:::
...
在 Android 上，這只會在 API 版本 23 及以上產生影響。
...
:::warning
由於 Android 15 引入了邊到邊強制執行，在 API 級別 35 中設置狀態列為半透明已被棄用，設置它將不會產生效果。您可以在此處閱讀更多關於我們的[邊到邊建議](https://github.com/react-native-community/discussions-and-proposals/discussions/827)。
:::
