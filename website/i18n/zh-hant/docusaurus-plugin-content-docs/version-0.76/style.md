...
在 React Native 中，您使用 JavaScript 來為應用程式設定樣式。所有核心元件都接受名為 `style` 的屬性。樣式名稱和[值](colors.md)通常與網頁上的 CSS 運作方式相符，但名稱使用駝峰式命名法，例如 `backgroundColor` 而非 `background-color`。
...
一個常見的模式是讓您的元件接受一個 `style` 屬性，該屬性又用於設定子元件的樣式。您可以使用此方式讓樣式像 CSS 一樣「層疊」。
