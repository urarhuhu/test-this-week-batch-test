...
> [`useWindowDimensions`](usewindowdimensions) 是 React 元件首選的 API。與 `Dimensions` 不同，它會隨著視窗尺寸的更新而更新。這與 React 的設計模式完美契合。
...
> 雖然尺寸可以立即取得，但它們可能會發生變化（例如由於設備旋轉、折疊式設備等），因此任何依賴這些常數的渲染邏輯或樣式都應該在每次渲染時調用此函數，而不是快取值（例如，使用內聯樣式而不是在 `StyleSheet` 中設置值）。

如果您針對的是折疊式設備或可以改變螢幕大小或應用程式視窗大小的設備，您可以使用 Dimensions 模組中提供的事件監聽器，如下例所示。
...
初始尺寸在 `runApplication` 被調用之前設置，因此它們應該在其他任何 require 運行之前就可取得，但可能會在之後更新。
...
> 對於 Android，`window` 尺寸將排除 `狀態欄`（如果不透明）和 `底部導航欄` 所使用的空間
