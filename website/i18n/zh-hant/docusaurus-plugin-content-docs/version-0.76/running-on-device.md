...
:::info
如果您使用 `create-expo-app` 來設置專案，您可以通過掃描運行 `npm start` 時顯示的 QR 碼，在 Expo Go 中於裝置上運行您的應用程式。有關更多信息，請參閱 Expo 指南中的[在您的裝置上運行專案](https://docs.expo.dev/get-started/expo-go/)。
:::
