...
在iOS上可指定任意數量的按鈕。每個按鈕可選擇性地設定樣式或標記為強調，可用選項由[AlertButtonStyle](#alertbuttonstyle-ios)枚舉和[AlertButton](alert#alertbutton)中的`isPreferred`欄位表示。
...
取消事件可透過在`options`參數中提供`onDismiss`回調函數來處理。
