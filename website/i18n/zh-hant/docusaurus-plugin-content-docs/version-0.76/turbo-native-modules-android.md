...
接著我們需要建立 `NativeLocalStoragePackage`。它透過將模組封裝為 Turbo Native Package，提供在 React Native 運行時註冊模組的物件：
...
最後，我們需要告知主應用程式中的 React Native 如何找到這個 `Package`。這稱為在 React Native 中「註冊」該套件。

在此案例中，您需將其加入 [getPackages](https://github.com/facebook/react-native/blob/8d8b8c343e62115a5509e1aed62047053c2f6e39/packages/react-native/ReactAndroid/src/main/java/com/facebook/react/ReactNativeHost.java#L233) 方法的回傳值中。

:::info
後續您將學習如何將 Turbo Native Modules 發佈為 [npm 套件](the-new-architecture/create-module-library.md#publish-the-library-on-npm)，我們的建置工具會自動為您處理連結。
:::
