...
若需要分節支援，請使用 [`<SectionList>`](sectionlist.md)。
...
- 透過傳遞 `extraData={selectedId}` 給 `FlatList`，我們確保當狀態變更時 `FlatList` 本身會重新渲染。未設定此屬性時，`FlatList` 不會知道需要重新渲染任何項目，因為它是 `PureComponent` 且屬性比較不會顯示任何變更。
...
此為 [`<VirtualizedList>`](virtualizedlist.md) 的便利封裝，因此繼承了其屬性（以及 [`<ScrollView>`](scrollview.md) 的屬性），此處未明確列出的屬性亦同，並附帶以下注意事項：

- 當內容滾出渲染視窗時，內部狀態不會保留。請確保所有資料皆擷取於項目資料或外部儲存（如 Flux、Redux 或 Relay）。
- 此為 `PureComponent`，意味著若 `props` 保持淺層相等則不會重新渲染。請確保 `renderItem` 函式依賴的所有內容皆以屬性傳遞（例如 `extraData`），且更新後不為 `===`，否則變更時 UI 可能不會更新。此包含 `data` 屬性與父元件狀態。
- 為限制記憶體使用並實現平滑滾動，內容會於螢幕外非同步渲染。這表示可能滾動速度快於填充速率，並暫時看到空白內容。此為可依應用需求調整的取捨，我們正於幕後持續改進。
...
提供額外中繼資料如 `index`（若您需要），以及更通用的 `separators.updateProps` 函式，讓您可設定任意屬性來變更前導分隔線或後續分隔線的渲染（在較常見的 `highlight` 與 `unhighlight`（設定 `highlighted: boolean` 屬性）不足以滿足使用情境時）。
...
- `index` (number)：對應此項目於 `data` 陣列中的索引。
- `separators` (Object)
  - `highlight` (Function)
...
在每個項目之間渲染（但不會出現在頂部或底部）。預設會提供 `highlighted` 和 `leadingItem` 屬性。`renderItem` 提供 `separators.highlight`/`unhighlight` 來更新 `highlighted` 屬性，但您也可以透過 `separators.updateProps` 添加自訂屬性。可以是 React 元件（例如 `SomeComponent`）或 React 元素（例如 `<SomeComponent />`）。
...
用於通知列表重新渲染的標記屬性（因其實作 `PureComponent`）。若您的 `renderItem`、Header、Footer 等函式依賴於 `data` 屬性之外的任何內容，請將其置於此處並視為不可變資料。
...
`getItemLayout` 是選用的效能優化選項，若您預先知道項目的尺寸（高度或寬度），可跳過動態內容的測量。對於固定尺寸的項目（例如以下情況）非常高效：
...
多欄佈局僅能在 `horizontal={false}` 時渲染，並會以類似 `flexWrap` 的鋸齒狀排列。所有項目應具有相同高度——不支援磚牆式佈局。
...
當列的可見性發生變化時呼叫（由 `viewabilityConfig` 屬性定義）。
...
參見 [`ViewabilityHelper.js`](https://github.com/facebook/react-native/blob/main/packages/react-native/Libraries/Lists/ViewabilityHelper.js) 以獲取流程類型與進一步文件說明。
...
至少需要設定 `viewAreaCoveragePercentThreshold` 或 `itemVisiblePercentThreshold` 其中一項。需在 `constructor` 中完成以避免以下錯誤（[參考](https://github.com/facebook/react-native/issues/17408)）：
...
視窗覆蓋百分比閾值（0-100），部分遮擋的項目需達到此比例才會被視為「可見」。完全可見的項目始終視為可見。數值為0表示項目在視窗中只要有一個像素即視為可見，100表示項目必須完全可見或覆蓋整個視窗才會視為可見。
...
滾動至內容末端。若未提供 `getItemLayout` 屬性，可能會有卡頓現象。
...
- 'animated' (布林值) - 滾動時是否啟用動畫。預設為 `true`。
- 'index' (數值) - 要滾動至的索引。必填。
- 'viewOffset' (數值) - 最終目標位置的固定像素偏移量。
- 'viewPosition' (數值) - 值為 `0` 時將指定索引的項目置頂，`1` 時置底，`0.5` 時居中。
