...
這會讓容器在所有平台上具有 `flex: 1`，在 Android 上具有綠色背景，在 iOS 上具有紅色背景，在其他平台上具有藍色背景。

由於對應平台鍵值的型別可以是 `any`，[`select`](platform.md#select) 方法也可用於回傳平台特定元件，如下所示：
