...
現在是時候編寫一些 Android 平台代碼來渲染網頁視圖了。你需要遵循的步驟如下：

- 執行 Codegen
- 編寫 `ReactWebView` 的代碼
- 編寫 `ReactWebViewManager` 的代碼
- 編寫 `ReactWebViewPackage` 的代碼
- 在應用程序中註冊 `ReactWebViewPackage`

### 1. 通過 Gradle 執行 Codegen

執行一次以生成你的 IDE 可以使用的樣板代碼。
...
Codegen 將生成你需要實現的 `ViewManager` 接口以及網頁視圖的 `ViewManager` 代理。

### 2. 編寫 `ReactWebView`

`ReactWebView` 是封裝 Android 原生視圖的組件，當使用我們的自定義組件時，React Native 將渲染該視圖。

在 `android/src/main/java/com/webview` 文件夾中創建一個 `ReactWebView.java` 或 `ReactWebView.kt` 文件，並包含以下代碼：
...
`ReactWebView` 繼承了 Android 的 `WebView`，因此你可以輕鬆重用平台已定義的所有屬性。

該類定義了三個 Android 構造函數，但它們的實際實現被委託給了私有的 `configureComponent` 函數。此函數負責初始化所有特定於組件的屬性：在此代碼中，你設置了 `WebView` 的佈局，並定義了用於自定義 `WebView` 行為的 `WebClient`。在這段代碼中，`ReactWebView` 通過實現 `WebClient` 的 `onPageFinished` 方法，在頁面加載完成時發出一個事件。

代碼隨後定義了一個輔助函數來實際發出事件。要發出事件，你需要：

- 獲取 `ReactContext` 的引用；
- 檢索你正在呈現的視圖的 `surfaceId`；
- 獲取與視圖關聯的 `eventDispatcher` 的引用；
- 使用 `WritableMap` 對象構建事件的負載；
- 創建需要發送到 JavaScript 的事件對象；
- 調用 `eventDispatcher.dispatchEvent` 來發送事件。

文件的最後部分包含了你發送事件所需的數據類型定義：
...
`WebViewManager` 是連接 React Native 運行時與原生視圖的類。

當 React 從應用程序接收到渲染特定組件的指令時，React 使用註冊的視圖管理器來創建視圖並傳遞所有必需的屬性。
...
`ReactWebViewManager` 繼承了 react 的 `SimpleViewManager` 類，並實現了由 Codegen 生成的 `CustomWebViewManagerInterface`。

它持有 `CustomWebViewManagerDelegate` 的引用，這是另一個由 Codegen 生成的元素。

然後它覆蓋了 `getName` 函數，該函數必須返回與 spec 中 `codegenNativeComponent` 函數調用所使用的相同名稱。

`createViewInstance` 函數負責實例化一個新的 `ReactWebView`。

接著，ViewManager 需要定義所有 React 組件屬性如何更新原生視圖。在示例中，你需要決定如何處理 React 將在 `WebView` 上設置的 `sourceURL` 屬性。

最後，如果元件能夠觸發事件，你需要透過覆寫 `getExportedCustomBubblingEventTypeConstants`（針對冒泡事件）或 `getExportedCustomDirectEventTypeConstants`（針對直接事件）來映射事件名稱。

### 4. 撰寫 `ReactWebViewPackage`

如同處理原生模組一樣，原生元件也需要實作 `ReactPackage` 類別。這個物件可用於在 React Native 運行時註冊元件。
...
`ReactWebViewPackage` 繼承自 `TurboReactPackage` 並實作所有必要方法以正確註冊我們的元件。

- `createViewManagers` 方法是工廠方法，用於建立管理自訂視圖的 `ViewManager`。
- `getModule` 方法根據 React Native 需要渲染的視圖返回對應的 ViewManager。
- `getReactModuleInfoProvider` 提供在運行時註冊模組所需的所有資訊。

### 5. 在應用程式中註冊 `ReactWebViewPackage`

最後，你需要在應用程式中註冊 `ReactWebViewPackage`。我們透過修改 `MainApplication` 檔案，將 `ReactWebViewPackage` 加入 `getPackages` 函數返回的套件清單中來實現這一點。
