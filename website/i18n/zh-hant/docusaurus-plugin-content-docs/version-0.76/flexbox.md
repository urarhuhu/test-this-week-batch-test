...
[`flex`](layout-props#flex) 屬性定義了子元素如何沿主軸**「填滿」**可用空間。空間將根據每個元素的 flex 屬性進行分配。
...
[`flexDirection`](layout-props#flexdirection) 控制節點子元素的排列方向，這也被稱為主軸。交叉軸則是與主軸垂直的軸線，或是換行排列的軸線。

- `column` (**預設值**) 從上到下排列子元素。若啟用換行，則下一行將從容器頂部第一個項目的右側開始。
- `row` 從左到右排列子元素。若啟用換行，則下一行將從容器左側第一個項目的下方開始。
- `column-reverse` 從下到上排列子元素。若啟用換行，則下一行將從容器底部第一個項目的右側開始。
- `row-reverse` 從右到左排列子元素。若啟用換行，則下一行將從容器右側第一個項目的下方開始。
...
佈局 [`direction`](layout-props#direction) 指定了子元素和文字在層級結構中的排列方向。佈局方向也會影響 `start` 和 `end` 所指向的邊緣。預設情況下，React Native 使用 LTR（左到右）佈局方向。在此模式下，`start` 指向左側，`end` 指向右側。
...
[`justifyContent`](layout-props#justifycontent) 描述了如何在容器的主軸上對齊子元素。例如，你可以使用此屬性在 `flexDirection` 設為 `row` 的容器中水平置中子元素，或在 `flexDirection` 設為 `column` 的容器中垂直置中子元素。
...
- `space-around` 在主軸上均勻分布子元素，剩餘空間環繞子元素分配。與 `space-between` 相比，使用 `space-around` 會在第一個子元素前和最後一個子元素後也分配空間。

- `space-evenly` 在主軸上完全均勻分布子元素。相鄰子元素之間、主軸起始端與第一個子元素之間、主軸末端與最後一個子元素之間的間距完全相同。
...
:::info
要使 `stretch` 生效，子元素在次要軸上不能有固定尺寸。在以下範例中，除非移除子元素的 `width: 50`，否則設置 `alignItems: stretch` 不會產生任何效果。
:::
...
[`alignSelf`](layout-props#alignself) 的選項和效果與 `alignItems` 相同，但它是針對單個子元素設置，以改變其在父容器內的對齊方式。`alignSelf` 會覆寫父容器通過 `alignItems` 設置的任何選項。
...
[alignContent](layout-props#aligncontent) 定義多行內容在交叉軸上的分布方式。此屬性僅在使用 `flexWrap` 將項目換行至多行時生效。

- `flex-start`（**預設值**）將換行後的內容對齊至交叉軸的起始端。

- `flex-end` 將換行後的內容對齊至交叉軸的末端。

- `stretch`（_在網頁版 Yoga 中的預設值_）拉伸換行後的內容以匹配容器交叉軸的高度。

- `center` 將換行後的內容對齊至交叉軸的中心。

- `space-between` 在交叉軸上均勻分布換行後的內容，剩餘空間分配於行與行之間。

- `space-around` 在交叉軸上均勻分布換行後的內容，剩餘空間環繞各行分配。容器兩端的空間大小為行間空間的一半。

- `space-evenly` 在交叉軸上完全均勻分布換行後的內容，剩餘空間完全均等分配。所有間距大小相同。
...
[`flexWrap`](layout-props#flexwrap) 屬性設置在容器上，用於控制當子元素在主軸方向上超出容器尺寸時的處理方式。默認情況下，子元素會被強制排列在單一行內（這可能會壓縮元素）。如果允許換行，則會根據需要沿主軸方向將項目換行至多行。
...
- [`flexBasis`](layout-props#flexbasis) 是一種與軸向無關的方式，用於設定項目在主軸上的默認尺寸。設置子元素的 `flexBasis` 類似於：當父容器為 `flexDirection: row` 時設定該子元素的 `width`，或當父容器為 `flexDirection: column` 時設定其 `height`。項目的 `flexBasis` 是該項目的默認尺寸，即在任何 `flexGrow` 和 `flexShrink` 計算之前的尺寸。

- [`flexGrow`](layout-props#flexgrow) 描述容器內的主軸空間應如何在其子元素之間分配。在佈局子元素後，容器會根據子元素指定的 flex grow 值分配剩餘空間。

  `flexGrow` 接受任何大於等於 0 的浮點數值，默認值為 0。容器會根據子元素的 `flexGrow` 值加權分配剩餘空間。

- [`flexShrink`](layout-props#flexshrink) 描述當子元素在主軸上的總尺寸超出容器主軸尺寸時，應如何收縮子元素。`flexShrink` 與 `flexGrow` 非常相似，可以將溢出的尺寸視為負的剩餘空間來理解。這兩個屬性也能很好地協同工作，允許子元素根據需要伸展和收縮。

  `flexShrink` 接受任何大於等於 0 的浮點數值，默認值為 0（在網頁上默認為 1）。容器會根據子元素的 `flexShrink` 值加權收縮其子元素。
...
您可以結合使用 `flexWrap`、`alignContent` 與 `gap` 來為項目添加一致的間距。
...
`width` 和 `height` 均可接受以下值：

- `auto`（**默認值**）React Native 會根據元素的內容（無論是其他子元素、文字還是圖片）計算其寬度/高度。

- `pixels` 以絕對像素值定義寬度/高度。根據組件上設置的其他樣式，這可能並非節點的最終尺寸。
...
- `absolute` 當元素採用絕對定位時，它不會參與正常的版面流動。而是獨立於其兄弟元素進行佈局。其位置由 `top`、`right`、`bottom` 和 `left` 的值決定，這些值會將元素定位相對於其包含區塊（containing block）。

- `static` 當元素採用靜態定位時，會依照正常的版面流動進行定位，並忽略 `top`、`right`、`bottom` 和 `left` 的值。此 `position` 設定也會導致元素不會為其絕對定位的子代形成包含區塊，除非存在其他優先的樣式屬性（例如 `transform`）。這允許 `absolute` 元素可以定位到非其父元素的物件上。請注意，**`static` 僅在新架構（New Architecture）中可用**。
...
套用於絕對定位元素的百分比長度（例如：`width: '50%'` 或 `padding: '10%'`）會根據其包含區塊的大小進行計算。例如，如果包含區塊的寬度為 100 點，則絕對定位元素上的 `width: 50%` 會使其寬度為 50 點。

以下列表可協助您判斷任何給定元素的包含區塊：

- 如果該元素的 `position` 類型為 `relative` 或 `static`，則其包含區塊為其父元素。
- 如果該元素的 `position` 類型為 `absolute`，則其包含區塊為滿足以下任一條件的最接近祖先元素：
