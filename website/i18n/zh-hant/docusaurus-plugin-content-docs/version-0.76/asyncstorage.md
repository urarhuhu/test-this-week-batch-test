...
由於 `AsyncStorage` 是全域運作，建議除了輕量使用外，應在其上封裝抽象層而非直接操作。

在 iOS 上，`AsyncStorage` 底層由原生程式碼實現，小量資料會儲存在序列化的字典中，較大資料則存於獨立檔案。Android 平台會根據可用性使用 [RocksDB](https://rocksdb.org/) 或 SQLite。

`AsyncStorage` 的 JavaScript 層是門面模式，提供清晰的 API、真實的 `Error` 物件和非批次操作方法。每個 API 方法都會回傳一個 `Promise` 物件。
...
為指定 `key` 設定數值並在完成時觸發回調函式。回傳一個 `Promise` 物件。
...
清除 _所有_ 用戶端、函式庫等的 `AsyncStorage` 資料。通常不應直接呼叫此方法，請改用 `removeItem` 或 `multiRemove` 僅清除你應用程式的鍵值。回傳一個 `Promise` 物件。
...
取得當前應用已知的 _所有_ 鍵值（包含所有呼叫者與函式庫）。回傳一個 `Promise` 物件。
...
此方法允許您批次獲取給定`key`陣列對應的項目。回調函數將被調用並傳入一個包含對應鍵值對的陣列：
...
使用此方法批次儲存多組鍵值對。操作完成時將收到單一回調函數並傳入可能發生的錯誤：
