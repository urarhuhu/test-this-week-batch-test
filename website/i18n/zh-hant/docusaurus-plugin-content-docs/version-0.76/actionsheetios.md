...
- `userInterfaceStyle` (字串) - 操作選單使用的介面風格，可設為 `light` 或 `dark`，否則將使用預設系統風格

'callback' 函式接收一個參數，即所選項目的零基索引。
...
'failureCallback' 函式接收一個參數，即錯誤物件。此物件唯一定義的屬性是選用的 `stack` 屬性，類型為 `string`。

'successCallback' 函式接收兩個參數：
