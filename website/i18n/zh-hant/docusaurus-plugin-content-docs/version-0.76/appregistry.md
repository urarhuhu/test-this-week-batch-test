...
`AppRegistry` 是所有 React Native 應用程式的 JavaScript 入口點。應用程式的根元件應透過 `AppRegistry.registerComponent` 註冊，之後原生系統可載入應用程式的套件包，並在準備就緒時透過調用 `AppRegistry.runApplication` 實際運行應用程式。
...
若要「停止」應用程式（當視圖需要銷毀時），請使用傳入 `runApplication` 的標籤調用 `AppRegistry.unmountApplicationComponentAtRootTag`。這些方法應始終配對使用。

`AppRegistry` 應在 `require` 序列的早期引入，以確保 JavaScript 執行環境在其他模組引入前已完成設定。
...
此方法可在應用程式處於背景時執行 JavaScript 任務，例如用於同步新資料、處理推播通知或播放音樂。
