...
將潛在敏感資訊放在環境中是常見做法。然而，在典型的 Docusaurus 網站中，`docusaurus.config.js` 檔案是與 Node.js 環境的唯一介面（參見 [我們的架構概述](advanced/architecture.mdx)），而其他所有內容（MDX 頁面、React 組件等）都是客戶端，無法直接存取 `process` 全域變數。在這種情況下，您可以考慮使用 [`customFields`](api/docusaurus.config.js.mdx#customFields) 將環境變數傳遞到客戶端。
...
沒有一個萬能的解決方案。您需要權衡您的需求和資源後做出選擇。

## 自架部署 {#self-hosting}

Docusaurus 可使用 [`docusaurus serve`](cli.mdx#docusaurus-serve-sitedir) 指令進行自架部署，透過 `--port` 參數變更連接埠，`--host` 參數變更主機位址。
...
相較於靜態網站託管服務/CDN，此並非最佳解決方案。

:::

:::warning

後續章節將介紹常見託管服務供應商及其針對 Docusaurus 網站的最佳化部署設定。Docusaurus 與這些服務無商業合作關係，本文僅為便利性提供參考資訊。部分內容由第三方提供，可能未及時反映最新 API 變更。若發現過時內容，歡迎提交 PR 修正。

由於我們僅能盡力維護現有內容，已停止接受新增託管方案的 PR。但您可將部署指南發表於外部平台（如個人部落格或服務商官網），並申請將連結收錄至本文件。

:::

## 部署至 Netlify {#deploying-to-netlify}

將 Docusaurus 網站部署至 [Netlify](https://www.netlify.com/) 前，請確認以下設定已正確配置：
...
站點設定過程中，請指定以下建置指令與目錄：

- 建置指令：`npm run build`
- 發佈目錄：`build`

若未預先設定這些建置選項，仍可於站點建立後，前往「站點設定」→「建置與部署」進行調整。
...
建議停用 Netlify 設定「後處理 > 資源優化 > 美化網址」以避免網址強制小寫、不必要的重新導向及 404 錯誤。

**請特別注意**：「停用資源優化」全域核取方塊存在缺陷，實際上無法真正停用「美化網址」設定。請務必獨立取消勾選此選項。
...
若要透過 [Vercel Git 整合功能](https://vercel.com/docs/concepts/git)部署 Docusaurus 專案，請確保專案已推送至 Git 儲存庫。

透過 [導入流程](https://vercel.com/import/git) 將專案導入 Vercel。在導入過程中，您會發現所有相關選項都已預先配置完成；不過，您仍可選擇變更其中任何 [選項](https://vercel.com/docs/build-step#build-&-development-settings)。

專案導入後，所有後續推送至分支的動作都會產生 [預覽部署](https://vercel.com/docs/platform/deployments#preview)，而對 [生產分支](https://vercel.com/docs/git-integrations#production-branch)（通常是 "main" 或 "master"）所做的任何變更，則會觸發 [生產環境部署](https://vercel.com/docs/platform/deployments#production)。
...
通常發布流程會涉及兩個儲存庫（或至少兩個分支）：包含原始碼檔案的分支，以及包含要透過 GitHub Pages 服務的建置輸出結果的分支。在以下教學中，它們將分別稱為 **「來源」** 和 **「部署」**。

每個 GitHub 儲存庫都關聯著一個 GitHub Pages 服務。若部署儲存庫名為 `my-org/my-project`（其中 `my-org` 為組織名稱或使用者名稱），部署後的網站將出現在 `https://my-org.github.io/my-project/`。若部署儲存庫名為 `my-org/my-org.github.io`（即 _組織 GitHub Pages 儲存庫_），網站則會出現在 `https://my-org.github.io/`。

:::info

若您想為 GitHub Pages 使用自訂網域，請在 `static` 目錄中建立一個 `CNAME` 檔案。`static` 目錄內的所有內容都會被複製到部署用的 `build` 目錄根目錄。使用自訂網域時，您應可將 `baseUrl: '/projectName/'` 改回 `baseUrl: '/'`，並將 `url` 設為您的自訂網域。

更多細節可參考 GitHub Pages 文件中的 [使用者、組織與專案頁面](https://help.github.com/en/articles/user-organization-and-project-pages)。

:::

GitHub Pages 會從預設分支（通常是 `master` 或 `main`）或 `gh-pages` 分支，以及根目錄或 `/docs` 資料夾中擷取已準備好部署的檔案（即 `docusaurus build` 的輸出結果）。您可透過儲存庫中的 `Settings > Pages` 進行配置。此分支將稱為「部署分支」。
...
以下工作流程範例假設您的網站原始碼位於儲存庫的 `main` 分支（_原始碼分支_ 為 `main`），且您的 [發佈來源](https://help.github.com/en/github/working-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site) 已設定為 [使用自訂 GitHub Actions 工作流程發佈](https://docs.github.com/en/pages/getting-started-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site#publishing-with-a-custom-github-actions-workflow)。

我們的目標是：

1. 當有新的拉取請求提交至 `main` 時，執行一個確保網站成功建置的動作，但不會實際部署。此工作稱為 `test-deploy`。
2. 當拉取請求合併至 `main` 分支或有人直接推送至 `main` 分支時，將建置並部署至 GitHub Pages。此工作稱為 `deploy`。

以下是兩種使用 GitHub Actions 部署文件的方法。根據您的部署儲存庫位置，選擇下方相關的標籤：

- 原始碼儲存庫和部署儲存庫為 **同一個** 儲存庫。
- 部署儲存庫為 **遠端** 儲存庫，與原始碼不同。此情境的說明假設 [發佈來源](https://help.github.com/en/github/working-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site) 為 `gh-pages` 分支。
...
雖然您可以將兩個工作定義在同一個工作流程檔案中，但原始的 `deploy` 工作流程在 PR 檢查套件狀態中會始終顯示為已跳過，這無法反映實際狀態且對審查過程無益。因此，我們建議將它們作為獨立的工作流程管理。
...
跨儲存庫發佈的設定較為複雜，因為您需要推送至另一個具有權限檢查的儲存庫。我們將使用 SSH 進行身份驗證。

1. 生成一組新的 [SSH 金鑰](https://help.github.com/en/github/authenticating-to-github/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent)。由於此 SSH 金鑰將用於 CI 環境，請確保不要設定任何密碼短語。
2. 預設情況下，您的公鑰應建立於 `~/.ssh/id_rsa.pub`；若使用其他名稱，請在 GitHub 部署金鑰設定頁面添加該金鑰。
3. 使用 `pbcopy < ~/.ssh/id_rsa.pub` 複製公鑰內容，並將其貼至部署儲存庫的 [部署金鑰](https://developer.github.com/v3/guides/managing-deploy-keys/#deploy-keys) 設定區。若命令列無效，請直接複製檔案內容。儲存前務必勾選 `允許寫入權限`。
4. 需將私鑰設為 [GitHub 加密憑證](https://help.github.com/en/actions/configuring-and-managing-workflows/creating-and-storing-encrypted-secrets) 以允許 Docusaurus 執行部署流程。
5. 使用 `pbcopy < ~/.ssh/id_rsa` 複製私鑰內容，在原始碼儲存庫建立名為 `GH_PAGES_DEPLOY` 的 GitHub 加密憑證。若命令列無效，請直接複製檔案內容並儲存。
6. 在 `.github/workflows/` 目錄建立 [文件工作流程](https://docs.github.com/en/actions/use-cases-and-examples/creating-an-example-workflow)，本範例使用 `deploy.yml` 檔案。

至此您應已完成以下設定：

- 原始碼儲存庫：已設定含私鑰的 GitHub 工作流程加密憑證
- 部署儲存庫：已於 GitHub 部署金鑰添加公鑰
...
持續整合 (CI) 服務通常用於在原始碼提交時執行例行任務，例如執行單元測試、整合測試、自動化建置、發布 npm 套件或部署網站更新。要實現網站自動化部署，只需在網站更新時調用 `yarn deploy` 腳本。以下說明如何透過熱門 CI 服務商 [Travis CI](https://travis-ci.com/) 實現此流程。

1. 前往 https://github.com/settings/tokens 生成新的 [個人存取權杖](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/)，建立時需授予 `repo` 權限範圍。
2. 使用 GitHub 帳號為目標儲存庫 [添加 Travis CI 應用程式](https://github.com/marketplace/travis-ci)。
3. 登入 Travis CI 儀表板 (網址格式為 `https://travis-ci.com/USERNAME/REPO`)，導航至儲存庫的 `更多選項 > 設定 > 環境變數` 區段。
4. 建立名為 `GH_TOKEN` 的環境變數（值為新生成的權杖），以及 `GH_EMAIL`（您的電子郵件）和 `GH_NAME`（GitHub 使用者名稱）。
...
現在，每當有新的提交推送至 `main` 分支時，Travis CI 將執行測試套件，若全部通過則透過 `yarn deploy` 腳本自動部署網站。

### 使用 Buddy 觸發部署 {#triggering-deployment-with-buddy}

[Buddy](https://buddy.works/) 是一款易用的 CI/CD 工具，可自動化將網站部署至 GitHub Pages 等環境。

請依以下步驟建立管道，當您推送變更至專案的指定分支時，將自動部署網站新版本：
...
[Flightcontrol](https://www.flightcontrol.dev/?ref=docusaurus) 是一項服務，可直接從您的 Git 儲存庫自動建置並將網頁應用程式部署至 AWS Fargate。它讓您能完整檢視並進行基礎架構變更，不受傳統 PaaS 的限制。
...
[Koyeb](https://www.koyeb.com) 是一個對開發者友善的無伺服器平台，可全球部署應用程式。該平台讓您無縫運行 Docker 容器、網頁應用程式和 API，具備基於 Git 的部署、原生自動擴展、全球邊緣網路，以及內建的服務網格和發現功能。查看 [Koyeb 的 Docusaurus 部署指南](https://www.koyeb.com/tutorials/deploy-docusaurus-on-koyeb) 開始使用。

## 部署到 Render {#deploying-to-render}

[Render](https://render.com) 提供 [免費的靜態網站託管](https://render.com/docs/static-sites) 服務，包含全託管的 SSL、自訂網域、全球 CDN，以及從您的 Git 倉庫持續自動部署。只需幾分鐘即可開始，請遵循 [Render 的 Docusaurus 部署指南](https://render.com/docs/deploy-docusaurus)。
...
2. 選擇要部署的專案。
   ...
   - 選擇您想要部署的專案。該專案必須包含專案檔案所在的目錄（例如 `website`）。
...
4. 部署。
   ...
   - 一旦開始，您將進入部署日誌。如果程式碼有任何問題，您將在日誌中收到警告或錯誤訊息，並指出問題原因。通常，日誌包含您所需的所有除錯資料。
...
使用 Surge 並按照以下步驟，幾秒內即可部署您的應用：

1. 首先，使用 npm 安裝 Surge，執行以下指令：
   ```bash npm2yarn
   npm install -g surge
   ```
2. 在專案的根目錄下，執行以下指令以建置專案的靜態檔案：
   ```bash npm2yarn
   npm run build
   ```
3. 然後，在專案的根目錄下執行以下指令：
...
確認您要發佈的網站位於 `build` 目錄中。系統會自動產生一個隨機子網域 `*.surge.sh`（可編輯）。
...
您可以將 Docusaurus 專案部署至 [Stormkit](https://www.stormkit.io)，這是一個用於靜態網站、單頁應用（SPA）和無伺服器函數的部署平台。詳細說明請參閱此[指南](https://www.stormkit.io/blog/how-to-deploy-docusarous)。
...
請參閱[文件](https://docs.quantcdn.io/docs/cli/continuous-integration)與[部落格](https://www.quantcdn.io/blog)以獲取更多部署至 QuantCDN 的範例與使用情境。

## 部署至 Layer0 {#deploying-to-layer0}

[Layer0](https://www.layer0.co) 是一站式平台，用於開發、部署、預覽、實驗、監控與運行無頭前端。其專注於大型動態網站，並透過 EdgeJS（基於 JavaScript 的內容傳遞網路）、預測性預取與效能監控提供頂尖效能。Layer0 提供免費方案，只需幾分鐘即可開始使用，請遵循 [Layer0 的 Docusaurus 部署指南](https://docs.layer0.co/guides/docusaurus)。

## 部署至 Cloudflare Pages {#deploying-to-cloudflare-pages}

[Cloudflare Pages](https://pages.cloudflare.com/) 是供前端開發者協作與部署網站的 Jamstack 平台。只需幾分鐘即可開始使用，請遵循[這篇文章](https://dev.to/apidev234/deploying-docusaurus-to-cloudflare-pages-565g)的指引。

## 部署至 Azure Static Web Apps {#deploying-to-azure-static-web-apps}

[Azure Static Web Apps](https://docs.microsoft.com/en-us/azure/static-web-apps/overview) 是一項服務，可直接從程式碼儲存庫自動建置並部署全端網頁應用程式至 Azure，簡化 CI/CD 的開發者體驗。Static Web Apps 將網頁應用的靜態資源與動態（API）端點分離。靜態資源透過全球分發的內容伺服器提供，使客戶端能透過就近的伺服器更快取得檔案。動態 API 則採用無伺服器架構進行擴展，使用事件驅動的函式型方法，更具成本效益並可依需求擴展。只需幾分鐘即可開始使用，請遵循[這份逐步指南](https://dev.to/azure/11-share-content-with-docusaurus-azure-static-web-apps-30hc)。

## 部署至 Kinsta {#deploying-to-kinsta}

[Kinsta 靜態網站託管服務](https://kinsta.com/static-site-hosting) 讓您免費部署最多 100 個靜態網站，包含自訂網域與 SSL、每月 100 GB 頻寬，以及 260+ 個 Cloudflare CDN 節點位置。

只需點擊幾下即可開始使用，請參閱我們的 [Kinsta 上的 Docusaurus](https://kinsta.com/docs/docusaurus-example/) 文章。