...
Docusaurus 的預設頁面外掛程式。經典模板預設配置了此外掛程式。此外掛程式提供[建立頁面](guides/creating-pages.mdx)功能。
...
若您使用 `@docusaurus/preset-classic` 預設配置，則無需將此外掛程式作為依賴項安裝。
...
Markdown 頁面可使用以下 Markdown [前置資料](../../guides/markdown-features/markdown-features-intro.mdx#front-matter)元資料欄位，並以 `---` 行包圍。
