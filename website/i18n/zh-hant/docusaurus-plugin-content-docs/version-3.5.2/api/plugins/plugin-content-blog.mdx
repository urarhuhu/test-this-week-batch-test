...
允許透過 [XSLT](https://developer.mozilla.org/en-US/docs/Web/XSLT) 為部落格 XML 訂閱源添加樣式，使瀏覽器能美觀渲染。

- 設為 `true` 時，部落格將使用內建的 `.xsl` 和 `.css` 文件為訂閱源添加樣式
- 設為假值（`undefined | null | false`）時將停用此功能
- 設為 `string` 時可指定部落格內容目錄下的自訂 `.xsl` 文件路徑。依慣例，需提供同名 `.css` 文件。
...
此檔案可包含預定義的 [全域部落格作者](../../blog.mdx#global-authors) 清單。透過 Markdown 文件中的 [`authors` 前置元數據](#markdown-front-matter) 可引用這些作者鍵值。
