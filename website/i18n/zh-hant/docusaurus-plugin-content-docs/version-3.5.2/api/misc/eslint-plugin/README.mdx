...
將 `plugin:@docusaurus/recommended` 加入你的 `.eslintrc` 設定檔的 `extends` 區段：
...
這將啟用 `@docusaurus` eslint 插件並使用 `recommended` 配置。請參閱下方的[支援規則](#supported-rules)以了解此配置會啟用的規則清單。
...
- 推薦配置：適用於大多數 Docusaurus 網站的推薦規則集，應從此處延伸。
- 全部規則：啟用**所有**規則。這會在次要版本間變更，因此若想避免意外的破壞性變更，不應使用此配置。
