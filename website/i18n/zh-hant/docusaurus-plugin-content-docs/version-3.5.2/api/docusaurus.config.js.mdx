...
若使用 [TypeScript](../typescript-support.mdx) 的 Docusaurus 程式碼庫，您的配置檔案可能命名為 `docusaurus.config.ts`。其語法與 `js` 配置檔案大致相同，但增加了類型定義。您可以在 [Docusaurus 官方網站](https://github.com/facebook/docusaurus/blob/main/website/docusaurus.config.ts)上查看範例。
...
網站的 URL。亦可視為頂層的主機名稱。例如，`https://facebook.github.io` 是 https://facebook.github.io/metro/ 的 URL，而 `https://docusaurus.io` 則是 https://docusaurus.io 的 URL。此欄位與 [`baseUrl`](#baseUrl) 欄位相關。
...
網站的基礎 URL。可視為主機名稱後的路徑。例如，`/metro/` 是 https://facebook.github.io/metro/ 的基礎 URL。對於沒有路徑的 URL，應將 `baseUrl` 設為 `/`。此欄位與 [`url`](#url) 欄位相關。開頭與結尾均需包含斜線。
...
網站圖示的路徑；必須為可用於連結 `href` 的 URL。例如，若您的圖示位於 `static/img/favicon.ico`：
...
允許自訂 URL/連結結尾是否包含斜線，以及靜態 HTML 檔案的生成方式：

- `undefined`（預設值）：保持 URL 不變，並為 `/docs/myDoc.md` 生成 `/docs/myDoc/index.html`
- `true`：為 URL/連結添加結尾斜線，並為 `/docs/myDoc.md` 生成 `/docs/myDoc/index.html`
- `false`：移除 URL/連結的結尾斜線，並為 `/docs/myDoc.md` 生成 `/docs/myDoc.html`
...
- `defaultLocale`: 該語言環境(1)不會在基礎URL中包含其名稱 (2)在未使用`--locale`選項時以`docusaurus start`啟動 (3)將用於`<link hrefLang="x-default">`標籤
- `locales`: 網站上部署的語言環境列表。必須包含`defaultLocale`。
- `path`: 所有語言環境文件夾的根目錄。可以是絕對路徑或相對於配置文件的相對路徑。預設為`i18n`。
- `localeConfigs`: 每個語言環境的個別選項。
  - `label`: 在語言環境下拉選單中顯示的標籤。
  - `direction`: `ltr`（預設）或`rtl`（適用於[從右至左的語言](https://developer.mozilla.org/en-US/docs/Glossary/rtl)如波斯語、阿拉伯語、希伯來語等）。用於選擇語言環境的CSS和HTML元屬性。
  - `htmlLang`: 用於`<html lang="...">`（或任何其他DOM標籤名稱）和`<link ... hreflang="...">`的BCP 47語言標籤
  - `calendar`: 用於計算日期紀元的[日曆](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/Locale/calendar)。注意它不控制實際顯示的字符串：`MM/DD/YYYY`和`DD/MM/YYYY`都屬於`gregory`。要選擇格式（`DD/MM/YYYY`或`MM/DD/YYYY`），請將語言環境名稱設為`en-GB`或`en-US`（`en`表示`en-US`）。
  - `path`: 該語言環境所有插件本地化文件夾的根目錄。將相對於`i18n.path`解析。預設為語言環境的名稱。注意：這不影響語言環境的`baseUrl`—基礎URL的定制正在開發中。

### `future` {#future}

- 類型: `Object`

`future`配置物件允許選擇加入即將推出/不穩定/實驗性的Docusaurus功能，這些功能尚未準備好正式使用。

這也是一種選擇加入下一個主要版本中即將到來的重大變更的方式，讓您可以在當前版本上為下一版本做好準備。[Remix Future Flags部落格文章](https://remix.run/blog/future-flags)很好地解釋了這一概念。

:::danger[次要版本中的重大變更]

帶有`experimental_`或`unstable_`前綴的功能可能在**次要版本**中變更，不被視為[語意化版本的重大變更](/community/release-process)。

帶有`v<MajorVersion>_`前綴的功能（`v6_`、`v7_`等）是未來標誌，預計在下一個主要版本中預設啟用。這些功能較不可能變更，但我們保留變更的可能性。

`future` API的重大變更應該易於處理，並會在次要/主要版本的部落格文章中記錄。
...
- `experimental_storage`: 全站瀏覽器儲存選項，主題開發者應盡量遵循。
  ...
  - `namespace`: 是否為瀏覽器儲存鍵添加命名空間，以避免當多個 Docusaurus 網站托管在同一網域或 localhost 時發生鍵衝突。可能值為 `string | boolean`。命名空間會附加在儲存鍵的末尾 `key-namespace`。使用 `true` 會根據網站 `url + baseUrl` 自動生成隨機命名空間。預設為 `false` (無命名空間，歷史行為)。
- `experimental_router`: 使用的路由類型。可能值為 `browser` 和 `hash`。預設為 `browser`。`hash` 路由僅適用於特殊情況，例如您希望退出靜態網站生成，擁有完全客戶端應用且僅有單一 `index.html` 入口文件。這對於將 Docusaurus 網站打包為 `.zip` 檔案並[無需運行網頁伺服器即可本地瀏覽](https://github.com/facebook/docusaurus/issues/3825)的情況很有用。
...
您伺服器的主機名稱。如果您使用 GitHub Enterprise，此設定會很有用。如果您不使用 `docusaurus deploy` 指令，則不需要此設定。
...
您伺服器的連接埠。如果您使用 GitHub Enterprise，此設定會很有用。如果您不使用 `docusaurus deploy` 指令，則不需要此設定。
...
將插入 HTML `<head>` 的標籤陣列。值必須是包含兩個屬性的物件：`tagName` 和 `attributes`。`tagName` 必須是決定所建立標籤的字串，例如 `"link"`。`attributes` 必須是屬性-值的對應。
...
使用 [Eta 語法](https://eta.js.org/docs/syntax#syntax-overview)編寫的 HTML 模板，用於渲染您的應用程式。可用於設定 `body` 標籤的自訂屬性、額外的 `meta` 標籤、自訂 `viewport` 等。請注意，Docusaurus 依賴模板的正確結構才能正常運作，一旦自訂模板，您必須確保模板符合上游的要求。
...
啟用後，當網站無法載入 CSS 或 JavaScript 檔案時，將顯示橫幅。這是一個非常常見的問題，通常與網站配置中的錯誤 `baseUrl` 有關。
...
![一個基本的 URL 問題橫幅範例。由於樣式表載入失敗，樣式非常原始。文字顯示「您的 Docusaurus 網站未正確載入...當前配置的 baseUrl = /（預設值）；我們建議嘗試 baseUrl = /build/](/img/baseUrlIssueBanner.png)

:::warning

此橫幅需要在所有資源載入因錯誤的 base URL 而失敗時內聯 CSS/JS。

如果您有嚴格的[內容安全策略](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP)，則應禁用此功能。
