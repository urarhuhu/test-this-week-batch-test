...
當需要在網站發布公告時，可添加公告欄。這是位於導航欄上方非固定且可選擇關閉的面板。所有配置皆位於`announcementBar`物件中。
...
標誌可置於[靜態資料夾](static-assets.mdx)中。標誌URL預設設為網站基礎URL。雖然可自訂標誌URL，但若為外部連結將在新分頁開啟。此外，可覆寫標誌連結的target屬性值，當文檔網站託管於主網站子目錄時特別有用，此情況下可能不需讓標誌連結至主網站時開啟新分頁。
...
您可以通過 `themeConfig.navbar.items` 向導航欄添加項目。
...
React Router 應自動為連結應用活動連結樣式，但在邊緣情況下可以使用 `activeBasePath`。對於連結應在多個不同路徑上處於活動狀態的情況（例如當您在同一側邊欄下有多個文檔文件夾時），可以使用 `activeBaseRegex`。`activeBaseRegex` 是 `activeBasePath` 的更靈活替代方案，並優先於它——Docusaurus 將其解析為正則表達式，並對當前 URL 進行測試。

出站（外部）連結會自動獲取 `target="_blank" rel="noopener noreferrer"` 屬性。
...
請注意，下拉選單的基礎項目也是一個可點擊的連結，因此該項目可以接收[普通導航欄連結](#navbar-link)的任何屬性。
...
如果您想連結到特定文檔，這種特殊的導航欄項目類型將渲染指向提供的 `docId` 的文檔連結。只要您瀏覽同一側邊欄的文檔，它就會獲取 `navbar__link--active` 類。
...
您可以將導航欄項目連結到給定側邊欄的第一個文檔連結（可以是文檔連結或生成的分類索引），而無需硬編碼文檔 ID。
...
若您使用版本化文檔功能，此特殊導航欄項目類型將渲染一個下拉選單，顯示您網站所有可用的版本。

使用者可在不同版本間切換，同時停留在同一份文檔上（只要該文檔ID在各版本間保持不變）。
...
#### 導航欄文檔版本連結 {#navbar-docs-version}

若您使用版本化文檔功能，此特殊導航欄項目類型將連結至當前瀏覽的文檔版本（根據URL判斷），若無則回退至最新版本。
...
您可以啟用此酷炫的UI功能：當使用者向下滾動頁面時自動隱藏導航欄，向上滾動時再次顯示。
...
目前提供兩種樣式選項：`dark`（深色）與`primary`（基於`--ifm-color-primary`主色）。樣式預覽請參閱[Infima說明文件](https://infima.dev/docs/components/navbar/)。
...
Docusaurus使用[Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer)實現程式碼區塊語法高亮。所有配置均位於`prism`物件中。
...
如果您使用行高亮標記語法，可能需要為深色模式語法高亮主題指定不同的高亮背景色。請參考[指引文件](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)。
...
您可以透過 `themeConfig.footer` 在頁尾添加標誌和版權聲明。標誌可放置於[靜態資料夾](static-assets.mdx)中。標誌 URL 的運作方式與導覽列標誌相同。
...
用於存取色彩模式的 React 鉤子。此上下文包含設定淺色與深色模式的函數，並公開布林變數顯示當前使用模式。
