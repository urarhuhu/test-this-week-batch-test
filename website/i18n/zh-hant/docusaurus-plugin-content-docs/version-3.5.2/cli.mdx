...
當您的網站初始化完成後，網站原始碼將包含可透過套件管理工具調用的 Docusaurus 腳本：
...
透過 [Webpack 開發伺服器](https://webpack.js.org/configuration/dev-server) 在本機建構並預覽您的網站。
...
清除 Docusaurus 網站生成的靜態資源、快取與建構產物。

建議在提交錯誤報告前、升級版本後，或任何遇到 Docusaurus 網站問題時執行此指令。
