...
本節與[樣式與佈局](./styling-layout.mdx)類似，但這次我們將客製化 React 元件本身，而非其外觀。我們將討論 Docusaurus 的核心概念：**元件替換 (swizzling)**，這項功能允許進行**更深層的網站客製化**。
...
該指令會在您的 `src/theme` 目錄下生成新元件，範例如下：
...
替換元件後，請**重新啟動開發伺服器**，以便 Docusaurus 識別新元件。

:::

:::warning[建議優先選擇安全方案]

請務必了解[哪些元件可安全替換](#what-is-safe-to-swizzle)。部分元件是主題的**內部實現細節**。
...
若要完整替換元件，請使用互動式 swizzle CLI 或搭配 `--eject` 參數：
...
此操作會將當前 `<Footer />` 元件的實作複製到您網站的 `src/theme` 目錄。Docusaurus 將使用此 `<Footer>` 副本而非原始元件。您現在可完全重新實作 `<Footer>` 元件。
...
完整替換[**非安全**](#what-is-safe-to-swizzle)元件時，可能會複製大量需自行維護的內部程式碼。這將增加 Docusaurus 升級難度，因為若接收的 props 或使用的內部主題 API 發生變更，您需自行遷移客製化內容。
...
在 Docusaurus 升級後，若要保持抽出的元件處於最新狀態，請重新執行抽出指令並使用 `git diff` 比較變更。建議在檔案頂部撰寫簡短註解說明您所做的修改，以便在重新抽出後能更輕鬆地重新套用您的變更。
...
若要包裝主題元件，請使用互動式混用 CLI 或搭配 `--wrap` 選項：
...
包裝主題是**在現有元件周圍添加額外元件**的絕佳方式，而無需[抽出](#ejecting)它。例如，您可以輕鬆在每篇部落格文章下方添加自訂評論系統：
...
## 哪些元件可以安全混用？ {#what-is-safe-to-swizzle}

> 能力越大，責任越大

某些主題元件是主題的**內部實作細節**。Docusaurus 允許您混用它們，但**可能具有風險**。
...
對於每個主題元件，混用 CLI 會標示主題作者宣告的**3種不同安全等級**：

- **安全**：此元件可安全混用，其公開 API 被視為穩定，且在主題**主要版本**內不應發生破壞性變更
- **不安全**：此元件是主題實作細節，不建議混用，且在主題**次要版本**中可能發生破壞性變更
- **禁止**：混用 CLI 會阻止您混用此元件，因為它根本不設計用於混用

:::note

某些元件可能安全用於包裝，但不安全用於抽出。

:::

:::info

不必過於**害怕混用不安全元件**：只需記住可能發生**破壞性變更**，且您可能需要在次要版本升級時手動更新自訂內容。

:::

:::note[回報使用案例]

如果您有**強烈需求需混用不安全元件**，請[**在此回報**](https://github.com/facebook/docusaurus/discussions/5468)，我們將共同尋找解決方案使其安全化。

:::

## 我應該混用哪個元件？ {#which-component-should-i-swizzle}

要達成預期效果，通常不清楚究竟該混用哪個元件。提供大多數主題元件的 `@docusaurus/theme-classic` 約有[100個元件](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-theme-classic/src/theme)！
...
1. **元件描述**：部分元件提供簡短說明，這是快速找到合適元件的好方法。
2. **元件名稱**：官方主題元件皆採用語義化命名，您可從名稱推斷其功能。Swizzle CLI 支援輸入部分元件名稱來篩選選項。例如執行 `yarn swizzle @docusaurus/theme-classic` 後輸入 `Doc`，將只列出與文檔相關的元件。
3. **從高階元件開始**：元件會形成樹狀結構，部分元件會引入其他元件。每條路由都會關聯一個最頂層的渲染元件（多數列於[內容外掛的路由系統](./advanced/routing.mdx#routing-in-content-plugins)）。例如所有部落格文章頁面的最頂層元件都是 `@theme/BlogPostPage`。您可以先swizzle此元件，再向下追蹤元件樹找到目標元件。找到正確元件後，請刪除其他swizzle過的檔案以避免維護過多元件。
4. **閱讀[主題原始碼](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-theme-classic/src/theme)**並善用搜尋功能。

:::tip[直接提問！]

若仍無法確定該swizzle哪個元件來實現需求，您可透過[支援管道](/community/support)尋求協助。

我們也希望更了解您的進階客製化需求，歡迎[提交案例討論](https://github.com/facebook/docusaurus/discussions/5468)。
...
2. **使用翻譯功能**：雖然看似意外，但翻譯本質上也是客製化文字標籤的方式。例如若網站預設語言為`en`，仍可執行`yarn write-translations -l en`後編輯生成的`code.json`，詳見[i18n教學](./i18n/i18n-tutorial.mdx)。

:::tip

**範圍越小越好**：若必須swizzle，請僅針對相關部分操作，並盡量減少自行維護的程式碼量。swizzle小型元件通常能降低升級時的**重大變更**風險。

相較於[完整抽離](#ejecting)，[封裝元件](#wrapping)是更安全的選擇。

:::

## 使用`<Root>`封裝您的網站 {#wrapper-your-site-with-root}

`<Root>`元件會渲染在React樹的**最頂層**，位於主題`<Layout>`之上且**永不卸載**，非常適合用來添加不該因導航而重新初始化的狀態邏輯（使用者認證狀態、購物車狀態等）。
