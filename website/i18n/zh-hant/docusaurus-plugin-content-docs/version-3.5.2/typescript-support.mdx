...
Docusaurus 支援編寫和使用 TypeScript 主題元件。若初始化模板提供 TypeScript 變體，您可直接透過 `--typescript` 參數初始化一個完整支援 TypeScript 的網站。
...
Docusaurus 不會使用此 `tsconfig.json` 來編譯專案。其存在僅為提供更完善的編輯器體驗，但您仍可選擇執行 `tsc` 來自行或在 CI 環境中進行型別檢查。
...
最佳 IDE（如 VS Code、WebStorm、IntelliJ...）將提供優異的自動完成體驗。

:::

## 替換 TypeScript 主題元件 {#swizzling-typescript-theme-components}

對於支援 TypeScript 主題元件的主題，您可在 `swizzle` 指令後方加入 `--typescript` 參數來取得 TypeScript 原始碼。例如以下指令會在 `src/theme/Footer` 目錄下產生 `index.tsx` 和 `styles.module.css`。
