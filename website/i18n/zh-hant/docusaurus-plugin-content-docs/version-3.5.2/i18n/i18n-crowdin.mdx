...
Crowdin 提供一個 [CLI](https://support.crowdin.com/cli-tool/) 來**上傳來源文件**和**下載翻譯**，讓您可以自動化翻譯流程。
...
您可以保留預設值 `CROWDIN_PERSONAL_TOKEN`，並將此環境變數設定在您的電腦和 CI 伺服器上，值為產生的存取權杖。
...
- `preserve_hierarchy`：在 Crowdin 介面中保留文件夾層級結構，而非扁平化顯示

### 安裝 Crowdin CLI {#install-the-crowdin-cli}

本教學使用 CLI 版本 `3.5.2`，但預期 `3.x` 版本均可正常運作。

將 Crowdin CLI 作為 npm 套件安裝至您的 Docusaurus 專案：
...
為預設語言產生 JSON 翻譯檔案至 `website/i18n/en`：
...
Crowdin 不支援多個同時進行的上傳/下載：最好僅在生產部署中包含翻譯，並保持部署預覽未翻譯狀態。
...
Crowdin 會將 JSX 語法視為嵌入式 HTML，並可能在翻譯下載時破壞 JSX 標記，導致網站因無效的 JSX 而無法建置。

使用簡單字串屬性的簡單 JSX 片段（如 `<Username name="Sebastien"/>`）可以正常運作；而使用物件/陣列屬性的複雜 JSX 片段（如 `<User person={{name: "Sebastien"}}/>`）則更容易因語法不像 HTML 而失敗。
...
當建立新版本時，原始字串通常與當前版本 (`website/docs`) 非常相似，您不會希望反覆翻譯新版本的文件。
...
當您的原始檔被重構時，您應該使用 Crowdin UI **手動更新 Crowdin 檔案**：
...
遺憾的是，由於技術原因目前尚無法正常運作，但我們相信未來可以解決這個問題。

Crowdin 會將 Markdown 字串替換為技術性 ID（如 `crowdin:id12345`），但替換過程過於激進，包括隱藏字串，並會破壞 front matter、admonitions 和 JSX...
...
Crowdin 近期對 Markdown 檔案格式進行重大變更，連結的處理方式與以往不同。過去連結被視為標籤，但現在顯示為純文字。由於這些變更，純文字連結會被傳送至 MT 引擎嘗試翻譯目標網址，導致翻譯錯誤（例如：字串 `Allez voir [ma merveilleuse page](/ma-merveilleuse-page)` 被翻譯為 `Check out [my wonderful page](/my-wonderful-page)`，這會破壞 docusaurus 的國際化工作流程，因為頁面名稱不應被翻譯）。

截至 2023 年 12 月 7 日，Crowdin 不計劃變更連結的處理邏輯，因此如果您計劃將 Crowdin 與 MT 搭配使用，請注意此問題。