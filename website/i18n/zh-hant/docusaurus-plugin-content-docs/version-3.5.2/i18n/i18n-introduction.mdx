...
透過國際化([i18n](https://en.wikipedia.org/wiki/Internationalization_and_localization))支援，**翻譯Docusaurus網站非常容易**。

## 目標 {#goals}

理解Docusaurus國際化支援背後的**設計決策**非常重要。
...
- **簡單**：只需將翻譯文件放在正確的檔案系統位置
- **靈活的翻譯流程**：可使用Git(單一倉庫、分支或子模組)、SaaS軟體、FTP
- **靈活的部署選項**：單一網域、多重網域或混合部署
- **模組化**：允許插件作者提供國際化支援
- **低運行開銷**：文檔主要是靜態內容，不需要繁重的JS函式庫或polyfill
- **可擴展的建置時間**：允許獨立建置和部署本地化網站
- **本地化資源**：網站圖片可能包含需要翻譯的文字
- **無耦合性**：不強制使用任何SaaS，但可整合
- **易於與[Crowdin](https://crowdin.com/)搭配使用**：許多Docusaurus v1網站使用Crowdin，應能遷移至v2
- **良好的SEO預設值**：我們為您設置有用的SEO標頭，如[`hreflang`](https://developers.google.com/search/docs/advanced/crawling/localized-versions)
- **RTL支援**：支援從右至左閱讀的語言(阿拉伯語、希伯來語等)且易於實作
...
建立翻譯版Docusaurus網站的工作流程概述：

1. **配置**：在[`docusaurus.config.js`](../api/docusaurus.config.js.mdx#i18n)中聲明預設語言和替代語言
2. **翻譯**：將翻譯文件放在正確的檔案系統位置
3. **部署**：使用單一或多網域策略建置並部署您的網站

### 翻譯文件 {#translation-files}

您將處理三種翻譯文件。

#### Markdown文件 {#markdown-files}

這是您Docusaurus網站的主要內容。

Markdown和MDX文檔需整體翻譯，以完整保留翻譯上下文，而非將每個句子拆分為獨立字串。

#### JSON文件 {#json-files}

JSON用於翻譯：

- 您的React程式碼：`src/pages`中的獨立React頁面或其他元件
- 透過`themeConfig`提供的佈局標籤：導覽列、頁尾
- 透過插件選項提供的佈局標籤：文檔側邊欄分類標籤、部落格側邊欄標題...

使用的JSON格式稱為**Chrome i18n**：
...
翻譯一個簡單的法語版Docusaurus網站會生成如下目錄結構：
...
JSON檔案可通過[`docusaurus write-translations`](../cli.mdx#docusaurus-write-translations-sitedir) CLI命令初始化。每個插件在對應資料夾下管理自己的翻譯內容，而`code.json`檔案則定義React代碼中使用的所有文本標籤。

每個內容插件或主題都有**獨立的翻譯檔案存放規則**：
