...
[Front Matter](./guides/markdown-features/markdown-features-intro.mdx#front-matter) 可用於為部落格文章添加更多元數據，例如作者資訊，但 Docusaurus 也能在沒有 front matter 的情況下推斷所有必要的元數據。所有可能的欄位請參閱 [API 文件](api/plugins/plugin-content-blog.mdx#markdown-front-matter)。

## 部落格列表 {#blog-list}

部落格的索引頁面（預設位於 `/blog`）是 _部落格列表頁面_，所有部落格文章會在此集中顯示。

在部落格文章中使用 `<!--truncate-->` 標記來表示在查看所有已發布部落格文章時將顯示的摘要。`<!--truncate-->` 上方的所有內容將成為摘要的一部分。請注意，截斷標記上方的部分必須是可獨立渲染的 Markdown。例如：
...
部落格側邊欄會顯示最近的部落格文章。預設顯示的項目數量為 5，但您可以透過插件配置中的 `blogSidebarCount` 選項自訂。設定 `blogSidebarCount: 0` 會完全停用側邊欄，並移除容器，這將增加主容器的寬度。特別地，如果設定 `blogSidebarCount: 'ALL'`，則會顯示 _所有_ 文章。

您也可以透過 `blogSidebarTitle` 選項修改側邊欄的標題文字。例如，如果設定 `blogSidebarCount: 'ALL'`，您可以將預設的「最近文章」改為「所有文章」：
...
- 為每位作者建立專屬頁面（[範例](/blog/authors/slorber)），列出其貢獻的所有部落格文章
- 作者索引頁面（[範例](/blog/authors)），按`authors.yml`中的順序列出所有作者

:::warning[關於內嵌作者]

僅[全域作者](#global-authors)可使用此功能。[內嵌作者](#inline-authors)不支援此特性。

:::

## 部落格文章標籤 {#blog-post-tags}

標籤需在前言資料中宣告，為文章分類提供另一種維度。

您可以直接定義內嵌標籤，或引用預先在[`標籤檔案`](api/plugins/plugin-content-blog.mdx#tags-file)（通常為`blog/tags.yml`）中宣告的標籤。

以下範例中：

- `docusaurus`引用自`blog/tags.yml`中預定義的標籤鍵
- `Releases`是內嵌標籤，因其未在`blog/tags.yml`中定義
...
Docusaurus會根據字數為每篇部落格文章生成閱讀時間估算。我們提供選項供您自訂此功能。
...
`readingTime`回調函數接收三個參數：部落格內容文字（字串形式）、前言資料（鍵值對記錄）以及預設閱讀時間計算函數。其返回值為數字（閱讀分鐘數）或`undefined`（禁用該頁面的閱讀時間顯示）。

預設閱讀時間計算可接受附加選項：`wordsPerMinute`（數字，預設值300）和`wordBound`（字串到布林值的函數）。當傳入`wordBound`的字串應被視為單詞邊界（預設為空格、製表符和換行符）時，該函數應返回`true`。
...
可透過設定 `feedOptions` 來生成 RSS/Atom/JSON 訂閱源。預設會生成 RSS 和 Atom 訂閱源。若要停用訂閱源生成，請將 `feedOptions.type` 設為 `null`。
...
您可以讓 Docusaurus 網站不設獨立首頁，直接將部落格文章列表頁設為首頁。將 `routeBasePath` 設為 `'/'`，即可讓部落格透過根路由 `example.com/` 而非子路由 `example.com/blog/` 提供服務。
...
若停用文件插件，請記得同時刪除設定檔中其他參照文件插件的地方。特別注意需移除導覽列中與文件相關的項目。

:::

:::tip

另有「純文件模式」供僅需使用文件功能的用戶。詳見[純文件模式](./guides/docs/docs-introduction.mdx)獲取詳細說明或更深入的 `routeBasePath` 解釋。

:::

### 多重部落格 {#multiple-blogs}

預設情況下，經典主題假設每個網站僅有一個部落格，因此僅包含一個部落格插件實例。若您想在單一網站上設置多個部落格，同樣可行！您可以在 `docusaurus.config.js` 的 `plugins` 選項中指定另一個部落格插件來新增部落格。

將 `routeBasePath` 設為您希望第二個部落格被訪問的 URL 路由。請注意，此處的 `routeBasePath` 必須與第一個部落格不同，否則可能發生路徑衝突！同時，將 `path` 設為包含您第二個部落格條目的目錄路徑。
