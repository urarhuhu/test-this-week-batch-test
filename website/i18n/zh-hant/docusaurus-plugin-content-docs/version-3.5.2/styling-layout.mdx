...
本節重點介紹通過樣式表進行樣式設定。如需更高級的自訂（DOM結構、React代碼等），請參考[元件替換指南](./swizzling.mdx)。

:::

Docusaurus 網站是一個單頁 React 應用程式。您可以像為 React 應用程式設定樣式一樣為其設定樣式。

根據您的偏好和您嘗試建立的網站類型，有幾種可行的方法/框架。對於高度互動且行為更像網路應用程式的網站，現代化的樣式設定方法（將樣式與元件放在一起）會更有優勢。當您希望自訂或替換元件時，元件樣式設定也特別有用。
...
您在該檔案中編寫的任何 CSS 都將全域可用，並可以直接使用字串字面量引用。
...
- **Infima 類別名稱**。這些類別名稱出現在經典主題中，通常遵循 [BEM 命名慣例](http://getbem.com/naming/)的 `block__element--modifier` 格式。它們通常是穩定的，但仍被視為實現細節，因此通常應避免針對它們。不過，您可以[修改 Infima CSS 變數](#styling-your-site-with-infima)。
- **CSS 模組類別名稱**。這些類別名稱在生產環境中帶有雜湊值（`codeBlockContainer_RIuc`），在開發環境中則附加了長檔案路徑。它們被視為實現細節，在自訂 CSS 中幾乎應始終避免針對它們。如果必須使用，可以使用[屬性選擇器](https://developer.mozilla.org/en-US/docs/Web/CSS/Attribute_selectors)（`[class*='codeBlockContainer']`）來忽略雜湊值。
...
如果找不到建立穩健 CSS 選擇器的方法，請[報告您的自訂用例](https://github.com/facebook/docusaurus/discussions/5468)，我們將考慮添加新的類別名稱。
...
`@docusaurus/preset-classic` 使用 [Infima](https://infima.dev/) 作為底層樣式框架。Infima 提供了適合以內容為中心的網站（部落格、文檔、登陸頁面）的靈活佈局和常見 UI 元件樣式。更多詳情，請查看 [Infima 網站](https://infima.dev/)。

當您使用 `create-docusaurus` 建立 Docusaurus 專案時，網站將使用基本的 Infima 樣式表和預設樣式生成。您可以全域覆蓋 Infima CSS 變數。
...
Docusaurus 使用 `996px` 作為行動裝置螢幕寬度與桌面的分界點。若您希望在行動裝置視圖中有不同的佈局，可以使用媒體查詢。
...
某些 React 元件（如標頭和側邊欄）在行動裝置視圖中會實作不同的 JavaScript 邏輯。如果您在自訂 CSS 中變更了斷點值，可能還需要透過[替換元件](./swizzling.mdx)來更新 `useWindowSize` 鉤子的調用，並傳遞明確的選項參數。

:::

## CSS 模組 {#css-modules}

若要使用 [CSS 模組](https://github.com/css-modules/css-modules)為元件設定樣式，請將樣式表檔案命名為 `.module.css` 後綴（例如 `welcome.module.css`）。Webpack 會將此類 CSS 檔案作為 CSS 模組載入，您需將類別名稱作為導入 CSS 模組的屬性來引用（而非使用純字串）。這與 [Create React App](https://facebook.github.io/create-react-app/docs/adding-a-css-modules-stylesheet) 中使用的慣例類似。
...
將你的樣式表文件命名為 `.module.scss` 後綴（例如 `welcome.module.scss`）而不是 `.css`。Webpack 將使用 `sass-loader` 預處理你的樣式表並將其加載為 CSS 模塊。
