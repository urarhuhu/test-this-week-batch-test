...
我們維護了一份[官方插件清單](./api/plugins/overview.mdx)，社群也開發了部分[非官方插件](/community/resources#community-plugins)。若您想新增任何功能：自動生成文檔頁面、執行自訂腳本、整合其他服務... 請務必查閱該清單：可能已有現成解決方案！

若您精力充沛，也可參閱[插件開發指南](./advanced/plugins.mdx)或[插件方法參考文檔](./api/plugin-methods/README.mdx)來學習如何自行開發插件。
...
最基本的插件使用方式，您僅需提供插件名稱或插件路徑即可。

但插件可透過在配置中以雙成員元組形式（包含插件名稱與選項物件）來指定選項。此風格通常稱為「Babel 風格」。
...
最多只能有一個插件實例作為「預設插件實例」，透過省略 `id` 屬性或使用 `id: 'default'` 來實現。

:::

## 使用主題 {#using-themes}

主題的載入方式與插件完全相同。從使用者角度來看，在安裝和配置插件時，`themes` 與 `plugins` 條目可互換使用。唯一細微差別在於主題會在插件之後載入，且[主題可覆蓋插件的預設主題元件](./advanced/client.mdx#theme-aliases)。
...
預設集是一個與[插件建構函式](./api/plugin-methods/README.mdx#plugin-constructor)結構相同的函式。它應返回一個 `{ plugins: PluginConfig[], themes: PluginConfig[] }` 物件，格式與網站設定中接受的格式相同。例如，您可以指定一個包含以下主題與插件的預設集：
...
其中 `moduleType` 為 `'preset'`、`'theme'` 或 `'plugin'` 之一，取決於模組名稱宣告在哪個欄位中。系統會載入第一個成功找到的模組名稱。

若名稱具有作用域（以 `@` 開頭），則會先以第一個斜線將名稱拆分為作用域與套件名稱：
