...
主要的重大變更是從 MDX v1 升級至 MDX v3。詳情請閱讀 [**MDX v2**](https://mdxjs.com/blog/v2/) 和 [**MDX v3**](https://mdxjs.com/blog/v3/) 的版本說明。MDX 現在會以**更嚴格**的方式編譯您的 Markdown 內容，並存在**細微差異**。
...
對於複雜的網站，我們還建議設定[**視覺回歸測試**](/blog/upgrading-frontend-dependencies-with-confidence-using-visual-regression-testing)，這是確保網站視覺效果保持不變的好方法。Docusaurus v3 主要升級了相依項目，預計不會產生任何視覺變更。

:::

:::note

請查閱 [**Docusaurus v3.0.0**](https://github.com/facebook/docusaurus/releases/tag/v3.0.0) 的版本說明，並瀏覽 pull-requests 以獲取更多有用資訊，以及了解此處提到的每個變更背後的動機。
...
如果您的網站使用了第三方社群外掛和主題，您可能需要升級它們。

在嘗試升級之前，請確保這些外掛與 Docusaurus v3 相容。
...
此命令也是估算需要完成多少工作以使您的內容相容的好方法。請記住，大部分工作可以通過[為 Docusaurus v3 準備您的內容](/blog/preparing-your-site-for-docusaurus-v3)提前執行。
...
升級至 MDX 將伴隨 [MDX v2](https://mdxjs.com/blog/v2/) 與 [MDX v3](https://mdxjs.com/blog/v3/) 發佈公告中記載的所有重大變更。大多數變更來自 MDX v2，而 MDX v3 是一個相對較小的版本。[MDX v2 遷移指南](https://mdxjs.com/migrating/v2/)中有一節關於如何[更新 MDX 文件](https://mdxjs.com/migrating/v2/#update-mdx-files)的內容對我們特別重要。同時請務必閱讀[MDX 疑難排解](https://mdxjs.com/docs/troubleshooting-mdx/)頁面，這能幫助您解讀常見的 MDX 錯誤訊息。

請同時閱讀我們更新後的 [**MDX 與 React**](../guides/markdown-features/markdown-features-react.mdx) 文件頁面。

### 使用 MDX 遊樂場

MDX 遊樂場是您的新好朋友。它能幫助您理解內容如何**被編譯為 React 元件**，並在隔離環境中排查編譯或渲染問題。
...
目標是重構有問題的內容，使其**能同時在兩個 MDX 版本中正常運作**。這樣當您升級至 Docusaurus v3 時，這些內容就能直接無縫運作。

:::

### 使用 MDX 檢查器 CLI

我們提供了一個 [docusaurus-mdx-checker](https://github.com/slorber/docusaurus-mdx-checker) CLI 工具，可輕鬆找出有問題的內容。在您的網站上執行此命令，即可獲得一份在 MDX v3 下會編譯失敗的文件清單。
...
然而，透過升級幾個 Docusaurus 網站，我們注意到大多數問題可歸納為幾種我們已為您記錄的案例。

#### 錯誤使用 `{`

`{` 字符用於開啟 [JavaScript 表達式](https://mdxjs.com/docs/what-is-mdx/#expressions)。若您置於 `{expression}` 中的內容不是有效表達式，MDX 現在會報錯。
...
字元 `<` 用於開啟 [JSX 標籤](https://mdxjs.com/docs/what-is-mdx/#jsx)。若 MDX 認為你的 JSX 無效，現在將會失敗。
...
> 在名稱前出現意外字元 `5` (U+0035)，預期應為可開始名稱的字元，如字母、`$` 或 `_`
...
> 名稱中出現意外字元 `@` (U+0040)，預期應為名稱字元如字母、數字、`$` 或 `_`；屬性前的空白；或標籤的結尾（注意：在 MDX 中建立連結，請使用 `[text](url)`）

> 在本地名稱前出現意外字元 `/` (U+002F)，預期應為可開始名稱的字元，如字母、`$` 或 `_`（注意：在 MDX 中建立連結，請使用 `[text](url)`）
...
對於提供 [自訂 `MDXComponent` 映射](../guides/markdown-features/markdown-features-react.mdx#mdx-component-scope) 的使用者，元件現在被「沙箱化」：

- `MDXComponent` 映射的 `h1` 僅用於 `# hi` 而非 `<h1>hi</h1>`
- **小寫**的自訂元件名稱將不再被其對應的 `MDXComponent` 元件替換

:::danger[視覺差異]

你的 [`MDXComponent` 元件映射](../guides/markdown-features/markdown-features-react.mdx#mdx-component-scope)可能不會像以前那樣應用，且你的自訂元件可能不再被使用。
...
新的 MDX 解析器現在嚴格遵循 CommonMark 規範。自 v0.14 起，CommonMark 規範針對空格和標點符號周圍的強調文字引入了新規則，這尤其會影響不使用空格分隔單詞的語言。

日文和中文受此影響最大，但其他某些語言（如泰文和高棉文）也可能受到影響，例如當您嘗試強調行內代碼或連結時。使用空格分隔單詞的語言受影響程度較小。

以下範例中的 `**`（除了 `` `**` `` 外）在 Docusaurus 2 中能正常解析，但在 Docusaurus 3 中已無法正常運作。
...
這實際上意味著您不能再使用 `require("remark-plugin")` 的寫法。

:::tip[升級方法]

Docusaurus v3 現在支援 [ES Modules](https://flaviocopes.com/es-modules/) 格式的設定檔。我們建議您將設定檔遷移至 ES 模組，這樣可以輕鬆導入 Remark 插件：
...
如果您想繼續使用 CommonJS 模組，可以使用動態導入作為變通方案，這允許您在 CommonJS 模組中導入 ES 模組。幸運的是，[Docusaurus 設定支援使用非同步函式](/docs/configuration#syntax-to-declare-docusaurus-config)來實現這一點。
...
如果您創建了自定義的 Remark 或 Rehype 插件，由於新 AST 的結構變化，您可能需要重構這些插件，甚至完全重寫。我們創建了一個[專屬支援討論串](https://github.com/facebook/docusaurus/discussions/9337)來幫助插件開發者升級他們的代碼。

:::

### 格式化工具

最常見的格式化工具 Prettier 目前僅支援舊版 MDX v1，截至 Docusaurus v3.0.0 尚未支援 v3。您可以在代碼不相容的部分前添加 `{/* prettier-ignore */}` 來使其與 Prettier 相容。
...
同時，請配置您的持續整合(CI)、內容傳遞網路(CDN)或主機服務以使用此新版本 Node.js。
...
React 18 自帶一些破壞性變更，但根據您為網站自訂的 React 程式碼量，處理起來應該相對容易。官方主題和外掛程式均已相容 React 18。
...
這是包含破壞性變更的新主版本，無法保證嚴格向後相容。雖然 [`prism-react-renderer` v2 發行說明](https://github.com/FormidableLabs/prism-react-renderer/releases/tag/prism-react-renderer%402.0.0)不夠詳盡，但 Docusaurus 用戶需注意以下 3 項重大變更。
...
對於使用 `@docusaurus/theme-live-codeblock` 套件的用戶，Docusaurus v3 將 [`react-live`](https://github.com/FormidableLabs/react-live) 升級至 v4.0+ 版本。
...
多數 Docusaurus 使用者無需任何操作。若有使用表情符號快捷碼，請查閱 [變更日誌](https://github.com/rhysd/remark-emoji/blob/master/CHANGELOG.md) 並確認表情符號仍能正常顯示。

> **破壞性變更** 將 [node-emoji](https://www.npmjs.com/package/node-emoji) 從 v1 升級至 v2。此變更新增了許多新表情符號支援，同時移除了 GitHub 上已失效的舊表情符號代碼。
...
此新套件版本將與其他 Docusaurus 核心套件同步更新，用於確保 TypeScript 的向下相容性與重大版本升級時的破壞性變更管理。
...
理論上，您無需進行任何操作，現有的設定檔應能如常運作。

但請注意，這是一次核心依賴庫的重大替換，可能導致細微的行為變化。

:::

### 警告型提示框注意事項

基於歷史因素，我們曾支援未公開的 `:::warning` 提示框樣式（實際渲染為紅色）。

:::danger[警告]

此為 Docusaurus v2 的 `:::warning` 提示框樣式。

:::

但該樣式的顏色與圖標始終存在錯誤。Docusaurus v3 正式重新引入 `:::warning` 提示框，修正顏色與圖標並納入文件。

:::warning

此為 Docusaurus v3 的 `:::warning` 提示框樣式。

:::

:::info[如何升級]

若您曾使用未公開的 `:::warning` 提示框，請逐一確認黃色是否適用於現有場景。如需維持紅色警示，請改用 `:::danger`。

Docusaurus v3 同時[棄用 `:::caution`](https://github.com/facebook/docusaurus/pull/9308) 提示框。請將現有 `:::caution`（黃色）重構為 `:::warning`（黃色）或 `:::danger`（紅色）。
...
此變更僅影響 **在 v2.0.0-beta.10 版（2021年12月）前啟用版本化功能** 的早期使用者。

當建立 v1.0.0 版本時，側邊欄檔案包含 `version-v1.0.0/` 前綴，[此格式在 Docusaurus v3 已不再支援](https://github.com/facebook/docusaurus/pull/9310)。
...
`@docusaurus/plugin-content-blog` 現在預設將 RSS 饋送限制為最近20篇文章。對於大型部落格站點，此為更合理的預設值，可避免饋送檔案過度膨脹。
...
對於曾替換（swizzle）文件相關主題元件（如 `@theme/DocPage`）的使用者，這些元件已進行重大重構以提升客製化便利性。

技術上**這不屬於破壞性變更**，因這些元件**標記為不建議替換**。但許多站點已替換文件相關元件，需注意自訂程式碼可能導致相容性問題。

:::info[如何升級]

刪除所有已替換的元件，重新執行替換操作，並在新版元件基礎上重新套用自訂修改。

或參考[拉取請求說明](https://github.com/facebook/docusaurus/pull/7966)了解新版主題元件樹結構，嘗試手動調整現有自訂元件。

:::

## 可選調整項目

以下變更非強制性，但了解這些特性可充分發揮 Docusaurus v3 優勢。
...
我們建議當你在 Markdown 檔案中使用 JSX、`import` 或 `export`（即 MDX 功能）時，使用 `.mdx` 副檔名。這在語義上更正確，並提高了與外部工具（IDE、格式化工具、linter 等）的兼容性。

在未來版本的 Docusaurus 中，`.md` 檔案將被解析為標準的 [CommonMark](https://commonmark.org/)，這不支援這些功能。在 Docusaurus v3 中，`.md` 檔案仍被編譯為 MDX 檔案，但可以[選擇使用 CommonMark](https://github.com/facebook/docusaurus/issues/3018)。

### 升級數學套件

如果你使用 Docusaurus 來渲染[數學方程式](../guides/markdown-features/markdown-features-math-equations.mdx)，你應該升級 MDX 插件。

確保在 Docusaurus v3（使用 MDX v3）中使用 `remark-math 6` 和 `rehype-katex 7`。我們無法保證其他版本能正常工作。
...
`hast-util-is-element` 在 Docusaurus v3 中不再需要。如果你已安裝它且未在其他地方使用，可以通過運行 `npm uninstall hast-util-is-element` 來移除它。
...
Docusaurus 現在使用 [Markdown 指令](https://talk.commonmark.org/t/generic-directives-plugins-syntax/444)（通過 [remark-directive](https://github.com/remarkjs/remark-directive) 實現）來實現警告，提供指令標籤的語法需要方括號：
...
此語法現在是無效的 MDX，需要轉義 `{` 字符：`{#my-explicit-id}`。
...
- 提供**最簡化**重現步驟，建議使用 [docusaurus.new](https://docusaurus.new) 建立範例
- 若網站能成功建置，請提供線上展示問題的部署網址
- 清楚描述問題，避免僅模糊表示「無法運作」
- 附上完整相關資料：程式碼片段、儲存庫網址、Git 分支連結、完整堆疊追蹤、截圖與影片
- 以清晰簡潔的方式呈現請求，展現您已盡力自助

您也可考慮付費請 [Docusaurus 服務供應商](https://github.com/facebook/docusaurus/discussions/9281) 代為升級。若專案為開源，亦可於社群尋求 [免費友善協助](https://github.com/facebook/docusaurus/discussions/9283)。