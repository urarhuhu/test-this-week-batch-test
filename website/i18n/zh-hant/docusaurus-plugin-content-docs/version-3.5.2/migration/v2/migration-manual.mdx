...
此手動遷移流程應在[自動遷移流程](./migration-automated.mdx)之後執行，以補全缺失部分或除錯遷移 CLI 輸出的問題。
...
這能明確區分 Docusaurus 官方套件與社群維護套件。換言之，所有 Docusaurus 官方套件皆歸屬於 `@docusaurus/` 命名空間下。
...
此外，CLI 指令更名為 `docusaurus <command>`（取代原先的 `docusaurus-command`）。
...
在 Docusaurus 1 中，所有建置產出物位於 `website/build/<PROJECT_NAME>`。
...
## 網站配置 {#site-configurations}

### `docusaurus.config.js` {#docusaurusconfigjs}

將 `siteConfig.js` 重新命名為 `docusaurus.config.js`。

Docusaurus 2 將各功能（部落格、文件、頁面）拆分為插件以實現模組化。預設集為插件組合包，為保持向後相容性，我們建立了 `@docusaurus/preset-classic` 預設集，其中包含 Docusaurus 1 中的核心插件組合。

請在 `docusaurus.config.js` 中添加以下預設集配置：
...
我們建議將 `docs` 資料夾移至 `website` 資料夾內，這也是 v2 的預設目錄結構。[Vercel](https://vercel.com) 支援[開箱即用的 Docusaurus 專案部署](https://github.com/vercel/vercel/tree/main/examples/docusaurus)，前提是 `docs` 目錄需位於 `website` 內。將文檔與網站程式碼集中存放在單一 `website` 目錄下，通常能帶來更好的協作體驗。

若您正在遷移 Docusaurus v1 網站且存在待處理的文檔拉取請求(pull requests)，可暫時保留 `/docs` 資料夾於原始位置，以避免產生衝突。
...
已棄用。我們為 Docusaurus 2 開發了名為 [Infima](https://infima.dev/) 的 CSS 框架，採用 CSS 變數實現主題化。目前文檔尚未完備，後續將更新指引。若要覆寫 Infima 的 CSS 變數，請建立自訂 CSS 檔案（如 `./src/css/custom.css`），並通過 `@docusaurus/preset-classic` 的選項全局導入：
...
在 Docusaurus 1 中，標題圖示與導航連結是 `siteConfig` 的根欄位：
...
已棄用。請改為通過 `@docusaurus/preset-classic` 的部落格選項傳遞：
...
- `blogSidebarTitle`
- `cleanUrl` - 現在預設使用簡潔網址。
- `defaultVersionShown` - 版本功能尚未移植。若您使用版本功能，將無法遷移至 Docusaurus 2。請持續關注。
- `disableHeaderTitle`
- `disableTitleTagline`
- `docsSideNavCollapsible` 現在可在 `docsPluginOptions.sidebarCollapsible` 中設定，且預設為啟用。
- `facebookAppId`
- `facebookComments`
- `facebookPixelId`
- `fonts`
...
若您在 v1 中設定 `cleanUrl: false`，可能有人已發布了指向 `/installation.html` 的連結。

為了 SEO 及避免連結失效，您應在託管服務商處設定伺服器端重新導向規則。
...
舊版本不支援巢狀側邊欄分類，且分類只能包含文件 ID。而 v2 允許無限層級巢狀結構，並提供多種[側邊欄項目類型](../../guides/docs/sidebar/items.mdx)（不局限於文件）。
...
不再需要 `website/core/Footer.js`。若要修改 Docusaurus 預設頁尾，請透過[元件替換](../../swizzling.mdx)功能：
...
請勿僅為在左側新增標誌而替換頁尾元件。v2 版刻意移除左側標誌並移至底部，請直接在 `docusaurus.config.js` 中透過 `themeConfig.footer` 設定：
...
請參閱[建立頁面](guides/creating-pages.mdx)了解 Docusaurus 2 的頁面運作機制。閱讀後請注意：需將 v1 版中的 `pages/en` 檔案移至 `src/pages`。

Docusaurus v1 的頁面會接收 `siteConfig` 物件作為 props。

Docusaurus v2 需改用 `useDocusaurusContext` 取得 `siteConfig` 物件。

v2 版需在每個頁面外層套用主題佈局元件。Layout 元件接受元資料 props。

v2 已棄用 `CompLibrary`，需自行編寫 React 元件或使用 Infima 樣式（相關文件即將推出，期間可檢查 V2 官網或參閱 https://infima.dev/ 查看可用樣式）。
...
Docusaurus 的部落格 front matter 欄位已從駝峰式命名改為蛇形命名，以與文件保持一致。
...
部落格 RSS 訂閱網址變更為 `/blog/rss.xml`（原為 `/blog/feed.xml`）。建議設定伺服器端重新導向，以維持使用者訂閱有效性。
