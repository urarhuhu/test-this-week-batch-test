...
它不再緊密耦合於 Crowdin，您可以改用 Git 或其他 SaaS 服務。
...
在 Docusaurus v1 中，您使用 `<translate>` 標籤翻譯頁面：
...
在 Docusaurus v2 中，則改用 `<Translate>` 標籤
...
這對國際化特別重要，因為若 Crowdin 上的翻譯品質不佳且使用了無效的標記，您的 v2 翻譯網站可能會建置失敗：您可能需要清理翻譯內容以修正這些錯誤。
...
本文件是為協助遷移而盡力提供的指南，若您找到更好的方法，請協助我們改進！

:::

首先，我們建議：

- 先在不包含翻譯內容的情況下將 v1 Docusaurus 網站遷移至 v2
- 熟悉 [Docusaurus v2 的新國際化系統](../../i18n/i18n-introduction.mdx)
- 使用全新且未翻譯的 Crowdin 專案，並依照 [Crowdin 教學](../../i18n/i18n-crowdin.mdx)讓 Crowdin 適用於您的 v2 網站

:::danger

請勿在未充分理解 Crowdin 和 Docusaurus v2 國際化的情況下嘗試遷移。

:::

### 建立新的 Crowdin 專案 {#create-a-new-crowdin-project}

為避免**生產環境中的 v1 網站發生故障的風險**，一種可能的策略是複製原始的 v1 Crowdin 專案。
...
- 先嘗試「100%匹配」（比「Perfect」會翻譯更多內容），並預翻譯你的原始檔案
- 下載Crowdin翻譯至本地端
- 嘗試執行/建置你的網站，查看是否有錯誤

首次嘗試時很可能會遇到錯誤：預翻譯可能會嘗試翻譯不該翻譯的內容（front matter、admonition、程式碼區塊等），且翻譯後的MD檔案可能對MDX解析器無效。

你需要修復所有錯誤直到網站能成功建置。可以透過修改本地端的翻譯MD檔案來解決，並使用`docusaurus build --locale fr`一次針對一個語系修復網站。
...
你可能需要重複此預翻譯流程，最終嘗試「Perfect」選項並限制僅對部分語系/檔案進行預翻譯。
...
Crowdin的Markdown解析器會隨時間演進，且每個Crowdin專案的解析器版本可能不同，這會導致預翻譯無法翻譯所有字串。
...
Crowdin有「上傳翻譯」功能，但根據我們的經驗，它對Markdown的處理效果不佳

:::

### 使用現有的Crowdin專案 {#use-the-existing-crowdin-project}

如果你不介意修改現有的Crowdin專案並承擔可能搞砸的風險，可以嘗試使用Crowdin的分支系統。
...
這樣你就不需要建立新Crowdin專案、轉移翻譯記憶、應用預翻譯，並嘗試修復預翻譯錯誤。

你可以為Docusaurus v2建立一個Crowdin分支，上傳v2原始檔案，並在準備好後將Crowdin分支合併至主分支。

### 使用Git替代Crowdin {#use-git-instead-of-crowdin}

可以選擇遷移離開Crowdin，將翻譯檔案加入Git中。
