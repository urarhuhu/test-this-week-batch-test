...
用它來包裹可能拋出錯誤的組件，當錯誤發生時顯示後備內容，而不是導致整個應用程序崩潰。
...
- `fallback`: 可選的渲染回調函數，返回一個 JSX 元素。它將接收一個包含 2 個屬性的對象：`error`，捕獲到的錯誤，和 `tryAgain`，一個函數 (`() => void`) 回調，用於重置組件中的錯誤並嘗試重新渲染。如果未提供，將渲染 `@theme/Error` 代替。`@theme/Error` 用於包裹整個網站的錯誤邊界，位於佈局之上。

:::warning

`fallback` 屬性是一個回調函數，**而不是一個 React 功能組件**。您不能在這個回調函數中使用 React 鉤子。
...
優先使用此組件而非普通的 `<a>` 標籤，因為如果您使用 `<Link>`，Docusaurus 會進行大量優化（例如檢測錯誤路徑、預取、應用基礎 URL 等）。
...
當[本地化您的網站](./i18n/i18n-introduction.mdx)時，`<Translate/>` 元件可為 React 元件（例如首頁）提供翻譯支援。`<Translate>` 元件支援[插值功能](#interpolate)。
...
`<Translate>` 元件支援插值。您也可以透過自訂程式碼和[命令式 `translate` API](#translate-imperative) 實作[字串複數化](https://github.com/facebook/docusaurus/pull/i18n/i18n-tutorial.mdx#pluralization)。
...
用於在字符串前附加站點 `baseUrl` 的 React 鉤子。
...
全域資料按插件名稱命名空間劃分，其次按插件 ID 劃分。

:::info

插件 ID 僅在單個插件被多次用於同一站點時有意義。每個插件實例可創建自己的全域資料。
...
存取特定插件創建的全域資料。給定插件名稱後，將按插件 ID 返回該名稱下所有插件實例的資料。
...
[`<Translate>`](#translate) 元件的命令式對應方法，同樣支援[佔位符插值](#interpolate)。
