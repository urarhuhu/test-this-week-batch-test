...
同樣地，您可以通過 `require` 現有資源並在 `video`、`a` 錨點連結等處使用返回的 URL 來連結這些資源。
...
若使用 Markdown 圖片或連結語法，Docusaurus 會將所有資源路徑解析為檔案路徑並自動轉換為 `require()` 調用。除非使用 JSX 語法（此時需自行處理），否則無需在 Markdown 中使用 `require()`。
...
若您需要通過 CSS 修改 SVG 圖像的局部樣式，此功能會非常實用。例如，您可以根據當前主題變更 SVG 的某部分顏色。
...
GitHub 使用其獨特的[圖片主題化方案](https://github.blog/changelog/2021-11-24-specify-theme-context-for-images-in-markdown/)，通過路徑片段實現，您可輕鬆自行實作此功能。

要使用路徑片段（GitHub 採用 `#gh-dark-mode-only` 和 `#gh-light-mode-only`）切換圖片可見性，請在自訂 CSS 中加入以下代碼（若不想與 GitHub 耦合，也可使用自訂後綴）：
...
若 Markdown 連結或圖片使用絕對路徑，該路徑將被視為檔案路徑，並從靜態目錄解析。例如，若您已設定[靜態目錄](../../static-assets.mdx)為 `['public', 'static']`，則對於以下圖片：

```md title="my-doc.md"
![An image from the static](/img/docusaurus.png)
```

Docusaurus 會嘗試在 `static/img/docusaurus.png` 和 `public/img/docusaurus.png` 兩個路徑中尋找該檔案。連結最終會被轉換為 `require()` 呼叫而非保留為 URL 格式，此設計具有兩項優勢：

1. 您無需考慮基礎 URL 問題，Docusaurus 會在提供資源時自動處理；
2. 圖片會進入 Webpack 建置流程，其檔名將附加雜湊值，這使得瀏覽器能積極快取圖片並提升網站效能。
