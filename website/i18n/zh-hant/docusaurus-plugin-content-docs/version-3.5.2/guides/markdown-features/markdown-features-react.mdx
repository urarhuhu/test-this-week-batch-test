...
雖然在 Markdown 中宣告元件對於簡單案例非常方便，但由於編輯器支援有限、解析錯誤風險高且可重用性低，難以維護。當元件涉及複雜 JS 邏輯時，請使用獨立的 `.js` 檔案：
...
在 Docusaurus 中，MDX 元件作用域由 `@theme/MDXComponents` 檔案提供。與大多數其他 `@theme/` 別名下的匯出不同，它本身並非 React 元件：而是一個從標籤名稱（如 `Highlight`）映射到其實作 React 元件的記錄。

如果您[替換](../../swizzling.mdx)此元件，將會發現所有已實作的標籤，並可透過替換相應的子元件（如 `@theme/MDXComponents/Code`）進一步自訂我們的實作（該元件用於渲染[Markdown 程式碼區塊](./markdown-features-code-blocks.mdx)）。
...
現在，您可在所有頁面中直接使用 `<Highlight>`，無需撰寫匯入語句：
...
[MDX 語法](https://mdxjs.com/docs/what-is-mdx/#mdx-syntax)大體相容於[CommonMark](https://commonmark.org/)，但更為嚴格，因為 `.mdx` 檔案可使用 JSX 並會被編譯為真實 React 元件（參閱[遊樂場](https://mdxjs.com/playground/)）。
...
按照慣例，使用 **文件名前綴 `_`** 不會創建任何文檔頁面，並表示該 Markdown 文件是一個 **「部分」**，將被其他文件導入。
