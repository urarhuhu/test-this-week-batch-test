...
所有文件將透過子路由 `docs/` 提供。但若**您的站點僅包含文件**，或希望將文件置於根目錄以優先展示該怎麼辦？
...
請注意，您**不一定需要放棄使用部落格**或其他插件；`routeBasePath: '/'` 的作用僅是將文件服務路徑從 `https://example.com/docs/some-doc` 改為站點根目錄 `https://example.com/some-doc`。若啟用部落格功能，仍可透過 `blog/` 子路由存取。
...
若您已透過 `slug: /` 將某文件設為首頁，應刪除現有的 `./src/pages/index.js` 首頁檔案，否則會出現兩個檔案映射至相同路由的情況！
