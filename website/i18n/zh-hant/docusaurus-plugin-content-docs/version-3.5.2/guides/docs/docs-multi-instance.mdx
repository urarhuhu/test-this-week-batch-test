...
若每個文檔實例體量龐大，您應考慮創建兩個獨立的 Docusaurus 站點。

當有人編輯 iOS 文檔時，重新構建所有內容（包括未變更的完整 Android 文檔）真的有必要嗎？

:::

### 版本化與非版本化文檔 {#versioned-and-unversioned-doc}

有時您希望部分文檔進行版本化，而其他文檔保持「全局性」，對其進行版本化顯得無意義。

我們在 Docusaurus 官方網站上採用此模式：
...
`@docusaurus/preset-classic` 已默認包含一個文檔插件實例！
...
每個插件實例將擁有專屬的 CLI 命令來標記新版本。執行以下命令可查看這些命令：
...
每個文件相關的[主題導航欄項目](../../api/themes/theme-configuration.mdx#navbar)都可選用 `docsPluginId` 屬性。
