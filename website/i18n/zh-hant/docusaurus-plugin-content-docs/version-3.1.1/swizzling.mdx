...
- [**完整替換 (Ejecting)**](#ejecting)：建立原始元件的**副本**，可進行全面**客製化**
- [**封裝替換 (Wrapping)**](#wrapping)：建立原始元件的**封裝層**，可進行**功能擴充**
...
替換元件後，請**重新啟動開發伺服器**以讓 Docusaurus 識別新元件。
...
完整替換主題元件是指**建立原始元件的副本**，您可對其進行**全面客製化與覆寫**。

要完整替換主題元件，請使用互動式 swizzle CLI 或搭配 `--eject` 選項：
...
此操作會將當前 `<Footer />` 元件的實作複製到您網站的 `src/theme` 目錄。Docusaurus 將改用此 `<Footer>` 副本而非原始元件。您現在可完全重新實作 `<Footer>` 元件。
...
替換[**非安全**](#what-is-safe-to-swizzle)元件時，可能需複製大量內部程式碼，後續需自行維護。這會增加 Docusaurus 升級難度，若接收的 props 或使用的內部主題 API 發生變更，您需自行遷移客製化內容。

**請盡可能優先選擇[封裝替換](#wrapping)**：需維護的程式碼量較少。
...
包裝主題是在不[抽出](#ejecting)的情況下**在現有元件周圍添加額外元件**的絕佳方式。例如，您可以輕鬆在每篇部落格文章下方加入自訂評論系統：
...
- **禁止**：混寫 CLI 會阻止您混寫此元件，因為它從設計上就不允許被混寫
...
不必過於**害怕混寫不安全元件**：只需記住可能發生**破壞性變更**，且在次要版本升級時您可能需要手動更新自訂內容。

:::

:::note[回報使用案例]

若您有**強烈需求需混寫不安全元件**，請[**在此回報**](https://github.com/facebook/docusaurus/discussions/5468)，我們將共同尋找解決方案使其變得安全。

:::

## 我該混寫哪個元件？ {#which-component-should-i-swizzle}

要精確知道該混寫哪個元件才能達成預期效果並非總是顯而易見。提供大多數主題元件的 `@docusaurus/theme-classic` 擁有約[100個元件](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-theme-classic/src/theme)！
...
3. **從高層級元件開始**。元件形成樹狀結構，部分元件會引入其他元件。每條路由都關聯一個頂層元件（多數列於[內容外掛路由](./advanced/routing.mdx#routing-in-content-plugins)）。例如所有部落格文章頁面的最頂層元件都是 `@theme/BlogPostPage`。您可以先swizzle此元件，再向下遍歷元件樹定位目標元件。找到正確元件後，請刪除其他swizzle產生的檔案以避免維護過多元件。
...
## 必須使用swizzle嗎？ {#do-i-need-to-swizzle}
...
相較於[完全彈出(ejecting)](#ejecting)，[包裝(wrapping)](#wrapping)是更安全的選擇。

:::

## 使用`<Root>`包裝您的網站 {#wrapper-your-site-with-root}

`<Root>`元件會渲染在React樹的**最頂層**，位於主題`<Layout>`之上且**永不卸載**，非常適合用於添加不該因導航而重新初始化的狀態邏輯（如使用者認證狀態、購物車狀態等）。
