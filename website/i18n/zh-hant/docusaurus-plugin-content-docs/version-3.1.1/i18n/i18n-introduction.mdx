...
如需更多背景資訊，可閱讀初始的 [RFC](https://github.com/facebook/docusaurus/issues/3317) 和 [PR](https://github.com/facebook/docusaurus/pull/3325)。
...
- **靈活的翻譯流程**：可使用 Git（單一倉庫、分支或子模組）、SaaS 軟體或 FTP
...
部分插件可能會讀取需整體本地化的外部資料檔案。例如，部落格插件使用的[`authors.yml`](../blog.mdx#global-authors)檔案，可透過在`i18n/[locale]/docusaurus-plugin-content-blog/authors.yml`路徑下建立副本來進行翻譯。
...
JSON檔案可透過[`docusaurus write-translations`](../cli.mdx#docusaurus-write-translations-sitedir) CLI指令初始化。各插件在對應資料夾下提供自有翻譯內容，而`code.json`檔案則定義React程式碼中使用的所有文字標籤。

每個內容插件或主題皆有差異，並**自行定義翻譯檔案位置**：
