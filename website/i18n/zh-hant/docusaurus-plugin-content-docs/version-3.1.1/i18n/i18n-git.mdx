...
一種**可能的翻譯策略**是透過 Git（或任何其他[版本控制系統](https://en.wikipedia.org/wiki/Version_control)）**對翻譯檔案進行版本控制**。
...
- **免費**（或無額外成本，假設您已使用 Git）
- **低門檻**：無需註冊外部工具
- **有成就感**：貢獻者樂於擁有良好的貢獻記錄

使用 Git 也存在一些缺點：

- **對非開發者困難**：他們不精通 Git 和拉取請求
...
請參考 [Docusaurus 國際化 RFC](https://github.com/facebook/docusaurus/issues/3317) 以了解我們研究這些系統的筆記和連結。
...
當未翻譯的 Markdown 文件被編輯時，**您有責任維護相應的翻譯檔案**，遺憾的是我們目前沒有好的方法來協助您完成此工作。
...
若您的翻譯存放在 Git 中，可使用 docs 和 blog 插件的 `editLocalizedFiles: true` 選項。
