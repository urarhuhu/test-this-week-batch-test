...
我們以 [Crowdin](https://crowdin.com/) 的使用為例進行說明，作為**一種**可能的**整合範例**。
...
使用這個 **[社群驅動的 GitHub 討論](https://github.com/facebook/docusaurus/discussions/4052)** 來討論任何與 Docusaurus + Crowdin 相關的內容。
...
Crowdin 提供 [CLI](https://support.crowdin.com/cli-tool/) 來**上傳原始檔案**和**下載翻譯**，讓您可以自動化翻譯流程。
...
使用 `Hide String` 功能確保翻譯人員**不會翻譯不該翻譯的內容**：
...
使用**[預先翻譯](https://support.crowdin.com/pre-translation-via-machine/)**功能處理網站內容，並**手動修正預先翻譯的錯誤**（請先在設定中啟用全域翻譯記憶庫）。

建議先使用 `Hide String` 功能，因為 Crowdin 的預先翻譯有時會過度樂觀地處理不該翻譯的內容。
...
建立 npm 腳本來 `sync` Crowdin（提取來源檔案、上傳來源檔案、下載翻譯檔案）：
...
保持部署預覽速度：不要下載翻譯檔案，針對功能分支使用 `npm run build -- --locale en`。
...
Crowdin **官方不支援 MDX**，但他們新增了 **.mdx 副檔名支援**，會將此類檔案視為 Markdown（而非純文字）處理。

#### MDX 問題 {#mdx-problems}

Crowdin 會將 JSX 語法誤認為嵌入式 HTML，可能在下載翻譯時破壞 JSX 標記，導致網站因無效 JSX 而無法建置。

使用簡單字串屬性的 JSX 片段（如 `<Username name="Sebastien"/>`）能正常運作；但使用物件/陣列屬性的複雜 JSX 片段（如 `<User person={{name: "Sebastien"}}/>`）更容易因語法不像 HTML 而失敗。
...
創建新版本時，源字符串通常與當前版本（`website/docs`）非常相似，而您不希望反復翻譯新版本文檔。
...
當您的源代碼被重構時，您應該使用 Crowdin UI **手動更新 Crowdin 文件**：

![Crowdin UI：重命名文件](/img/crowdin/crowdin-files-rename.png)

### VCS（Git）集成 {#vcs-git-integrations}

Crowdin 為 [GitHub](https://support.crowdin.com/github-integration/)、GitLab、Bitbucket 提供了多種 VCS 集成。
...
原本這可能有助於在 Git 和 Crowdin 中編輯翻譯，並在兩個系統之間實現**雙向同步**。

實際上，**它的可靠性並不高**，原因如下：

- Crowdin -> Git 同步工作正常（通過拉取請求）
- Git -> Crowdin 同步是手動的（您必須按下按鈕）
- Crowdin 用於匹配現有 Markdown 翻譯與現有 Markdown 源的啟發式方法並非 100% 可靠，您必須在每次從 Git 同步後在 Crowdin UI 上驗證結果
- 兩個用戶同時在 Git 和 Crowdin 上編輯可能導致翻譯丟失
- 它要求 `crowdin.yml` 文件位於存儲庫的根目錄

### 上下文本地化 {#in-context-localization}

Crowdin 具有[上下文本地化](https://support.crowdin.com/in-context-localization/)功能。

:::warning

不幸的是，由於技術原因，它目前還無法工作，但我們有希望解決這個問題。

Crowdin 將 Markdown 字符串替換為技術 ID，如 `crowdin:id12345`，但它做得過於激進，包括隱藏字符串，並弄亂了 front matter、admonitions、JSX...
...
您可能更希望編輯按鈕連結到 Crowdin 介面，可以透過使用 `editUrl` 函數來根據不同語言環境自訂編輯網址。
