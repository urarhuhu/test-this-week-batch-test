...
Docusaurus 支援編寫和使用 TypeScript 主題元件。若初始化模板提供 TypeScript 變體，您可直接透過 `--typescript` 參數初始化一個具備完整 TypeScript 支援的網站。
...
Docusaurus 不會使用此 `tsconfig.json` 來編譯您的專案。其僅為提升編輯器體驗而添加，但您仍可選擇執行 `tsc` 來自行或在 CI 上進行程式碼型別檢查。
...
## 提取 TypeScript 主題元件 {#swizzling-typescript-theme-components}

對於支援 TypeScript 主題元件的主題，您可在 `swizzle` 指令結尾添加 `--typescript` 參數來取得 TypeScript 原始碼。例如以下指令將在 `src/theme/Footer` 中生成 `index.tsx` 和 `styles.module.css`。
