...
安裝 Docusaurus 最簡單的方法是使用命令行工具，它能幫助您建立一個 Docusaurus 網站的基本骨架。您可以在新的空倉庫或現有倉庫中的任何位置執行此命令，它將創建一個包含骨架文件的新目錄。

```bash
npx create-docusaurus@latest my-website classic
```

我們推薦使用 `classic` 模板，以便您能快速開始，它包含了 Docusaurus 1 中的功能。`classic` 模板包含 `@docusaurus/preset-classic`，其中包括標準文檔、博客、自定義頁面和 CSS 框架（支援暗黑模式）。使用經典模板，您可以極快地啟動並運行，並在熟悉 Docusaurus 後進行自定義。

您也可以通過傳遞 `--typescript` 標誌使用模板的 TypeScript 變體。更多信息請參閱 [TypeScript 支援](./typescript-support.mdx)。
...
如果您正在為 Meta 開源專案設置新的 Docusaurus 網站，請在內部倉庫中執行此命令，該命令會帶有一些有用的 Meta 特定默認設置：
...
- `/src/` - 非文件類檔案，例如頁面或自訂 React 元件。您不一定要將非文件檔案嚴格放置於此，但集中存放於單一目錄下，便於在需要進行某種 linting/處理時指定
...
- `/package.json` - Docusaurus 網站是一個 React 應用程式。您可以在其中安裝並使用任何您喜歡的 npm 套件
- `/sidebars.js` - 供文件使用，用於指定側邊欄中文檔的排序

### 單體儲存庫 (Monorepos) {#monorepos}

若您正在使用 Docusaurus 為現有專案編寫文件，單體儲存庫可能是您的解決方案。單體儲存庫允許您在相似專案間共享依賴項。例如，您的網站可以使用本地套件來展示最新功能，而非依賴已發布的版本。如此一來，貢獻者就能在實作功能時同步更新文件。以下是一個單體儲存庫的目錄結構範例：
...
若您使用 Netlify 或 Vercel 等託管服務提供商，則需將網站的 `Base directory` 變更為 Docusaurus 根目錄所在位置。在此案例中，應設為 `./website`。更多關於設定忽略指令的資訊，請參閱[部署文件](./deployment.mdx#deploying-to-netlify)。

更多關於單體儲存庫的資訊，請參閱 [Yarn 文件](https://yarnpkg.com/features/workspaces)（Yarn 並非設定單體儲存庫的唯一方式，但它是常見解決方案），或查看 [Docusaurus](https://github.com/facebook/docusaurus) 和 [Jest](https://github.com/facebook/jest) 以獲取實際案例。

## 執行開發伺服器 {#running-the-development-server}

若要預覽您在編輯檔案時的變更，可執行本地開發伺服器來提供網站服務並即時反映最新修改。
...
恭喜！您已成功建立第一個 Docusaurus 網站！請瀏覽網站內容以查看可用功能。

## 建置 {#build}

Docusaurus 是一個現代靜態網站產生器，因此我們需要將網站建置為靜態內容目錄，並將其放置於網頁伺服器上以供檢視。若要建置網站：

```bash npm2yarn
npm run build
```

建置完成後，內容將生成於 `/build` 目錄內，可將其複製到任何靜態檔案託管服務，例如 [GitHub Pages](https://pages.github.com/)、[Vercel](https://vercel.com/) 或 [Netlify](https://www.netlify.com/)。更多細節請參閱[部署文件](deployment.mdx)。
