...
預設建議將這些資源放在 `static` 資料夾中。**該目錄下的所有檔案**都會被複製到生成的 `build` 資料夾根目錄，並保持目錄層級結構。例如若在 static 資料夾中添加名為 `sun.jpg` 的檔案，它會被複製到 `build/sun.jpg`。
...
在 JSX 中，您可以使用絕對 URL 從 `static` 資料夾引用資源，但這並非理想做法，因為變更網站 `baseUrl` 會**導致這些連結失效**。對於 `<img src="/img/docusaurus.png" />` 圖片，若網站部署在 `https://example.com/test` 路徑下，瀏覽器會嘗試從 URL 根目錄解析，即 `https://example.com/img/docusaurus.png`，但實際上圖片是透過 `https://example.com/test/img/docusaurus.png` 提供，這將導致載入失敗。

建議使用 `import()` 或 `require()` 載入靜態資源（推薦做法），或使用 `useBaseUrl` 工具函數：兩者都會自動為路徑添加 `baseUrl` 前綴。
...
在 Markdown 中撰寫連結或圖片時**使用 Markdown 語法**可保持使用絕對路徑，因為 Docusaurus 在解析 Markdown 時會將其視為 `require` 調用而非 URL。詳見 [Markdown 靜態資源](./guides/markdown-features/markdown-features-assets.mdx)。
...
在 CSS 中，常用 `url()` 函數引用字型或圖片等資源。引用靜態資源時請使用絕對路徑：
