...
由於所有文檔文件都是使用 MDX 解析的，任何看起來像 HTML 的內容實際上是 JSX。因此，如果您需要內聯樣式一個元件，請遵循 JSX 風格並提供樣式物件。
...
如果您在多個文件中使用相同的元件，不需要在每個地方都匯入它——考慮將其添加到全域範圍中。[請參閱下方](#mdx-component-scope)
...
如果您[替換](../../swizzling.mdx)此元件，您將找到所有已實作的標籤，並且可以通過替換相應的子元件（如 `@theme/MDXComponents/Code`，用於渲染[Markdown 代碼塊](./markdown-features-code-blocks.mdx)）來進一步自訂我們的實作。

如果您想註冊額外的標籤名稱（如上述的 `<Highlight>` 標籤），您應該考慮[包裝 `@theme/MDXComponents`](../../swizzling.mdx#wrapping)，這樣您就不需要維護所有現有的對應。由於替換 CLI 目前不允許包裝非元件檔案，您應該手動創建包裝器：
...
[MDX 語法](https://mdxjs.com/docs/what-is-mdx/#mdx-syntax)大多與[CommonMark](https://commonmark.org/)兼容，但由於您的 `.mdx` 檔案可以使用 JSX 並被編譯為真正的 React 元件（請查看[遊樂場](https://mdxjs.com/playground/)），因此更加嚴格。
...
Docusaurus v3 可以通過以下選項選擇支持較不嚴格、標準的[CommonMark](https://commonmark.org/)：
...
您必須使用 `<CodeBlock>` 而不是 Markdown 的三重反引號 ` ``` `，因為後者會將其任何內容原樣輸出，但您希望在此處插入導入的文本。
...
按照慣例，使用 **`_` 文件名前綴** 將不會創建任何文檔頁面，並且意味著該 Markdown 文件是一個 **「部分」**，將被其他文件導入。
...
- `contentTitle`: Markdown 標題，即 Markdown 文本中的第一個 `h1` 標題。如果沒有（例如在前置元數據中指定的標題），則為 `undefined`。
