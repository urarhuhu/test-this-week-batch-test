...
除了基本的 Markdown 語法外，我們還提供特殊的提示框語法：用三組冒號包裹文字，並在冒號後加上類型標籤。
...
若您使用 [Prettier](https://prettier.io) 格式化 Markdown 文件，Prettier 可能會將程式碼自動格式化為無效的提示框語法。為避免此問題，請在起始和結束指令周圍添加空行。這也是為何此處範例都在內容周圍保留空行。
...
接受的類型與上述相同：`note`、`tip`、`danger`、`info`、`warning`。您也可以選擇性地傳遞 JSX 元素或字串作為圖標，或是指定標題：
...
預設情況下，主題不知道如何處理自訂提示框關鍵字 (如 `:::my-custom-admonition`)。您需自行將每個提示框關鍵字映射到 React 元件，以便主題知道如何渲染它們。

如果您透過以下配置註冊了一個新的提示區塊類型 `my-custom-admonition`：
...
您可以為 `:::my-custom-admonition` 提供對應的 React 元件，方法是建立以下檔案（由於這不是 React 元件檔案，因此無法透過 swizzle 功能替換）：
...
現在您可以在 Markdown 檔案中使用新的提示區塊關鍵字，它將根據您的自訂邏輯進行解析和渲染：
