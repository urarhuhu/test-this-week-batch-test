...
此功能僅對[版本化文件](./versioning.mdx)有用。建議在閱讀本頁面前先熟悉文件版本控制。若您只需要[多側邊欄](./sidebar/multiple-sidebars.mdx)，在單一插件內即可實現。

:::

## 使用情境 {#use-cases}

有時您會希望一個 Docusaurus 站台能托管兩套（或更多）獨立的文件系統。

這些文件系統甚至可能擁有不同的版本控制/發布週期。

### 移動端 SDK 文件 {#mobile-sdks-documentation}

若您開發跨平台移動端 SDK，可能會擁有兩套文件：

- Android SDK 文件（`v1.0`、`v1.1`）
- iOS SDK 文件（`v1.0`、`v2.0`）

此情況下，可為每個移動端 SDK 文件使用獨立的插件實例。

:::warning

若每個文件實例體量龐大，您應該建立兩個獨立的 Docusaurus 站台。

當有人編輯 iOS 文件時，真的需要重新構建所有內容（包括未變更的完整 Android 文件）嗎？

:::

### 版本化與非版本化文件 {#versioned-and-unversioned-doc}

有時您希望部分文件進行版本控制，而其他「全局性」文件則無需版本化。

我們在 Docusaurus 官方網站就採用此模式：

- [/docs/\*](/docs) 區段採用版本控制
- [/community/\*](/community/support) 區段未版本化
...
我們將 `product` 實例視為最重要實例，透過不分配 ID 使其成為「預設」實例。
...
舉例來說，若您希望為每個行動 SDK（iOS 和 Android）設置獨立的版本下拉選單，可以這樣設定：
