...
我們維護了[官方插件列表](./api/plugins/overview.mdx)，社群也開發了部分[非官方插件](/community/resources#community-plugins)。若您想新增任何功能：自動生成文檔頁面、執行自訂腳本、整合第三方服務...請務必查閱該列表：可能已有人為您實現了該功能！

若您精力充沛，也可閱讀[插件指南](./advanced/plugins.mdx)或[插件方法參考文檔](./api/plugin-methods/README.mdx)來了解如何自行開發插件。

## 安裝插件 {#installing-a-plugin}

插件通常是 npm 套件，因此您可像其他 npm 套件一樣使用 npm 進行安裝。

```bash npm2yarn
npm install --save docusaurus-plugin-name
```

接著在您網站的 `docusaurus.config.js` 的 `plugins` 選項中添加該插件：
...
但插件可透過在配置中以雙成員元組形式（通常稱為「Babel 風格」）包裝名稱與選項物件來指定參數。
...
所有 Docusaurus 內容插件皆支援多實例運行。例如，可能需要運行[多個文檔插件實例](./guides/docs/docs-multi-instance.mdx)或[多個部落格](./blog.mdx#multiple-blogs)。必須為每個插件實例分配唯一 ID，預設情況下插件 ID 為 `default`。
...
主題的載入方式與插件完全相同。從使用者角度來看，在安裝和配置插件時，`themes` 與 `plugins` 條目可互換使用。唯一細微差別在於主題會在插件之後載入，且[主題可覆寫插件的預設主題組件](./advanced/client.mdx#theme-aliases)。

:::tip

`themes` 和 `plugins` 選項會導致不同的[模組簡寫解析方式](#module-shorthands)，若想利用簡寫功能，請務必使用正確的條目！
...
接著，在您網站的 `docusaurus.config.js` 中的 `presets` 選項裡加入它：
...
預設集是一個與 [插件建構函式](./api/plugin-methods/README.mdx#plugin-constructor) 結構相同的函式。它應該回傳一個 `{ plugins: PluginConfig[], themes: PluginConfig[] }` 物件，其格式與網站設定中接受的格式相同。例如，您可以指定一個包含以下主題與插件的預設集：
...
然後在您的 Docusaurus 設定中，您可以配置這個預設集：
...
以下是一些在 `plugins` 欄位註冊的插件範例。請注意，與 [ESLint](https://eslint.org/docs/user-guide/configuring/plugins#configuring-plugins) 或 [Babel](https://babeljs.io/docs/en/options#name-normalization) 不同，這些工具強制要求插件遵循一致的命名慣例，而 Docusaurus 允許更大的命名自由，因此解析方式並非絕對，而是遵循上述優先順序。
