...
此手動遷移程序應在[自動遷移程序](./migration-automated.mdx)完成後執行，以補足缺失部分或除錯遷移 CLI 輸出的問題。
...
在 Docusaurus 2 中，我們使用範圍化套件名稱：

- `docusaurus` → `@docusaurus/core`

這能明確區分 Docusaurus 官方套件與社群維護套件。換言之，所有 Docusaurus 官方套件均歸類在 `@docusaurus/` 命名空間下。
...
請使用最新的 Docusaurus 2 版本，您可在此查看[最新版本](https://www.npmjs.com/package/@docusaurus/core)（使用 `latest` 標籤）。

:::

#### CLI 指令 {#cli-commands}

同時，CLI 指令更名為 `docusaurus <command>`（取代原先的 `docusaurus-command`）。

您的 `package.json` 中 `"scripts"` 區塊應更新如下：
...
### 更新對 `build` 目錄的參照 {#update-references-to-the-build-directory}
...
Docusaurus 2 將各功能（部落格、文件、頁面）拆分為插件以實現模組化。預設集是插件的組合包，為保持向後兼容性，我們建立了 `@docusaurus/preset-classic` 預設集，其中包含 Docusaurus 1 中的大多數核心插件。

在您的 `docusaurus.config.js` 中添加以下預設集配置：
...
如果您正在遷移 Docusaurus v1 網站，且存在待處理的文檔拉取請求(pull requests)，可以暫時保留 `/docs` 資料夾在原位置，以避免產生衝突。
...
已棄用。我們為 Docusaurus 2 開發了名為 [Infima](https://infima.dev/) 的自訂 CSS 框架，該框架使用 CSS 變數進行主題設定。相關文檔尚未完全準備好，我們會在完成後更新此處內容。若要覆寫 Infima 的 CSS 變數，請建立自己的 CSS 檔案（例如 `./src/css/custom.css`），並通過將其作為選項傳遞給 `@docusaurus/preset-classic` 來全局導入：
...
您可以聯繫 DocSearch 團隊 (@shortcuts, @s-pace) 尋求支援。他們可以為您更新配置並觸發重新爬取您的網站以恢復搜索功能（否則您可能需要等待最多 24 小時直到下一次排程爬取）
...
已棄用。請改為在 `static` 資料夾中創建一個 `CNAME` 文件，並填入您的自訂網域。`static` 資料夾中的文件將在執行建置命令時被複製到 `build` 資料夾的根目錄。
...
- `blogSidebarTitle`
- `cleanUrl` - 現在預設使用簡潔網址。
- `defaultVersionShown` - 版本控制功能尚未移植。如果您正在使用版本控制，將無法遷移至 Docusaurus 2。請持續關注。
- `disableHeaderTitle`
- `disableTitleTagline`
- `docsSideNavCollapsible` 現在可在 `docsPluginOptions.sidebarCollapsible` 中設定，且此功能現在預設為開啟。
- `facebookAppId`
- `facebookComments`
- `facebookPixelId`
- `fonts`
...
- `separateCss` - 可以像上面提到的 `custom.css` 一樣導入。
- `scrollToTop`
- `scrollToTopOptions`
...
在 v1 中，所有頁面均可透過帶或不帶 `.html` 副檔名的網址訪問。
...
出於 SEO 考量及避免連結失效，您應在託管服務提供商處設定伺服器端重定向規則。

作為應急方案，您可以使用 [@docusaurus/plugin-client-redirects](../../api/plugins/plugin-client-redirects.mdx) 創建從 `/installation.html` 到 `/installation` 的客戶端重定向。
...
若您希望保留 `.html` 副檔名作為頁面的標準網址，可在文件 front matter 中宣告 `slug: installation.html`。

## 元件 {#components}

### 側邊欄 {#sidebar}

舊版本中不允許嵌套側邊欄分類，且分類只能包含文件 ID。然而 v2 版本允許無限嵌套側邊欄，並提供多種[側邊欄項目類型](../../guides/docs/sidebar/items.mdx)而不僅限於文件。

若您的側邊欄包含分類類型，需進行遷移。請將 `subcategory` 更名為 `category`，`ids` 更名為 `items`。
...
不再需要 `website/core/Footer.js`。若需修改 Docusaurus 預設頁尾，請透過[替換元件](../../swizzling.mdx)實現：
...
請勿僅為在左側添加標誌而替換頁尾元件。v2 版本已刻意移除左側標誌並移至底部。請直接在 `docusaurus.config.js` 中透過 `themeConfig.footer` 配置頁尾：
...
請參閱[建立頁面](guides/creating-pages.mdx)了解 Docusaurus 2 頁面運作方式。閱讀後請注意，需將 v1 的 `pages/en` 檔案移至 `src/pages` 目錄。

In Docusaurus v1, pages received the `siteConfig` object as props.

在 Docusaurus v2 中，請改用 `useDocusaurusContext` 獲取 `siteConfig` 物件。

v2 版本中，需在每個頁面套用主題佈局元件。Layout 元件接受元資料屬性。

v2 已棄用 `CompLibrary`，您需自行編寫 React 元件或使用 Infima 樣式（相關文件即將推出，敬請見諒！期間可檢查 V2 網站或查看 https://infima.dev/ 了解可用樣式）。
...
- 幫助/支援頁面 - [Docusaurus 2](https://github.com/facebook/docusaurus/blob/main/website/src/pages/help.js/)、[Flux](http://facebook.github.io/flux/support)

## 內容 {#content}

### 替換 AUTOGENERATED_TABLE_OF_CONTENTS {#replace-autogenerated_table_of_contents}

此功能已被[行內目錄](../../guides/markdown-features/markdown-features-toc.mdx#inline-table-of-contents)取代

### 將 Markdown 語法更新為 MDX 相容格式 {#update-markdown-syntax-to-be-mdx-compatible}

Docusaurus 2 中，Markdown 語法已改為 [MDX](https://mdxjs.com/)。因此現有文件可能出現需修復的語法問題。常見範例如 HTML 中有效的自閉合標籤 `<img>` 和 `<br>`，現在需明確閉合（`<img/>` 和 `<br/>`）。MDX 文件中的所有標籤都必須是有效的 JSX。

Front matter 由 [gray-matter](https://github.com/jonschlinkert/gray-matter) 解析。若您的 front matter 使用特殊字符如 `:`，現在需要加上引號：`title: Part 1: my part1 title` → `title: "Part 1: my part1 title"`。

**提示**：您可以使用 [HTML 轉 JSX](https://transform.tools/html-to-jsx) 等線上工具來簡化遷移流程。
...
`authorFBID` 和 `authorTwitter` 欄位已被棄用。這些欄位僅用於生成作者頭像，現在可透過 `authors` 欄位實現相同功能。

## 部署 {#deployment}

GitHub Pages 使用的 `CNAME` 檔案不再自動生成，若您使用自訂網域，請確保已在 `/static/CNAME` 建立該檔案。

部落格 RSS 訂閱網址變更為 `/blog/rss.xml`（原為 `/blog/feed.xml`）。建議設定伺服器端重新導向，以維持使用者訂閱功能。

## 測試您的網站 {#test-your-site}
