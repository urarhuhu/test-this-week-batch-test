...
主要的重大變更是從 MDX v1 升級至 MDX v3。詳情請參閱 [**MDX v2**](https://mdxjs.com/blog/v2/) 和 [**MDX v3**](https://mdxjs.com/blog/v3/) 的發布說明。MDX 現在會**更嚴格**且**有細微差異**地編譯您的 Markdown 內容。
...
對於複雜的網站，我們還建議設定[**視覺回歸測試**](/blog/upgrading-frontend-dependencies-with-confidence-using-visual-regression-testing)，這是確保網站視覺效果保持一致的好方法。Docusaurus v3 主要升級了依賴項，預計不會產生任何視覺變化。

:::

:::note

請查閱 [**Docusaurus v3.0.0**](https://github.com/facebook/docusaurus/releases/tag/v3.0.0) 的發布說明，並瀏覽相關 pull-requests 以獲取更多有用資訊及本文提及的每項變更背後的動機。
...
升級至MDX伴隨著[MDX v2](https://mdxjs.com/blog/v2/)和[MDX v3](https://mdxjs.com/blog/v3/)發布部落格中記載的所有破壞性變更。大多數破壞性變更來自MDX v2。[MDX v2遷移指南](https://mdxjs.com/migrating/v2/)中有關於如何[更新MDX文件](https://mdxjs.com/migrating/v2/#update-mdx-files)的章節，這對我們特別重要。同時請務必閱讀[MDX疑難排解](https://mdxjs.com/docs/troubleshooting-mdx/)頁面，該頁面能幫助您解讀常見的MDX錯誤訊息。

請同時閱讀我們更新後的[**MDX與React**](../guides/markdown-features/markdown-features-react.mdx)文件頁面。

### 使用MDX遊樂場

MDX遊樂場是您的新好朋友。它能幫助您理解內容如何**被編譯為React元件**，並在隔離環境中排查編譯或渲染問題。

- [MDX遊樂場 - 當前版本](https://mdxjs.com/playground/)
- [MDX遊樂場 - v1](https://mdx-git-renovate-babel-monorepo-mdx.vercel.app/playground/)
...
並排使用兩個MDX遊樂場時，您會很快注意到某些內容在v2中的編譯方式不同或無法編譯。

:::tip[讓您的內容具備未來兼容性]

目標是重構有問題的內容，使其**能同時兼容兩個版本的MDX**。這樣當您升級至Docusaurus v3時，這些內容就能直接無縫運作。

:::

### 使用MDX檢查器CLI

我們提供了[docusaurus-mdx-checker](https://github.com/slorber/docusaurus-mdx-checker) CLI工具，可輕鬆定位有問題的內容。在您的網站上運行此命令，即可獲取在MDX v3下會編譯失敗的文件清單。

```bash
npx docusaurus-mdx-checker
```

對於每個編譯問題，CLI會記錄文件路徑和需查看的行號。

![終端機截圖顯示MDX檢查器CLI輸出範例，包含數個錯誤訊息](@site/blog/2023/09-29-preparing-your-site-for-docusaurus-v3/img/mdx-checker-output.png)

:::tip

使用此CLI評估讓您的內容兼容MDX v3所需的工作量。

:::

:::warning

此CLI僅為最佳努力工具，且**僅會報告編譯錯誤**。
...
Docusaurus無法詳盡記錄MDX帶來的所有變更。這是[MDX v2](https://mdxjs.com/migrating/v2/)和[MDX v3](https://mdxjs.com/migrating/v3/)遷移指南的責任。

然而，透過升級數個Docusaurus網站，我們注意到大多數問題可歸納為幾種常見案例，我們已為您記錄這些案例。

#### 錯誤使用`{`

`{`字符用於開啟[JavaScript表達式](https://mdxjs.com/docs/what-is-mdx/#expressions)。若您置於`{expression}`中的內容不是有效表達式，MDX現在會編譯失敗。
...
在 MDX v3 中，現在可以更輕鬆地交織使用 JSX 和 Markdown，而無需額外的換行。在多行上編寫內容也可能產生新的預期 `<p>` 標籤。
...
日文和中文受此影響最大，但其他一些語言也可能受到影響（例如泰文和高棉文），例如當您嘗試強調行內程式碼或連結時。使用空格分隔單詞的語言受影響程度較小。

以下範例中的 `**`（除了 `` `**` `` 外）在 Docusaurus 2 中被正確解析為強調，但在 Docusaurus 3 中則不再如此。
...
MDX 生態系統中的所有官方套件（Unified、Remark、Rehype...）現在僅支援 [**ES 模組**](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)，不再支援 [CommonJS](https://nodejs.org/api/modules.html#modules-commonjs-modules)。

實際上這意味著您不能再使用 `require("remark-plugin")` 了。
...
如果您創建了自定義的 Remark 或 Rehype 插件，由於新 AST 的結構，您可能需要重構這些插件，甚至完全重寫它們。我們創建了一個[專屬支援討論](https://github.com/facebook/docusaurus/discussions/9337)來幫助插件開發者升級他們的程式碼。

:::

### 格式化工具

最常見的格式化工具 Prettier 目前僅支援舊版的 MDX v1，截至 Docusaurus v3.0.0 尚未支援 v3。您可以在程式碼的不相容部分前添加 `{/* prettier-ignore */}` 來使其與 Prettier 兼容。
...
如果您厭倦了頻繁添加 `{/* prettier-ignore */}`，可以考慮在 Prettier 開始支援 MDX v3 之前，通過在 `.prettierignore` 文件中添加以下內容來禁用 Prettier 對 MDX 的格式化：
...
同時，配置您的持續集成、CDN 或主機以使用這個新的 Node.js 版本。

您還可以更新網站的 `package.json` 以防止使用不再支援的舊版本：
...
React 18 自帶了一些破壞性變更，這些變更的處理難度取決於您為網站編寫的自訂 React 程式碼量。官方主題和外掛程式已相容 React 18。

:::info[升級指南]

閱讀官方 [React v18.0](https://react.dev/blog/2022/03/29/react-v18) 和 [如何升級至 React 18](https://react.dev/blog/2022/03/08/react-18-upgrade-guide)，並檢查您的 React 程式碼以確認哪些元件可能受此升級影響。
...
Docusaurus 對這些功能的支援目前處於實驗階段。未來可能需要調整整合方式，導致運行時行為變化。
...
這是包含破壞性變更的新主版本，我們無法保證嚴格的向後相容性。[`prism-react-renderer` v2 版本說明](https://github.com/FormidableLabs/prism-react-renderer/releases/tag/prism-react-renderer%402.0.0)不夠詳盡，但 Docusaurus 使用者需注意 3 項主要變更。
...
先前 `react-prism-render` v1 [預設包含更多語言](https://github.com/FormidableLabs/prism-react-renderer/blob/v1.3.5/src/vendor/prism/includeLangs.js)。從 v2.0+ 開始，[預設包含的語言較少](https://github.com/FormidableLabs/prism-react-renderer/blob/prism-react-renderer%402.1.0/packages/generate-prism-languages/index.ts#L9)。您可能需要在 Docusaurus 設定中添加額外語言：
...
理論上您無需任何操作，現有的互動式程式碼區塊應能繼續正常工作。

但這是包含破壞性變更的新主版本，我們無法保證嚴格的向後相容性。若遇到問題，請查閱 [v3](https://github.com/FormidableLabs/react-live/releases/tag/v3.0.0) 和 [v4](https://github.com/FormidableLabs/react-live/releases/tag/v4.0.0) 版本說明。
...
多數 Docusaurus 使用者無需任何操作。使用表情符號短代碼的使用者應閱讀 [變更日誌](https://github.com/rhysd/remark-emoji/blob/master/CHANGELOG.md) 並確認表情符號渲染效果符合預期。

> **重大變更** 將 [node-emoji](https://www.npmjs.com/package/node-emoji) 從 v1 升級至 v2。此變更新增了許多新表情符號支援，並移除了 GitHub 上已失效的舊表情符號短代碼。
...
理論上無需任何操作，現有圖表應可繼續正常運作。

但此為重大版本更新，我們無法保證完全向下相容。若遇到問題請參閱 [v10 變更日誌](https://github.com/mermaid-js/mermaid/releases/tag/v10.0.0)。
...
此新套件與其他 Docusaurus 核心套件同步版本，將用於確保 TypeScript 的向下相容性與重大版本升級時的破壞性變更。
...
Docusaurus v3 將內部配置載入函式庫從 [`import-fresh`](https://github.com/sindresorhus/import-fresh) 更換為 [`jiti`](https://github.com/unjs/jiti)。此載入器負責處理如 `docusaurus.config.js`、`sidebars.js` 及 Docusaurus 插件等檔案。

:::info[升級指南]

理論上無需任何操作，現有配置檔案應可繼續正常運作。

但此為核心依賴項更換，可能出現細微行為差異。

:::

### 警告提示框變更

因歷史因素，我們曾支援未列入文件的紅色 `:::warning` 提示框。
...
然而其顏色與圖示始終存在錯誤。Docusaurus v3 正式重新引入 `:::warning` 提示框，將其列入文件並修正顏色與圖示。
...
若您先前使用未公開的 `:::warning` 警示框，請確認每個使用情境是否適合改用黃色標示。若需維持紅色警示，請改用 `:::danger`。
...
此重大變更僅影響 **在 `v2.0.0-beta.10`（2021年12月）前啟用版本化功能** 的 Docusaurus v2 早期採用者。
...
`@docusaurus/plugin-content-blog` 現在預設將 RSS 饋送限制為最近 20 篇文章。對於大型 Docusaurus 部落格，這是更合理的預設值，可避免 RSS 檔案過度膨脹。

:::info[如何升級]

若不希望採用此新預設行為，可透過新增的 `limit: false` 饋送選項恢復原先「無限制」的行為：
...
對於替換過文件相關主題元件（如 `@theme/DocPage`）的使用者，這些元件已大幅重構以簡化自訂流程。

技術上**這不屬於重大變更**，因這些元件**標記為不建議替換**，但許多 Docusaurus 網站仍替換了文件相關元件，需注意自訂內容可能導致 Docusaurus 異常。

:::info[如何升級]

刪除所有替換過的元件，重新替換後，在新版元件上重新套用您的自訂內容。

或參閱[拉取請求說明](https://github.com/facebook/docusaurus/pull/7966)了解新版主題元件樹結構，嘗試手動修補替換過的元件。
...
未使用 React API 的 JSX 檔案不再需要導入 React。
...
我們建議當你在 Markdown 檔案中使用 JSX、`import` 或 `export`（即 MDX 功能）時，一律使用 `.mdx` 副檔名。這樣在語意上更為正確，也能提升與外部工具（如 IDE、格式化工具、linter 等）的相容性。

在未來版本的 Docusaurus 中，`.md` 檔案將被解析為標準的 [CommonMark](https://commonmark.org/)，而不再支援這些功能。在 Docusaurus v3 中，`.md` 檔案仍會被編譯為 MDX 檔案，但未來將可以[選擇啟用 CommonMark](https://github.com/facebook/docusaurus/issues/3018)。

### 升級數學公式套件

如果你使用 Docusaurus 來渲染[數學公式](../guides/markdown-features/markdown-features-math-equations.mdx)，你應該升級 MDX 插件。
...
此選項允許在 MDX 中使用 HTML 註解，儘管官方已不再支援 HTML 註解。
...
Docusaurus 現在使用 [Markdown Directives](https://talk.commonmark.org/t/generic-directives-plugins-syntax/444)（透過 [remark-directive](https://github.com/remarkjs/remark-directive) 實現）來實作警告框，而指定標籤的語法需要使用方括號：
...
- 確保所有文件能在 [MDX 遊樂場](https://mdxjs.com/playground/)中編譯，或使用 [`npx docusaurus-mdx-checker`](https://github.com/slorber/docusaurus-mdx-checker)
- 刪除 `node_modules` 並重新執行 `npm install`
- 執行 `docusaurus clear` 清除快取
- 移除可能不支援 Docusaurus v3 的第三方插件
- 刪除所有你 swizzle 過的元件

嘗試以上步驟後，你可以透過以下支援管道尋求協助：
...
請注意**我們的時間非常寶貴**。為了確保您的支援請求不會被忽略，我們誠摯地請求您：

- 提供一個**最小化**且我們能輕鬆執行的重現範例，理想情況下使用 [docusaurus.new](https://docusaurus.new) 建立
...
或者，您可以尋找付費的 [Docusaurus 服務提供商](https://github.com/facebook/docusaurus/discussions/9281) 來為您執行此升級。如果您的網站是開源的，您也可以向我們的社群尋求 [免費的善意協助](https://github.com/facebook/docusaurus/discussions/9283)。