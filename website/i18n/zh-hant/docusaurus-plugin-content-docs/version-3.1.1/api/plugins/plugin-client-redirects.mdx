...
本插件會讀取 [`siteConfig.onDuplicateRoutes`](../docusaurus.config.js.mdx#onDuplicateRoutes) 配置項，當多個文件將被輸出至相同路徑時，據此調整日誌級別。
...
因此同個"目標"可對應多個"來源"（生成多個重定向至相同目標的HTML文件），但單個"來源"不能對應多個"目標"（生成的HTML文件必須有確定的重定向終點）。
