...
此外，`warn` 和 `error` 會為**整個**訊息著色以引起更好的注意。如果您要打印關於錯誤的大塊幫助文本，最好使用 `logger.info`。

:::

### 使用模板字面量標籤

模板字面量標籤評估模板和嵌入的表達式。`interpolate` 返回一個新字符串，而其他日誌記錄函數則打印它。以下是典型用法：
...
嵌入的表達式可以選擇在前面加上一個標誌，形式為 `[a-z]+=`（幾個小寫字母，後跟等號，直接位於嵌入的表達式之前）。如果表達式前面沒有任何標誌，則按原樣打印。否則，它會由其中一個格式化器格式化：
