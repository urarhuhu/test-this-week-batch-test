...
這些資料可以通過 [`useGlobalData`](../../docusaurus-core.mdx#useGlobalData) 和 [`usePluginData`](../../docusaurus-core.mdx#usePluginData) 鉤子在客戶端/主題程式碼中存取。
...
修改內部 webpack 配置。如果返回值是一個 JavaScript 物件，它將使用 [`webpack-merge`](https://github.com/survivejs/webpack-merge) 合併到最終配置中。如果是一個函數，它將被呼叫並接收 `config` 作為第一個參數和一個 `isServer` 標誌作為第二個參數。
...
`configureWebpack` 會同時被呼叫並帶有插件載入的內容。
...
`injectHtmlTags` 會同時被呼叫並帶有插件載入的內容。
