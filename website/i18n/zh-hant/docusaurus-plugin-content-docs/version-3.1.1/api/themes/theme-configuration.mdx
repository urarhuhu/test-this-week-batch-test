...
標誌可置於[靜態資料夾](static-assets.mdx)。標誌URL預設為網站根路徑。您可自訂標誌URL，若為外部連結將在新分頁開啟。此外可覆寫標誌連結的target屬性值，當文件網站託管於主站子目錄時特別有用（此時可能不需讓標誌連結至主站時開啟新分頁）。

為強化深色模式支援，可為此模式設定不同標誌。
...
可透過 `themeConfig.navbar.items` 添加項目至導覽列。
...
項目行為可依據 `type` 欄位有所不同，以下章節將介紹所有可用的導覽列項目類型。

#### 導覽列連結 {#navbar-link}

預設情況下，導航欄項目是常規連結（內部或外部）。

React Router 應會自動為連結套用活動連結樣式，但在特殊情況下可使用 `activeBasePath`。若某連結需在多個路徑下保持活動狀態（例如同一側邊欄下有多個文檔資料夾），可使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更靈活的替代方案且優先級更高——Docusaurus 會將其解析為正則表達式並與當前 URL 進行匹配測試。

外部連結會自動添加 `target="_blank" rel="noopener noreferrer"` 屬性。
...
請注意，下拉選單基底項目本身也是可點擊的連結，因此該項目可接受[純導航欄連結](#navbar-link)的任何屬性。
...
若需連結至特定文檔，此特殊導航欄項目類型會渲染指向所提供 `docId` 文檔的連結。只要您瀏覽同一側邊欄下的文檔，該連結就會獲得 `navbar__link--active` 類別。
...
若您使用版本化文檔，此特殊導航欄項目類型會渲染包含所有可用版本的下拉選單。

使用者可在同一文檔（只要文檔 ID 在各版本間保持不變）切換不同版本。
...
若您使用版本化文件，此特殊導覽列項目類型會連結至當前瀏覽的文件版本（依據目前網址決定），並在無效時自動回退至最新版本。
...
若啟用[國際化功能](../../i18n/i18n-introduction.mdx)，此特殊導覽列項目會顯示包含所有可用語言的下拉選單。

使用者可切換不同語言版本，同時保持停留在相同頁面。
...
#### 導覽列搜尋框 {#navbar-search}

若啟用[搜尋功能](../../search.mdx)，搜尋框預設會出現在導覽列最右側。

透過此特殊項目類型，可變更搜尋框的預設位置。
...
#### 自訂HTML導覽列項目 {#navbar-with-custom-html}

您亦可使用此類型在導覽列項目中渲染自訂HTML標記。
...
可啟用此UI特效：當使用者向下滾動頁面時自動隱藏導覽列，向上滾動時重新顯示。
...
您可設定靜態導覽列樣式，同時保留主題切換功能。所選樣式將始終生效，不受使用者選擇的主題影響。

目前提供兩種樣式選項：`dark`（深色）與`primary`（基於`--ifm-color-primary`主色）。樣式預覽請參閱[Infima文件](https://infima.dev/docs/components/navbar/)。
...
Docusaurus使用[Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer)實現程式碼高亮，所有設定皆位於`prism`物件中。
...
預設採用[Palenight](https://github.com/FormidableLabs/prism-react-renderer/blob/master/packages/prism-react-renderer/src/themes/palenight.ts)語法高亮主題。您可從[可用主題清單](https://github.com/FormidableLabs/prism-react-renderer/tree/master/packages/prism-react-renderer/src/themes)選擇自訂主題，亦可為深色模式指定不同高亮主題。
...
若您使用行標記的 Markdown 語法，可能需要為深色模式的語法高亮主題指定不同的標記背景顏色。請參閱[行標記指南文件](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)以獲取指引。
...
您可透過`themeConfig.footer`添加標誌與版權聲明至頁尾。標誌可置於[靜態資源目錄](static-assets.mdx)，其URL運作方式與導航列標誌相同。
...
您可透過`themeConfig.footer.links`在頁尾添加連結。頁尾配置分為兩種類型：**多欄式頁尾**與**簡易頁尾**。

多欄式頁尾連結需為每欄設定`title`及一組`FooterItem`清單。
...
您可透過`themeConfig.tableOfContents`調整預設目錄設定。
...
此React鉤子用於存取色彩模式上下文。該上下文包含設定亮色/暗色模式的函式，並暴露當前使用模式的布林值變數。
...
- **多實例路徑**: 不適用
- **JSON檔案**: 透過[`docusaurus write-translations`](../../cli.mdx#docusaurus-write-translations-sitedir)指令提取
- **Markdown檔案**: 不適用
