...
Docusaurus 提供了一些客戶端 API，可在構建網站時為您提供幫助。
...
優先使用此組件而非普通的 `<a>` 標籤，因為 Docusaurus 在使用 `<Link>` 時會進行許多優化（例如檢測錯誤路徑、預取、應用基礎 URL 等）。
...
當程式碼無法在 Node.js 中執行時（例如需要存取 `window` 或 `document` 物件），請使用此元件。
...
當[本地化您的網站](./i18n/i18n-introduction.mdx)時，`<Translate/>` 元件將為 React 元件（例如您的主頁）提供**翻譯支援**。`<Translate>` 元件支援[插值](#interpolate)。
...
`<Translate>` 元件支援插值。您還可以透過一些自訂程式碼和 [`translate` 命令式 API](#translate-imperative) 來實作[字串複數化](https://github.com/facebook/docusaurus/pull/i18n/i18n-tutorial.mdx#pluralization)。
...
用於存取 Docusaurus Context 的 React hook。該 context 包含來自 [docusaurus.config.js](api/docusaurus.config.js.mdx) 的 `siteConfig` 物件和一些額外的網站元資料。
...
React Hook 用於存取 Docusaurus 的失效連結檢查 API，提供方式讓 Docusaurus 頁面回報並收集其連結與錨點。 :::warning 這是**進階** API，**大多數 Docusaurus 使用者無需直接使用**。
...
若您需實作自訂的 `<Heading>` 或 `<Link>` 元件，才需使用 `useBrokenLinks()`。
...
此模組提供若干布林變數，用以檢查當前渲染環境。

:::warning

若為 React 渲染邏輯，請改用 [`useIsBrowser()`](#useIsBrowser) 或 [`<BrowserOnly>`](#browseronly)。
