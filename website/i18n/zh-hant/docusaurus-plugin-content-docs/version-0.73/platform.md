...
`select` 方法會回傳最適合當前執行平台的值。也就是說，如果你在手機上執行，`android` 和 `ios` 鍵會優先被考慮。如果這些鍵沒有指定，則會使用 `native` 鍵，然後是 `default` 鍵。
...
這將導致容器在所有平台上都有 `flex: 1`，在 Android 上有綠色背景色，在 iOS 上有紅色背景色，在其他平台上有藍色背景色。
