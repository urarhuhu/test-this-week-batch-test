...
當應用程式運行時，您可以在終端機中使用以下指令來顯示 iOS 或 Android 應用的控制台日誌：
...
:::info
如果您使用的是 Expo CLI，控制台日誌會與打包工具（bundler）的輸出顯示在同一個終端機中。
:::

## 除錯原生程式碼

當您處理原生程式碼時，例如編寫原生模組，您可以從 Android Studio 或 Xcode 啟動應用程式，並利用原生除錯功能（如設定中斷點等），就像在開發標準原生應用時一樣。
