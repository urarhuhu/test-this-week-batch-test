...
用於驅動2D動畫（如平移手勢）的二維數值。其API幾乎與標準的[`Animated.Value`](animatedvalue)相同，但採用多路復用機制。底層封裝了兩個常規的`Animated.Value`。
...
設定一個偏移量，該偏移量會疊加到無論通過`setValue`、動畫或`Animated.event`設定的數值上。適用於補償平移手勢起始點等場景。
...
停止任何正在運行的動畫或追蹤。停止後會通過`callback`回傳最終值，這對於將狀態更新至與動畫佈局位置同步非常有用。
