...
[TypeScript][ts] 是一種透過添加類型定義來擴展 JavaScript 的語言。新建立的 React Native 專案預設支援 TypeScript，同時也兼容 JavaScript 和 Flow。

## TypeScript 入門指南

透過 [React Native CLI](/docs/environment-setup#creating-a-new-application) 或熱門模板（如 [Ignite][ignite]）建立的新專案將預設使用 TypeScript。

TypeScript 也可與 [Expo][expo] 搭配使用，後者提供 TypeScript 模板，或在專案中添加 `.ts` 或 `.tsx` 文件時自動提示安裝並配置 TypeScript。
...
1. 將 TypeScript、類型定義和 ESLint 插件加入您的專案。
...
2. 添加 TypeScript 配置文件。在專案根目錄建立 `tsconfig.json`：
...
> 應保留 `./index.js` 入口文件不變，否則可能在打包生產版本時遇到問題。

4. 執行 `yarn tsc` 對新的 TypeScript 文件進行類型檢查。

## 使用 JavaScript 替代 TypeScript

React Native 新專案預設使用 TypeScript，但仍可選擇 JavaScript。副檔名為 `.jsx` 的文件會被視為 JavaScript 而非 TypeScript，且不會進行類型檢查。TypeScript 模組仍可導入 JavaScript 模組，反之亦然。

## TypeScript 與 React Native 的協作原理

預設情況下，TypeScript 原始碼會在打包時由 [Babel][babel] 轉換。建議僅將 TypeScript 編譯器用於類型檢查，這是新專案中 `tsc` 的預設行為。若需將現有 TypeScript 程式碼移植到 React Native，使用 Babel 替代 TypeScript 時需注意 [一兩個注意事項][babel-7-caveats]。

## React Native + TypeScript 實例展示

您可以透過 `React.Component<Props, State>` 為 React 元件的 [Props](props) 和 [State](state) 提供介面，這將在 JSX 中操作該元件時啟用類型檢查和編輯器自動補全功能。
...
- [React + TypeScript 速查表][cheat] 提供完整的 React 與 TypeScript 整合指南

## 在 TypeScript 中使用自訂路徑別名

要讓自訂路徑別名同時兼容 Babel 和 TypeScript，需進行以下配置：

1. 修改 `tsconfig.json` 設定 [自訂路徑映射][path-map]。將 `src` 根目錄下的內容設為可直接引用，並允許透過 `tests/File.tsx` 格式存取測試文件：
