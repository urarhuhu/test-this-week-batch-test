...
在下一次佈局發生時，自動將視圖動畫過渡到新位置。

常見的使用方式是：在函數式組件中更新狀態鉤子前調用此API，或在類組件中調用`setState`前使用。

請注意，要在 **Android** 上實現此功能，需透過`UIManager`設置以下標誌：
...
- `springDamping`：彈簧阻尼係數（數字，僅與`type: Type.spring`配合使用時可選）
...
輔助函數，創建一個包含`create`、`update`和`delete`字段的對象，用於傳入[`configureNext`](layoutanimation.md#configurenext)。`type`參數為[動畫類型](layoutanimation.md#types)，`creationProp`參數為[佈局屬性](layoutanimation.md#properties)。
...
動畫類型的枚舉，用於[`create`](layoutanimation.md#create)方法或[`configureNext`](layoutanimation.md#configurenext)的`create`/`update`/`delete`配置中。（用法示例：`LayoutAnimation.Types.easeIn`）
...
佈局屬性的枚舉，用於動畫化，可在[`create`](layoutanimation.md#create)方法或[`configureNext`](layoutanimation.md#configurenext)的`create`/`update`/`delete`配置中使用。（用法示例：`LayoutAnimation.Properties.opacity`）
