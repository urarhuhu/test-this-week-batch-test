...
手指並非最精確的工具，使用者常會意外激活錯誤元素或錯過激活區域。為此，`Pressable` 提供了一個可選的 `HitRect`，可用於定義觸控可註冊的距離範圍。按壓可以在 `HitRect` 內的任何位置開始。

`PressRect` 允許按壓在維持激活狀態的同時，移動超出元素及其 `HitRect` 的範圍，並仍符合「按壓」資格——想像您按住一個按鈕時，手指慢慢滑開的情景。
...
> `Pressable` 使用 React Native 的 `Pressability` API。有關 Pressability 的狀態機流程及其運作方式的更多資訊，請查閱 [Pressability](https://github.com/facebook/react-native/blob/main/packages/react-native/Libraries/Pressability/Pressability.js#L350) 的實作。
...
在呼叫 `onPressIn` 前，等待按壓按下的持續時間（以毫秒為單位）。
...
在此元件邊界外的額外距離範圍內，觸碰仍會被視為按壓動作（直到觸發 `onPressOut`）。
...
可接受一般視圖樣式，或接收一個反映元件當前按壓狀態的布林值並回傳視圖樣式的函式。
