...
- 預設情況下，如果你嘗試在前景應用程式時執行任務，應用程式會崩潰。這是為了防止開發者在任務中執行大量工作而拖慢 UI。你可以傳遞第四個 `boolean` 參數來控制此行為。
- 如果你從 `BroadcastReceiver` 啟動服務，請確保在從 `onReceive()` 返回之前呼叫 `HeadlessJsTaskService.acquireWakeLockNow()`。

## 使用範例

可以從 Java API 啟動服務。首先你需要決定何時啟動服務，並據此實作你的解決方案。以下是一個響應網路連接變更的範例。
...
廣播接收器然後在 `onReceive` 函式中處理廣播的意圖。這是檢查應用程式是否處於前景的好地方。如果應用程式不在前景，我們可以準備要啟動的意圖，不帶資訊或使用 `putExtra` 捆綁額外資訊（請注意，捆綁只能處理可打包的值）。最後啟動服務並取得 wakelock。
