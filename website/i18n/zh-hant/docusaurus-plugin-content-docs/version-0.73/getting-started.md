...
**若您剛接觸行動開發**，最簡單的方式是使用 Expo Go。Expo 是一套圍繞 React Native 建構的工具與服務，雖然它擁有眾多[功能](https://docs.expo.dev)，目前對我們最相關的特性是能在數分鐘內開始撰寫 React Native 應用程式。您僅需最新版 Node.js 和一支手機或模擬器。若想在安裝任何工具前先透過網頁瀏覽器試用 React Native，可嘗試 [Snack](https://snack.expo.dev/)。

**若您已熟悉行動開發**，可考慮使用 React Native CLI。此方式需先安裝 Xcode 或 Android Studio。若已安裝其中一項工具，應能在數分鐘內開始開發。若尚未安裝，預期需花費約一小時進行安裝與環境設定。
