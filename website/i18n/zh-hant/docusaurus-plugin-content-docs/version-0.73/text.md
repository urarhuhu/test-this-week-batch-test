...
在以下範例中，嵌套的標題和正文文字將繼承 `styles.baseText` 的 `fontFamily`，但標題提供了自己的額外樣式。由於實際換行符號，標題和正文將垂直堆疊：
...
Android 和 iOS 都允許您通過為字串的特定範圍添加格式（如粗體或彩色文字）來顯示格式化文字（iOS 上的 `NSAttributedString`，Android 上的 `SpannableString`）。實際上，這非常繁瑣。對於 React Native，我們決定使用網頁範式來實現相同的效果，您可以嵌套文字來達成。
...
`<Text>` 元素在佈局方面是獨特的：內部所有內容不再使用 Flexbox 佈局，而是使用文字佈局。這意味著 `<Text>` 內部的元素不再是矩形，而是在看到行尾時會自動換行。
...
您也失去了為整個子樹設定預設字體的能力。同時，`fontFamily` 僅接受單一字體名稱，這與 CSS 中的 `font-family` 不同。建議在應用程式中使用一致字體和大小的方式是創建一個包含這些設定的元件 `MyAppText`，並在整個應用程式中使用此元件。您還可以使用此元件來創建更具體的元件，如 `MyAppHeaderText`，用於其他類型的文字。
...
- (開發者) React 元件設計時考慮了強隔離性：您應該能夠將元件放置在應用程式的任何位置，並相信只要屬性相同，它將以相同的方式外觀和行為。可能從屬性外部繼承的文字屬性將破壞這種隔離性。

- (實作者) React Native 的實現也簡化了。我們不需要在每個元素上都有一個 `fontFamily` 字段，也不需要每次顯示文字節點時都遍歷樹到根節點。樣式繼承僅編碼在本機 Text 元件內部，不會洩漏到其他元件或系統本身。
...
輔助功能提示能幫助使用者理解當他們對該輔助功能元素執行操作時會發生什麼結果，尤其是當該結果無法從輔助功能標籤中明確得知時。
...
指定螢幕閱讀器與該元素互動時應使用的語言，需符合 [BCP 47 規範](https://www.rfc-editor.org/info/bcp47)。
...
輔助功能操作允許輔助技術以程式化方式觸發元件的動作。`accessibilityActions` 屬性應包含一組動作物件，每個物件需包含 name 和 label 欄位。
...
在 iOS 上應用於此元素的[動態字型](https://developer.apple.com/documentation/uikit/uifont/scaling_fonts_automatically)級別。
...
> 在 Android 上，當 `numberOfLines` 設定為大於 `1` 的值時，只有 `tail` 值能正確運作。
...
用於從原生代碼定位此視圖。優先級高於 `nativeID` 屬性。
...
用於在計算文本佈局（包括換行）後，以省略號截斷文本，使總行數不超過此數值。將此屬性設為 `0` 會取消此限制，意味著不會應用任何行數限制。
...
響應者權限已從視圖中移除。可能是在調用 `onResponderTerminationRequest` 後被其他視圖取得，也可能是由操作系統未經詢問直接取得（例如 iOS 上的控制中心/通知中心）。
...
當其他 `View` 試圖成為觸控回應者時，會要求當前 `View` 釋放回應權限。返回 `true` 表示允許釋放。
...
當滾動視圖禁用時，此參數定義觸控點可偏離按鈕多遠才會停用按鈕。停用後若移回範圍內，按鈕會重新啟用。建議傳入常數以減少記憶體分配。
...
`role` 向輔助技術使用者傳達元件用途，優先級高於 [`accessibilityRole`](text#accessibilityrole) 屬性。
...
設為 `true` 時，文字按下時不會顯示視覺反饋。預設按下會顯示灰色橢圓形高亮。
...
設定 Android API 23+ 的文字斷行策略，可選值為 `simple`、`highQuality`、`balanced`。
...
設定 iOS 14+ 的換行策略，可選值為 `none`、`standard`、`hangul-word` 及 `push-out`。
...
`TextLayout` 物件為 [`TextLayoutEvent`](text#textlayoutevent) 回調的一部分，包含 `Text` 行測量數據。
...
`TextLayoutEvent` 物件會在元件佈局變更時透過回調函數返回。它包含一個名為 `lines` 的鍵，其值是一個陣列，其中每個元素都是對應到每行渲染文字的 [`TextLayout`](text#textlayout) 物件。
