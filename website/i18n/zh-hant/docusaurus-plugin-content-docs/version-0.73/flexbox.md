...
:::caution
Flexbox 在 React Native 中的運作方式與網頁 CSS 基本相同，但存在一些例外情況：
預設值有所不同，`flexDirection` 預設為 `column` 而非 `row`，`alignContent` 預設為 `flex-start` 而非 `stretch`，`flexShrink` 預設為 `0` 而非 `1`，且 `flex` 參數僅支援單一數值。
:::

## Flex 彈性佈局

[`flex`](layout-props#flex) 屬性決定了子元素如何沿主軸「填充」可用空間。空間將根據每個元素的 flex 屬性值進行分配。

在以下範例中，紅色、橙色和綠色視圖都是設有 `flex: 1` 的容器視圖的子元素。紅色視圖使用 `flex: 1`，橙色視圖使用 `flex: 2`，綠色視圖使用 `flex: 3`。**1+2+3 = 6**，這表示紅色視圖將獲得 `1/6` 的空間，橙色視圖獲得 `2/6`，綠色視圖則獲得 `3/6` 的空間。
...
- `column`（**預設值**）從上到下排列子元素。若啟用換行，則下一行會從頂部第一個元素的右側開始。
- `row` 從左到右排列子元素。若啟用換行，則下一行會從左側第一個元素的下方開始。
- `column-reverse` 從下到上排列子元素。若啟用換行，則下一行會從底部第一個元素的右側開始。
- `row-reverse` 從右到左排列子元素。若啟用換行，則下一行會從右側第一個元素的下方開始。
...
佈局[`direction`](layout-props#direction) 指定了子元素和文字在層級結構中的排列方向，同時影響「起始(start)」和「結束(end)」所對應的邊緣。React Native 預設採用 LTR（左至右）佈局方向，此時「起始」指左側，「結束」指右側。

- `LTR`（**預設值**）文字和子元素從左至右排列。應用於元素起始處的外邊距(margin)和內邊距(padding)會作用於左側。
...
[`justifyContent`](layout-props#justifycontent) 描述如何沿容器的主軸對齊子元素。例如，當 `flexDirection` 設為 `row` 時可用此屬性水平置中子元素，設為 `column` 時則可垂直置中。
...
- `baseline` 沿著共同的基線對齊容器的子元素。可以將個別子元素設置為其父元素的參考基線。

:::info
要使 `stretch` 生效，子元素在次要軸上不能有固定尺寸。在以下範例中，設置 `alignItems: stretch` 不會有任何效果，直到移除子元素的 `width: 50`。
:::
...
[`alignSelf`](layout-props#alignself) 的選項和效果與 `alignItems` 相同，但它不是影響容器內的子元素，而是可以應用於單個子元素以改變其在父元素中的對齊方式。`alignSelf` 會覆蓋父元素通過 `alignItems` 設置的任何選項。
...
[alignContent](layout-props#aligncontent) 定義了沿著交叉軸的行的分布方式。此屬性僅在使用 `flexWrap` 將項目換行到多行時有效。
...
- `space-around` 在交叉軸上均勻分布換行的行，剩餘空間分配在行周圍。與 `space-between` 相比，使用 `space-around` 會在第一行的起始端和最後一行的末端也分配空間。
...
[`flexWrap`](layout-props#flexwrap) 屬性設置在容器上，用於控制當子元素沿主軸溢出容器尺寸時的行為。默認情況下，子元素會被強制排列在單一行內（這可能會壓縮元素）。如果允許換行，則會在必要時沿主軸將項目換行至多行。
...
- [`flexBasis`](layout-props#flexbasis) 是一種與軸向無關的方式，用於設定項目在主軸上的默認尺寸。設置子元素的 `flexBasis` 類似於在父容器為 `flexDirection: row` 時設定該子元素的 `width`，或在父容器為 `flexDirection: column` 時設定其 `height`。`flexBasis` 是項目在進行任何 `flexGrow` 和 `flexShrink` 計算前的初始尺寸。

- [`flexGrow`](layout-props#flexgrow) 描述容器內的主軸剩餘空間應如何分配給其子元素。在佈局子元素後，容器會根據子元素指定的 `flexGrow` 值分配剩餘空間。

  `flexGrow` 接受任何大於或等於 0 的浮點數，默認值為 0。容器會按照子元素的 `flexGrow` 值加權分配剩餘空間。

- [`flexShrink`](layout-props#flexshrink) 描述當子元素在主軸上的總尺寸超出容器尺寸時，應如何收縮子元素。`flexShrink` 與 `flexGrow` 非常相似，可將溢出的尺寸視為負剩餘空間來理解。這兩個屬性通常配合使用，使子元素能根據需要伸展或收縮。

  `flexShrink` 接受任何大於或等於 0 的浮點數，默認值為 0（在網頁上默認為 1）。容器會根據子元素的 `flexShrink` 值加權收縮子元素。
...
- [`gap`](layout-props#gap) 同時設定行與列的間距大小，是 `rowGap` 和 `columnGap` 的簡寫形式。

可結合使用 `flexWrap`、`alignContent` 與 `gap` 來為項目添加一致的間距。
...
`width` 屬性指定元素內容區域的寬度，`height` 屬性則指定元素內容區域的高度。

`width` 和 `height` 可接受以下值：

- `auto`（**默認值**）: React Native 會根據元素內容（無論是其他子元素、文字或圖片）自動計算寬度/高度。
- `pixels`: 以絕對像素值定義寬度/高度。根據組件上設置的其他樣式，這可能不是節點的最終尺寸。
...
- `relative` (**預設值**) 預設情況下，元素採用相對定位。這意味著元素會依照正常的佈局流來定位，然後根據 `top`、`right`、`bottom` 和 `left` 的值進行相對偏移。此偏移不會影響任何兄弟元素或父元素的位置。

- `absolute` 當採用絕對定位時，元素不會參與正常的佈局流。它會獨立於其兄弟元素進行佈局。其位置由 `top`、`right`、`bottom` 和 `left` 的值決定。
...
查看互動式的 [yoga playground](https://www.yogalayout.dev/playground)，可以幫助你更深入理解 flexbox 的運作方式。

我們已經涵蓋了基礎知識，但還有許多其他你可能需要用到的佈局樣式。控制佈局的完整屬性列表記錄在[這裡](./layout-props.md)。

此外，你也可以參考 [Wix 工程師](https://medium.com/wix-engineering/the-full-react-native-layout-cheat-sheet-a4147802405c)提供的一些範例。