...
在 iOS 中，回傳一個 Promise，該 Promise 將被調用並帶有一個包含 `action` 和 `activityType` 的物件。如果用戶關閉對話框，Promise 仍會以 action 為 `Share.dismissedAction` 且其他鍵為 undefined 的方式解析。請注意，某些分享選項在 iOS 模擬器上不會出現或無法運作。

在 Android 中，回傳一個 Promise，該 Promise 總是會以 action 為 `Share.sharedAction` 的方式解析。
