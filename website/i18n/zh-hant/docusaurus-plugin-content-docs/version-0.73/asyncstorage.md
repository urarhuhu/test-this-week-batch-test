...
`AsyncStorage` 是一個未加密、非同步、持久化且全域於應用的鍵值儲存系統。應優先使用它來替代 LocalStorage。

建議在輕度使用情境外，應基於 `AsyncStorage` 進行抽象封裝而非直接調用，因其運作於全域層級。

在 iOS 上，`AsyncStorage` 底層由原生程式碼實現，小量值會序列化儲存於字典，大量值則存於獨立檔案。Android 端會根據可用性選用 [RocksDB](https://rocksdb.org/) 或 SQLite 作為儲存引擎。

JavaScript 層的 `AsyncStorage` 為門面模式，提供清晰的 API 介面、真實的 `Error` 物件及非批次操作方法，所有 API 均返回 `Promise` 物件。
...
為指定 `key` 設定值並於完成時觸發回調。返回 `Promise` 物件。
...
清除 _所有_ 用戶端、函式庫等的 `AsyncStorage` 資料。通常不應直接調用此方法，建議使用 `removeItem` 或 `multiRemove` 僅清除應用自身的鍵值。返回 `Promise` 物件。
...
取得應用已知的 _所有_ 鍵值（含其他呼叫者及函式庫）。返回 `Promise` 物件。
...
此方法允許您批次獲取給定`key`陣列對應的項目。回調函數將被調用並傳入一組對應的鍵值對：
...
使用此方法作為批次儲存多組鍵值對的操作。當操作完成時，您將收到一個包含任何錯誤的單一回調：
