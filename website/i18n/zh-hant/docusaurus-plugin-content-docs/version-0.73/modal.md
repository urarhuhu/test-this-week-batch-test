...
`onRequestClose` 回調函式在使用者點擊 Android 的硬體返回按鈕或 Apple TV 的選單按鈕時被呼叫。由於此為必要屬性，請注意只要模態框處於開啟狀態，`BackHandler` 事件將不會被觸發。
在 iOS 上，當 `presentationStyle` 為 `pageSheet` 或 `formSheet` 時，此回調會在透過拖動手勢關閉模態框時被呼叫。
...
`statusBarTranslucent` 屬性決定模態框是否應位於系統狀態列下方。
...
`supportedOrientations` 屬性允許模態視窗旋轉至指定的任何方向。在 iOS 上，模態視窗仍會受到應用程式 Info.plist 中 UISupportedInterfaceOrientations 欄位所指定的限制。
