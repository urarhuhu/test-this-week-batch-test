...
此手動遷移程序應在[自動遷移程序](./migration-automated.mdx)後執行，以補足缺失部分或除錯遷移 CLI 輸出的問題。
...
在 Docusaurus 2 中，我們使用作用域套件名稱：

- `docusaurus` → `@docusaurus/core`

這提供了 Docusaurus 官方套件與社群維護套件間的明確區別。換言之，所有 Docusaurus 官方套件皆命名於 `@docusaurus/` 之下。

同時，Docusaurus 1 預設提供的文件站台功能現由 `@docusaurus/preset-classic` 提供。因此，我們也需添加此相依套件：
...
同時，CLI 指令更名為 `docusaurus <command>`（而非 `docusaurus-command`）。

您的 `package.json` 中的 `"scripts"` 區段應更新如下：
...
在 Docusaurus 1 中，所有建置成品皆位於 `website/build/<PROJECT_NAME>`。

在 Docusaurus 2 中，現已移至 `website/build`。請確保更新您的部署配置，以從正確的 `build` 目錄讀取生成檔案。

若您部署至 GitHub 頁面，請確保執行 `yarn deploy` 而非 `yarn publish-gh-pages` 腳本。

### `.gitignore` {#gitignore}

您的 `website` 中的 `.gitignore` 應包含：
...
D1 網站可能已有現有的 README 檔案。您可以修改它以反映 D2 變更，或複製預設的 [Docusaurus v2 README](https://github.com/facebook/docusaurus/blob/main/packages/create-docusaurus/templates/shared/README.md)。
...
在 Docusaurus 2 中，我們將每項功能（部落格、文件、頁面）拆分為插件以實現模組化。預設集是插件的組合包，為向後兼容，我們建立了 `@docusaurus/preset-classic` 預設集，其組合了 Docusaurus 1 中的大多數核心插件。

將以下預設集配置添加至您的 `docusaurus.config.js`。
...
已棄用。我們為 Docusaurus 2 開發了名為 [Infima](https://infima.dev/) 的自訂 CSS 框架，使用 CSS 變數進行主題設定。相關文檔尚未完全準備好，我們會在完成後更新此處。若要覆蓋 Infima 的 CSS 變數，請建立自己的 CSS 檔案（例如 `./src/css/custom.css`），並通過將其作為選項傳遞給 `@docusaurus/preset-classic` 來全局導入：
...
- `disableHeaderTitle`
- `disableTitleTagline`
- `docsSideNavCollapsible` 現在可在 `docsPluginOptions.sidebarCollapsible` 中找到，且此功能現在預設為開啟。
- `facebookAppId`
- `facebookComments`
- `facebookPixelId`
- `fonts`
...
我們計劃在未來將許多已棄用的配置欄位實現為插件。歡迎協助！

## 網址 {#urls}

在 v1 中，所有頁面都可以選擇是否使用 `.html` 副檔名。
...
如果您在 v1 中設置了 `cleanUrl: false`，可能有人發布了指向 `/installation.html` 的連結。
...
舊版本不允許嵌套側邊欄分類，且分類只能包含文件 ID。但 v2 允許無限嵌套側邊欄，並提供多種[側邊欄項目類型](../../guides/docs/sidebar/items.mdx)而不僅限於文件。
...
不再需要 `website/core/Footer.js`。若要修改 Docusaurus 預設頁尾，請透過[元件替換](../../swizzling.mdx)實現：

```bash npm2yarn
npm run swizzle @docusaurus/theme-classic Footer
```

此操作會將當前主題使用的 `<Footer />` 元件複製到網站根目錄下的 `src/theme/Footer` 目錄，後續可編輯此元件進行客製化。

請勿僅為在左側添加標誌而替換頁尾元件。v2 版已刻意移除左側標誌並移至底部，請直接在 `docusaurus.config.js` 中透過 `themeConfig.footer` 配置頁尾：
...
請參閱[建立頁面](guides/creating-pages.mdx)了解 Docusaurus 2 頁面運作機制。閱讀後請注意需將 v1 的 `pages/en` 檔案移至 `src/pages`。

Docusaurus v1 中，頁面會接收 `siteConfig` 物件作為 props。

Docusaurus v2 需改從 `useDocusaurusContext` 獲取 `siteConfig` 物件。

v2 版需在每個頁面套用主題佈局元件。Layout 元件接受元資料 props。

v2 已棄用 `CompLibrary`，需自行編寫 React 元件或使用 Infima 樣式（相關文件即將推出，敬請見諒！期間可檢查 V2 官網或查看 https://infima.dev/ 了解可用樣式）。
...
### 更新 Markdown 語法以相容 MDX {#update-markdown-syntax-to-be-mdx-compatible}
