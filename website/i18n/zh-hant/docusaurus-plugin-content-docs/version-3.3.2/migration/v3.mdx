...
Docusaurus v3 是一個**重大版本更新**，包含需要您相應調整網站的**破壞性變更**。我們將在此過程中提供指引，並提及一些可選建議。

這並非全面重寫，且破壞性變更相對容易處理。最簡單的網站最終可能只需更新 npm 相依項即可完成升級。
...
在升級前，我們建議先[**為 Docusaurus v3 準備您的網站**](/blog/preparing-your-site-for-docusaurus-v3)。有些變更您可以在**Docusaurus v2 環境下逐步處理**。這樣做有助於減少最終升級至 Docusaurus v3 所需的工作量。

對於複雜的網站，我們還建議設定[**視覺回歸測試**](/blog/upgrading-frontend-dependencies-with-confidence-using-visual-regression-testing)，這是確保網站視覺效果保持一致的好方法。Docusaurus v3 主要升級相依項，預計不會產生任何視覺變化。

:::

:::note

請查閱 [**Docusaurus v3.0.0**](https://github.com/facebook/docusaurus/releases/tag/v3.0.0) 的版本說明，並瀏覽相關 pull-requests 以獲取更多實用資訊及本文提及各項變更的背後動機。
...
升級至 MDX 伴隨著所有在 [MDX v2](https://mdxjs.com/blog/v2/) 和 [MDX v3](https://mdxjs.com/blog/v3/) 發布部落格文章中記載的重大變更。大多數重大變更來自 MDX v2，而 MDX v3 是一個相對較小的版本。[MDX v2 遷移指南](https://mdxjs.com/migrating/v2/)中有一個關於如何[更新 MDX 文件](https://mdxjs.com/migrating/v2/#update-mdx-files)的章節，這對我們特別相關。同時，請確保閱讀[MDX 疑難排解](https://mdxjs.com/docs/troubleshooting-mdx/)頁面，這可以幫助您解讀常見的 MDX 錯誤訊息。

請同時閱讀我們更新的[**MDX 與 React**](../guides/markdown-features/markdown-features-react.mdx) 文件頁面。

### 使用 MDX 遊樂場

MDX 遊樂場是您的新好朋友。它讓您了解您的內容如何**被編譯成 React 元件**，並在隔離環境中疑難排解編譯或渲染問題。
...
我們提供了一個 [docusaurus-mdx-checker](https://github.com/slorber/docusaurus-mdx-checker) CLI，可以輕鬆找出問題內容。在您的網站上運行此命令，以獲取在 MDX v3 下無法編譯的文件列表。
...
使用此 CLI 來估算使您的內容與 MDX v3 兼容所需的工作量。
...
`<` 字符用於開啟 [JSX 標籤](https://mdxjs.com/docs/what-is-mdx/#jsx)。若 MDX 認為你的 JSX 無效，現在會直接報錯。
...
> 在名稱前出現意外字符 `5` (U+0035)，預期應為字母、`$` 或 `_` 等可作為名稱開頭的字符
...
> 名稱中出現意外字符 `@` (U+0040)，預期應為字母、數字、`$` 或 `_` 等名稱字符；屬性前的空白；或標籤結束（注意：在 MDX 中建立連結請使用 `[文字](網址)`）

> 在本地名稱前出現意外字符 `/` (U+002F)，預期應為字母、`$` 或 `_` 等可作為名稱開頭的字符（注意：在 MDX 中建立連結請使用 `[文字](網址)`）
...
對於提供[自定義 `MDXComponent` 映射](../guides/markdown-features/markdown-features-react.mdx#mdx-component-scope)的用戶，元件現在會被「沙盒化」：

- `h1` 的 `MDXComponent` 映射僅適用於 `# 你好` 而不適用於 `<h1>你好</h1>`
- **小寫**的自定義元素名稱將不再被其對應的 `MDXComponent` 元件替換

:::danger[視覺差異]

你的 [`MDXComponent` 元件映射](../guides/markdown-features/markdown-features-react.mdx#mdx-component-scope)可能不會像以前那樣生效，且自定義元件可能不再被使用。
...
指令會被解析，目的是讓其他 Remark 插件處理。未處理的指令將被忽略，並且不會以其原始形式渲染回來。
...
日文和中文受此影響最大，但其他某些語言（如泰文和高棉文）也可能受到影響，例如當您嘗試強調行內代碼或連結時。使用空格分隔單詞的語言所受影響則小得多。

以下範例中的 `**`（除了 `` `**` `` 外）在 Docusaurus 2 中能正常解析為強調效果，但在 Docusaurus 3 中已不再支援。
...
MDX 生態系統中的所有官方套件（Unified、Remark、Rehype...）現已[僅支援 ES Modules](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)，不再支援 [CommonJS](https://nodejs.org/api/modules.html#modules-commonjs-modules)。

這意味著您無法再使用 `require("remark-plugin")` 這種寫法。

:::tip[升級指南]

Docusaurus v3 現已支援 [ES Modules](https://flaviocopes.com/es-modules/) 設定檔。我們建議您將設定檔遷移至 ES 模組，這樣就能輕鬆導入 Remark 插件：
...
若您希望繼續使用 CommonJS 模組，可使用動態導入作為變通方案，讓您能在 CommonJS 模組中導入 ES 模組。幸運的是，[Docusaurus 設定支援使用非同步函式](/docs/configuration#syntax-to-declare-docusaurus-config)來實現此功能。
...
如果您創建了自訂的 Remark 或 Rehype 插件，由於新版 AST 的結構變更，您可能需要重構甚至完全重寫這些插件。我們已建立[專屬支援討論串](https://github.com/facebook/docusaurus/discussions/9337)來協助插件開發者升級程式碼。

:::

### 程式碼格式化工具

目前最常見的格式化工具 Prettier 僅支援舊版 MDX v1，截至 Docusaurus v3.0.0 尚未支援 v3。您可在程式碼不相容的部分前添加 `{/* prettier-ignore */}` 來讓 Prettier 正常運作。
...
若您厭倦頻繁插入 `{/* prettier-ignore */}`，可考慮在 Prettier 開始支援 MDX v3 前，透過在 `.prettierignore` 檔案中添加以下內容來停用 Prettier 的 MDX 格式化功能：
...
Node.js 16 [已終止支援](https://nodejs.org/en/blog/announcements/nodejs16-eol)，Docusaurus v3 現在需要 **Node.js >= 18.0**。
...
在升級至 Docusaurus v3 前，先將您的 Docusaurus v2 網站升級至 Node.js 18 環境。
...
React 18 自帶了一些破壞性變更，根據您為網站編寫的自訂 React 程式碼量，處理這些變更的難度會有所不同。官方主題和外掛程式都已相容 React 18。

:::info[如何升級]

閱讀官方 [React v18.0](https://react.dev/blog/2022/03/29/react-v18) 和 [升級至 React 18 指南](https://react.dev/blog/2022/03/08/react-18-upgrade-guide)，並檢查您自己的 React 程式碼，找出哪些元件可能會受到此次升級影響。
...
這些功能在 Docusaurus 中的支援被視為實驗性質。未來我們可能需要調整整合方式，這可能導致運行時行為發生變化。
...
但請注意這是新的主要函式庫版本，包含破壞性變更，我們無法保證完全向下相容。若遇到問題，請查閱 [v3](https://github.com/FormidableLabs/react-live/releases/tag/v3.0.0) 和 [v4](https://github.com/FormidableLabs/react-live/releases/tag/v4.0.0) 的變更日誌。
...
多數 Docusaurus 使用者無需任何操作。使用表情符號快捷碼的使用者應閱讀 [變更日誌](https://github.com/rhysd/remark-emoji/blob/master/CHANGELOG.md) 並確認表情符號仍能正常顯示。

> **破壞性變更** 將 [node-emoji](https://www.npmjs.com/package/node-emoji) 從 v1 更新至 v2。此變更新增了許多新表情符號支援，同時移除了 GitHub 上不再有效的舊表情符號快捷碼。
...
但請注意這是新的主要函式庫版本，包含破壞性變更，我們無法保證完全向下相容。若遇到問題，請查閱 [v10](https://github.com/mermaid-js/mermaid/releases/tag/v10.0.0) 變更日誌。
...
官方 Docusaurus TypeScript 配置已從外部套件 [`@tsconfig/docusaurus`](https://www.npmjs.com/package/@tsconfig/docusaurus) 重新內化至我們新的單體儲存庫套件 [`@docusaurus/tsconfig`](https://www.npmjs.com/package/@docusaurus/tsconfig)。

此新套件與其他 Docusaurus 核心套件同步版本，將用於確保 TypeScript 的向下相容性及主要版本升級時的破壞性變更管理。
...
Docusaurus v3 將其內部配置載入函式庫從 [`import-fresh`](https://github.com/sindresorhus/import-fresh) 更換為 [`jiti`](https://github.com/unjs/jiti)。此函式庫負責載入如 `docusaurus.config.js`、`sidebars.js` 等檔案，以及 Docusaurus 插件。

:::info[如何升級]

理論上，您無需進行任何操作，現有的設定檔案應能如常運作。

然而，這是一次主要的依賴庫更換，可能會出現細微的行為變化。

:::

### 警告提示框注意事項

基於歷史原因，我們支援一個未公開的警告提示框語法`:::warning`，其會以紅色呈現。
...
然而，其顏色與圖示始終存在錯誤。Docusaurus v3 正式重新引入 `:::warning` 提示框，將其納入文件並修正顏色與圖示。
...
若您先前使用未公開的 `:::warning` 提示框，請逐一確認黃色是否適用於各使用情境。若需維持紅色，請改用 `:::danger`。

Docusaurus v3 亦[棄用 `:::caution`](https://github.com/facebook/docusaurus/pull/9308) 提示框。請將 `:::caution`（黃色）重構為 `:::warning`（黃色）或 `:::danger`（紅色）。
...
此重大變更僅影響 **在 `v2.0.0-beta.10` 版（2021年12月）前啟用文件版本化功能** 的早期 Docusaurus v2 使用者。

當建立 `v1.0.0` 版本時，側邊欄檔案包含 `version-v1.0.0/` 前綴，[此語法在 Docusaurus v3 已不再支援](https://github.com/facebook/docusaurus/pull/9310)。
...
對於替換過文件相關主題元件（如 `@theme/DocPage`）的使用者，這些元件已大幅重構以提升自訂便利性。

技術上**這不屬於重大變更**，因這些元件**標記為不建議替換**，但許多 Docusaurus 站點仍抽換了文件相關元件，需注意其自訂內容可能導致 Docusaurus 異常。

:::info[如何升級]

刪除所有替換過的元件，重新執行替換程序，並在新版元件基礎上重新套用自訂內容。
...
部分變更非強制性，但了解這些變更有助於充分發揮 Docusaurus v3 功能。
...
我們建議在 Markdown 檔案中使用 JSX、`import` 或 `export`（即 MDX 功能）時，使用 `.mdx` 副檔名。這在語義上更為正確，並提高了與外部工具（IDE、格式化工具、linter 等）的兼容性。
...
`hast-util-is-element` 在 Docusaurus v3 中已不再需要。如果您已安裝且未在其他地方使用，可以通過運行 `npm uninstall hast-util-is-element` 來移除它。
...
Docusaurus 現在使用 [Markdown 指令](https://talk.commonmark.org/t/generic-directives-plugins-syntax/444)（通過 [remark-directive](https://github.com/remarkjs/remark-directive) 實現）來實現警告，提供指令標籤的語法需要使用方括號：
...
請注意**我們的時間相當寶貴**。為確保您的支援請求不被忽略，請務必：

- 提供**最小化**且可立即執行的重現案例，建議使用 [docusaurus.new](https://docusaurus.new) 建立
- 若網站能成功建置，請提供展示問題的線上部署網址
- 清楚描述問題細節，避免僅籠統表示「無法運作」
- 附上所有相關材料：程式碼片段、儲存庫網址、Git 分支連結、完整堆疊追蹤、螢幕截圖或影片
- 以清晰簡潔的方式提出請求，展現您已盡力協助我們理解問題

您亦可考慮付費委託 [Docusaurus 服務供應商](https://github.com/facebook/docusaurus/discussions/9281) 代為升級。若您的專案屬開源性質，也可在社群尋求 [免費友善協助](https://github.com/facebook/docusaurus/discussions/9283)。