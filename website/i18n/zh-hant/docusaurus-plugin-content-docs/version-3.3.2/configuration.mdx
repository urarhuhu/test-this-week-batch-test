...
Docusaurus 對於配置有獨特的見解。我們鼓勵您將網站相關信息集中到一個地方。我們會保護此文件的字段，並促進讓這個數據對象在您的網站中可訪問。

維護良好的 `docusaurus.config.js` 可以幫助您、您的協作者以及開源貢獻者能夠專注於文檔，同時仍然能夠自定義網站。
...
- **必需：** 使用 `export default /* 您的配置 */`（或 `module.exports`）來導出您的 Docusaurus 配置
- **可選：** 使用 `import Lib from 'lib'`（或 `require('lib')`）來導入 Node.js 包
...
即使您在開發網站時，也不應該從頭開始編寫 `docusaurus.config.js`。所有模板都帶有一個 `docusaurus.config.js`，其中包含常見選項的默認值。
...
網站元數據包含基本的全局元數據，例如 `title`、`url`、`baseUrl` 和 `favicon`。

它們在多個地方使用，例如您的網站標題和標題、瀏覽器標籤圖標、社交分享（Facebook、X）信息，甚至用於生成正確的路徑來提供您的靜態文件。

### 部署配置 {#deployment-configurations}

部署配置（例如 `projectName`、`organizationName` 和可選的 `deploymentBranch`）在您使用 `deploy` 命令部署網站時使用。
...
若要為預設集中捆綁的插件或主題指定選項，需透過 `presets` 欄位傳遞選項。在此範例中，`docs` 指代 `@docusaurus/plugin-content-docs`，`theme` 指代 `@docusaurus/theme-classic`。
...
對於新的 Docusaurus 專案，我們會在專案根目錄自動生成 `babel.config.js`。
...
大多數情況下，此配置都能正常運作。若您想自訂 Babel 配置（例如新增 Flow 支援），可以直接編輯此檔案。要使變更生效，您需要重新啟動 Docusaurus 開發伺服器。