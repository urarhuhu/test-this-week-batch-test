...
**Docusaurus 核心本身不提供任何功能。** 所有功能都委託給個別插件實現：[文檔功能](./guides/docs/docs-introduction.mdx)由[文檔插件](./api/plugins/plugin-content-docs.mdx)提供；[部落格功能](./blog.mdx)由[部落格插件](./api/plugins/plugin-content-blog.mdx)提供；獨立[頁面](./guides/creating-pages.mdx)則由[頁面插件](./api/plugins/plugin-content-pages.mdx)提供。若未安裝任何插件，網站將不會包含任何路由。
...
若您精力充沛，也可閱讀[插件開發指南](./advanced/plugins.mdx)或[插件方法參考文檔](./api/plugin-methods/README.mdx)來了解如何自行開發插件。
...
路徑應為絕對路徑或相對於配置檔案的相對路徑。

## 配置插件 {#configuring-plugins}

最基本的插件使用方式，是僅提供插件名稱或插件路徑。

但插件可透過在配置中將名稱與選項物件包裝成二元組來指定選項。此風格通常稱為「Babel 風格」。
...
所有 Docusaurus 內容插件皆支援多實例運行。例如，可能需要[多個文檔插件實例](./guides/docs/docs-multi-instance.mdx)或[多個部落格](./blog.mdx#multiple-blogs)。必須為每個插件實例分配唯一ID，預設情況下插件ID為 `default`。
...
主題的載入方式與插件完全相同。從使用者角度來看，在安裝和配置插件時，`themes` 與 `plugins` 條目可互換使用。唯一細微差別在於主題會在插件之後載入，且[主題可覆寫插件的預設主題元件](./advanced/client.mdx#theme-aliases)。

:::tip

`themes` 和 `plugins` 選項會導致不同的[簡寫解析規則](#module-shorthands)，因此若您想利用簡寫功能，請務必使用正確的條目！
...
Docusaurus 支援插件、主題與預設集的簡寫。當它看到一個插件/主題/預設集名稱時，會依以下順序嘗試載入：
...
若名稱不存在（如 `@jquery`），則會載入 `[scope]/docusaurus-[moduleType]`（例如 `@jquery/docusaurus-plugin`）。否則，系統會嘗試以下路徑：

- `[scope]/[name]`（如 `@jquery/content-docs`）
- `[scope]/docusaurus-[moduleType]-[name]`（如 `@jquery/docusaurus-plugin-content-docs`）

以下是一些在 `plugins` 欄位註冊外掛的範例。請注意，與 [ESLint](https://eslint.org/docs/user-guide/configuring/plugins#configuring-plugins) 或 [Babel](https://babeljs.io/docs/en/options#name-normalization) 強制要求外掛遵循一致的命名規範不同，Docusaurus 允許更大的命名自由度，因此解析結果並非絕對，而是依據上述優先級定義。
