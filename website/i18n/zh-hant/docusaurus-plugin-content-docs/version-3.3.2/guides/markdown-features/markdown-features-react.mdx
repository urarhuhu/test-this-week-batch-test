...
由於所有文檔文件都使用 MDX 解析，任何看似 HTML 的內容實際上是 JSX。因此，如果您需要內聯樣式一個元件，請遵循 JSX 風格並提供樣式物件。
...
您也可以匯入在其他文件中定義的自訂元件，或透過 npm 安裝的第三方元件。
...
雖然在 Markdown 中宣告元件對於簡單情況非常方便，但由於編輯器支援有限、解析錯誤風險高以及可重用性低，維護起來會變得困難。當您的元件涉及複雜的 JS 邏輯時，請使用獨立的 `.js` 文件：
...
若要註冊額外標籤名稱（如上述 `<Highlight>` 標籤），建議採用[封裝 `@theme/MDXComponents`](../../swizzling.mdx#wrapping)方式，如此便無需維護所有現有映射。由於替換 CLI 尚不支援封裝非元件檔案，您需手動建立封裝器：
...
- `mdx.format: md` 前置資料
- 結合 `siteConfig.markdown.format: "detect"` 設定的 `.md` 副檔名
...
按照慣例，使用 **`_` 文件名前綴** 將不會創建任何文檔頁面，並且表示該 Markdown 文件是一個 **「部分」**，將被其他文件導入。
...
- `contentTitle`：Markdown 標題，即 Markdown 文本中的第一個 `h1` 標題。如果沒有（例如，在前端元數據中指定了標題），則為 `undefined`。
