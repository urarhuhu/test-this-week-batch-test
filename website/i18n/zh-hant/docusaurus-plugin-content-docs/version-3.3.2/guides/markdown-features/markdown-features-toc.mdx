...
每份 Markdown 文件右上角會顯示目錄。預設情況下，目錄僅顯示 h2 和 h3 標題，這通常足以呈現頁面結構。如需調整顯示的標題層級範圍，您可以針對單一頁面或全域設定最小與最大標題層級。
...
即使已設定全域選項，您仍可透過 front matter 進行局部覆寫。

:::note

`themeConfig` 選項會影響網站上所有目錄（包含[行內目錄](#inline-table-of-contents)），但 front matter 參數僅影響右上角的目錄。您需要使用 `minHeadingLevel` 和 `maxHeadingLevel` 屬性來個別自訂每個 `<TOCInline />` 元件。

:::

## 行內目錄 {#inline-table-of-contents}

透過 MDX 功能，您也可以在 Markdown 文件內直接嵌入行內目錄。

MDX 文件中可透過 `toc` 變數取得所有標題資訊。預設僅顯示 `h2` 和 `h3` 標題。您可透過為個別 `TOCInline` 元件設定 `minHeadingLevel` 或 `maxHeadingLevel` 來調整顯示的標題層級範圍。
...
請注意 `toc` 是扁平化陣列，因此您可以輕鬆過濾不需要的節點或插入額外節點，重新建立目錄樹狀結構。
...
目前尚無法直觀地插入額外標題或忽略特定標題，此功能仍在開發中。若此功能對您至關重要，請在 [此議題](https://github.com/facebook/docusaurus/issues/6201) 中回報您的使用案例。

---

:::warning

以下僅為虛擬內容，用於在當前頁面生成更多目錄項目。
