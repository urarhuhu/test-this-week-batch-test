...
如果您試用過 [MDX 遊樂場](https://mdxjs.com/playground/)，會注意到 MDX 轉譯有兩個中間步驟：Markdown 抽象語法樹（MDAST）和超文本抽象語法樹（HAST），最終才會輸出 JSX。MDX 插件也分為兩種形式：
...
許多官方 Remark/Rehype 插件僅支援 [**ES Modules**](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)（JavaScript 模組系統），而 Docusaurus 支援此規範。建議使用 [**ES Modules**](https://flaviocopes.com/es-modules/) 格式的設定檔，以便更輕鬆導入這類套件。

:::

接著導入插件，並透過 `docusaurus.config.js` 中的插件或預設配置將其加入選項：
...
部分插件可接受自訂配置選項。此時請使用 `[plugin, pluginOptions]` 語法，如下所示：
...
以下說明**並非**建立外掛程式的完整指南，僅示範如何使其在 Docusaurus 中運作。如需更深入的工作原理說明，請參閱 [Remark](https://github.com/remarkjs/remark/blob/main/doc/plugins.md#create-plugins) 或 [Rehype](https://github.com/rehypejs/rehype/blob/main/doc/plugins.md#create-plugins) 的官方文件。

:::

舉例來說，我們建立一個會遍歷所有 `h2` 標題並加上 `第 X 節. ` 前綴的外掛程式。首先，在任何位置建立外掛程式原始檔——您甚至可以將其發佈為獨立的 npm 套件，並依照前述方式安裝。我們會將外掛程式放在 `src/remark/section-prefix.js`。remark/rehype 外掛程式本質上是一個接收 `options` 並返回操作 AST 的 `transformer` 的函式。
...
`transformer` 函式具有第二個參數 [`vfile`](https://github.com/vfile/vfile)，當您需要存取當前 Markdown 檔案的路徑時非常實用。
...
Docusaurus 的預設外掛程式會先於自訂 remark 外掛程式執行，這意味著圖片或連結已被轉換為帶有 `require()` 呼叫的 JSX。例如在上例中，即使所有 `h2` 標題都加上了 `第 X 節. ` 前綴，生成的目錄仍保持不變，因為目錄生成外掛程式在我們的自訂外掛程式之前執行。若您需要在預設外掛程式處理前操作 MDAST，請使用 `beforeDefaultRemarkPlugins` 和 `beforeDefaultRehypePlugins`。
