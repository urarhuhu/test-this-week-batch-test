...
若您使用 [Prettier](https://prettier.io) 格式化 Markdown 文件，Prettier 可能會將提示框語法自動轉換為無效格式。為避免此問題，請在起始與結束指令周圍添加空行。這也是為何此處所有範例都在內容周圍保留空行的原因。
...
您可透過 [swizzling](../../swizzling.mdx) 自訂每種提示框類型的渲染方式。通常只需簡單的封裝即可達成目標。例如以下範例中，我們僅替換了 `info` 類型提示框的圖標。
...
您可以為 `:::my-custom-admonition` 提供對應的 React 組件，方法是建立以下文件（由於這不是 React 組件文件，因此無法透過 swizzle 功能自動生成）：
