...
您可以在[10分鐘內學會 Markdown](https://commonmark.org/help/)。
...
**選項設定**：選擇正確格式（MDX 或 CommonMark）以及 Docusaurus 使用的以下插件：`remark-gfm`、`remark-directive`、`rehype-raw`。

:::

## MDX 與 CommonMark 比較 {#mdx-vs-commonmark}

Docusaurus 使用 MDX 編譯器將 `.md` 和 `.mdx` 文件編譯為 React 組件，但**語法解析方式可能因設定而異**。
...
基於歷史因素，**Docusaurus v3 預設對所有文件（包括 `.md` 文件）使用 MDX 格式**。
