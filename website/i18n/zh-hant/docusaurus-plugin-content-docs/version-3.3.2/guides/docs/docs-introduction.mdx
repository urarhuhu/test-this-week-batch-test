...
本指南將依序介紹：從[如何配置單獨頁面](./docs-create-doc.mdx)開始，到[如何建立單一或多個側邊欄](./sidebar/index.mdx)，再到[如何建立與管理版本](./versioning.mdx)，最後是[如何使用多個文件插件實例](./docs-multi-instance.mdx)。
...
所有文件都將在子路由 `docs/` 下提供。但如果**您的站點只有文件**，或希望將文件置於根目錄以優先顯示該怎麼辦？
...
若您已為某文件添加 `slug: /` 使其成為首頁，應刪除現有的 `./src/pages/index.js` 首頁文件，否則將有兩個檔案映射至同一路由！
...
對於僅想使用 Docusaurus 部落格功能的用戶，也有「純部落格模式」。您可使用上述相同方法，按照[純部落格模式](../../blog.mdx#blog-only-mode)的設定說明操作。
