...
用它來包裹可能拋出錯誤的組件，當錯誤發生時顯示後備內容，而不是導致整個應用崩潰。
...
- `fallback`: 可選的渲染回調函數，返回一個 JSX 元素。它將接收一個包含 2 個屬性的對象：`error`，捕獲到的錯誤，以及 `tryAgain`，一個函數 (`() => void`) 回調，用於重置組件中的錯誤並嘗試重新渲染。如果未提供，則會渲染 `@theme/Error` 代替。`@theme/Error` 用於包裹整個網站的錯誤邊界，位於佈局之上。
...
此組件支持鏈接到內部頁面，以及一個強大的性能特性稱為預加載。預加載用於預取資源，以便用戶通過此組件導航時資源已經被獲取。我們使用 `IntersectionObserver` 在 `<Link>` 進入視口時發起低優先級請求，然後使用 `onMouseOver` 事件在用戶可能導航到請求的資源時觸發高優先級請求。

此組件是 react-router 的 `<Link>` 組件的封裝，添加了 Docusaurus 特有的增強功能。所有屬性都會傳遞給 react-router 的 `<Link>` 組件。
...
相比普通的 `<a>` 標籤，更推薦使用此組件，因為 Docusaurus 在您使用 `<Link>` 時會進行許多優化（例如檢測錯誤路徑、預取、應用基礎 URL 等）。
...
當程式碼因存取 `window` 或 `document` 物件而無法在 Node.js 環境執行時，可使用此元件進行整合。

:::

#### 屬性 {#browseronly-props}

- `children`: 渲染函數屬性，返回僅在瀏覽器中執行的 JSX。不會在 Node.js 中執行。
...
當[本地化您的網站](./i18n/i18n-introduction.mdx)時，`<Translate/>` 元件將為 React 元件（如您的主頁）提供**翻譯支援**。`<Translate>` 元件支援[插值功能](#interpolate)。
...
`<Translate/>` 的屬性**必須是硬編碼的字串**。

除了用於插值的 `values` 屬性外，**無法使用變數**，否則靜態提取將無法運作。
...
`<Translate>` 元件支援插值。您還可以透過一些自訂程式碼和[`translate` 命令式 API](#translate-imperative) 實現[字串複數化](https://github.com/facebook/docusaurus/pull/i18n/i18n-tutorial.mdx#pluralization)。
...
React 鉤子，用於存取 Docusaurus 上下文。上下文包含來自 [docusaurus.config.js](api/docusaurus.config.js.mdx) 的 `siteConfig` 物件和一些額外的網站元數據。
...
在 React 渲染邏輯中請使用此鉤子而非 `typeof windows !== 'undefined'`。
...
對於[靜態資源](./guides/markdown-features/markdown-features-assets.mdx)，建議改用 `require()` 調用：
...
插件 ID 僅在單個插件被多次使用時有意義。每個插件實例均可建立自己的全域資料。
...
存取特定插件建立的全域資料。給定插件名稱後，將按插件 ID 返回該名稱下所有插件實例的資料。
...
React Hook 用於存取 Docusaurus 的失效連結檢查 API，提供一種方式讓 Docusaurus 頁面回報並收集其連結與錨點。

:::warning

這是一個**進階** API，**大多數 Docusaurus 使用者無需直接使用**。
...
僅在實作自訂的 `<Heading>` 或 `<Link>` 元件時才需使用 `useBrokenLinks()`。
...
[`<Translate>`](#translate) 元件的命令式對應實現，同時支援[佔位符插值](#interpolate)。
