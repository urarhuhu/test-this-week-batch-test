...
Docusaurus 提供便捷的**互動式 CLI** 來替換元件。通常您只需記住以下指令：
...
彈出主題元件是指**建立原始主題元件的副本**，您可對其進行**完整客製化與覆寫**。

要彈出主題元件，請使用互動式元件替換 CLI 或搭配 `--eject` 選項：
...
彈出[**不安全**](#what-is-safe-to-swizzle)元件時，可能會複製大量內部代碼，這些代碼現在需要自行維護。這可能使 Docusaurus 升級更加困難，因為若接收的 props 或使用的內部主題 API 發生變化，您需要遷移自訂內容。

**盡可能優先選擇[包裝](#wrapping)**：需維護的代碼量較少。
...
如果您有**強烈需求需混寫不安全元件**，請[**在此回報**](https://github.com/facebook/docusaurus/discussions/5468)，我們將共同尋找解決方案使其安全化。
...
3. **從高層級元件開始**。元件會形成樹狀結構，某些元件會導入其他元件。每條路由都會關聯一個頂層元件（大多數列於[內容外掛的路由系統](./advanced/routing.mdx#routing-in-content-plugins)）。例如所有部落格文章頁面的最頂層元件都是 `@theme/BlogPostPage`。您可以先 swizzle 此元件，再向下遍歷元件樹定位目標元件。找到正確元件後，請刪除其他 swizzle 產生的文件以避免維護過多元件。
4. **閱讀[主題原始碼](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-theme-classic/src/theme)** 並善用搜尋功能。

:::tip[直接提問！]

若仍無法確定該 swizzle 哪個元件來實現需求，可透過我們的[支援管道](/community/support)尋求協助。

我們也希望更了解您的客製化需求，歡迎[提交案例討論](https://github.com/facebook/docusaurus/discussions/5468)。

:::

## 是否必須 swizzle？ {#do-i-need-to-swizzle}

Swizzle 本質上意味著您需維護與 Docusaurus 內部 API 互動的 React 程式碼。在客製化網站時，請優先考慮以下替代方案：

1. **使用 CSS**。CSS 規則和選擇器通常能實現相當程度的客製化，詳見[樣式與佈局](./styling-layout.mdx)。
2. **使用翻譯功能**。雖然看似不直觀，但翻譯本質上也是一種文字標籤客製化方式。例如若網站預設語言為 `en`，仍可執行 `yarn write-translations -l en` 後編輯生成的 `code.json` 文件，詳見[國際化教程](./i18n/i18n-tutorial.mdx)。

:::tip

**範圍越小越好**。若必須 swizzle，請僅針對相關部分操作，並盡量減少自行維護的程式碼量。swizzle 小型元件通常能降低升級時的**破壞性變更**風險。

相較於[完整抽離](#ejecting)，[封裝元件](#wrapping)是更安全的替代方案。

:::

## 使用 `<Root>` 封裝您的網站 {#wrapper-your-site-with-root}

`<Root>` 元件會渲染在 React 樹結構的**最頂層**（高於主題 `<Layout>`），且**永不卸載**。非常適合用來添加不應在導航時重新初始化的狀態邏輯（使用者認證狀態、購物車狀態等）。

請手動 swizzle 此元件，在 `src/theme/Root.js` 建立檔案：
...
可用此元件來渲染 React Context providers。
