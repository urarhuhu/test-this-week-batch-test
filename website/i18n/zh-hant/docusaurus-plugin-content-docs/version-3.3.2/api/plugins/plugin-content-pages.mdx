...
Docusaurus 的預設頁面外掛程式。經典模板預設配置了此外掛程式，提供[建立頁面](guides/creating-pages.mdx)功能。
...
多數 Docusaurus 使用者會透過預設選項來配置此外掛程式。
...
Markdown 頁面可使用以下 Markdown [前置資料](../../guides/markdown-features/markdown-features-intro.mdx#front-matter)元資料欄位，需以 `---` 行包夾。
