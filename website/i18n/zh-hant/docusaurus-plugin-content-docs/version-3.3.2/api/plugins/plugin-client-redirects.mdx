...
此插件會為您的靜態站點生成額外的 HTML 頁面，透過 JavaScript 將用戶重定向至現有的 Docusaurus 頁面。

:::warning[僅在生產環境生效]

此插件在開發環境始終處於非活躍狀態，且僅在生產環境生效，因其運作機制依賴構建輸出結果。
...
本插件會同時讀取 [`siteConfig.onDuplicateRoutes`](../docusaurus.config.js.mdx#onDuplicateRoutes) 配置，以調整當多個文件將被生成至相同路徑時的日誌級別。
...
「來源」與「目標」的概念是本插件的核心設計。「來源」表示您要創建的路徑，即一個將被寫入的額外HTML文件；「目標」表示您要重定向至的路徑，通常是Docusaurus已識別的路由。

因此同一個「目標」可對應多個「來源」：我們會創建多個最終指向同一目的地的HTML文件。但一個「來源」絕不能對應多個「目標」：生成的HTML文件必須有確定的重定向終點。
