...
Docusaurus 插件，使用 [Workbox](https://developers.google.com/web/tools/workbox) 添加 PWA 支援。此插件僅在生產環境構建時生成 [Service Worker](https://developers.google.com/web/fundamentals/primers/service-workers)，讓您能創建完全符合 PWA 標準的文檔站點，支援離線瀏覽與安裝功能。
...
僅安裝 service worker 並不足以使您的應用成為 PWA。您至少需要包含 [Web 應用清單](https://developer.mozilla.org/en-US/docs/Web/Manifest) 並在 `<head>` 中添加正確的標籤（參見 [選項 > pwaHead](#pwahead)）。

部署後，您可以使用 [Lighthouse](https://developers.google.com/web/tools/lighthouse) 對站點進行審核。

有關如何使您的站點成為 PWA 的更詳盡列表，請參考 [PWA 檢查清單](https://developers.google.com/web/progressive-web-apps/checklist)

## 應用安裝支援 {#app-installation-support}

如果您的瀏覽器支援，您應該能夠將 Docusaurus 站點安裝為應用程式。

![應用安裝過程的螢幕錄製。瀏覽器地址欄中出現一個按鈕，點擊後顯示「安裝此應用程式？」的對話框。點擊「安裝」按鈕後，操作系統中會開啟一個新應用程式，顯示 Docusaurus 首頁。](/img/pwa_install.gif)
...
我們通過 service-worker 預緩存技術，讓用戶能夠離線瀏覽 Docusaurus 站點。

[workbox-precaching](https://developers.google.com/web/tools/workbox/modules/workbox-precaching) 頁面解釋了核心概念：

> Service worker 的一個特性是能在安裝階段將一組文件保存到緩存中，這通常稱為「預緩存」，因為您是在 service worker 被使用前就緩存內容。
>
> 這樣做的主要原因是讓開發者能控制緩存，意味著他們可以決定文件何時被緩存、緩存多久，以及不經過網路直接提供給瀏覽器，這可用來創建能離線工作的網路應用。
>
> Workbox 通過簡化 API 和確保資源高效下載，大幅降低了預緩存的實現難度。

預設情況下，當站點被安裝為應用時會啟用離線模式。詳見 `offlineModeActivationStrategies` 選項說明。

當站點完成預緩存後，service worker 會為後續訪問提供緩存響應。當新版本部署並伴隨新 service worker 時，新的 worker 會開始安裝並最終進入等待狀態。在此等待狀態期間，會顯示重新載入彈窗，要求用戶刷新頁面以獲取新內容。在用戶清除應用緩存或點擊彈窗中的 `reload` 按鈕前，service worker 會持續提供舊內容。

:::warning

離線模式/預緩存需要提前下載站點所有靜態資源，可能消耗不必要的頻寬。對於某些類型的站點，啟用此功能可能並非好主意。
...
- `standalone`: 對以獨立應用模式運行的使用者啟用 (通常是 PWA 安裝後的情況)
...
傳遞給 `workbox.injectManifest()` 的 [Workbox 選項](https://developer.chrome.com/docs/workbox/reference/workbox-build/#type-InjectManifestOptions)。此選項讓您控制哪些資源會被預先快取並可在離線時使用。
...
包含 `tagName` 和屬性鍵值對的物件陣列，用於注入 `<head>` 標籤。技術上您可透過此方式注入任何 head 標籤，但理想情況下用於使網站符合 PWA 規範的標籤。以下是使應用完全合規的標籤列表:
...
適用於額外的 Workbox 規則。您可在此執行 service worker 能做的任何操作，並充分利用 workbox 函式庫的功能。程式碼會被轉譯，因此可使用現代 ES6+ 語法。
...
在 Docusaurus 應用前添加條目，使註冊可在應用運行前完成。預設的 `registerSW.js` 檔案足以進行簡單註冊。
