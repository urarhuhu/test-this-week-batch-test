...
這些檔案將由 [`write-translations` CLI](../../cli.mdx#docusaurus-write-translations-sitedir) 寫入插件的 i18n 子資料夾，並會在呼叫 [`translateContent()`](#translateContent) 和 [`translateThemeConfig()`](#translateThemeConfig) 前，根據當前語言環境讀取。
...
應回傳 `Record<string, string>` 格式的訊息，其中鍵為翻譯 ID，值為使用網站當前語言環境本地化的訊息（不包含描述）。
