...
在建置過程中，插件會平行載入以獲取各自的內容並將其渲染為路由。插件還可以配置 webpack 或對生成的文件進行後處理。

## `async loadContent()` {#loadContent}

插件應使用此生命週期從數據源（文件系統、遠程 API、無頭 CMS 等）獲取數據或進行一些伺服器端處理。返回值是它需要的內容。
...
在 `loadContent` 中加載的數據將在 `contentLoaded` 中被使用。它可以被渲染為路由、註冊為全局數據等。

### `content` {#content}

`contentLoaded` 將在 `loadContent` 完成後被調用。`loadContent()` 的返回值將作為 `content` 傳遞給 `contentLoaded`。
...
一個聲明式的回調，用於創建靜態數據（通常是 JSON 或字符串），這些數據稍後可以作為 props 提供給你的路由。接收文件名和要存儲的數據，並返回實際數據文件的路徑。
...
全局數據是...全局的：它的大小會影響網站所有頁面的加載時間，因此請盡量保持其體積小。盡可能優先使用 `createData` 和頁面特定數據。
...
修改內部 webpack 配置。如果返回值是一個 JavaScript 對象，它將使用 [`webpack-merge`](https://github.com/survivejs/webpack-merge) 合併到最終配置中。如果是一個函數，它將被調用並接收 `config` 作為第一個參數和 `isServer` 標誌作為第二個參數。

:::warning

`configureWebpack` 的 API 將來會修改為接受一個對象（`configureWebpack({config, isServer, utils, content})`）

:::

### `config` {#config}

`configureWebpack` 被調用時會傳入根據客戶端/伺服器端建置生成的 `config`。你可以將其視為要合併的基礎配置。
...
`configureWebpack` 會同時被呼叫並帶入外掛載入的內容。
...
`injectHtmlTags` 會同時被呼叫並帶入外掛載入的內容。
