...
當設定`respectPrefersColorScheme: true`時，`defaultMode`會被使用者系統偏好覆寫。
...
當需要在網站發布公告時，可添加公告欄。這是位於導覽列上方、非固定且可選擇關閉的面板。所有配置皆位於`announcementBar`物件中。
...
標誌可置於[靜態資源目錄](static-assets.mdx)。標誌URL預設為網站根路徑。您可自訂標誌URL，若為外部連結將在新分頁開啟。此外，可覆寫標誌連結的target屬性值，當文件網站託管於主站子目錄時特別有用（此時可能不需讓標誌連結至主站時開啟新分頁）。
...
可透過`themeConfig.navbar.items`添加導覽列項目。
...
React Router 應會自動為連結套用活動連結樣式，但在邊緣情況下可使用 `activeBasePath`。若某連結需在多個路徑下保持活動狀態（例如同一側邊欄下有多個文件夾），可使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更靈活的替代方案且優先級更高——Docusaurus 會將其解析為正則表達式並與當前 URL 進行匹配測試。
...
請注意，下拉選單基底項目本身也是可點擊連結，因此該項目可接受[純導航欄連結](#navbar-link)的任何屬性。
...
若需連結至特定文件，此特殊導航欄項目類型會渲染指向所提供 `docId` 文件的連結。只要您瀏覽同一側邊欄的文件，該連結就會獲得 `navbar__link--active` 類別。
...
使用者可在同一文件（只要文件 ID 跨版本保持一致）切換不同版本。
...
若您使用版本化文件，此特殊導航欄項目類型會連結至當前瀏覽的文件版本（取決於當前URL），並在無匹配版本時自動回退至最新版本。
...
若啟用[國際化功能](../../i18n/i18n-introduction.mdx)，此特殊導航欄項目會顯示包含所有可用語言的下拉選單。
...
#### 導航欄搜索框 {#navbar-search}

若啟用[搜索功能](../../search.mdx)，搜索框預設會出現在導航欄最右側。
...
#### 自定義HTML導航欄項目 {#navbar-with-custom-html}

此項目類型允許在導航欄中直接渲染自定義HTML標記。
...
啟用此UI特性後，當用戶向下滾動頁面時導航欄會自動隱藏，向上滾動時重新顯示。
...
可設定靜態導航欄樣式而不影響主題切換功能。所選樣式將始終生效，不受用戶主題選擇影響。

目前支援兩種樣式選項：`dark`（暗色）與`primary`（基於`--ifm-color-primary`主色）。樣式預覽參見[Infima文檔](https://infima.dev/docs/components/navbar/)。
...
Docusaurus使用[Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer)實現代碼高亮，所有配置均位於`prism`對象中。
...
預設採用[Palenight](https://github.com/FormidableLabs/prism-react-renderer/blob/master/packages/prism-react-renderer/src/themes/palenight.ts)語法高亮主題。可從[可用主題列表](https://github.com/FormidableLabs/prism-react-renderer/tree/master/packages/prism-react-renderer/src/themes)選擇自定義主題，並為暗色模式指定不同高亮主題。
...
您可為未在開頭三個反引號（即```）後指定語言的代碼塊設置預設語言。請注意，必須傳入有效的[語言名稱](https://prismjs.com/#supported-languages)。
...
可透過`themeConfig.footer`添加標誌與版權聲明至頁尾。標誌可置於[靜態資源目錄](static-assets.mdx)，其URL規則與導航欄標誌相同。
...
可透過`themeConfig.footer.links`添加連結至頁尾。頁尾配置分兩種類型：**多欄式頁尾**與**簡易頁尾**。

多欄式頁尾連結需為每欄定義`title`及一組`FooterItem`列表。
...
用於存取色彩模式的React鉤子。此上下文包含設定淺色/深色模式的函數，並暴露當前使用模式的布林值。
