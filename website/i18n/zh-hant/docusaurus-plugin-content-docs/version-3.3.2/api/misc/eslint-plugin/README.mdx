...
[ESLint](https://eslint.org/) 是一個靜態分析程式碼的工具，能透過編輯器提示和命令行報告問題或建議最佳實踐。Docusaurus 提供了一個 ESLint 插件來強制執行 Docusaurus 的最佳實踐。
...
這將啟用 `@docusaurus` eslint 插件並使用 `recommended` 配置。請參閱下方的 [支援的規則](#supported-rules) 以了解這將啟用的規則列表。

### 手動配置

如需更細緻的控制，你也可以手動啟用插件並直接配置你想使用的規則：
...
- 全部：啟用**所有**規則。這會在次要版本之間變更，因此如果你希望避免意外的破壞性變更，則不應使用此配置。
