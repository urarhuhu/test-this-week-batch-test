...
該服務對任何開發者文件或技術部落格**免費**：請務必閱讀[檢查清單](https://docsearch.algolia.com/docs/who-can-apply/)並[申請加入 DocSearch 計畫](https://docsearch.algolia.com/apply)。

DocSearch 每週爬取您的網站一次（排程可透過網頁介面設定），並將所有內容彙總至 Algolia 索引中。這些內容隨後會透過 Algolia API 直接從前端查詢。
...
申請通過並部署後，您將收到一封包含所有詳細資訊的電子郵件，以便將 DocSearch 加入專案。可透過[網頁介面](https://crawler.algolia.com/)編輯和管理爬取作業。索引在部署後立即可用，因此通常無需手動配置。
...
Docusaurus 自身的 `@docusaurus/preset-classic` 支援 Algolia DocSearch 整合。若使用經典預設配置，無需額外安裝。
...
若搜尋在重大變更後失效，請透過 Algolia 儀表板 **手動觸發新一輪爬取**。
...
同理，瀏覽法文站點時，若顯示英文文件的搜尋結果也不合理。

為解決此問題，上下文搜尋會動態識別您正在瀏覽的特定文件版本與語言，並自動建立對應的搜尋篩選條件。
...
詳見 [Algolia 面搜尋相關文件](https://www.algolia.com/doc/guides/managing-results/refine-results/faceting/)。
...
DocSearch 預載符合無障礙標準的精心調校主題，確保色彩與對比度達標。

您仍可透過編輯 `/src/css/custom.css` 檔案，套用 Docusaurus 的 [Infima CSS 變數](styling-layout.mdx#styling-your-site-with-infima) 來調整 DocSearch 樣式。
...
若需直接編輯 Algolia 搜尋的 React 元件，可對 `@docusaurus/theme-search-algolia` 中的 `SearchBar` 元件進行 [swizzle 操作](swizzling.mdx)：
...
我們[推薦特定爬蟲配置](#algolia-index-configuration)有其原因。若您選擇不同配置，我們將無法提供支援。

:::

您可透過以下步驟修正索引配置問題：

1. 使用[推薦爬蟲配置](#algolia-index-configuration)
2. 透過介面刪除現有索引
3. 透過介面觸發重新爬取
4. 確認索引已重建並包含以下分面欄位：`docusaurus_tag`、`language`、`lang`、`version`、`type`
5. 此時應能正常顯示搜尋結果（即使啟用[情境式搜尋](#contextual-search)）

### 支援服務 {#algolia-support}

Algolia DocSearch 團隊可協助您排查網站搜尋問題。
...
若搜尋索引體積較小，可讓使用者在瀏覽網站時下載至本地端，此時適合採用本地搜尋外掛。
...
此操作會在專案目錄建立 `src/theme/SearchBar` 檔案。重新啟動開發伺服器並編輯該元件後，Docusaurus 將會改用您的自訂 `SearchBar` 元件。

**注意**：您也可選擇[從 Algolia SearchBar 進行替換](#editing-the-algolia-search-component)，並以此為基礎建立自訂搜尋元件。