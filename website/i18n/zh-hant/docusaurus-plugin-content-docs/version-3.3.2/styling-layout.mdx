...
Docusaurus 站點是一個單頁 React 應用程式。您可以像設定 React 應用程式樣式一樣來設定其樣式。

根據您的偏好和您試圖建立的網站類型，有幾種可行的方法/框架。對於行為更像網路應用程式且具有高度互動性的網站，將樣式與元件共置的現代樣式設定方法會更為有益。當您希望自訂或替換元件時，元件樣式設定也特別有用。
...
如果要為任何元素添加 CSS，可以在瀏覽器中打開開發者工具來檢查其類別名稱。類別名稱有以下幾種：

- **主題類別名稱**。這些類別名稱在[下一小節](#theme-class-names)中有詳盡列出。它們沒有任何預設屬性。在自訂 CSS 中，您應始終優先針對這些穩定的類別名稱。
- **Infima 類別名稱**。這些類別名稱出現在經典主題中，通常遵循 [BEM 慣例](http://getbem.com/naming/)的 `block__element--modifier` 格式。它們通常是穩定的，但仍被視為實現細節，因此通常應避免針對它們。不過，您可以[修改 Infima CSS 變數](#styling-your-site-with-infima)。
...
我們提供了一些穩定的 CSS 類別名稱，用於實現穩健且易於維護的全域佈局樣式設定。這些名稱與主題無關，旨在供自訂 CSS 使用。
...
如果您計劃透過 iframe 在另一個網站中嵌入部分 Docusaurus 頁面，建立替代顯示模式並使用如 `https://mysite.com/docs/myDoc?docusaurus-data-mode=iframe` 的 iframe 網址會很有幫助。您需自行提供額外樣式，並決定要保留或隱藏哪些 UI 元素。

:::

### 行動裝置視圖 {#mobile-view}

Docusaurus 使用 `996px` 作為行動裝置螢幕寬度與桌機的切分點。若您希望行動視圖的版面有所不同，可以使用媒體查詢。
...
部分 React 元件（如標頭和側邊欄）在行動視圖中會實作不同的 JavaScript 邏輯。若您在自訂 CSS 中變更斷點值，可能還需要透過[替換元件](./swizzling.mdx)來更新 `useWindowSize` 鉤子的調用，並傳入明確的選項參數。
...
將樣式表文件命名為 `.module.scss` 後綴（例如 `welcome.module.scss`）而非 `.css`。Webpack 將使用 `sass-loader` 預處理您的樣式表並將其作為 CSS 模塊加載。
