...
透過[網站設定](./configuration.mdx#site-metadata)為整個網站提供全域元屬性。所有元數據將使用鍵值對作為屬性名稱和值，渲染在 HTML 的 `<head>` 中。`metadata` 屬性是宣告 `<meta>` 標籤的便捷方式，但也可以透過 `headTags` 屬性在 `<head>` 中注入任意標籤。
...
Docusaurus 會自動為每個 Markdown 頁面添加 `description`、`title`、規範 URL 連結和其他有用的元數據。這些可以透過[前置資料](./guides/markdown-features/markdown-features-intro.mdx#front-matter)進行配置：
...
對於 `description` 和 `keywords` 等字段，建議使用[前置資料](./guides/markdown-features/markdown-features-intro.mdx#front-matter)：Docusaurus 會自動將其應用於 `description` 和 `og:description`，而使用 `<head>` 標籤時則需要手動宣告兩個元數據標籤。
...
Docusaurus 部落格預設支援[豐富搜尋結果](https://search.google.com/test/rich-results)，以獲得最佳搜尋引擎體驗。這些資訊會根據您在部落格/全域設定中的元數據自動生成。若要充分利用豐富搜尋資訊的優勢，請填寫文章發布日期、作者和圖片等相關資訊。更多關於元數據的說明請參閱[此文件](./blog.mdx)。
...
若要阻止整個 Docusaurus 網站被索引，請使用 [`noIndex`](./api/docusaurus.config.js.mdx#noIndex) 站點設定。某些[託管服務提供商](./deployment.mdx)可能也允許您配置 `X-Robots-Tag: noindex` HTTP 標頭（GitHub Pages 不支援此功能）。
...
Docusaurus 提供預設內建於 `preset-classic` 的 [`@docusaurus/plugin-sitemap`](./api/plugins/plugin-sitemap.mdx) 外掛程式。它會自動生成 `sitemap.xml` 檔案，在正式環境建置後可透過 `https://example.com/[baseUrl]/sitemap.xml` 存取。此網站地圖元數據有助於搜尋引擎爬蟲更準確地抓取您的網站。
...
搜尋引擎依賴 `<h2>`、`<table>` 等 HTML 標記來理解網頁結構。當 Docusaurus 渲染您的頁面時，會使用 `<aside>`、`<nav>`、`<main>` 等語義標記來劃分頁面的不同區塊，幫助搜尋引擎定位側邊欄、導航列和主要內容等部分。
