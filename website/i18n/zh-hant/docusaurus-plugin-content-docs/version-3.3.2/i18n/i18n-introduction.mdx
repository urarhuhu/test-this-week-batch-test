...
- **靈活的部署選項**：單一、多個網域或混合部署
- **模組化**：允許插件作者提供國際化支援
- **低運行開銷**：文件主要是靜態的，不需要繁重的 JS 函式庫或填充程式
- **可擴展的建置時間**：允許獨立建置和部署本地化網站
- **本地化資源**：網站圖片可能包含需要翻譯的文字
- **無耦合**：不強制使用任何 SaaS，但可以整合
- **易於與 [Crowdin](https://crowdin.com/) 搭配使用**：許多 Docusaurus v1 網站使用 Crowdin，應該能夠遷移到 v2
...
- **自動語言環境檢測**：這是有主見的，最好在[伺服器（您的託管服務提供商）](../deployment.mdx)上完成
- **翻譯 SaaS 軟體**：您需要自行了解所選的外部工具
- **翻譯網址片段**：技術上複雜，SEO 價值低
...
部分插件可能從需整體本地化的外部資料檔案讀取內容。例如部落格插件使用的[`authors.yml`](../blog.mdx#global-authors)檔案，可通過在`i18n/[locale]/docusaurus-plugin-content-blog/`下建立副本進行翻譯

### 翻譯檔案存放位置 {#translation-files-location}

翻譯檔案需存放在正確的檔案系統路徑下

每個語言區域與插件都有專屬的`i18n`子目錄：
...
翻譯一個簡易法語版Docusaurus網站會產生以下目錄結構：
...
JSON檔案可透過[`docusaurus write-translations`](../cli.mdx#docusaurus-write-translations-sitedir) CLI指令初始化。各插件在對應目錄下管理自身翻譯內容，而`code.json`則定義React程式碼中使用的文字標籤
