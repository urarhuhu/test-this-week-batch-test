...
當下一次佈局發生時，自動將視圖動畫到新位置。

常見的使用方式是在函數式組件中更新狀態鉤子前調用此API，或在類組件中調用`setState`前使用。

請注意，要在**Android**上正常運作，需透過`UIManager`設置以下標記：
...
輔助函數，創建一個對象（包含`create`、`update`和`delete`字段）傳入[`configureNext`](layoutanimation.md#configurenext)。`type`參數是[動畫類型](layoutanimation.md#types)，`creationProp`參數是[佈局屬性](layoutanimation.md#properties)。
