...
顯示在按鈕內部的文字。在 Android 平台上，給定的標題文字會自動轉換為大寫形式。
...
指定螢幕閱讀器與元件互動時應使用的語言值。需遵循 [BCP 47 規範](https://www.rfc-editor.org/info/bcp47)。
...
無障礙操作允許輔助技術以程式化方式觸發元件的動作。`accessibilityActions` 屬性應包含操作物件列表，每個操作物件需包含名稱欄位與標籤。
...
當使用者執行無障礙操作時觸發。此函式的唯一參數是包含待執行操作名稱的事件物件。
