...
雖然配色方案可以立即取得，但這可能會改變（例如在日出或日落時預定的配色方案變更）。任何依賴使用者偏好配色方案的渲染邏輯或樣式，應該在每次渲染時調用此函數，而不是緩存該值。例如，你可以使用 [`useColorScheme`](usecolorscheme) React 鉤子，因為它提供並訂閱配色方案的更新，或者你可以使用內聯樣式，而不是在 `StyleSheet` 中設置值。
...
表示當前使用者偏好的配色方案。該值可能會在之後更新，無論是通過直接的使用者操作（例如在設備設置中選擇主題）還是按照計劃（例如跟隨日夜循環的淺色和深色主題）。
