...
> 注意：Android 上僅支援 `keyboardDidShow` 和 `keyboardDidHide` 事件。若您的 activity 在 Android 10 及以下版本中將 `android:windowSoftInputMode` 設為 `adjustNothing`，則不會觸發這些事件。
...
用於同步 TextInput（或其他鍵盤附屬視圖）的尺寸或位置變化與鍵盤移動。
