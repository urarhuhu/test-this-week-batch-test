...
現在是時候編寫一些 iOS 平台代碼，以確保 `localStorage` 在應用程式關閉後仍能保留資料。
...
- `RCT_EXPORT_MODULE` 使用我們在 JavaScript 環境中存取模組的識別碼 `NativeLocalStorage` 來匯出和註冊模組。詳情請參閱這些 [文件](./legacy/native-modules-ios#module-name)。
- 你可以使用 Xcode 跳轉到 Codegen 生成的 `@protocol NativeLocalStorageSpec`。你也可以使用 Xcode 為你生成存根。
