...
2. _如何讓瀏覽文件 Y 時顯示側邊欄 X，但側邊欄 X 不包含 Y 的連結？_ 例如當 Y 是「文件首頁」而側邊欄僅用於導航時？

Front matter 選項 `displayed_sidebar` 可強制設定側邊欄關聯。沿用相同範例，您仍可使用簡寫形式而無需特殊配置：
...
這會明確告知 Docusaurus 在瀏覽 `commonDoc` 時顯示 `apiSidebar`。透過同樣方法，您也可以讓未包含文件 Y 的側邊欄 X 出現在文件 Y 上：
...
若設定 `displayed_sidebar: null`，該頁面將不會顯示任何側邊欄，連帶也不會產生分頁導航。

## 生成分頁導航 {#generating-pagination}

Docusaurus 使用側邊欄來產生每個文件頁面底部的「下一頁」和「上一頁」分頁連結。它嚴格依據當前顯示的側邊欄來生成：若無關聯側邊欄，則不會產生分頁導航。但被連結為「下一頁」或「上一頁」的文件不一定會顯示相同側邊欄：它們雖被包含在此側邊欄中，但其 front matter 可能設定了不同的 `displayed_sidebar`。

若透過 front matter `displayed_sidebar` 設定的側邊欄未包含當前文件本身，則不會顯示分頁導航。
...
`ref` 類型與 [`doc` 類型](./items.mdx#sidebar-item-doc)在各方面完全相同，唯一的區別在於它不會參與生成導航元數據。它僅註冊為一個連結。當[生成分頁](#generating-pagination)和[顯示側邊欄](#sidebar-association)時，`ref` 項目會被完全忽略。
