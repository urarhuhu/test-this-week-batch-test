...
如果您使用 [@docusaurus/plugin-ideal-image](../../api/plugins/plugin-ideal-image.mdx)，則需要按照文檔使用專用的圖片組件。
...
:::info[Markdown 連結始終是文件路徑]

如果您使用 Markdown 圖片或連結語法，Docusaurus 會將所有資源路徑解析為文件路徑並自動轉換為 `require()` 調用。除非您使用 JSX 語法（此時需要自行處理），否則無需在 Markdown 中使用 `require()`。
...
這在您希望通過 CSS 修改 SVG 圖像的部分時非常有用。例如，您可以根據當前主題更改 SVG 的某種顏色。
...
Docusaurus 支持主題化圖片：內置於主題中的 `ThemedImage` 組件允許您根據當前主題切換圖片來源。
...
要使用路徑片段切換圖片的可見性（GitHub 使用 `#gh-dark-mode-only` 和 `#gh-light-mode-only`），請將以下代碼添加到您的自定義 CSS 中（如果您不希望與 GitHub 耦合，也可以使用自己的後綴）：
...
如果 Markdown 連結或圖片使用絕對路徑，該路徑將被視為文件路徑並從靜態目錄解析。例如，如果您將[靜態目錄](../../static-assets.mdx)配置為 `['public', 'static']`，則對於以下圖片：

```md title="my-doc.md"
![An image from the static](/img/docusaurus.png)
```

Docusaurus 會嘗試在 `static/img/docusaurus.png` 和 `public/img/docusaurus.png` 兩個路徑中尋找該檔案。該連結最終會被轉換為 `require()` 呼叫而非保留為 URL 格式。這種處理方式在以下兩個方面具有優勢：
