...
每份 Markdown 文件右上角會顯示目錄。預設僅顯示 h2 和 h3 標題，這通常足以呈現頁面結構概覽。若需調整顯示的標題層級範圍，可針對單一頁面或全域設定最小與最大標題層級。
...
即使已設定全域選項，仍可透過 front matter 進行局部覆蓋。

:::note

`themeConfig` 選項會套用至網站上所有目錄（包含[行內目錄](#inline-table-of-contents)），而 front matter 選項僅影響右上角的目錄。您需要使用 `minHeadingLevel` 和 `maxHeadingLevel` 屬性來客製化每個 `<TOCInline />` 元件。
...
`toc` 變數可在任何 MDX 文件中使用，包含該文件的所有標題。預設僅顯示 `h2` 和 `h3` 標題。您可以透過為個別 `TOCInline` 元件設定 `minHeadingLevel` 或 `maxHeadingLevel` 來調整顯示的標題層級範圍。
...
請注意 `toc` 全域變數是扁平化陣列，因此您可以輕鬆過濾不需要的節點或插入額外節點，重新組織目錄樹狀結構。
...
## 自訂目錄生成方式 {#customizing-table-of-contents-generation}

目錄是透過 [Remark 插件](./markdown-features-plugins.mdx) 解析 Markdown 原始碼所生成。在某些邊緣情況下可能會產生誤判或漏判。
...
非 Markdown 標題不會顯示在目錄中。這個特性可用來解決上述問題。
