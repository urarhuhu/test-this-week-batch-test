...
若您試用過 [MDX 互動式教學](https://mdxjs.com/playground/)，會注意到 MDX 轉譯過程有兩個中間階段：Markdown 抽象語法樹（MDAST）和超文本抽象語法樹（HAST），最終才輸出 JSX。MDX 插件也分為兩種形式：
...
Docusaurus 在 Markdown 處理過程中會注入[預設的 Remark 插件](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-mdx-loader/src/remark)，這些插件會：
...
這些都是 Remark 插件的典型應用案例，也可作為開發自訂插件時的參考範例。
...
許多官方 Remark/Rehype 插件僅支援 [ES Modules](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c) 模組系統。Docusaurus 支援此規範，建議使用 [ES Modules 設定檔](https://flaviocopes.com/es-modules/)以便導入這類套件。
...
## 建立自訂 rehype/remark 插件 {#creating-new-rehyperemark-plugins}

若現有的套件無法滿足您的客製化需求，您可以自行建立 MDX 外掛程式。

:::note

以下說明**並非**建立外掛程式的完整指南，僅示範如何使其在 Docusaurus 中運作。如需更深入的工作原理說明，請參閱 [Remark](https://github.com/remarkjs/remark/blob/main/doc/plugins.md#create-plugins) 或 [Rehype](https://github.com/rehypejs/rehype/blob/main/doc/plugins.md#create-plugins) 文件。

:::

舉例來說，我們建立一個外掛程式，為每個 `h2` 標題加上 `第 X 節` 前綴。首先，在任何位置建立外掛程式原始檔（甚至可發佈為獨立 npm 套件並依前述方式安裝）。我們將外掛程式置於 `src/remark/section-prefix.js`。remark/rehype 外掛程式本質上是接收 `options` 並返回操作 AST 的 `transformer` 函式。
...
現在您可於 `docusaurus.config.js` 中導入此外掛程式，並如同已安裝的外掛程式般使用！
...
`transformer` 的第二參數 [`vfile`](https://github.com/vfile/vfile) 可用於存取當前 Markdown 檔案路徑：
...
例如我們的 `transformImage` 外掛程式便利用此參數將相對圖片參照轉換為 `require()` 呼叫。

:::

:::note

Docusaurus 的預設外掛程式會先於自訂 remark 外掛程式執行，這意味著圖片或連結已被轉換為帶有 `require()` 呼叫的 JSX。例如上方案例中，即使所有 `h2` 標題皆加上 `第 X 節` 前綴，生成的目錄仍保持原樣，因為目錄生成外掛程式先於自訂外掛程式執行。若需在預設外掛程式前處理 MDAST，請使用 `beforeDefaultRemarkPlugins` 與 `beforeDefaultRehypePlugins`：
