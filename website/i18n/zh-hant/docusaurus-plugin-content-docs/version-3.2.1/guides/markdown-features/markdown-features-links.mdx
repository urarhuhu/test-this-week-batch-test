...
相對檔案路徑是根據當前檔案所在的目錄來解析。而絕對檔案路徑則是相對於 **內容根目錄** 來解析，通常是 `docs/`、`blog/` 或 [本地化版本](../../i18n/i18n-tutorial.mdx) 如 `i18n/zh-Hans/plugin-content-docs/current`。

絕對檔案路徑也可以相對於網站目錄。但要注意，以 `/docs/` 或 `/blog/` 開頭的連結 **不可移植**，因為如果你建立新的文件版本或進行本地化時，需要手動更新這些連結。
...
Markdown 檔案引用僅在來源檔案和目標檔案由同一個插件實例處理時才能正常運作。這是我們 Markdown 處理架構的技術限制，未來將會修復。如果你需要在不同插件之間連結檔案（例如從部落格文章連結到文件頁面），則必須使用 URL 連結。
