...
`context`是與插件無關的，同一個對象會被傳遞給用於Docusaurus網站的所有插件。`context`對象包含以下字段：
...
`options`是[使用插件時的第二個可選參數](../../using-plugins.mdx#configuring-plugins)。`options`是插件特定的，由用戶在`docusaurus.config.js`中使用時指定。如果導出了[`validateOptions`](./static-methods.mdx#validateOptions)函數，則`options`會事先被驗證和標準化。
