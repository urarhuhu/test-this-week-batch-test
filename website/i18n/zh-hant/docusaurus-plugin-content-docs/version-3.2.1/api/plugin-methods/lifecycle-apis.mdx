...
在 `contentLoaded` 中會消耗 `loadContent` 載入的資料。可將其渲染為路由、註冊為全域資料等。
...
例如，下方插件建立了一個顯示「您的朋友是：Yangshun、Sebastien」的 `/friends` 頁面：
...
修改內部 webpack 配置。若返回值為 JavaScript 物件，將使用 [`webpack-merge`](https://github.com/survivejs/webpack-merge) 將其合併至最終配置中。若為函式，則會呼叫該函式並接收 `config` 作為第一個參數，`isServer` 標誌作為第二個參數。
...
例如，以下這個插件修改了 webpack 配置來轉譯 `.foo` 檔案。
...
`configureWebpack` 會同時被呼叫並帶入插件載入的內容。
...
`injectHtmlTags` 會同時被呼叫並帶入插件載入的內容。
