...
有時您需要在網站發布公告。為此可添加公告欄——這是一個非固定且可選擇性關閉的面板，位於導航欄上方。所有配置皆位於 `announcementBar` 物件中。
...
標誌可置於[靜態資料夾](static-assets.mdx)。標誌URL預設設為網站根路徑。您可自訂標誌URL，若為外部連結將在新分頁開啟。此外，可覆寫標誌連結的target屬性值，這在將文檔網站託管於主站子目錄時特別有用（此時可能不需讓標誌連結至主站時開啟新分頁）。
...
項目行為會根據 `type` 欄位而異。以下章節將介紹所有可用的導航欄項目類型。
