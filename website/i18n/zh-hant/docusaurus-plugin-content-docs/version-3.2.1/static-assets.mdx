...
預設建議將這些資源放在 `static` 資料夾中。**該目錄下的所有檔案**都會被複製到生成的 `build` 資料夾根目錄，並保持目錄結構。例如若在 static 資料夾中添加名為 `sun.jpg` 的檔案，它會被複製到 `build/sun.jpg`。
...
在 JSX 中，您可以使用絕對 URL 從 `static` 資料夾引用資源，但這並非理想做法，因為變更網站 `baseUrl` 會**導致這些連結失效**。對於 `<img src="/img/docusaurus.png" />` 圖片，當網站部署在 `https://example.com/test` 時，瀏覽器會嘗試從 URL 根目錄解析，即 `https://example.com/img/docusaurus.png`，這將失敗因為實際路徑應為 `https://example.com/test/img/docusaurus.png`。
...
在 Markdown 中，**使用 Markdown 語法**撰寫連結或圖片時可堅持使用絕對路徑，因為 Docusaurus 在解析 Markdown 時會將其視為 `require` 調用而非 URL。詳見 [Markdown 靜態資源](./guides/markdown-features/markdown-features-assets.mdx)。
...
在 CSS 中，`url()` 函數常用於引用字型、圖片等資源。要引用靜態資源，請使用絕對路徑：
...
- 撰寫 JSX 時假設基礎 URL 為 `/test/`，避免使用 `src="/img/thumbnail.png"` 這類絕對 URL 路徑，改用 `require` 引用資源。
...
- 預設情況下，`static` 資料夾中的任何檔案都不會經過後處理、雜湊或最小化。
  - 但如先前所示，我們通常能將其轉換為 `require` 調用以便進行處理。這有利於實現積極緩存和提升使用者體驗。
- 若透過硬編碼的絕對路徑引用缺失檔案，將無法在編譯時被偵測到，並會導致 404 錯誤。
- GitHub Pages 預設會使用 [Jekyll](https://jekyllrb.com/) 處理發布檔案。由於 Jekyll 會忽略所有以 `_` 開頭的檔案，若您使用 GitHub Pages 作為託管服務，建議在 `static` 目錄中添加名為 `.nojekyll` 的空檔案來停用 Jekyll。