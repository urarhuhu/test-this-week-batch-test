...
該服務對任何開發者文件或技術部落格**免費**：只需閱讀[檢查清單](https://docsearch.algolia.com/docs/who-can-apply/)並[申請 DocSearch 計劃](https://docsearch.algolia.com/apply)。

DocSearch 每週爬取您的網站一次（排程可透過網頁介面配置），並將所有內容匯總至 Algolia 索引中。這些內容隨後會直接透過 Algolia API 從前端查詢。

若您的網站[不符合](https://docsearch.algolia.com/docs/who-can-apply)免費託管版 DocSearch 的資格，或網站位於防火牆後且非公開，則可[自行運行](https://docsearch.algolia.com/docs/run-your-own/) DocSearch 爬蟲。
...
申請通過並部署後，您將收到一封包含所有詳細資訊的電子郵件，以便將 DocSearch 添加到專案中。可透過[網頁介面](https://crawler.algolia.com/)編輯和管理爬取任務。索引在部署後立即可用，通常無需手動配置。
...
若更新 `initialIndexSettings` 爬蟲設定，可透過介面手動更新索引，但 [Algolia 團隊建議刪除索引後重新開始爬取](https://github.com/facebook/docusaurus/issues/9200#issuecomment-1667338492)，以完全使用新設定重新初始化。
...
若搜尋在重大變更後失效，請使用 Algolia 儀表板 **手動觸發新爬取**。
...
當瀏覽 v2 文件時，若出現 v1 文件的搜尋結果會顯得突兀。有時 v1 與 v2 文件內容高度相似，可能導致相同查詢出現重複結果（每個版本各一筆）。

同理，瀏覽法文站點時，若出現英文文件的搜尋結果也不合理。

為解決此問題，上下文搜尋會動態識別您當前瀏覽的文件版本與語言，並自動建立對應的搜尋篩選條件。
...
若有特殊需求，可關閉 `contextualSearch` 並自訂 `facetFilters`：
...
若僅在關閉上下文搜尋時才能獲得結果，極可能是因 [索引設定問題](#algolia-no-search-results) 所致。
...
我們[建議特定的爬蟲配置](#algolia-index-configuration)有其原因。若您選擇不同配置，我們將無法提供支援。
...
您可透過[Algolia支援頁面](https://algolia.com/support)或[Discord](https://discord.gg/wr2m5j948P)聯繫他們。
...
- 在自有伺服器上[自主託管](https://typesense.org/docs/latest/guide/install-typesense.html#option-2-local-machine-self-hosting)
...
- [docusaurus-theme-search-typesense](https://github.com/typesense/docusaurus-theme-search-typesense) - 可整合至網站的搜尋列UI元件

請參閱[如何執行 typesense-docsearch-scraper 的逐步指南](https://typesense.org/docs/latest/guide/docsearch.html#step-1-set-up-docsearch-scraper)，以及[如何在 Docusaurus 網站安裝搜尋列](https://typesense.org/docs/latest/guide/docsearch.html#option-a-docusaurus-powered-sites)。

## 👥 使用本地搜尋 {#using-local-search}

當搜尋索引體積較小、可直接下載至使用者瀏覽器時，您可使用本地搜尋外掛程式。
...
這將在專案目錄中建立`src/theme/SearchBar`檔案。重新啟動開發伺服器並編輯該元件後，Docusaurus 便會使用您的自訂`SearchBar`元件。

**注意**：您也可以[從 Algolia SearchBar 開始替換](#editing-the-algolia-search-component)，並以此為基礎建立自訂搜尋元件。