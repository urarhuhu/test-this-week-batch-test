...
Docusaurus 2 則改為 `website/build`，請確認部署配置已更新為從正確的 `build` 目錄讀取檔案。
...
D1 專案可能已有 README 文件，可修改內容以反映 D2 變更，或直接複製預設的 [Docusaurus v2 README](https://github.com/facebook/docusaurus/blob/main/packages/create-docusaurus/templates/shared/README.md)。
...
Docusaurus 2 將各功能（部落格、文件、頁面）拆分為模組化插件。預設集為插件組合包，為保持向後兼容性，我們建立了 `@docusaurus/preset-classic` 預設集，其包含 Docusaurus 1 的大部分核心插件。
...
已棄用。我們為 Docusaurus 2 開發了名為 [Infima](https://infima.dev/) 的自訂 CSS 框架，該框架使用 CSS 變數進行主題設定。相關文檔尚未完全準備就緒，我們將在完成後更新此處內容。若要覆蓋 Infima 的 CSS 變數，請建立自己的 CSS 檔案（例如 `./src/css/custom.css`），並通過將其作為選項傳遞給 `@docusaurus/preset-classic` 來全局導入：
...
網站元資訊（如資源、SEO、版權資訊）現在由主題處理。若要自訂這些內容，請在 `docusaurus.config.js` 中使用 `themeConfig` 欄位：
...
已棄用。請改為在 `static` 資料夾中創建一個 `CNAME` 檔案，並填入您的自訂網域。`static` 資料夾中的檔案將在建置命令執行期間被複製到 `build` 資料夾的根目錄。
...
- `disableHeaderTitle`
- `disableTitleTagline`
- `docsSideNavCollapsible` 現在可在 `docsPluginOptions.sidebarCollapsible` 中找到，且此選項現在預設為開啟。
- `facebookAppId`
- `facebookComments`
- `facebookPixelId`
- `fonts`
- `highlight` - 我們現在使用 [Prism](https://prismjs.com/) 而非 [highlight.js](https://highlightjs.org/)。
- `markdownOptions` - 在 v2 中我們使用 MDX 而非 Remarkable。您的 Markdown 選項必須轉換為 Remark/Rehype 插件。
- `markdownPlugins` - 在 v2 中我們使用 MDX 而非 Remarkable。您的 Markdown 插件必須轉換為 Remark/Rehype 插件。
- `manifest`
- `onPageNav` - 此功能現在預設為開啟。
- `separateCss` - 可以按照上述 `custom.css` 的方式導入。
- `scrollToTop`
- `scrollToTopOptions`
...
我們計劃在未來將許多已棄用的配置欄位實現為插件。歡迎貢獻幫助！

## 網址 {#urls}

在 v1 中，所有頁面都可以在有或沒有 `.html` 副檔名的情況下訪問。
...
出於 SEO 考量並避免連結失效，您應在託管服務提供商處配置伺服器端重定向規則。
...
舊版本不允許嵌套側邊欄分類，且分類只能包含文件 ID。但 v2 版本支援無限嵌套側邊欄，並提供多種[側邊欄項目類型](../../guides/docs/sidebar/items.mdx)（不僅限於文件）。
...
Docusaurus v2 需改用 `useDocusaurusContext` 獲取 `siteConfig` 物件。

v2 版本需在每個頁面套用主題佈局。Layout 元件接受元資料 props。
...
Docusaurus 2 採用 [MDX](https://mdxjs.com/) 語法，因此現有文件可能需修正不相容語法。常見範例如 HTML 中有效的自閉合標籤 `<img>` 和 `<br>`，現在需明確閉合為 `<img/>` 和 `<br/>`。MDX 文件中的所有標籤都必須符合 JSX 規範。

Front matter 由 [gray-matter](https://github.com/jonschlinkert/gray-matter) 解析。若您的 front matter 使用特殊字符如 `:`，現在需加上引號：`title: Part 1: my part1 title` → `title: "Part 1: my part1 title"`。
...
GitHub Pages 使用的 `CNAME` 檔案不再自動生成，若使用自訂網域請務必在 `/static/CNAME` 手動建立。
