...
這對國際化特別重要，因為若 Crowdin 上的翻譯品質不佳且使用了無效的標記語言，您的 v2 翻譯網站可能會建置失敗：您可能需要清理部分翻譯內容來修復這些錯誤。
...
本文件是為協助遷移所做的最大努力，若您找到更好的方法，請協助我們改進！

:::

首先，我們建議您：

- 先不帶翻譯內容將 v1 Docusaurus 網站遷移至 v2
- 熟悉 [Docusaurus v2 的新國際化系統](../../i18n/i18n-introduction.mdx)
- 使用全新且未翻譯的 Crowdin 專案，配合 [Crowdin 教學](../../i18n/i18n-crowdin.mdx) 讓 Crowdin 適用於您的 v2 網站
...
為避免任何可能導致生產環境中 v1 網站中斷的風險，一種可行策略是複製原始的 v1 Crowdin 專案。
...
- 先嘗試「100% match」（會比「Perfect」選項翻譯更多內容），並預翻譯您的源文件
- 下載Crowdin翻譯至本地端
- 嘗試運行/建置您的網站，檢查是否有錯誤

首次嘗試時很可能會遇到錯誤：預翻譯可能會嘗試翻譯不該翻譯的內容（front matter、admonition、代碼區塊等），且翻譯後的MD文件可能對MDX解析器無效。

您需要修復所有錯誤直到網站能成功建置。可以透過修改本地翻譯的MD文件，並使用`docusaurus build --locale fr`一次針對一個語言修復網站。

我們無法提供修復這些錯誤的終極指南，但常見錯誤原因包括：

- 未在Crowdin標記足夠的「隱藏字串」，導致預翻譯嘗試翻譯這些字串。
- 存在不良的v1翻譯，導致v2中出現無效標記：翻譯中包含錯誤的HTML元素和未閉合的標籤
- 任何被MDX解析器拒絕的內容，例如使用HTML元素而非JSX元素（使用[MDX playground](https://mdxjs.com/playground/)進行除錯）

您可能需要重複此預翻譯流程，最終嘗試「Perfect」選項並限制僅對某些語言/文件進行預翻譯。

:::tip

對有問題的Markdown元素使用[`mdx-code-block`](../../i18n/i18n-crowdin.mdx#mdx-solutions)：Crowdin較不容易在代碼區塊中出錯。
...
此解析器版本未公開說明，您需要聯繫Crowdin支援以確認專案的解析器版本並固定特定版本。
...
Crowdin有「上傳翻譯」功能，但根據我們的經驗，對於Markdown文件效果不佳

:::

### 使用現有的Crowdin專案 {#use-the-existing-crowdin-project}

如果您不介意修改現有Crowdin專案並承擔可能搞砸的風險，可以嘗試使用Crowdin的分支系統。
...
這樣您就無需創建新Crowdin專案、轉移翻譯記憶、應用預翻譯並嘗試修復預翻譯錯誤。

您可以為Docusaurus v2創建一個Crowdin分支，上傳v2源文件，並在準備就緒後將分支合併至主幹。

### 改用Git而非Crowdin {#use-git-instead-of-crowdin}

可以完全遷移出Crowdin，將翻譯文件直接加入Git版本控制。

使用Crowdin CLI下載v1翻譯文件，並將這些翻譯文件放置到Docusaurus v2正確的文件系統位置。