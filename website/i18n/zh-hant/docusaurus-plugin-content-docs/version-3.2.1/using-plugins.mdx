...
然而，插件可以透過在配置中將名稱和選項物件包裝成一個二元組來指定選項。這種風格通常稱為「Babel 風格」。
...
主題的載入方式與插件完全相同。從使用者角度來看，在安裝和配置插件時，`themes` 和 `plugins` 條目是可互換的。唯一的細微差別是主題在插件之後載入，且[主題可以覆蓋插件的預設主題組件](./advanced/client.mdx#theme-aliases)。
...
預設集是一個與 [插件建構函式](./api/plugin-methods/README.mdx#plugin-constructor) 相同結構的函式。它應該返回一個 `{ plugins: PluginConfig[], themes: PluginConfig[] }` 物件，與網站設定中接受的形式相同。例如，您可以指定一個包含以下主題與插件的預設集：
...
以下是一些在 `plugins` 欄位中註冊外掛的範例。請注意，與 [ESLint](https://eslint.org/docs/user-guide/configuring/plugins#configuring-plugins) 或 [Babel](https://babeljs.io/docs/en/options#name-normalization) 不同，這些工具強制要求外掛遵循一致的命名慣例，而 Docusaurus 允許更大的命名自由度，因此解析方式並非絕對，而是依照上述優先順序進行。
