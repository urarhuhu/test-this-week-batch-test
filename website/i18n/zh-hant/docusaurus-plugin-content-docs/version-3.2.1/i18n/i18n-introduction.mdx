...
Docusaurus 透過其國際化 ([i18n](https://en.wikipedia.org/wiki/Internationalization_and_localization)) 支援，讓**翻譯網站變得簡單**。
...
- **靈活的翻譯流程**：可使用 Git（單一儲存庫、分支或子模組）、SaaS 軟體或 FTP
- **靈活的部署選項**：單一網域、多網域或混合部署
- **模組化**：允許插件作者提供國際化支援
- **低運行開銷**：文件主要是靜態內容，無需繁重的 JS 函式庫或 polyfill
...
- **自動語言檢測**：此功能較為主觀，最好由[伺服器（您的託管服務商）](../deployment.mdx)處理
- **翻譯 SaaS 軟體**：您需自行了解所選的外部工具
- **網址縮略名翻譯**：技術複雜且 SEO 價值低
...
- 透過插件選項提供的佈局標籤：文件側邊欄分類標籤、部落格側邊欄標題...
...
某些插件可能會讀取需整體本地化的外部資料檔案。例如，部落格插件使用的[`authors.yml`](../blog.mdx#global-authors)檔案，可透過在`i18n/[locale]/docusaurus-plugin-content-blog/authors.yml`路徑下建立副本來進行翻譯。

### 翻譯檔案存放位置 {#translation-files-location}

翻譯檔案需建立在正確的檔案系統位置。

每個語言環境與插件都有專屬的`i18n`子資料夾：
...
為簡單的法文版Docusaurus網站進行翻譯後，目錄結構如下：
...
JSON檔案可透過[`docusaurus write-translations`](../cli.mdx#docusaurus-write-translations-sitedir) CLI指令初始化。各插件在對應資料夾下提供自身翻譯內容，而`code.json`檔案則定義React程式碼中使用的所有文字標籤。

每個內容插件或主題皆不同，並**自行定義翻譯檔案存放位置**：
