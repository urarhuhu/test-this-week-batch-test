...
請參考 [Docusaurus 國際化 RFC](https://github.com/facebook/docusaurus/issues/3317) 以查看我們研究這些系統的筆記和連結。
...
使用 [write-translations](../cli.mdx#docusaurus-write-translations-sitedir) CLI 命令初始化法文語系的 JSON 翻譯檔案：
...
`Hello` 會顯示為 `(fr) Hello`，這清楚地表明需要翻譯。
...
保持翻譯檔案與原始檔案**一致**可能**具有挑戰性**，特別是對於 Markdown 文件。

### Markdown 翻譯 {#markdown-translations}

當未翻譯的 Markdown 文件被編輯時，**您有責任維護相應的翻譯檔案**，我們目前沒有好的方法來幫助您完成這項工作。

為了保持翻譯站點的一致性，當 `website/docs/doc1.md` 文檔被編輯時，您需要**將這些修改反向移植**到 `i18n/fr/docusaurus-plugin-content-docs/current/doc1.md`。
...
當用戶瀏覽 `/fr/doc1` 頁面時，編輯按鈕預設會連結到未本地化的文檔 `website/docs/doc1.md`。

您的翻譯存放在 Git 中，可以使用 docs 和 blog 插件的 `editLocalizedFiles: true` 選項。

編輯按鈕將連結到本地化文檔 `i18n/fr/docusaurus-plugin-content-docs/current/doc1.md`。