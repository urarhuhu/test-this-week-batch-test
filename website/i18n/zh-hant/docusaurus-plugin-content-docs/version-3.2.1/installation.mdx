...
安裝 Docusaurus 最簡單的方法是使用命令行工具，它可以幫助您建立一個 Docusaurus 網站的基本框架。您可以在新的空倉庫或現有倉庫中的任何位置運行此命令，它將創建一個包含框架文件的新目錄。
...
- `/src/` - 非文件類檔案（如頁面或自訂 React 元件）。您不一定要嚴格將非文件檔案放置於此，但集中存放於單一目錄下，能更方便進行程式碼檢查/處理等操作
...
- `/docusaurus.config.js` - 包含網站設定的組態檔。此檔案相當於 Docusaurus v1 的 `siteConfig.js`
- `/package.json` - Docusaurus 網站本質是 React 應用程式，您可安裝並使用任何 npm 套件
- `/sidebars.js` - 供文件功能使用，用於指定側邊欄中文檔的排序

### 單體式儲存庫 {#monorepos}

若您使用 Docusaurus 為現有專案建立文件，單體式儲存庫（monorepo）可能是適合的解決方案。單體式儲存庫允許您在相似專案間共享依賴項。例如，您的網站可使用本地套件來展示最新功能，而非依賴已發布版本。如此一來，貢獻者能在實作功能時同步更新文件。下方為單體式儲存庫的範例目錄結構：
...
若使用 Netlify 或 Vercel 等託管服務，需將網站的 `Base directory` 變更為 Docusaurus 根目錄位置（本例中為 `./website`）。更多關於設定忽略指令的資訊，請參閱[部署文件](./deployment.mdx#deploying-to-netlify)。

更多單體式儲存庫相關資訊，請參閱 [Yarn 文件](https://yarnpkg.com/features/workspaces)（Yarn 並非唯一設定方式，但屬常見解決方案），或參考 [Docusaurus](https://github.com/facebook/docusaurus) 與 [Jest](https://github.com/facebook/jest) 的實際案例。

## 執行開發伺服器 {#running-the-development-server}

要即時預覽檔案編輯後的變更，可執行本地開發伺服器來提供網站服務並反映最新修改。
...
預設情況下，瀏覽器視窗將在 [`http://localhost:3000`](http://localhost:3000) 自動開啟。

恭喜！您已成功建立第一個 Docusaurus 網站！請瀏覽站點內容以查看可用功能。

## 建置 {#build}

Docusaurus 是現代靜態網站生成器，需將網站建置為靜態內容目錄後部署至網頁伺服器才能檢視。建置網站指令如下：

```bash npm2yarn
npm run build
```

建置完成後，內容將生成於 `/build` 目錄內，可將其部署至任何靜態檔案託管服務如 [GitHub Pages](https://pages.github.com/)、[Vercel](https://vercel.com/) 或 [Netlify](https://www.netlify.com/)。更多細節請參閱[部署文件](deployment.mdx)。

## 更新 Docusaurus 版本 {#updating-your-docusaurus-version}

更新 Docusaurus 版本有多種方式。其中一種確保有效的方法是手動修改 `package.json` 中的版本號至目標版本。請注意所有 `@docusaurus/` 命名空間的套件都應使用相同版本。
