...
本章節將介紹如何在 Docusaurus 中進行版面配置的自訂。

> 似曾相識...？

本節與[樣式與版面](./styling-layout.mdx)類似，但這次我們將自訂 React 元件本身，而非其外觀。我們將討論 Docusaurus 的核心概念：**元件替換 (swizzling)**，這項技術能實現**更深層的網站客製化**。

實務上，元件替換允許您**用自己實作的元件取代主題元件**，主要分為兩種模式：
...
使用 `--help` 參數查看所有可用 CLI 選項，或參考 [swizzle CLI 文件](./cli.mdx#docusaurus-swizzle)。
...
完整替換主題元件是指**建立原始主題元件的副本**，您可對其進行**完整客製化與覆寫**。
...
此操作會將當前 `<Footer />` 元件的實作複製到您網站的 `src/theme` 目錄。Docusaurus 將使用此 `<Footer>` 元件副本取代原始元件。您現在可以完全重新實作 `<Footer>` 元件。
...
完整替換[**不安全**](#what-is-safe-to-swizzle)元件時，可能會複製大量內部程式碼，後續需自行維護。這可能增加 Docusaurus 升級難度，若接收的 props 或使用的內部主題 API 發生變更，您需自行遷移客製化內容。

**請盡可能優先選擇[包裝替換](#wrapping)**：需維護的程式碼量較少。

:::

:::tip[重新混寫]

為了在 Docusaurus 升級後保持抽出的元件處於最新狀態，請重新執行抽出命令並使用 `git diff` 比較變更。建議在檔案頂部寫下簡短註解說明您所做的修改，以便在重新抽出後能更輕鬆地重新套用變更。
...
包裝主題是**在現有元件周圍添加額外元件**而不需[抽出](#ejecting)的絕佳方式。例如，您可以輕鬆在每篇部落格文章下方加入自訂留言系統：
...
不必過度**害怕混寫不安全元件**：只需記住可能發生**破壞性變更**，您或許需要在次要版本升級時手動更新自訂內容。

:::

:::note[回報使用案例]

如果您有**強烈需求必須混寫不安全元件**，請[**在此回報**](https://github.com/facebook/docusaurus/discussions/5468)，我們將共同尋找解決方案使其變得安全。

:::

## 應該混寫哪個元件？ {#which-component-should-i-swizzle}

要精確判斷該混寫哪個元件才能達成預期效果並非總是顯而易見。提供大多數主題元件的 `@docusaurus/theme-classic` 約有[100個元件](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-theme-classic/src/theme)！
...
2. **元件名稱**。官方主題元件採用語義化命名，您可從名稱推斷其功能。swizzle CLI 允許輸入部分元件名稱來縮小選擇範圍。例如執行 `yarn swizzle @docusaurus/theme-classic` 後輸入 `Doc`，將只列出與文檔相關的元件。
3. **從高層級元件開始**。元件形成樹狀結構，部分元件會導入其他元件。每條路由都會關聯一個頂層元件（大多列於[內容外掛路由](./advanced/routing.mdx#routing-in-content-plugins)）。例如所有部落格文章頁面的最頂層元件都是 `@theme/BlogPostPage`。您可以先swizzle此元件，再向下遍歷元件樹定位目標元件。找到正確元件後，請刪除其他swizzle產生的檔案以避免維護過多元件。
4. **閱讀[主題原始碼](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-theme-classic/src/theme)**並善用搜尋功能。

:::tip[直接提問！]

若仍不確定該swizzle哪個元件來實現需求，可透過我們的[支援管道](/community/support)尋求幫助。
...
2. **使用翻譯功能**。雖然看似不直觀，但翻譯本質上是自訂文字標籤的方式。例如若網站預設語言為`en`，仍可執行`yarn write-translations -l en`後編輯生成的`code.json`，詳見[i18n教學](./i18n/i18n-tutorial.mdx)。
...
`<Root>`元件會渲染在React樹的**最頂層**（高於主題`<Layout>`）且**永不卸載**，非常適合用於添加不該因導航而重新初始化的狀態邏輯（使用者認證狀態、購物車狀態等）。
