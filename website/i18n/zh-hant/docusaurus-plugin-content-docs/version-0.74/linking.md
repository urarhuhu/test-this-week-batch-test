...
每個連結 (URL) 都有一個 URL 方案，某些網站以 `https://` 或 `http://` 為前綴，其中的 `http` 就是 URL 方案。我們簡稱它為 scheme。

除了 `https`，您可能也熟悉 `mailto` 方案。當您開啟一個帶有 mailto 方案的連結時，您的作業系統會開啟已安裝的郵件應用程式。同樣地，也有用於撥打電話和發送簡訊的方案。更多關於[內建 URL](#built-in-url-schemes) 方案的資訊請參閱下文。

就像使用 mailto 方案一樣，也可以透過自訂的 url 方案連結到其他應用程式。例如，當您從 Slack 收到一封 **Magic Link** 電子郵件時，**Launch Slack** 按鈕是一個錨點標籤，其 href 看起來像這樣：`slack://secret/magic-login/other-secret`。與 Slack 一樣，您可以告訴作業系統您想要處理一個自訂方案。當 Slack 應用程式開啟時，它會接收到用於開啟它的 URL。這通常被稱為深度連結。更多關於如何[獲取深度連結](#get-the-deep-link)到您的應用程式中的資訊請參閱下文。

自訂 URL 方案並不是在行動裝置上開啟應用程式的唯一方式。您不希望在使用電子郵件中的連結時使用自訂 URL 方案，因為這樣連結在桌面上會失效。相反，您希望使用常規的 `https` 連結，例如 `https://www.myapp.io/records/1234546`，並在行動裝置上讓該連結開啟您的應用程式。Android 稱之為 **Deep Links** (Universal Links - iOS)。

### 內建 URL 方案

如引言所述，有一些用於核心功能的 URL 方案存在於每個平台上。以下是一個非詳盡的清單，但涵蓋了最常用的方案。
...
您可以使用 `Linking.addEventListener('url', callback)` 來處理這些事件 - 它會呼叫 `callback({url})` 並帶有連結的 URL

#### 2. 如果應用程式尚未開啟，它會被開啟並且 url 會作為 initialURL 傳入

您可以使用 `Linking.getInitialURL()` 來處理這些事件 - 它會返回一個 Promise，該 Promise 會解析為 URL（如果有的話）。
...
透過監聽 `url` 事件類型並提供處理程序，來添加一個處理程序以處理 Linking 的變更。
...
該方法返回一個 `Promise` 物件。當確定給定 URL 能否被處理時，Promise 會被解析，第一個參數表示該 URL 是否能被打開。

在 Android 上，如果無法檢查 URL 能否被打開，或者當目標為 Android 11 (SDK 30) 時未在 `AndroidManifest.xml` 中指定相關的 intent 查詢，`Promise` 會被拒絕。類似地，在 iOS 上，如果未在 `Info.plist` 的 `LSApplicationQueriesSchemes` 鍵中添加特定 scheme（見下文），Promise 也會被拒絕。
...
>
> - 如果你的應用鏈接到較早版本的 iOS 但在 iOS 9.0 或更高版本上運行，你可以調用此方法最多 50 次。達到限制後，後續調用總是解析為 `false`。如果用戶重新安裝或升級應用，iOS 會重置此限制。
>
> 從 iOS 9 開始，你的應用還需要在 `Info.plist` 中提供 `LSApplicationQueriesSchemes` 鍵，否則 `canOpenURL()` 總是解析為 `false`。

> 當目標為 Android 11 (SDK 30) 時，你必須在 `AndroidManifest.xml` 中指定要處理的 scheme 的 intent。常見 intent 列表可參考 [這裡](https://developer.android.com/guide/components/intents-common)。
>
...
嘗試使用任何已安裝的應用打開給定的 `url`。

你可以使用其他 URL，例如位置（如 Android 上的 `"geo:37.484847,-122.148386"` 或 iOS 上的 `"https://maps.apple.com/?ll=37.484847,-122.148386"`）、聯繫人，或任何可以用已安裝應用打開的 URL。

該方法返回一個 `Promise` 物件。如果用戶確認打開對話框或 URL 自動打開，Promise 會被解析。如果用戶取消打開對話框或沒有註冊應用可以處理該 URL，Promise 會被拒絕。
