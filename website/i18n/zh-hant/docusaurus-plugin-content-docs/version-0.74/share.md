...
在 iOS 中，回傳一個 Promise，該 Promise 將被調用並帶有一個包含 `action` 和 `activityType` 的物件。如果用戶關閉了對話框，Promise 仍將被解析，其中 action 為 `Share.dismissedAction`，其他所有鍵均為未定義。請注意，某些分享選項在 iOS 模擬器上不會出現或無法使用。

在 Android 中，回傳一個 Promise，該 Promise 將始終被解析，其中 action 為 `Share.sharedAction`。
