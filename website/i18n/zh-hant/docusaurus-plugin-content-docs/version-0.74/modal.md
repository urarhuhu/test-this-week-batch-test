...
`onRequestClose` 回調函式會在 Android 使用者點擊硬體返回按鈕或 Apple TV 選單按鈕時被呼叫。由於此屬性是必需的，請注意只要模態框開啟，`BackHandler` 事件將不會被觸發。
...
`supportedOrientations` 屬性允許模態視窗旋轉至指定的任何方向。在 iOS 上，模態視窗仍會受到應用程式 Info.plist 中 UISupportedInterfaceOrientations 字段所指定的限制。
