...
2. 在您的 Mac 上，開啟 Safari 並啟用「開發」選單。這可以在 Safari > 設定... > 進階標籤中找到，然後選擇「顯示網頁開發者功能」。
3. 在「開發」選單下找到您的裝置，並從子選單中選擇「JSContext」項目。這將開啟 Safari 的網頁檢查器，其中包含與 Chrome DevTools 類似的「主控台」和「來源」面板。
...
遠端 JavaScript 偵錯將外部網頁瀏覽器（Chrome）連接到您的應用程式，並在網頁中執行 JavaScript 程式碼。這讓您能像偵錯任何網頁應用程式一樣使用 Chrome 的偵錯工具。請注意，瀏覽器環境可能非常不同，且並非所有 React Native 模組在這種偵錯方式下都能正常運作。
...
從該頁面，可以透過以下方式開啟 Chrome DevTools：
...
:::note
在 Android 上，如果偵錯工具與裝置之間的時間出現偏差，動畫和事件行為等可能無法正常運作。可以透過執行 ``adb shell "date `date +%m%d%H%M%Y.%S%3N`"`` 來修復此問題。使用實體裝置時需要 root 權限。
:::

### 在實體裝置上偵錯

:::info
若您使用 Expo CLI，此功能已預設為您配置完成。
:::
...
:::note
若遇到問題，可能是某個 Chrome 擴充功能與偵錯工具發生非預期交互。建議停用所有擴充功能後逐一重新啟用，直到找出造成問題的擴充套件。
:::