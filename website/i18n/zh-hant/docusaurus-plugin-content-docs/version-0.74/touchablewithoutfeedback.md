...
`TouchableWithoutFeedback` 僅支援單一子元素。若需包含多個子元件，請將其包裹在 View 中。需特別注意，`TouchableWithoutFeedback` 的工作原理是克隆其子元素並套用回應屬性，因此任何中介元件都必須將這些屬性傳遞至底層的 React Native 元件。
...
當值為 `true` 時，表示該視圖為無障礙元素。預設情況下，所有可觸碰元素皆具無障礙特性。
...
指定螢幕閱讀器在使用者與元素互動時應使用的語言，其值應符合 [BCP 47 規範](https://www.rfc-editor.org/info/bcp47)。
...
當無障礙標籤無法明確表達操作結果時，無障礙提示能幫助使用者理解對該元素執行動作後的預期效果。
...
- `'combobox'` - 當元素代表下拉式選單，允許用戶從多個選項中選擇時使用。
- `'menu'` - 當元件是選單選項時使用。
...
無障礙操作允許輔助技術以程式化方式呼叫元件的操作。`accessibilityActions`屬性應包含操作對象列表，每個操作對象應包含名稱和標籤欄位。
...
這定義了你的觸碰可以從按鈕多遠的距離開始。當移出按鈕時，這會添加到 `pressRetentionOffset`。

> 觸碰區域永遠不會超出父視圖的邊界，並且如果觸碰擊中兩個重疊的視圖，兄弟視圖的 Z-index 始終優先。
...
如果在 `onPressIn` 之後的時間超過 370 毫秒，則呼叫。此時間段可以通過 [`delayLongPress`](#delaylongpress) 自定義。
...
當觸碰被釋放時呼叫，但如果被取消（例如被搶奪響應者鎖的滾動）則不呼叫。第一個函數參數是一個 [PressEvent](pressevent) 形式的事件。
...
在觸碰元件被按下時立即呼叫，甚至在 onPress 之前呼叫。這在進行網絡請求時很有用。第一個函數參數是一個 [PressEvent](pressevent) 形式的事件。
...
在觸碰被釋放時立即呼叫，甚至在 onPress 之前。第一個函數參數是一個 [PressEvent](pressevent) 形式的事件。
...
當滾動視圖被禁用時，這定義了你的觸碰可以從按鈕移動多遠的距離，然後停用按鈕。一旦停用，嘗試將其移回，你會看到按鈕再次被激活！在滾動視圖被禁用時來回移動幾次。確保傳入一個常量以減少內存分配。
