...
手指並非最精確的工具，使用者常會意外激活錯誤元素或錯過激活區域。為此，`Pressable` 提供了一個可選的 `HitRect`，可用於定義觸控能夠在包裝元素多遠的距離外仍被註冊。按壓可以在 `HitRect` 內的任何位置開始。

`PressRect` 允許按壓在保持激活狀態的同時，移動超出元素及其 `HitRect` 的範圍，並仍有資格成為「按壓」——想像一下當你按住一個按鈕時，手指慢慢滑開的情況。
...
> `Pressable` 使用 React Native 的 `Pressability` API。如需更多關於 Pressability 的狀態機流程及其運作方式的資訊，請查看 [Pressability](https://github.com/facebook/react-native/blob/main/packages/react-native/Libraries/Pressability/Pressability.js#L350) 的實作。
...
在呼叫 `onPressIn` 前，等待按壓開始的持續時間（以毫秒為單位）。
...
在此元件外部額外的觸碰距離範圍內，仍會被視為按壓狀態，直到觸發 `onPressOut`。
