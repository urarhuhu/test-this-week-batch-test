...
當網站初始化完成後，專案源碼將包含可透過套件管理器調用的 Docusaurus 腳本：
...
使用 [Webpack Dev Server](https://webpack.js.org/configuration/dev-server) 本地建構並預覽網站。
...
請注意部分功能（例如錨點連結）在開發模式無法運作，這些功能將在正式環境正常運作。

:::

:::info[網路開發環境]

當從遠端伺服器或虛擬機（如 GitHub Codespaces）轉發 3000 端口時，可透過 `0.0.0.0` 參數讓開發伺服器監聽本地 IP。
...
為實現 CSS 套件進階壓縮，我們採用 [cssnano 進階預設配置](https://github.com/cssnano/cssnano/tree/master/packages/cssnano-preset-advanced)（搭配多個 PostCSS 插件）與 [clean-css 第二級優化](https://github.com/jakubpawlowicz/clean-css#level-2-optimizations)。若此進階壓縮導致 CSS 異常，可透過環境變數 `USE_SIMPLE_CSS_MINIFIER=true` 改用 [cssnano 默認預設配置](https://github.com/cssnano/cssnano/tree/master/packages/cssnano-preset-default) 壓縮 CSS。**若遭遇 CSS 壓縮錯誤，請[提交問題報告](https://github.com/facebook/docusaurus/issues/new?labels=bug%2C+needs+triage&template=bug.md)。**
...
清除 Docusaurus 網站產生的資源、快取及建置成品。

建議在回報錯誤、升級版本後，或任何遇到 Docusaurus 網站問題時執行此指令。
...
為網站中的 Markdown 文件添加 [明確標題 ID](./guides/markdown-features/markdown-features-toc.mdx#heading-ids)。
