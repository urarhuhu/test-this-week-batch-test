...
**Docusaurus 核心本身不提供任何功能。** 所有功能都委派給個別插件實現：[文檔功能](./guides/docs/docs-introduction.mdx)由[文檔插件](./api/plugins/plugin-content-docs.mdx)提供；[部落格功能](./blog.mdx)由[部落格插件](./api/plugins/plugin-content-blog.mdx)提供；獨立[頁面](./guides/creating-pages.mdx)則由[頁面插件](./api/plugins/plugin-content-pages.mdx)提供。若未安裝任何插件，網站將不會包含任何路由。

您無需逐一手動安裝常見插件：它們可以透過[預設配置](#using-presets)以套件形式分發。對多數使用者而言，插件是透過預設配置進行設定的。

我們維護了一份[官方插件清單](./api/plugins/overview.mdx)，社群也開發了部分[非官方插件](/community/resources#community-plugins)。若您想新增任何功能：自動生成文檔頁面、執行自訂腳本、整合第三方服務...請務必查閱該清單：可能已有現成解決方案！

若您精力充沛，也可閱讀[插件開發指南](./advanced/plugins.mdx)或[插件方法參考文檔](./api/plugin-methods/README.mdx)來學習如何自行開發插件。

## 安裝插件 {#installing-a-plugin}

插件通常是 npm 套件，因此您可使用 npm 像安裝其他套件一樣安裝它們。

```bash npm2yarn
npm install --save docusaurus-plugin-name
```

接著在站點的 `docusaurus.config.js` 配置檔案的 `plugins` 選項中新增：
...
最基本的插件使用方式，您僅需提供插件名稱或插件路徑。

但插件可透過在配置中使用「Babel 風格」的二元組（包含插件名稱與選項物件）來指定選項。
...
所有 Docusaurus 內容插件皆支援多實例運行。例如，您可能需要[多個文檔插件實例](./guides/docs/docs-multi-instance.mdx)或[多個部落格](./blog.mdx#multiple-blogs)。每個插件實例必須分配唯一ID，預設情況下插件ID為 `default`。
...
主題的載入方式與插件完全相同。從使用者角度來看，安裝和配置插件時，`themes` 與 `plugins` 條目可互換使用。唯一細微差別在於主題會在插件之後載入，且[主題可覆蓋插件的預設主題組件](./advanced/client.mdx#theme-aliases)。
...
經典預設集預設會隨新建立的 Docusaurus 網站（透過 [`create-docusaurus`](./installation.mdx#scaffold-project-website) 建立）一同安裝。它包含以下主題與插件：
...
預設集通常是 npm 套件，因此您可以像安裝其他 npm 套件一樣使用 npm 安裝它們。
...
預設集是一個與[插件建構函式](./api/plugin-methods/README.mdx#plugin-constructor)具有相同結構的函式。它應該返回一個 `{ plugins: PluginConfig[], themes: PluginConfig[] }` 物件，與它們在網站設定中被接受的方式相同。例如，您可以指定一個包含以下主題與插件的預設集：
...
這在當某些插件與主題需要一起使用時特別有用。您甚至可以將它們的選項連結在一起，例如將一個選項傳遞給多個插件。

## 模組簡寫 {#module-shorthands}

Docusaurus 支援插件、主題與預設集的簡寫。當它看到一個插件/主題/預設集名稱時，會嘗試按以下順序載入：
...
以下是一些範例，針對在 `plugins` 欄位中註冊的插件。請注意，與 [ESLint](https://eslint.org/docs/user-guide/configuring/plugins#configuring-plugins) 或 [Babel](https://babeljs.io/docs/en/options#name-normalization) 不同，它們強制要求插件遵循一致的命名慣例，而 Docusaurus 允許更大的命名自由，因此解析方式並非絕對，但會遵循上述定義的優先順序。
