...
有關 PWA 標準的完整要求清單，請參考 [PWA 檢查表](https://developers.google.com/web/progressive-web-apps/checklist)

## 應用安裝支援 {#app-installation-support}

如果瀏覽器支援，您可將 Docusaurus 站點作為應用安裝。

![應用安裝過程的螢幕錄影。瀏覽器地址欄中出現按鈕，點擊後顯示「安裝此應用？」對話框。點擊「安裝」按鈕後，操作系統中會開啟新應用窗口並載入 Docusaurus 首頁。](/img/pwa_install.gif)
...
[workbox-precaching](https://developers.google.com/web/tools/workbox/modules/workbox-precaching) 頁面解釋了核心原理：

> Service worker 的重要特性是在安裝時能將文件組保存到緩存中，此過程常稱為「預緩存」。
>
> 這樣做的主要優勢是讓開發者能完全控制緩存，可精確決定文件何時被緩存、緩存多久，並能直接向瀏覽器提供緩存內容而無需網路請求，從而創建可離線運行的網路應用。
>
> Workbox 通過簡化 API 和優化資源下載流程，大幅降低了預緩存的實現難度。

預設情況下，當站點作為應用安裝時會啟用離線模式。詳見 `offlineModeActivationStrategies` 選項說明。

站點完成預緩存後，service worker 會為後續訪問提供緩存響應。當新版本部署並伴隨新 service worker 時，新 worker 會開始安裝並最終進入等待狀態。在此等待期間，將顯示重新載入彈窗提示用戶更新內容。在用戶清除應用緩存或點擊彈窗中的 `reload` 按鈕前，service worker 會持續提供舊版內容。
...
目前無法以非常可靠的方式檢測應用程式是否以 PWA 形式安裝。
...
[`standalone` 策略](https://petelepage.com/blog/2019/07/is-my-pwa-installed/)是啟用離線模式的良好備用方案（至少在執行已安裝應用程式時有效）。
...
包含 `tagName` 和屬性鍵值對的物件陣列，用於注入 `<head>` 標籤。技術上您可以透過此方式注入任何 head 標籤，但理想情況下應用於使網站符合 PWA 規範的標籤。以下是使應用程式完全符合規範的標籤列表：
...
在 Docusaurus 應用程式之前新增一個進入點，以便在應用程式執行前完成註冊。預設的 `registerSW.js` 檔案已足夠進行簡單註冊。
...
當新服務工作者等待安裝時，會渲染 `@theme/PwaReloadPopup` 元件，並向使用者建議重新載入頁面。您可以[替換](../../swizzling.mdx)此元件並實作自己的 UI。該元件會接收一個 `onReload` 回調函式作為 props，應在使用者點擊「重新載入」按鈕時呼叫此函式。這將指示服務工作者安裝等待中的服務工作者並重新載入頁面。
