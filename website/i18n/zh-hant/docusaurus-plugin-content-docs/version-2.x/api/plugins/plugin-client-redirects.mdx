...
本插件還會讀取 [`siteConfig.onDuplicateRoutes`](../docusaurus.config.js.mdx#onDuplicateRoutes) 配置，以便在多個文件將被發送到相同位置時調整其日誌級別。
...
「來源」與「目標」的概念是本插件的核心。「來源」指您希望創建的路徑，即一個將被寫入的額外 HTML 文件；「目標」則是您希望重定向至的路徑，通常是 Docusaurus 已知的路由。

因此，同一個「目標」可以對應多個「來源」：我們會創建多個 HTML 文件，它們都重定向至同一個目的地。反之，一個「來源」不能對應多個「目標」：寫入的 HTML 文件必須有確定的目的地。
