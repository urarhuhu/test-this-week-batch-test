...
在建置過程中，插件會並行載入以獲取自身內容並渲染至路由。插件亦可配置 webpack 或對生成文件進行後處理。
...
`configureWebpack` 呼叫時會傳入根據客戶端/伺服器建置生成的 `config`。您可將其視為待合併的基礎配置。

### `isServer` {#isServer}

`configureWebpack` 會在伺服器建置和客戶端建置時被呼叫。伺服器建置會收到 `true` 作為 `isServer` 參數，而客戶端建置會收到 `false`。
...
例如，以下插件會修改 webpack 配置以轉譯 `.foo` 檔案。
...
`configureWebpack` 會同時被呼叫並傳入插件載入的內容。
...
`injectHtmlTags` 會同時被呼叫並傳入插件載入的內容。
...
舉例來說，要讓你的主題從使用者傳入的 `options` 載入 `customCss` 或 `customJs` 檔案路徑：
