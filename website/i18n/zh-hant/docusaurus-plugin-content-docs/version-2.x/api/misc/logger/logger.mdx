...
此外，`warn` 和 `error` 會為整個訊息著色以引起更好的注意。如果您要打印關於錯誤的大塊幫助文本，最好使用 `logger.info`。
...
嵌入的表達式可以選擇在前面加上一個標誌，形式為 `[a-z]+=`（幾個小寫字母，後跟等號，直接位於嵌入的表達式之前）。如果表達式前面沒有任何標誌，則按原樣打印。否則，它會由以下格式化器之一格式化：
