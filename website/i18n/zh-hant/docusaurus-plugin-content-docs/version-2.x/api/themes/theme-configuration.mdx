...
有時您需要在網站發布公告。為此可添加公告欄——這是位於導覽列上方、非固定且可選擇性關閉的面板。所有配置皆位於 `announcementBar` 物件中。
...
React Router 應會自動為連結套用活動連結樣式，但在邊緣情況下可使用 `activeBasePath`。若某連結需在多個路徑下保持活動狀態（例如同一側邊欄下有多個文件夾），可使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更靈活的替代方案且優先級更高——Docusaurus 會將其解析為正則表達式並與當前 URL 進行匹配。
...
除上述字段外，您還可指定其他任意屬性，這些屬性將應用於 HTML 連結。
...
- [自定義 HTML 導航欄](#navbar-with-custom-html)
...
若需連結至特定文件，此特殊導航欄項目類型會渲染指向所提供 `docId` 文件的連結。只要您瀏覽同一側邊欄下的文件，該連結就會獲得 `navbar__link--active` 類別。
...
使用者可在不同版本間切換，同時停留在同一文件（只要文件 ID 在各版本間保持一致）。
...
若您使用版本化文件，此特殊導航欄項目類型將連結至當前瀏覽的文件版本（依據網址判斷），並在無匹配版本時自動回退至最新版本。
...
若啟用[搜尋功能](../../search.mdx)，搜尋框預設會出現在導航欄最右側。
...
啟用此UI特性後，當使用者向下滾動頁面時導航欄會自動隱藏，向上滾動時則重新顯示。
...
目前提供兩種樣式選項：`dark`（深色）與`primary`（基於主色調）。樣式預覽可參閱[Infima文件](https://infima.dev/docs/components/navbar/)。
...
Docusaurus採用[Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer)實現代碼高亮，所有配置均位於`prism`對象中。
...
預設使用[Palenight](https://github.com/FormidableLabs/prism-react-renderer/blob/master/packages/prism-react-renderer/src/themes/palenight.ts)語法高亮主題。可從[可用主題列表](https://github.com/FormidableLabs/prism-react-renderer/tree/master/src/themes)選擇自訂主題，並為深色模式指定不同高亮主題。
...
若使用行高亮標記語法，可能需要為深色模式語法主題指定不同的高亮背景色。詳見[行高亮指南](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)。
...
您可以透過`themeConfig.footer`添加標誌和版權資訊至頁尾。標誌可置於[靜態資料夾](static-assets.mdx)中，其URL運作方式與導覽列標誌相同。
...
您可透過`themeConfig.footer.links`添加連結至頁尾。頁尾配置有兩種類型：**多欄式頁尾**和**簡易頁尾**。

多欄式頁尾連結需為每欄設定`title`和一組`FooterItem`清單。
...
此React鉤子用於存取色彩模式上下文。該上下文包含設定亮色/暗色模式的函式，並暴露布林變數顯示當前使用模式。
...
- **多實例路徑**: 不適用
- **JSON檔案**: 透過[`docusaurus write-translations`](../../cli.mdx#docusaurus-write-translations-sitedir)命令提取
- **Markdown檔案**: 不適用
