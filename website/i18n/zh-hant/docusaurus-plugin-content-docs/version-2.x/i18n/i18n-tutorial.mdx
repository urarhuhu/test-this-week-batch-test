...
JSX 元素是一個 _物件_，而非字串。在預期字串的上下文中使用它（例如 [`<option>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/option) 的子元素）會 [強制轉換為字串](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/toString)，從而返回 `"[object Object]"`。雖然我們鼓勵您將 `<Translate>` 用作 JSX 子元素，但僅在實際有效的情況下使用元素形式。
...
Docusaurus 特意提供了一個 **非常輕量級的翻譯運行時**，僅支援基本的 [佔位符插值](../docusaurus-core.mdx#interpolate)，使用 [ICU 訊息格式](https://formatjs.io/docs/core-concepts/icu-syntax/) 的子集。

大多數文檔網站通常是 **靜態的**，不需要進階的 i18n 功能（**複數**、**性別** 等）。對於更進階的使用場景，請使用像 [react-intl](https://www.npmjs.com/package/react-intl) 這樣的庫。

:::

`docusaurus write-translations` 命令會靜態分析您網站中使用的所有 React 代碼文件，提取對這些 API 的調用，並將它們聚合到 `code.json` 文件中。翻譯文件將存儲為從 ID 到翻譯訊息物件（包括翻譯後的標籤和標籤的描述）的映射。在調用翻譯 API（`<Translate>` 或 `translate()`）時，您需要指定默認的未翻譯訊息或 ID，以便 Docusaurus 正確將每個翻譯條目與 API 調用關聯起來。

:::warning[文字標籤必須是靜態的]

`docusaurus write-translations` 命令僅對您的代碼進行 **靜態分析**。它實際上並不運行您的網站。因此，無法提取動態訊息，因為訊息是一個 _表達式_，而不是 _字串_：
...
function ItemsList() {
  return (
    <ul>
      {/* 不要這樣做：無法與 write-translations 命令一起使用 */}
      {items.map((item) => (
        <li key={item.id}>
...
這在運行時仍然行為正確。然而，未來我們可能會提供一種「無運行時」機制，允許通過 Babel 轉換直接將翻譯內聯到 React 代碼中，而不是在運行時調用 API。因此，為了未來兼容性，您應該始終偏好靜態可分析的訊息。例如，我們可以將上面的代碼重構為：
...
您可以將對翻譯 API 的調用視為純粹的 _標記_，告訴 Docusaurus「這裡有一個文字標籤需要替換為翻譯後的訊息」。

:::

#### 複數形式 {#pluralization}

當您運行 `write-translations` 時，您會注意到一些標籤使用了複數形式：
...
每種語言都會有一系列 [可能的複數類別](https://unicode-org.github.io/cldr-staging/charts/37/supplemental/language_plural_rules.html)。Docusaurus 會按照 `["zero", "one", "two", "few", "many", "other"]` 的順序排列它們。例如，因為英語 (`en`) 有兩種複數形式（"one" 和 "other"），所以翻譯訊息會有兩個用管道符 (`|`) 分隔的標籤。對於波蘭語 (`pl`)，它有三種複數形式（"one"、"few" 和 "many"），您需要按順序提供三個標籤，並用管道符連接。
