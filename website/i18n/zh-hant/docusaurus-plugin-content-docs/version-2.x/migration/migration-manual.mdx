...
Docusaurus 2 則簡化為 `website/build`，請確認部署配置已更新至正確讀取新路徑。
...
D1 專案可能已有 README 文件，可修改內容反映 D2 變更，或直接套用[預設 Docusaurus v2 README](https://github.com/facebook/docusaurus/blob/main/packages/create-docusaurus/templates/shared/README.md)。
...
Docusaurus 2 將各功能（部落格、文件、頁面）模組化為獨立插件。預設集（preset）為插件組合包，為保持相容性，我們提供 `@docusaurus/preset-classic` 預設集，包含 Docusaurus 1 的核心功能。
...
請參閱下方遷移指南，了解 `siteConfig.js` 中每個欄位的調整方式。
...
已棄用。我們為 Docusaurus 2 開發了名為 [Infima](https://infima.dev/) 的自訂 CSS 框架，使用 CSS 變數進行主題設定。相關文檔尚未完備，我們將在準備完成後更新此處說明。若要覆寫 Infima 的 CSS 變數，請建立自己的 CSS 檔案（例如 `./src/css/custom.css`），並通過將其作為選項傳遞給 `@docusaurus/preset-classic` 來全局導入：
...
建議使用 [ColorBox](https://www.colorbox.io/) 工具為您選擇的主色尋找不同色階。

或者，使用以下工具生成網站所需的各種色階，並將變數複製到 `src/css/custom.css` 中。
...
您可以聯繫 DocSearch 團隊 (@shortcuts, @s-pace) 獲取支援。他們可以為您更新配置並觸發重新爬取網站以恢復搜索功能（否則您需要等待最多 24 小時直到下次排程爬取）
...
為了 SEO 考量並避免連結失效，您應在託管服務商處設定伺服器端重新導向規則。
...
舊版不允許嵌套側邊欄分類，且分類僅能包含文件 ID。而 v2 版本支援無限嵌套側邊欄，並提供多種[側邊欄項目類型](../guides/docs/sidebar/items.mdx)（不局限於文件）。
...
此操作會將主題當前使用的 `<Footer />` 元件複製到網站根目錄下的 `src/theme/Footer` 目錄，後續可編輯此元件進行客製化。
...
請參閱[建立頁面](guides/creating-pages.mdx)了解 Docusaurus 2 的頁面運作機制。閱讀後請注意：需將 v1 版的 `pages/en` 檔案移至 `src/pages` 目錄。

Docusaurus v1 的頁面會接收 `siteConfig` 物件作為 props。

Docusaurus v2 需改用 `useDocusaurusContext` 獲取 `siteConfig` 物件。

v2 版本中，每個頁面都需套用主題佈局元件。Layout 元件接收元數據 props。
...
此功能已被[內嵌目錄](../guides/markdown-features/markdown-features-toc.mdx#inline-table-of-contents)取代

### 將 Markdown 語法更新為 MDX 相容格式 {#update-markdown-syntax-to-be-mdx-compatible}

Docusaurus 2 的 Markdown 語法已改為 [MDX](https://mdxjs.com/)，因此現有文件可能需要修正語法。常見範例如：HTML 中有效的自閉合標籤 `<img>` 和 `<br>`，現在需明確閉合為 `<img/>` 和 `<br/>`。MDX 文件中的所有標籤都必須符合 JSX 規範。
