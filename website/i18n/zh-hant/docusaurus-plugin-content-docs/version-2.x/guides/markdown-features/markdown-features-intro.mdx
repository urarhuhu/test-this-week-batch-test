...
文件是產品與使用者之間的介面之一。一套撰寫良好且組織完善的說明文件，能幫助使用者快速理解您的產品。我們的共同目標是協助使用者以最快速度找到並理解所需資訊。

Docusaurus 2 採用現代化工具，讓您輕鬆編寫互動式文件。您可以嵌入 React 元件，或建立即時編碼區塊讓使用者現場操作程式碼。開始透過讓觀眾無法忽視的程式碼分享您的靈光時刻，這或許是吸引潛在使用者最有效的方式。

:::info

本節假設您正在使用官方的 Docusaurus 內容插件。

:::

## 標準功能 {#standard-features}

Markdown 是一種讓您能用易讀語法編寫格式化內容的標記語言。
...
前置元數據用於為 Markdown 文件添加元資訊。所有內容插件都有各自的前置元數據結構，並利用這些數據來強化從內容或其他配置推斷出的預設元數據。

前置元數據位於文件頂部，由三個破折號 `---` 包圍。其內容以 [YAML](https://yaml.org/spec/1.2.2/) 格式解析。
...
Markdown 可嵌入 HTML 元素，而 [`details`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/details) HTML 元素經過精美樣式設計：
