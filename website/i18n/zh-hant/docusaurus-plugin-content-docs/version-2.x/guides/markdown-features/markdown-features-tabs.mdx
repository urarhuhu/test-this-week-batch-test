...
預設會顯示第一個標籤頁。若要覆寫此行為，可在任一標籤項目中添加 `default` 屬性，或為 `Tabs` 元件的 `defaultValue` 屬性指定您偏好的標籤值。例如在上方範例中，無論是為 `value="apple"` 的標籤設定 `default` 屬性，或是為 `Tabs` 設定 `defaultValue="apple"`，都會強制預設開啟「Apple」標籤頁。

若為 `Tabs` 提供的 `defaultValue` 指向不存在的值，Docusaurus 會拋出錯誤。若希望預設不顯示任何標籤頁，請使用 `defaultValue={null}`。

## 同步標籤選擇 {#syncing-tab-choices}

您可能希望同類型的標籤選擇能彼此同步。例如，您可能想為 Windows 和 macOS 使用者提供不同操作說明，並希望透過單擊切換所有作業系統專用的說明標籤頁。為實現此功能，可為所有相關標籤設定相同的 `groupId` 屬性。請注意，此設定會將選擇狀態持久化至 `localStorage`，且所有具有相同 `groupId` 的 `<Tab>` 實例會在任一標籤值變更時自動更新。需注意群組 ID 為全域命名空間。
...
對於所有具有相同 `groupId` 的標籤群組，其可能值不必相同。若某標籤群組選擇的值在另一相同 `groupId` 的標籤群組中不存在，則該標籤群組不會切換標籤頁。您可從以下範例觀察此行為：嘗試選擇 Linux 後，上方標籤群組不會改變。
...
您也可透過 `attributes` 欄位獨立自訂每個標籤標題。額外屬性可透過 `Tabs` 的 `values` 屬性傳遞，或如同宣告 `label` 的方式透過每個 `TabItem` 的屬性傳遞。
