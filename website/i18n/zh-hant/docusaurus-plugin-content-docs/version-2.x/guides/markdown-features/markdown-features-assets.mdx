...
有時您會希望直接從 Markdown 檔案連結靜態資源（如 docx 文件、圖片等），此時將資源與使用它的 Markdown 檔案放在同一目錄下會非常方便。
...
您可以使用三種方式顯示圖片：Markdown 語法、CJS require 或 ES 導入語法。
...
若使用 Markdown 圖片或連結語法，Docusaurus 會將所有資源路徑解析為檔案路徑並自動轉換為 `require()` 調用。除非使用 JSX 語法（需自行處理），否則在 Markdown 中無需手動使用 `require()`。
...
這對於需要通過 CSS 修改 SVG 部分內容的場景非常有用。例如，您可以根據當前主題變更 SVG 的某個顏色。
...
要使用路徑片段（GitHub 採用 `#gh-dark-mode-only` 和 `#gh-light-mode-only`）切換圖片可見性，請在自訂 CSS 中添加以下內容（若不想與 GitHub 耦合，也可使用自訂後綴）：
...
若 Markdown 連結或圖片使用絕對路徑，該路徑將被視為檔案路徑，並從靜態目錄解析。例如，若您配置[靜態目錄](../../static-assets.mdx)為 `['public', 'static']`，則以下圖片：

```md title="my-doc.md"
![An image from the static](/img/docusaurus.png)
```

Docusaurus 會嘗試同時在 `static/img/docusaurus.png` 和 `public/img/docusaurus.png` 路徑下尋找該檔案。該連結將被轉換為 `require()` 調用而非保持為 URL 格式，此設計具有兩項優勢：

1. 您無需擔心基礎 URL 問題，Docusaurus 會在提供資源時自動處理；
2. 圖片將進入 Webpack 的建置流程，其檔名會被附加雜湊值，使瀏覽器能積極快取圖片並提升網站效能。
